...
**[生物](Creatures.md "Creatures")** 的体型表明其大小。
...
- 当生物的体型改变时，其重量也会根据需要进行调整，以符合上述类别。
...
| [狂野魔法：猫形态](Wild_Magic_colon__Cat_Form_(Condition).md "Wild Magic: Cat Form (Condition)") | 微型 | [狂野魔法：猫和狗](Wild_Magic_colon__Cats_and_Dogs.md "Wild Magic: Cats and Dogs") | 持续时间：2 回合 |
...
此外，可以对角色的体型进行额外修改，将其转换到相邻的类别，如下表所示。
...
| ALCH_ELIXIR | [巨像灵药](Elixir_of_The_Colossus_(Condition).md "Elixir of The Colossus (Condition)") | +1 | [巨像灵药](Elixir_of_the_Colossus.md "Elixir of the Colossus") | 持续时间：直至[长休](Long_Rest.md "Long rest") |
  ...
  - 例如，一个[灰矮人](Duergar.md "Duergar")被施加了[巨化](Enlarged_(Condition).md "Enlarged (Condition)")，体型变为大型，使用[谁是大块头？](Whossa_Large_Fellow_q_.md "Whossa Large Fellow?")会先取消巨化，然后重新应用。
- 对处于[缩小](Reduced_(Condition).md "Reduced (Condition)")状态的角色使用[巨化](Enlarge.md "Enlarge")会先取消后一状态（无论初始来源如何），然后应用巨化，可能使其整体体型改变 2 个类别。
...
- 具有*不同*堆叠 ID 的状态*可以*叠加，根据需要改变角色的体型。
  ...
  - 具有[巨像灵药](Elixir_of_The_Colossus_(Condition).md "Elixir of The Colossus (Condition)")的生物，如果随后被缩小，会先恢复到原始体型，然后在任一效果结束或被替换时再次根据需要改变。
...
- 任何 4 种巨化状态（[巨化](Enlarged_(Condition).md "Enlarged (Condition)")等）在生物最终变为巨型或超巨型时引起的重量增加（而非体型）是不稳定的；它可能会在执行动作或获得/失去状态时丢失或恢复。
- [变形](Polymorphed_(status_group).md) 除了星界形态外，会将[缩小](Reduced_(Condition).md "Reduced (Condition)")和预先存在的巨化状态对体型类别的所有调整（而非重量或视觉大小）加倍。例如，一个被缩小的角色[伪装](Disguise_Self_(Condition).md "Disguise Self (Condition)")成精灵是微型，而非小型，但重量仍为 32 公斤（64 磅）。

## 备注

1. [↑](#cite_ref-1) 尽管身材较矮且[移动速度](Resources.md#Movement_speed "Movement speed")降低，矮人确实是中型生物，并相应地与其他游戏机制互动。
1. [↑](#cite_ref-2) 安苏的体型在死亡时显示为巨型，存活时显示为大型。这是由于两个角色属性条目，一个 GameSize 为巨型，一个重量为 4000 公斤。死亡时，安苏的尸体被视为游戏对象（类似于可拾取的尸体），游戏引用 GameSize 条目，将安苏归类为巨型。存活时，安苏被视为变形角色，游戏引用 Weight 条目，由于其值将安苏归类为大型。因此，存活时安苏会受到通常不会对巨型角色生效的法术和攻击的影响。
