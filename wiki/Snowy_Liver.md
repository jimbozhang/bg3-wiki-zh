# 雪白肝脏

**雪白肝脏** 是一项被动功能，可增强体质伤害和抗性。

## 描述

当处于 [醉酒](Drunk_(Condition).md "醉酒 (状态)") 状态时，你在进行 [体质](Constitution.md "体质") [豁免检定](Saving_throw.md "豁免检定") 时具有 [优势](Advantage.md "优势")，并在你的 [驱散](Turned_(Condition).md "驱散") 开始时获得等于你 [体质](Constitution.md "体质") [调整值](Modifier.md "调整值") 的 [临时生命值](Temporary_Hit_Points.md "临时生命值")。

## 如何习得

由以下装备提供：

- [醉酒服饰](Drunken_Cloth.md "醉酒服饰")

## 备注

- 雪白肝脏适用于 [醉酒](Drunk_(Condition).md "醉酒 (状态)")、[酒](Alcohol_(Condition).md "酒 (状态)") 或 \_(状态)[醉酒 (醉拳大师)](Drunk_(Drunken_Master)_(Condition).md "醉酒 (醉拳大师) (状态)") 状态

---
*Source: [Snowy Liver](https://bg3.wiki/wiki/Snowy_Liver)*