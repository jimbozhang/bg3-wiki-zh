...
**陷阱**是一种隐藏的危险，可以突然造成不想要的[状态](Conditions.md "Conditions")和/或伤害。
...
除了上述之外，游戏中还有多个地点具有独特的陷阱类型，这些类型在其他地方不会出现。这些陷阱可能因豁免检定失败而造成独特的状态，有时它们本身只对某种类型的伤害脆弱，对所有其他伤害具有抗性。尽管如此，它们可以像游戏中所有其他陷阱一样被解除。

_在[第一幕](Act_One.md "Act_One")地点：_
...
- [莎尔铁手神殿](Gauntlet_of_Shar.md "Gauntlet of Shar")：前厅的通风口产生[虚空](Void.md "Void")云——一种类似于[哈达之饥渴](Hunger_of_Hadar.md "Hunger of Hadar")效果的区域，但没有酸伤害。在[寂静图书馆](Silent_Library.md "Silent Library")中，所有书架都设伏了[酸液飞溅](Acid_Splash.md "Acid Splash")法术，而“暗夜之谜”基座周围的四个通风口形成了类似于[艾伐黑触手](Evard's_Black_Tentacles.md "Evard's Black Tentacles")的地表。
...
- [底城废墟](Undercity_Ruins.md "Undercity Ruins")/[巴尔神殿](Bhaal_Temple.md "Bhaal Temple")：通往神殿本身的桥梁上的[血色通风口](Sanguine_Vent.md "Sanguine Vent")排放[不稳血液](Unstable_Blood.md "Unstable Blood")水坑，如果点燃会燃烧。
...
此外，游戏中的某些物体可能受到法术保护（除了其他安全措施，例如物理锁），因此被队伍成员识别为“已设伏”。最常见的是储存各种物品的容器，无论是贵重物品还是对任何任务都至关重要的物品。在某些情况下，触发已设伏的容器而不先解除它可能会连同其中的珍贵物品一起摧毁它。此外，已设伏的物体可能会阻挡通往目标物品或任务检查点的道路。

陷阱可以独立放置，也可以组合成一个系统，当所有元素在与环境的某些细节（最常见的是踩到[大理石板](Marble_Plate.md "Marble Plate")）互动后同时触发。在这种情况下，触发物品也会被角色识别为陷阱，解除它会使整个系统失效。

队伍成员可以设置的唯一陷阱类型是[守卫刻文](Glyph_of_Warding.md "Glyph_of Warding")法术。

## 相关内容

已解除的陷阱碎片

[察觉技能](Perception.md "Perception")技能用于探测陷阱。这是每当队伍成员接近一个活跃但未被发现的陷阱时执行的被动技能检定。每个队伍成员独立进行检定，因此如果一个队伍成员检定失败，另一个成员可能更幸运并成功发现陷阱。

在大多数情况下，如果几个陷阱组合成一个联合系统，触发其中任何一个都会启用[回合制模式](Turn-based_mode.md "Turn-based_mode")，这使得队伍即使不能避免有害影响，至少也可以仔细选择行动以将其最小化。

要解除陷阱，互动的队伍成员需要使用他们的[巧手](Sleight_of_Hand.md "Sleight of Hand")技能（需要掷骰子）并使用[解除陷阱工具包](Trap_Disarm_Toolkit.md "Trap Disarm Toolkit")。如果掷骰成功，陷阱被解除（通常可视化为爆裂成进一步失效的碎片），并且解除陷阱工具包**不会被消耗**。如果掷骰失败或搞砸，解除陷阱工具包**会被消耗**，并且陷阱触发。

与[撬锁](Lockpick.md "Lockpick")不同，解除陷阱不能通过花费另一个解除陷阱工具包来重新掷骰。然而，失败和搞砸的掷骰可以使用[激励点](Inspiration.md "Inspiration")重新掷骰，就像游戏中大多数其他基于掷骰的检定一样。

某些陷阱可以通过游戏情节提供的特定方式停用，或通过武器或魔法攻击物理摧毁。气体[通风口](Vent.md "Vent")也可以通过放置/投掷足够重的物体到其顶部来禁用（为此，通风口应首先被成功探测到）。

有些陷阱可能无需察觉检定即可看到，但除非角色探测到它们，否则无法解除，只能避开。
...
- [IsTrap（法术标志）](IsTrap_(spell_flag).md) 确定被视为陷阱的动作
