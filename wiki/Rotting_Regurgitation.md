# 腐蚀回流

**腐蚀回流**是[西索博得·索姆](Thisobald_Thorm.md "西索博得·索姆")在拥有[腐烂反流](Rotten_Reflux_(Condition).md "腐烂反流（状态）")状态时可用的动作。他会在区域内呕吐出一种黯蚀液体，并变得略微更加醉醺醺。

## 描述

啜饮一口酿酒师的特制烈酒会导致他呕吐出一种黯蚀液体。

## 属性

消耗
[动作](Actions.md#Resources "动作")
伤害：8~80 + 调整值

8d10 + [力量调整值](Strength.md#Strength_modifier_chart "力量")⁠[黯蚀](Necrotic.md "黯蚀")

详情
[体质](Constitution.md "体质") [豁免检定](Saving_throws.md "豁免检定")（[DC](DC.md "DC") 18）（豁免成功：目标仍承受一半伤害。）
范围：4 米（13 英尺）
区域效果：3 米（10 英尺）半径
充能：每回合

## 技术细节

UID

`Projectile_Belch_Necro_Brewer`

法术标志

`[IgnoreSilence](IgnoreSilence_(spell_flag).md)`, `[IsEnemySpell](https://bg3.wiki/w/index.php?title=IsEnemySpell_\(spell_flag\)&action=edit&redlink=1) "IsEnemySpell \(spell flag\) \(page does not exist\)")`, `[IsHarmful](IsHarmful_(spell_flag).md)`

## 状态：中度醉醺醺

**[中度醉醺醺](Moderately_Inebriated_(Condition).md "中度醉醺醺（状态）")**

- 每啜饮一口他的特制烈酒，西索博得都会变得更加醉醺醺。
- 他在魅力和敏捷[检定](Ability_Check.md "属性检定")上具有[劣势](Disadvantage.md "劣势")，并开始幻视小魔鬼。

## 如何习得

由生物使用：[西索博得·索姆](Thisobald_Thorm.md "西索博得·索姆")

## 错误

- 根据游戏数据，本应创建一个名为[黑色水洼](Black_Puddle_(surface).md "黑色水洼（地表）")的地表，但这似乎没有触发。

---
*Source: [Rotting Regurgitation](https://bg3.wiki/wiki/Rotting_Regurgitation)*