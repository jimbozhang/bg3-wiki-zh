# 夜刃

本文介绍的内容**[无法通过正常游戏获取](Category_colon_Inaccessible.md "Category:Inaccessible")**。但仍可通过第三方工具访问。

**夜刃**是[阿兰德拉的浪涛](Allandra's_Whelm.md "阿兰德拉的浪涛")赋予的[武器动作](Weapon_action.md "武器动作")。

## 描述

你将你的[熟练项加值](Proficiency_Bonus.md "熟练项加值")作为⁠[寒冷](Cold.md "寒冷")伤害添加。

当攻击命中时，施加[冻伤](Frostbitten_(Condition).md "冻伤 (状态)")。该生物在进行武器攻击时受到1d4⁠⁠[寒冷](Cold.md "寒冷")伤害。

## 属性

消耗
[动作](Actions.md#Resources "动作")
伤害：

普通武器伤害

\+ [熟练项加值](Proficiency_Bonus.md "熟练项加值")⁠[寒冷](Cold.md "寒冷")

详情
近战武器[攻击掷骰](Attack_roll.md "攻击掷骰")
[体质](Constitution.md "体质")[豁免检定](Saving_throws.md "豁免检定")（[武器动作DC](Dice_rolls.md#Weapon_action_DC "掷骰") + 2）
范围：普通武器范围
充能：[短休](Short_rest.md "短休")
必须[熟练](Proficiency.md#Weapon_proficiency "熟练")使用该武器

## 技术细节

UID

`Target_MAG_WeaponAction_FrigidBlade`

法术标志

`[IsDefaultWeaponAction](https://bg3.wiki/w/index.php?title=IsDefaultWeaponAction_\(spell_flag\)&action=edit&redlink=1) "IsDefaultWeaponAction \(spell flag\) \(page does not exist\)")`, `[IsHarmful](IsHarmful_(spell_flag).md)`, `[IsMelee](IsMelee_(spell_flag).md)`

## 状态：冻伤

**[冻伤](Frostbitten_(Condition).md "冻伤 (状态)")**

持续时间：3 驱散

[体质](Constitution.md "体质")[豁免检定](Saving_throws.md "豁免检定")（[武器动作DC](Dice_rolls.md#Weapon_action_DC "掷骰") + 2）

- 受影响实体在使用武器攻击时受到1d4⁠⁠[寒冷](Cold.md "寒冷")伤害。

## 如何习得

由物品赋予：

- [阿兰德拉的浪涛](Allandra's_Whelm.md "阿兰德拉的浪涛")

---
*Source: [Frigid Blade](https://bg3.wiki/wiki/Frigid_Blade)*