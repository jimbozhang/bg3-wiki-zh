# 圣火 (状态)

**圣火**

- 每回合受到 1~4⁠⁠[光耀](Radiant.md "光耀") 伤害。

## 属性

[堆叠ID](Stack_ID.md "堆叠ID"): `BURNING` [状态组](Status_groups.md "状态组"): [SG_Surface](SG_Surface.md "SG Surface"), [SG_Light](SG_Light.md "SG Light")

[持续时间减少](Conditions.md#Duration "状态"): 回合开始时

[更多属性](Status_properties.md "状态属性"):

- [引发战斗](Status_properties/InitiateCombat.md "状态属性/引发战斗")
- [燃烧](Status_properties/Burning.md "状态属性/燃烧")
- [禁用头顶显示](Status_properties/DisableOverhead.md "状态属性/禁用头顶显示")
- [忽略休息](Status_properties/IgnoreResting.md "状态属性/忽略休息")

## 圣火的来源

- [圣火](Holy_Fire.md "圣火")
- [月光打击](Lunar_Smite.md "月光打击")

## 拥有圣火的生物

*维基数据库中未定义*

## 具有相同堆叠ID的状态

| 状态 | 效果 |
| --- | --- |
| [燃烧](Burning_(Orthonic)_(Condition).md "燃烧 (奥瑟通) (状态)") | 每回合受到 1d4⁠⁠[火焰](Fire.md "火焰") 伤害。无法通过 [协助](Help.md "协助") 动作、使用 [治疗类药水](Potion_of_Healing.md "治疗药水") 或获得 [濡湿](Wet_(Condition).md "濡湿 (状态)") 来移除。 |
| [燃烧](Burning_(Condition).md "燃烧 (状态)") | 每回合受到 1d4⁠⁠[火焰](Fire.md "火焰") 伤害。可以通过 [协助](Help.md "协助") 动作、使用 [治疗药水](Potion_of_Healing.md "治疗药水") 或获得 [濡湿](Wet_(Condition).md "濡湿 (状态)") 来移除。如果处于 [濡湿](Wet_(Condition).md "濡湿 (状态)") 状态则免疫。[蘸取](Dip.md "蘸取") 动作可用于燃烧的角色或物体。 |
| [灼烧](Burning_Fiercely_(Condition).md "灼烧 (状态)") | 每回合受到 1d10⁠⁠[火焰](Fire.md "火焰") 伤害。 |
| 圣火 | 每回合受到 1~4⁠⁠[光耀](Radiant.md "光耀") 伤害。 |
| [融化](Melting_(Condition).md "融化 (状态)") | 每回合受到 10d6⁠⁠[火焰](Fire.md "火焰") 伤害。 |
| [翻腾地狱火](Roiling_Hellfire_(Condition).md "翻腾地狱火 (状态)") | 来自阿弗纳斯核心的火焰吞噬此实体，每回合造成 6d6⁠⁠[火焰](Fire.md "火焰") 伤害。 |
| [灼热光环](Scorching_Aura_(Condition).md "灼热光环 (状态)") | 此实体造成的攻击额外造成 1d6⁠⁠[火焰](Fire.md "火焰") 伤害，除非目标处于 [濡湿](Wet_(Condition).md "濡湿 (状态)") 状态。 |

---
*Source: [Holy Fire (Condition)](https://bg3.wiki/wiki/Holy_Fire_(Condition)*