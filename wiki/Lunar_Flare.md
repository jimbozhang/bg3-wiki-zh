# 月光闪耀

**月光闪耀**是[法术](Spells.md "法术")。此法术由[艾琳女士](Dame_Aylin.md "Dame_Aylin")使用，以光耀飞弹击中敌人，造成[光耀](Radiant.md "Radiant")伤害、[圣火](Holy_Fire_(Condition).md), 并对针对它们进行的[攻击掷骰](Attack_rolls.md "Attack Rolls")授予[优势](Advantage.md "Advantage")。

## 描述

发射一团月光，在其冲击点2米半径范围内洒下翻腾的[圣火](Holy_Fire_(Condition).md "Holy Fire (Condition)")。

## 属性

消耗
[动作](Actions.md#Resources "Actions") + [2级法术位](Spells.md#Spell_slots "Spells")
伤害：5~30

5d6[光耀](Radiant.md "Radiant")

详情
[攻击掷骰](Attack_roll.md "Attack Roll")
射程：18米（60英尺）
创造区域：圣火

## 更高环阶

以更高环阶施放此法术不会获得额外收益。

## 技术细节

UID

`Projectile_MOO_Nightsong_GuidingBolt`

法术标志

`[HasHighGroundRangeExtension](HasHighGroundRangeExtension_(spell_flag).md)`, `[HasSomaticComponent](HasSomaticComponent_(spell_flag).md)`, `[HasVerbalComponent](HasVerbalComponent_(spell_flag).md)`, `[IsHarmful](IsHarmful_(spell_flag).md)`, `[IsSpell](IsSpell_(spell_flag).md)`, `[RangeIgnoreVerticalThreshold](https://bg3.wiki/w/index.php?title=RangeIgnoreVerticalThreshold_\(spell_flag\)&action=edit&redlink=1) "RangeIgnoreVerticalThreshold \(spell flag\) \(page does not exist\)")`

## 状态：曳光弹

**[曳光弹](Guiding_Bolt_(Condition).md "Guiding Bolt (Condition)")**

持续时间：2驱散

- 下一次针对此生物的[攻击掷骰](Attack_roll.md "Attack Roll")具有[优势](Advantage.md "Advantage")。

## 区域：圣火

**[圣火](Holy_Fire.md "Holy Fire")**

持续时间：3驱散
范围效果：2米（7英尺）未知形状

每驱散造成1d6[光耀](Radiant.md "Radiant")伤害。

类型：[地表](Area.md#Surface "Area")

### 状态：圣火

**[圣火](Holy_Fire_(Condition).md "Holy Fire (Condition)")**

- 每驱散受到1~4[光耀](Radiant.md "Radiant")伤害。

## 如何习得

使用者：[艾琳女士](Dame_Aylin.md "Dame Aylin")

## 备注

- 工具提示显示这是一个1级塑能学派法术，但需要2级法术位。

---
*Source: [Lunar Flare](https://bg3.wiki/wiki/Lunar_Flare)*