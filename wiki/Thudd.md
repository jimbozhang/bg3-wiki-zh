# 萨德

## 萨德

- 游戏内
- 模型
- 肖像画

### 身份

男性

### 声音 / 演员

- [Frazer Blaxland](https://www.imdb.com/name/nm7257504/)

### 阵营

[烈焰氏族](Clan_Flameshade.md "Clan Flameshade")

## 属性

### 体型

[中级](Medium.md "Medium")

### 类型

[类人生物](Humanoid.md "Humanoid")

### 种族

[矮人](Dwarf.md "Dwarf")

### 亚种

[灰矮人](Duergar.md "Duergar")

### 职业

[野蛮人](Barbarian.md "Barbarian")

- 平衡

- 硬核

## 平衡难度属性

### 等级

5

### 生命值

80

### 护甲等级

13

### 先攻

+1

### 熟练项

+3

### 移动速度

7.5 米 (25 英尺)

### 重量

75 千克 (150 磅)

### 高级黑暗视觉

24 米 (80 英尺)

### 经验值奖励

75

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 16 / (+3) | 12 / (+1) | 16 / (+3) | 10 / (0) | 12 / (+1) | 12 / (+1) |

## 抗性 [(info)](Resistances.md "Resistances")

- ## 额外资源

- 3

## 特性

- [危机感知](Danger_Sense.md "危机感知")

对陷阱、法术、地表的敏捷豁免具有优势。不能被目盲或失能。

- [灰矮人坚韧](Duergar_Resilience.md "灰矮人坚韧")

对[幻术学派](Illusion.md "Illusion")和被[魅惑](Charmed_(status_group).md)或[麻痹](Paralysed_(Condition).md)的[豁免检定](Saving_throws.md "Saving throws")具有[优势](Advantage.md "Advantage")。

- [矮人活力](Dwarven_Resilience.md "矮人活力")

对被[中毒](Poisoned_(Condition).md)的[豁免检定](Saving_throw.md "Saving throw")具有[优势](Advantage.md "Advantage")，并对[中毒](Poison.md "Poison")伤害具有[抗性](Resistance.md "Resistance")。

- [额外攻击](Extra_Attack.md "额外攻击")

在徒手或武器攻击后可以进行一次额外攻击。

- [快速移动](Fast_Movement.md "快速移动")

当不穿着[重甲](Heavy_Armour.md "Heavy Armour")时，你的[移动速度](Movement_speed.md "Movement Speed")增加 3 米 / 10 英尺。

- [背水一战](Last_Stand.md "背水一战")

在战斗中，当穿戴者在其回合开始时生命值为 50% [生命值](Hit_Points.md "Hit Points")或更低时，他们获得[怒火](Wrath_(Condition).md "Wrath (Condition)")，持续 2 回合。

- [借机攻击](Opportunity_Attack.md "借机攻击")

攻击离开攻击范围的敌人。

- [日照敏感](Sunlight_Sensitivity.md "日照敏感")

在阳光下时，攻击掷骰和[察觉技能](Perception.md "Perception")检定具有劣势。

- [高级黑暗视觉](Superior_Darkvision.md "高级黑暗视觉")

可以在黑暗中看到 24 米 / 80 英尺。

- [**韧性**](Tenacity_(passive_feature).md "Tenacity (passive feature)")

近战攻击未命中时，仍会造成等于[力](STR.md "力")调整值或最小值为 1 的[钝击](Bludgeoning.md "Bludgeoning")伤害。

## 硬核难度属性

### 等级

5

### 生命值

80

### 护甲等级

13

### 先攻

+1

### 熟练项

+3

### 移动速度

7.5 米 (25 英尺)

### 重量

75 千克 (150 磅)

### 高级黑暗视觉

24 米 (80 英尺)

### 经验值奖励

75

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | +6 | +1 | +6 | 0 | +1 | +1 |

## 抗性 [(info)](Resistances.md "Resistances")

- ## 额外资源

- 3

## 特性

- [危机感知](Danger_Sense.md "危机感知")

对陷阱、法术、地表的敏捷豁免具有优势。不能被目盲或失能。

- [灰矮人坚韧](Duergar_Resilience.md "灰矮人坚韧")

对[幻术学派](Illusion.md "Illusion")和被[魅惑](Charmed_(status_group).md)或[麻痹](Paralysed_(Condition).md)的[豁免检定](Saving_throws.md "Saving throws")具有[优势](Advantage.md "Advantage")。

- [矮人活力](Dwarven_Resilience.md "矮人活力")

对被[中毒](Poisoned_(Condition).md)的[豁免检定](Saving_throw.md "Saving throw")具有[优势](Advantage.md "Advantage")，并对[中毒](Poison.md "Poison")伤害具有[抗性](Resistance.md "Resistance")。

- [额外攻击](Extra_Attack.md "额外攻击")

在徒手或武器攻击后可以进行一次额外攻击。

- [快速移动](Fast_Movement.md "快速移动")

当不穿着[重甲](Heavy_Armour.md "Heavy Armour")时，你的[移动速度](Movement_speed.md "Movement Speed")增加 3 米 / 10 英尺。

- [背水一战](Last_Stand.md "背水一战")

在战斗中，当穿戴者在其回合开始时生命值为 50% [生命值](Hit_Points.md "Hit Points")或更低时，他们获得[怒火](Wrath_(Condition).md "Wrath (Condition)")，持续 2 回合。

- [借机攻击](Opportunity_Attack.md "借机攻击")

攻击离开攻击范围的敌人。

- [日照敏感](Sunlight_Sensitivity.md "日照敏感")

在阳光下时，攻击掷骰和[察觉技能](Perception.md "Perception")检定具有劣势。

- [高级黑暗视觉](Superior_Darkvision.md "高级黑暗视觉")

可以在黑暗中看到 24 米 / 80 英尺。

- [**韧性**](Tenacity_(passive_feature).md "Tenacity (passive feature)")

近战攻击未命中时，仍会造成等于[力](STR.md "力")调整值或最小值为 1 的[钝击](Bludgeoning.md "Bludgeoning")伤害。

## 技术

###

UID

`S_UND_DuergarRebelGuard_03` ###

UUID

`baac0ae3-7b2e-47e6-85e4-579f70d4b4fa` ###

Stats

`UND_KC_Thudd` **萨德**是一名[烈焰氏族](Clan_Flameshade.md "Clan Flameshade")灰矮人野蛮人，可以在[复仇之炉](Grymforge.md "Grymforge")的[第一幕](Act_One.md "Act_One")中找到。

> “
>
> 真魂者把自己困住了。让他窒息吧，如果至上真神救不了他。
>
> „

— 萨德

## 目录

- [1 登场](#登场)
  - [1.1 第一幕](#第一幕)
- [2 战斗](#战斗)
  - [2.1 攻击与能力](#攻击与能力)
  - [2.2 战利品](#战利品)
- [3 相关任务](#相关任务)
- [4 图库](#图库)

## 登场

### 第一幕

萨德忠于烈焰氏族。如果与[布里特瓦尔](Brithvar.md "Brithvar")结盟，他会与小队一起对抗[尼讷](Nere.md "Nere")。

## 战斗

### 攻击与能力

[震荡猛击](Concussive_Smash.md "Concussive Smash") () – [巨棒](Greatclub.md "Greatclub")

伤害：4~11

1d8 + 3⁠[钝击](Bludgeoning.md "Bludgeoning")

全力击中敌人造成伤害，并可能使其[眩晕](Dazed_(Condition).md "Dazed (Condition)")。

[体质](Constitution.md "Constitution") [豁免检定](Saving_throws.md "Saving throws")
范围：正常武器范围
充能：[短休](Short_rest.md "Short rest")

[巨化](Enlarge.md "Enlarge") (+ )
使一个生物变大。其武器造成额外 **1d4** 伤害。其在[力量](Strength.md "Strength") [属性检定](Ability_Check.md "属性检定")和[豁免检定](Saving_throw.md "Saving throw")上具有[优势](Advantage.md "Advantage")。

范围：9 米 (30 英尺)

[主手攻击](Main_Hand_Attack.md "Main Hand Attack") () – [巨棒](Greatclub.md "Greatclub")

伤害：4~11

1d8 + 3⁠[钝击](Bludgeoning.md "Bludgeoning")

用装备的武器进行近战攻击。

近战：1.5 米 (5 英尺)

[狂暴](Rage.md "Rage") (+ )

近战和即兴武器以及投掷造成 **2** 额外伤害。

获得对物理伤害的[抗性](Resistance.md "Resistance")，并在力量[检定](Checks.md "Checks")和[豁免检定](Saving_throws.md "Saving Throws")上具有[优势](Advantage.md "Advantage")。

范围：自身

[鲁莽攻击](Reckless_Attack.md "Reckless Attack") ()

伤害：

正常武器伤害

不顾一切地获得近战[攻击掷骰](Attack_roll.md "Attack Roll")的[优势](Advantage.md "Advantage")。

直到你的下一回合，你在攻击掷骰上具有优势，但敌人在对你进行攻击掷骰时也具有优势。

范围：正常武器范围

[韧性](Tenacity.md "Tenacity") () – [巨棒](Greatclub.md "Greatclub")

伤害：3

3⁠[钝击](Bludgeoning.md "Bludgeoning")

当你攻击未命中时，仍会造成等于你的[力量](Strength.md "Strength") [调整值](Ability_Scores.md#Ability_score_modifiers "Ability scores")的[钝击](Bludgeoning.md "Bludgeoning")伤害（最小值为 1）。

[投掷](Throw.md "Throw") () – [标枪](Javelin.md "Javelin")

伤害：4~9

1d6 + 3⁠[穿刺](Piercing.md "Piercing")

投掷世界或你物品栏中的角色或物品。

你的[力量](Strength.md "Strength")影响你能投掷的重量。更重的物品造成更多伤害。

具有[投掷攻击](Thrown.md "投掷攻击")属性的武器的伤害与该武器的近战伤害相同。

范围：3-18 米 (10-60 英尺)

### 战利品

- [狂怒头盔](Cap_of_Wrath.md "Cap of Wrath")
- [巨棒](Greatclub.md "Greatclub")
- [标枪](Javelin.md "Javelin") - 2x（如果未使用）

## 相关任务

- ⁠[除掉至上真神的眼线](Blind_the_Absolute.md "Blind the Absolute")
- ⁠[交送尼讷的头](Deliver_Nere's_Head.md "Deliver Nere's Head")
- ⁠[救出真魂者尼讷](Free_True_Soul_Nere.md "Free True Soul Nere")
- ⁠[解救月出之塔的侏儒](Rescue_the_Gnomes_in_Moonrise_Towers.md "Rescue the Gnomes in Moonrise Towers")
- ⁠[拯救复仇之炉的侏儒们](Save_the_Grymforge_Gnomes.md "Save the Grymforge Gnomes")

## 图库

- 游戏内模型

---
*Source: [Thudd](https://bg3.wiki/wiki/Thudd)*