# 狂野魔法：传送 (术士)

本文介绍的是狂野魔法术士使用的版本。关于狂野魔法野蛮人版本，请参见 [狂野魔法：传送 (野蛮人)](Wild_Magic_colon__Teleport_(Barbarian)..md)

**狂野魔法：传送** 是 [狂野魔法术士](Wild_Magic_Sorcerer.md "狂野魔法术士") 的一个 [自由动作](Actions.md#Resources "动作")，当被 [狂野魔法](Wild_Magic_(passive_feature).md "狂野魔法 (被动特性)") 掷骰触发时随机使用；它允许他们通过一个 [附赠动作](Actions.md#Resources "动作") [传送](Misty_Step.md "迷踪步") 到附近的地点。

## 描述

直到你的下一个 [驱散](Actions.md#Resources "动作") 结束，你可以使用一个 [附赠动作](Actions.md#Resources "动作") 传送至多 9 米 (30 英尺)。

## 属性

详情
目标：自身

## 技术细节

UID

`Shout_WildMagic_Teleport`

## 状态：狂野魔法：传送 (术士)

**\_(状态)[狂野魔法：传送](Wild_Magic_colon__Teleport_(Sorcerer)_(Condition).md "狂野魔法：传送 (术士) (状态)")**

持续时间：2 驱散

- 可以使用一个 [附赠动作](Actions.md#Resources "动作") 传送至多 9 米 (30 英尺)。

## 如何学习

职业：

- 职业等级 1：[狂野魔法术士](Wild_Magic_Sorcerer.md "狂野魔法术士")

---
*Source: [Wild Magic: Teleport (Sorcerer)](https://bg3.wiki/wiki/Wild_Magic:_Teleport_(Sorcerer)*