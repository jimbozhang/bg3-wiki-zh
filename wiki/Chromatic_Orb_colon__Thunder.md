# 繁彩球：雷鸣

**繁彩球：雷鸣** 是一个 [等级1 塑能学派法术](Spells.md "Spells")。此法术是 [繁彩球](Chromatic_Orb.md "Chromatic Orb") 的一个变体。它允许施法者投掷一个雷鸣球，造成雷鸣伤害。

## 描述

投掷一个能量球，造成 3d8⁠⁠[雷鸣](Thunder.md "Thunder") 伤害。

## 属性

消耗
[动作](Actions.md#Resources "Actions") + [等级1 法术位](Spells.md#Spell_slots "Spells")
伤害: 3~24

3d8⁠[雷鸣](Thunder.md "Thunder")

详情
[攻击掷骰](Attack_roll.md "Attack Roll")
射程: 18 米 (60 英尺)

## 升环施法

- [升环施法](Upcasting.md "Upcasting")：以更高的法术位等级施放此法术时，每比1环高一环，额外造成 1d8⁠⁠[雷鸣](Thunder.md "Thunder") 伤害。

## 技术细节

UID

`Projectile_ChromaticOrb_Thunder`

法术标志

`[HasHighGroundRangeExtension](HasHighGroundRangeExtension_(spell_flag).md)`, `[HasSomaticComponent](HasSomaticComponent_(spell_flag).md)`, `[HasVerbalComponent](HasVerbalComponent_(spell_flag).md)`, `[IsHarmful](IsHarmful_(spell_flag).md)`, `[IsSpell](IsSpell_(spell_flag).md)`, `[RangeIgnoreVerticalThreshold](https://bg3.wiki/w/index.php?title=RangeIgnoreVerticalThreshold_\(spell_flag\)&action=edit&redlink=1) "RangeIgnoreVerticalThreshold \(spell flag\) \(page does not exist\)")`

## 如何习得

此法术是以下法术的变体：
[繁彩球](Chromatic_Orb.md "Chromatic Orb")

## 备注

- 这是繁彩球唯一一个不会为额外效果创建地表的变体。相反，它只是额外造成 1d8 伤害。

## 视觉效果

加载视频

本地文件

本地文件可能会收集个人数据。

继续 关闭

<https://bg3.wiki/wiki/File:Chromatic_Orb_Visuals.mp4>

---
*Source: [Chromatic Orb: Thunder](https://bg3.wiki/wiki/Chromatic_Orb:_Thunder)*