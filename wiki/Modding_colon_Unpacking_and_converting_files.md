...
- LSLIB by Norbyte
- _注意：此步骤主要是为了从旧版LSLIB获取 **granny2.dll**。最新发布版本可能已包含此文件，如果是这样，您可以直接安装最新版本。_
  - 首先安装 [1.15.13](https://github.com/Norbyte/lslib/releases/tag/v1.15.13)，然后安装任意 [最新](https://github.com/Norbyte/lslib/releases) 版本。
...
**该工具有一个官方Wiki，现在详细说明了该工具的解包和其他部分：[BG3 Modders Multitool Wiki](https://github.com/ShinyHobo/BG3-Modders-Multitool/wiki)**

请使用该Wiki获取最新信息。

现在来看ExportTool。

此项目需要 **granny2.dll** 文件才能正常工作。由于许可问题，Norbyte在ExportTool的某些后期版本中移除了此文件，但它存在于旧版本中，例如v1.15.13，链接如上。

验证 **granny2.dll** 文件是否存在于ExportTool的 **根文件夹** 和 **Tools** 子文件夹中。如果文件在这两个文件夹中都存在，则无需对ExportTool进行任何其他操作，您可以跳到本指南的下一节。

如果最新版本的ExportTool的 **根文件夹** 和/或 **Tools** 子文件夹中缺少 **granny2.dll** 文件，则需要从旧版本v1.15.13复制该文件。打开存档 **ExportTool-v1.15.13.zip** ，复制 **granny2.dll** 文件，并将其粘贴到您解压最新版本ExportTool的 **根文件夹** 和 **Tools** 子文件夹中。无需对ExportTool进行任何其他操作。

要运行ExportTool，请打开ConverterApp.exe。

## 解包过程

目前，由于Multitool的更新，我们不再需要直接解包游戏pak文件。相反，您可以索引文件并按需解包文件。这可以节省一些硬盘空间，因为BG3有很多文件。

### 索引

**[Multitools关于索引和搜索的Wiki帖子](https://github.com/ShinyHobo/BG3-Modders-Multitool/wiki/Index-Search)**

在Multitool中，转到 工具 > 索引 > 索引Pak文件，这将索引所有已解包的pak文件。比解包所有pak文件更快、更易于维护。

- 当我们有新的热修复时，请按照上述步骤再次索引新的热修复pak。

但是，每当有完整补丁发布时，建议先清除索引并使用以下方法重新索引：工具 > 索引 > 清除索引，然后工具 > 索引 > 索引Pak文件

如果您仍然想手动解包游戏pak，而不是通过新方法解包单个文件，可以使用：**工具** > **游戏文件操作** > **解包游戏文件**。

- - 从补丁4开始，当新补丁发布时，热修复pak会被合并到主pak中。

#### 索引搜索解包

如果您知道要解包资源的通用名称，可以在索引完成后使用索引搜索来解包特定资源。例如，假设我们要创建头部模组，因此需要从资源中解包头部模型。

这里我们使用过滤器仅显示GR2资源，如下所示：

然后我们搜索NKD_Head，如下所示：

现在我们选择全部并提取所选内容。

请注意，这将提取的不仅仅是我们可以扮演的种族，但如果我们的目标只是创建头部模组，那么这比提取整个models.pak占用的空间要少得多。您也可以使用此功能浏览列表以选择要提取的特定资源。这不仅限于GR2，只需选择要过滤的资源，然后提取即可。
...
- 通常，只有当您有足够的硬盘空间专门用于解包文件时才建议这样做，因为它们可能会占用相当多的空间。
- 将出现一个类似于以下内容的菜单：
  - （从补丁4开始，当新补丁发布时，热修复包会被合并到主pak中）

常见文件：

- English.pak — 本地化文件所在位置，即游戏中显示的所有文本字符串。
- Gustav.pak — .lsf格式的文本文件。关卡数据、对话文件、装备属性、法术。
- Materials.pak — 材质（着色器）。它本身没有用，但您可以查看游戏中有哪些材质以将其分配给您的模型。
- Models.pak — .gr2格式的模型。头部、头发、护甲、建筑等。
- Textures.pak — .dds格式的纹理。这里没有太多纹理，因为大多数纹理都保存在虚拟纹理中。这些大多是颜色ID遮罩，用于将护甲划分为着色区域。但也有面部、头发、纹身的纹理。一些环境纹理。
...
来自 **Models.pak** 的模型（网格）文件。这些可以使用ExportTool转换。在GR2 Tools程序的第一个选项卡中指定所需 `.gr2` 文件的路径，以及下方转换后文件的保存路径。**!!!** 您必须在名称后添加 `.dae` **!!!**

首先单击导入按钮导入游戏 `.gr2` 模型，然后导出以将其转换为 `.dae` 格式。`.dae` 可以使用任何3D编辑器打开。

您也可以直接将 `.gr2` 导入 Blender 3.x.x+。Norbyte更新了他们的导入和导出模型插件，最新版本可以在这里找到 [here](https://github.com/Norbyte/dos2de_collada_exporter)。在插件设置中，我们指定ExportTool的 **divine.exe** 路径。

现在您可以直接将 `.gr2` 导入 Blender。

### .dds

来自 **Textures.pak** 的纹理文件。这些可以通过安装Nvidia网站的 [Adobe Photoshop插件](https://developer.nvidia.com/nvidia-texture-tools-exporter) 来打开。对于没有Photoshop的用户，免费的 [Gimp](https://www.gimp.org/downloads/) 从2.10.10版本开始支持.dds。[Paint.net](https://www.getpaint.net/) 也可以打开.dds文件，并且是免费的。
...
来自 **Voice.pak** 的音频文件。对于 `.wem`，您需要来自Nexus Mods的 [Automatic .wem to .ogg Converter](https://www.nexusmods.com/monsterhunterworld/mods/1912?tab=description) 脚本。它是为Monster Hunter: World制作的，但与BG3文件完美兼容。存档内有一个文件夹 WEM to OGG Converter。您需要将其中的文件放入 .wem 文件所在的文件夹并运行 `script.bat`。脚本将自动将所有 `.wem` 转换为 `.ogg` 文件，这些文件可以用标准媒体播放器打开。
