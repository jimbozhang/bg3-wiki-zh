# 弗莱德

## 弗莱德

清洁助理

- 游戏内
- 模型
- 肖像画

### 全名

弗莱德

### 声音 / 演员

- [詹姆斯·布拉德威尔](https://www.imdb.com/name/nm10525457/)

### 阵营

[终焉马戏团](Circus_of_the_Last_Days.md "终焉马戏团")

## 属性

### 体型

小型

### 类型

[妖精](Fey.md "妖精")

### 种族

[红帽子](Redcap.md "红帽子")

- 平衡
- 探索者
- 硬核

## 平衡难度属性

### 等级

4

### 生命值

45

### 护甲等级

14

### 先攻

+1

### 熟练项

+2

### 移动

9 米 (30 英尺)

### 重量

35 千克 (70 磅)

### 高级黑暗视觉

18 米 (60 英尺)

### 经验值奖励

200

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 18 / (+4) | 13 / (+1) | 18 / (+4) | 10 / (0) | 12 / (+1) | 9 / (-1) |

## 特性

- [借机攻击](Opportunity_Attack.md "借机攻击")

攻击离开触及范围的敌人。

- [沉浸杀戮](Steeped_in_Slaughter.md "沉浸杀戮")

对[流血](Bleeding_(Condition).md "流血 (状态)")的敌人进行[攻击掷骰](Attack_rolls.md "攻击掷骰")时具有[优势](Advantage.md "优势")。

- [高级黑暗视觉](Darkvision.md "高级黑暗视觉")

可以在黑暗中看到一定距离。

- [**多重攻击：嗜血**](Multiattack_colon__Bloodlust.md "多重攻击：嗜血")

红帽子在[嗜血](Bloodlust_(Condition).md "嗜血 (状态)")效果下可以进行[多重攻击](Multiattack_(Redcap).md "多重攻击 (红帽子)")。

## 探索者难度属性

### 等级

4

### 生命值

31

### 护甲等级

14

### 先攻

+1

### 熟练项

+2

### 移动

9 米 (30 英尺)

### 重量

35 千克 (70 磅)

### 高级黑暗视觉

18 米 (60 英尺)

### 经验值奖励

200

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | +4 | +1 | +4 | 0 | +1 | -1 |

## 特性

- [借机攻击](Opportunity_Attack.md "借机攻击")

攻击离开触及范围的敌人。

- [沉浸杀戮](Steeped_in_Slaughter.md "沉浸杀戮")

对[流血](Bleeding_(Condition).md "流血 (状态)")的敌人进行[攻击掷骰](Attack_rolls.md "攻击掷骰")时具有[优势](Advantage.md "优势")。

- [高级黑暗视觉](Darkvision.md "高级黑暗视觉")

可以在黑暗中看到一定距离。

- [**多重攻击：嗜血**](Multiattack_colon__Bloodlust.md "多重攻击：嗜血")

红帽子在[嗜血](Bloodlust_(Condition).md "嗜血 (状态)")效果下可以进行[多重攻击](Multiattack_(Redcap).md "多重攻击 (红帽子)")。

## 硬核难度属性

### 等级

4

### 生命值

58

### 护甲等级

14

### 先攻

+1

### 熟练项

+2

### 移动

9 米 (30 英尺)

### 重量

35 千克 (70 磅)

### 高级黑暗视觉

18 米 (60 英尺)

### 经验值奖励

200

| 属性值 | 运动 | 体操 | 察觉技能 |
| --- | --- | --- | --- |
|  | 6 | 1 | 3 |

## 特性

- [借机攻击](Opportunity_Attack.md "借机攻击")

攻击离开触及范围的敌人。

- [沉浸杀戮](Steeped_in_Slaughter.md "沉浸杀戮")

对[流血](Bleeding_(Condition).md "流血 (状态)")的敌人进行[攻击掷骰](Attack_rolls.md "攻击掷骰")时具有[优势](Advantage.md "优势")。

- [高级黑暗视觉](Darkvision.md "高级黑暗视觉")

可以在黑暗中看到一定距离。

- [**多重攻击：嗜血**](Multiattack_colon__Bloodlust.md "多重攻击：嗜血")

红帽子在[嗜血](Bloodlust_(Condition).md "嗜血 (状态)")效果下可以进行[多重攻击](Multiattack_(Redcap).md "多重攻击 (红帽子)")。

## 技术信息

###

UID

`S_WYR_Circus_RedcapCleaner_001` ###

UUID

`f76cc3eb-dacc-4a85-9b0f-d119d3292c74` ###

Stats

`RedCap` **弗莱德**是受雇于[终焉马戏团](Circus_of_the_Last_Days.md "终焉马戏团")的[红帽子](Redcap.md "红帽子")，在[第三幕](Act_Three.md "第三幕")担任清洁工。

> “
>
> 呸！肮脏的球袋——让开！我得打扫了。
>
> „

— 弗莱德

## 战斗

### 攻击与能力

[铁意追击](Ironbound_Pursuit.md "铁意追击") ()

伤害：7~28

3d8 + 4⁠[钝击](Bludgeoning.md "钝击")

发动一记飞踢，将目标击退 2 米 / 7 英尺并使其[倒伏](Prone_(Condition).md "倒伏 (状态)")。

[敏捷](Dexterity.md "敏捷") [豁免检定](Saving_throws.md "豁免检定")
近战：1.5 米 (5 英尺)

[主手攻击](Main_Hand_Attack.md "主手攻击") () – [镰刀](Sickle.md "镰刀")

伤害：5~8

1d4 + 4⁠[挥砍](Slashing.md "挥砍")

用装备的武器进行近战攻击。

近战：1.5 米 (5 英尺)

仅在[嗜血](Bloodlust_(Condition).md "嗜血 (状态)")效果下可用：

[多重攻击](Multiattack_(Redcap).md "多重攻击 (红帽子)") ()

伤害：

正常武器伤害

\+ 正常武器伤害

\+ 正常武器伤害

对一个目标进行多次攻击

近战：1.5 米 (5 英尺)
充能：每回合

---
*Source: [Fred](https://bg3.wiki/wiki/Fred)*