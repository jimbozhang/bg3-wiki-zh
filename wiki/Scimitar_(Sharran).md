# 莎尔弯刀 (Sharran Scimitar)

**莎尔弯刀**是[弯刀](Scimitars.md "Scimitars")武器家族的一种常见变体。它是一种单手挥舞的[武器](Weapons.md "武器")。作为[灵巧](Finesse.md "Finesse")武器，它可以从使用者的**敏捷**中获益，而不仅仅是**力量**。它是一种[轻型](Light_(weapon_property).md) 武器，任何人都无需特殊训练即可双持。

## 属性

伤害

1d8 (1~8) + [力量或敏捷调整值](Damage_Roll.md#Modifiers "伤害掷骰") ⁠[挥砍](Slashing.md "挥砍")

详情
[弯刀](Scimitars.md "Scimitars")
稀有度：普通
惑控学派：无
单手持用
[灵巧](Finesse.md "Finesse")
[轻型](Light_(weapon_property).md "轻型（武器属性）")
近战：1.5 米（5 英尺）
重量：1.35 千克（2.7 磅）
价格：30 金币
UID `WPN_HUM_Sword_Shar_A` UUID `afdc3411-7dad-4483-8ee1-e6a8f37b3ee4` ### 武器动作

_如果你拥有[熟练项](Proficiency.md "熟练项")，装备在**主手**可获得：_

[华舞](Flourish.md "华舞")
佯攻以可能使你的对手[失衡](Off_Balance_(Condition).md "失衡（状态）"). (充能：[短休](Short_rest.md "短休").)

[割裂](Lacerate.md "割裂")
劈砍目标的要害点使其[流血](Bleeding_(Condition).md "流血（状态）"). (充能：[短休](Short_rest.md "短休").)

## 获取地点

- 由[哀伤之邸](House_of_Grief.md "哀伤之邸")中的数名莎尔信徒携带
- 由部分[审判复仇者](Justiciar_Avenger.md "审判复仇者")携带

## 备注

- 值得注意的是，这些弯刀的基础伤害骰为 1d8，而其他弯刀通常为 1d6。

---
*Source: [Scimitar (Sharran)](https://bg3.wiki/wiki/Scimitar_(Sharran)*