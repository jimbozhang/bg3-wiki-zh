# 染血之刃

本文介绍的内容**[无法通过正常游戏](Category_colon_Inaccessible.md "Category:Inaccessible")**获取，但仍可通过第三方工具访问。

染血之刃是一把[刺剑](Rapier.md "刺剑")[刺剑](Rapier.md "刺剑")，每击杀一名敌人（最多3名），即可提升[攻击掷骰](Attack_roll.md "攻击掷骰")、伤害和[法术](Spell.md "法术")豁免DC。

鲜血在动荡的时代中，对人们施加着迷人的统治。其酒红色的污渍甚至能为和平主义者施以膏抹。

## 属性

伤害

1d8 + 1 (2~9) + [力量或敏捷调整值](Damage_Roll.md#Modifiers "伤害掷骰") ⁠[穿刺](Piercing.md "穿刺")

详情
[刺剑](Rapiers.md "刺剑")
稀有度：稀有
惑控学派：**+ 1**
单手
[灵巧](Finesse.md "灵巧")
[可蘸取](Dippable.md "可蘸取")
近战：1.5 米 (5 英尺)
重量：0.9 千克 (1.8 磅)
价格：gp
UID `MAG_Zhentarim_BloodfeederBlade_Rapier` UUID `a8dc9927-1aa3-45ee-8fea-4106e431c194` ### 特殊

**持有此物品者获得：**

- **[猩红再生](Scarlet_Regeneration.md "猩红再生")：** 如果你拥有来自[猩红叠加](Scarlet_Remittance_+3_(Condition).md "猩红叠加 +3 (状态)")的 +3 奖励，则消耗它以恢复 3d10⁠⁠[治疗](Healing.md "治疗")（充能：[长休](Long_Rest.md "长休")）。

[猩红叠加](Scarlet_Remittance.md "猩红叠加")
每击杀一名敌人，获得 +1 奖励至[攻击掷骰](Attack_roll.md "攻击掷骰")、伤害和[法术](Spell.md "法术")豁免DC，最高可达 +3。

### 武器动作

_如果你拥有[熟练项](Proficiency.md "熟练项")，装备在**主手**以获得：_

[华舞](Flourish.md "华舞")
佯攻以可能使对手[失衡](Off_Balance_(Condition).md "失衡 (状态)")。（充能：[短休](Short_rest.md "短休")。）

[穿刺打击](Piercing_Strike.md "穿刺打击")
刺击敌人并可能造成[开放伤口](Gaping_Wounds_(Condition).md "开放伤口 (状态)")。（充能：[短休](Short_rest.md "短休")。）

[弱化打击](Weakening_Strike.md "弱化打击")
以非致命攻击瞄准敌人的手部，可能造成[弱腕](Weak_Grip_(Condition).md "弱腕 (状态)")。（充能：[短休](Short_rest.md "短休")。）

## 注释

_关于猩红叠加：_

- 自补丁 5 起，猩红叠加仅在击杀存活的敌对目标时生效。此前，此特性会触发于任何拥有生命条的物体，包括板条箱和小袋子等。
- 当被动特性被卸下时，持有带有猩红叠加的武器的生物将失去该奖励。

---
*Source: [Sanguine Blade](https://bg3.wiki/wiki/Sanguine_Blade)*