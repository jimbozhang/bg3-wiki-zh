# 斯库娜

## 执法者 斯库娜

- 游戏内
- 肖像画

### 身份

女性

### 声优 / 演员

- [Jacqui Bardelang](https://www.imdb.com/name/nm11624408/)

### 家乡

[博德之门](Baldur's_Gate.md "Baldur's Gate")

### 职业

[精灵之歌酒馆](Elfsong_Tavern.md "Elfsong Tavern")的保镖

## 属性

### 体型

[中级](Medium.md "Medium")

### 类型

[类人生物](Humanoid.md "Humanoid")

### 种族

[半兽人](Half-Orc.md "Half-orc")

### 职业

[战士](Fighter.md "Fighter")

### 副职

[冠军勇士](Champion.md "Champion")

- 平衡

- 探索者

- 硬核

## 平衡难度属性

### 等级

11

### 生命值

86

### 护甲等级

19

### 先攻

0

### 熟练项

+4

### 移动

9 米 (30 英尺)

### 重量

100 千克 (200 磅)

### 高级黑暗视觉

18 米 (60 英尺)

### 经验值奖励

320

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 20 / (+5) | 10 / (0) | 16 / (+3) | 10 / (0) | 10 / (0) | 10 / (0) |

## 特性

- [高级黑暗视觉](Darkvision.md "高级黑暗视觉")

能在黑暗中看到一定距离。

- [防御](Defence.md "防御")

穿着[护甲](Armour.md "Armour")时，[护甲等级](AC.md "AC") +1。

- [双持客](Dual_Wielder.md "双持客")

即使使用非[轻型](Light_(weapon_property)的单手武器.md)，也能使用双武器战斗。

- [**双持客：附赠护甲等级**](Dual_Wielder_colon__Bonus_Armour_Class.md "Dual Wielder: Bonus Armour Class")

双持时，[护甲等级](AC.md "AC") +1。

- [额外攻击](Extra_Attack.md "额外攻击")

在徒手或武器攻击后，可以进行一次额外攻击。

- [精通重击](Improved_Critical_Hit.md "精通重击")

攻击时，需要掷出[重击](Critical_Hit.md "Critical Hit")的数值降低 1。此效果可叠加。

- [精通额外攻击](Improved_Extra_Attack.md "精通额外攻击")

主手武器攻击后，可以再进行两次攻击。

- [借机攻击](Opportunity_Attack.md "借机攻击")

攻击离开攻击范围的敌人。

- [坚韧不屈](Relentless_Endurance.md "坚韧不屈")

生命值降至 0 时，恢复 1 点生命值，每[长休](Long_Rest.md "Long rest")一次。

- [凶蛮打击](Savage_Attacks.md "凶蛮打击")

近战[重击](Critical_Hit.md "Critical hit")命中时，额外掷一个伤害骰。

- [**精良材质**](Superior_Material_(-1).md "Superior Material (-1)")

受到的[挥砍](Slashing.md "Slashing")伤害减少 1 点。

- [双武器战斗](Two-Weapon_Fighting.md "双武器战斗")

进行副手攻击时，可以将你的[属性值调整值](Ability_score_modifier.md "Ability Score Modifier")加到攻击伤害上。

## 探索者难度属性

### 等级

11

### 生命值

60

### 护甲等级

19

### 先攻

0

### 熟练项

+6

### 移动

9 米 (30 英尺)

### 重量

100 千克 (200 磅)

### 高级黑暗视觉

18 米 (60 英尺)

### 经验值奖励

320

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | +9 | 0 | +7 | 0 | 0 | 0 |

## 特性

- [高级黑暗视觉](Darkvision.md "高级黑暗视觉")

能在黑暗中看到一定距离。

- [防御](Defence.md "防御")

穿着[护甲](Armour.md "Armour")时，[护甲等级](AC.md "AC") +1。

- [双持客](Dual_Wielder.md "双持客")

即使使用非[轻型](Light_(weapon_property)的单手武器.md)，也能使用双武器战斗。

- [**双持客：附赠护甲等级**](Dual_Wielder_colon__Bonus_Armour_Class.md "Dual Wielder: Bonus Armour Class")

双持时，[护甲等级](AC.md "AC") +1。

- [额外攻击](Extra_Attack.md "额外攻击")

在徒手或武器攻击后，可以进行一次额外攻击。

- [精通重击](Improved_Critical_Hit.md "精通重击")

攻击时，需要掷出[重击](Critical_Hit.md "Critical Hit")的数值降低 1。此效果可叠加。

- [精通额外攻击](Improved_Extra_Attack.md "精通额外攻击")

主手武器攻击后，可以再进行两次攻击。

- [借机攻击](Opportunity_Attack.md "借机攻击")

攻击离开攻击范围的敌人。

- [坚韧不屈](Relentless_Endurance.md "坚韧不屈")

生命值降至 0 时，恢复 1 点生命值，每[长休](Long_Rest.md "Long rest")一次。

- [凶蛮打击](Savage_Attacks.md "凶蛮打击")

近战[重击](Critical_Hit.md "Critical hit")命中时，额外掷一个伤害骰。

- [**精良材质**](Superior_Material_(-1).md "Superior Material (-1)")

受到的[挥砍](Slashing.md "Slashing")伤害减少 1 点。

- [双武器战斗](Two-Weapon_Fighting.md "双武器战斗")

进行副手攻击时，可以将你的[属性值调整值](Ability_score_modifier.md "Ability Score Modifier")加到攻击伤害上。

## 硬核难度属性

### 等级

11

### 生命值

111

### 护甲等级

19

### 先攻

0

### 熟练项

+4

### 移动

9 米 (30 英尺)

### 重量

100 千克 (200 磅)

### 高级黑暗视觉

18 米 (60 英尺)

### 经验值奖励

320

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 20 / (+5) | 10 / (0) | 16 / (+3) | 10 / (0) | 10 / (0) | 10 / (0) |

## 特性

- [高级黑暗视觉](Darkvision.md "高级黑暗视觉")

能在黑暗中看到一定距离。

- [防御](Defence.md "防御")

穿着[护甲](Armour.md "Armour")时，[护甲等级](AC.md "AC") +1。

- [双持客](Dual_Wielder.md "双持客")

即使使用非[轻型](Light_(weapon_property)的单手武器.md)，也能使用双武器战斗。

- [**双持客：附赠护甲等级**](Dual_Wielder_colon__Bonus_Armour_Class.md "Dual Wielder: Bonus Armour Class")

双持时，[护甲等级](AC.md "AC") +1。

- [额外攻击](Extra_Attack.md "额外攻击")

在徒手或武器攻击后，可以进行一次额外攻击。

- [精通重击](Improved_Critical_Hit.md "精通重击")

攻击时，需要掷出[重击](Critical_Hit.md "Critical Hit")的数值降低 1。此效果可叠加。

- [精通额外攻击](Improved_Extra_Attack.md "精通额外攻击")

主手武器攻击后，可以再进行两次攻击。

- [借机攻击](Opportunity_Attack.md "借机攻击")

攻击离开攻击范围的敌人。

- [坚韧不屈](Relentless_Endurance.md "坚韧不屈")

生命值降至 0 时，恢复 1 点生命值，每[长休](Long_Rest.md "Long rest")一次。

- [凶蛮打击](Savage_Attacks.md "凶蛮打击")

近战[重击](Critical_Hit.md "Critical hit")命中时，额外掷一个伤害骰。

- [**精良材质**](Superior_Material_(-1).md "Superior Material (-1)")

受到的[挥砍](Slashing.md "Slashing")伤害减少 1 点。

- [双武器战斗](Two-Weapon_Fighting.md "双武器战斗")

进行副手攻击时，可以将你的[属性值调整值](Ability_score_modifier.md "Ability Score Modifier")加到攻击伤害上。

## 技术数据

###

UID

`S_LOW_Elfsong_Skoona` ###

UUID

`9a4e3f08-739f-4576-a50c-e051ce5126d1` ###

Stats

`LOW_Skoona` **执法者 斯库娜**是[半兽人](Half-orc.md "Half-orc")<sup>[\[1\]](#cite_note-1)</sup> [精灵之歌酒馆](Elfsong_Tavern.md "Elfsong Tavern")的员工，位于[博德之门](Baldur's_Gate.md "Baldur's Gate")的[下城区](Lower_City.md "Lower City")。

## 目录

- [1 出场](#出场)
- [2 战斗](#战斗)
  - [2.1 攻击与能力](#攻击与能力)
  - [2.2 可观战利品](#可观战利品)
- [3 闲谈](#闲谈)
- [4 注释](#注释)
- [5 外部链接](#外部链接)

## 出场

斯库娜在酒馆前门和厨房后门之间巡逻。

## 战斗

### 攻击与能力

[主手攻击](Main_Hand_Attack.md "Main Hand Attack") () – [弯刀 +1](Scimitar_+1.md "Scimitar +1")

伤害：7~12

1d6 + 6[挥砍](Slashing.md "Slashing")

用装备的武器进行近战攻击。

近战：1.5 米 (5 英尺)

[副手攻击（近战）](Off-Hand_Attack_(Melee).md "Off-Hand Attack (Melee)") () – [弯刀 +1](Scimitar_+1.md "Scimitar +1")

伤害：7~12

1d6 + 6[挥砍](Slashing.md "Slashing")

用副手武器进行近战攻击。

近战：1.5 米 (5 英尺)

[鲁莽攻击](Reckless_Attack.md "Reckless Attack") ()

伤害：

正常武器伤害

孤注一掷，获得[优势](Advantage.md "Advantage")进行近战[攻击掷骰](Attack_roll.md "Attack Roll")。

直到你的下个回合，你在攻击掷骰上具有优势，但敌人在攻击你时也具有优势。

射程：正常武器射程

### 可观战利品

- [链甲 +1](Chain_Mail_+1.md "Chain Mail +1")
- [手斧](Handaxe.md "Handaxe") - x2
- [弯刀 +1](Scimitar_+1.md "Scimitar +1") - x2

## 闲谈

- 斯库娜
  凭[格乌什](Gruumsh.md "Gruumsh")之名！这些顾客看起来一天比一天邋遢。

- 斯库娜
  休息时间一到，我就要去和厨房的猫玩，我才不管里沃<sup>[\[2\]](#cite_note-2)</sup>怎么说。

## 注释

1. [↑](#cite_ref-1) 斯库娜是《[博德之门：阿凡纳斯之陷](https://forgottenrealms.fandom.com/wiki/Baldur%27s_Gate:_Descent_into_Avernus)》中提及的角色，在那里她被描述为一个粗鲁的女性半食人魔（而不是游戏中的半兽人）。她在那里是为了保护其他工作人员，而不是顾客，并且除非工作人员卷入战斗，否则不会介入打架。
1. [↑](#cite_ref-2) 可能指的是[里沃](Roveer.md "Roveer")，这可能是有意为之，也可能是拉瑞安的笔误。

## 外部链接

- ⁠[斯库娜](https://forgottenrealms.fandom.com/wiki/Skoona) 在 [被遗忘的国度维基](https://forgottenrealms.fandom.com/wiki/Main_Page)

---
*Source: [Skoona](https://bg3.wiki/wiki/Skoona)*