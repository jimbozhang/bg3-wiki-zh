# 莎尔之刺

本文介绍的内容**[无法通过正常游戏](Category_colon_Inaccessible.md "Category:Inaccessible")**获取，但仍可通过第三方工具访问。

莎尔之刺是一把稀有度为**不常见**的+1 [匕首](Daggers.md "Daggers")，命中时额外造成1d4点[黯蚀](Necrotic.md "Necrotic")伤害。

刀刃锋利，握柄坚实。然而，这把匕首散发出的死亡气息，犹如一具新鲜的尸体。

## 属性

伤害

1d4 + 1 (2~5) + [力量或敏捷调整值](Damage_Roll.md#Modifiers "伤害掷骰") ⁠[穿刺](Piercing.md "穿刺")

额外伤害

1d4 (1~4) ⁠[黯蚀](Necrotic.md "黯蚀")

详情
[匕首](Daggers.md "Daggers")
稀有度：不常见
惑控学派：**+ 1**
单手
[灵巧](Finesse.md "Finesse")
[轻型](Light_(weapon_property).md "Light (weapon property)")
[投掷攻击](Thrown.md "Thrown")
[可蘸取](Dippable.md "Dippable")
近战：1.5 m (5 ft)
重量：0.45 kg (0.9 lb)
价格：65 gp
UID `TWN_SharDagger` UUID `7773610f-f246-4837-a75f-2d260f815718` Stats `TWN_SharDagger` ### 武器动作

_如果你拥有[熟练项](Proficiency.md "Proficiency")，装备在**主手**以获得：_

[穿刺打击](Piercing_Strike.md "Piercing_Strike")
刺击敌人并可能施加[开放伤口](Gaping_Wounds_(Condition).md "Gaping Wounds (Condition)")。（充能：[短休](Short_rest.md "Short rest")。）

## 备注

- 这把匕首与[莎尔的仪式匕首](Ritual_Dagger_of_Shar.md "Ritual Dagger of Shar")完全相同，只是名称不同。

---
*Source: [Shar's Sting](https://bg3.wiki/wiki/Shar's_Sting)*