# 防御华舞（远程）

另请参阅：[防御华舞（近战）](Defensive_Flourish_(Melee).md)

**防御华舞（远程）**是[剑舞学院](College_of_Swords.md "剑舞学院")吟游诗人的武器动作。此华舞策略允许吟游诗人在进行一次远程武器攻击后，花费[诗人激励](Bardic_Inspiration_(resource).md "诗人激励")进入防御姿态，使护甲等级提高4点。

## 描述

进行防御性攻击，若命中则使你的[护甲等级](Armour_Class.md "护甲等级")提高4点。

你必须装备远程武器。

## 属性

消耗
[动作](Actions.md#Resources "动作")
命中时消耗
[诗人激励](Bardic_Inspiration_(resource).md "诗人激励")
伤害：1~6

普通武器伤害

\+ 1d6⁠[武器](Weapon.md "武器")

详情
远程武器[攻击掷骰](Attack_roll.md "攻击掷骰")
射程：普通武器射程

## 更高等级

- 在5级时，拥有[精通诗人激励](Improved_Bardic_Inspiration_(passive_feature).md "精通诗人激励（被动特性）")，额外伤害提高至1d8。
- 在10级时，拥有[精通诗人激励](Improved_Bardic_Inspiration_(passive_feature).md "精通诗人激励（被动特性）")，额外伤害提高至1d10。

## 技术细节

UID

`Projectile_BladeFlourish_Defensive`

法术标志

`[HasHighGroundRangeExtension](HasHighGroundRangeExtension_(spell_flag).md)`, `[IsHarmful](IsHarmful_(spell_flag).md)`, `[RangeIgnoreVerticalThreshold](https://bg3.wiki/w/index.php?title=RangeIgnoreVerticalThreshold_\(spell_flag\)&action=edit&redlink=1) "RangeIgnoreVerticalThreshold \(spell flag\) \(page does not exist\)")`

## 变体

- [防御华舞（近战）](Defensive_Flourish_(Melee).md "防御华舞（近战）")

## 状态：防御华舞

**[防御华舞](Defensive_Flourish_(Condition).md "防御华舞（状态）")**

持续时间：1驱散

- [护甲等级](Armour_Class.md "护甲等级")提高4点

## 学习方式

职业：

- 职业等级3：[剑舞学院](College_of_Swords.md "剑舞学院")

---
*Source: [Defensive Flourish (Ranged)](https://bg3.wiki/wiki/Defensive_Flourish_(Ranged)*