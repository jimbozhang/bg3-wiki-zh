# 恶毒嘲笑 (埃赛尔婶婶)

另见：[恶毒嘲笑 (消歧义)](Vicious_Mockery_(disambiguation).md)

**恶毒嘲笑** 是一种 [戏法 (惑控学派)](Spells.md "Spells")。它是 [恶毒嘲笑](Vicious_Mockery.md "Vicious Mockery") 的强化变体，拥有更高的伤害和射程，并消耗一个 [附赠动作](Bonus_action.md "Bonus action")。队伍成员无法学习此法术。

## 描述

侮辱一个生物：其下一次 [攻击掷骰](Attack_roll.md "Attack Roll") 将具有 [劣势](Disadvantage.md "Disadvantage")。

## 属性

消耗
[附赠动作](Actions.md#Resources "Actions")
伤害：2~8

2d4⁠[心灵](Psychic.md "Psychic")

详情
[感知](Wisdom.md "Wisdom") [豁免检定](Saving_throws.md "Saving throws") ([法术豁免 DC](Dice_rolls.md#Spell_save_DC "Dice rolls"))
射程：26 米 (87 英尺)

## 技术细节

UID

`Target_HAG_ViciousMockery`

法术标志

`[HasVerbalComponent](HasVerbalComponent_(spell_flag).md)`, `[IsHarmful](IsHarmful_(spell_flag).md)`, `[IsSpell](IsSpell_(spell_flag).md)`

## 状态：恶毒嘲笑

**[恶毒嘲笑](Vicious_Mockery_(Condition).md "恶毒嘲笑 (状态)")**

持续时间：2 驱散

- 下一次 [攻击掷骰](Attack_roll.md "Attack Roll") 具有 [劣势](Disadvantage.md "Disadvantage")。

## 如何习得

由以下生物使用：[埃赛尔婶婶](Auntie_Ethel.md "Auntie Ethel")

## 备注

- 埃赛尔婶婶施放此法术时的 [侮辱性言论](List_of_Vicious_Mockery_insults.md "恶毒嘲笑言论列表") 与普通的 [恶毒嘲笑](Vicious_Mockery.md "Vicious Mockery") 台词不同，是专门针对目标身份的。这包括嘲笑目标的种族或 [出身](Origin.md "Origin") 背景。

---
*Source: [Vicious Mockery (Auntie Ethel)](https://bg3.wiki/wiki/Vicious_Mockery_(Auntie_Ethel)*