# 嘘！静默！

**嘘！静默！** 是由 [巫术破除](Witchbreaker.md "巫术破除") 赋予的 [武器动作](Weapon_action.md "武器动作")。

## 描述

劈砍目标并可能使其 [沉默](Silenced_(Condition).md "沉默（状态）")。

## 属性

消耗
[动作](Actions.md#Resources "动作")

伤害：

普通武器伤害

\+ [熟练项加值](Proficiency_Bonus.md "熟练项加值")⁠[挥砍](Slashing.md "挥砍")

详情
近战武器 [攻击掷骰](Attack_roll.md "攻击掷骰")
[体质](Constitution.md "体质") [豁免检定](Saving_throws.md "豁免检定")（[武器动作DC](Dice_rolls.md#Weapon_action_DC "掷骰") + 2）
近战：1.5 米（5 英尺）
充能：[短休](Short_rest.md "短休")
必须 [熟练](Proficiency.md#Weapon_proficiency "熟练项") 该武器

## 状态：沉默

**[沉默](Silenced_(Condition).md "沉默（状态）")**

持续时间：2 驱散

[体质](Constitution.md "体质") [豁免检定](Saving_throws.md "豁免检定")（[武器动作DC](Dice_rolls.md#Weapon_action_DC "掷骰") + 2）

- 生物无法说话或施放具有语言成分的法术，并且免疫 ⁠[雷鸣](Thunder.md "雷鸣") 伤害。

## 如何习得

由物品赋予：

- [巫术破除](Witchbreaker.md "巫术破除")

## 视觉效果

<https://bg3.wiki/wiki/File:Hush_You!_Action.mp4>

---
*Source: [Hush You!](https://bg3.wiki/wiki/Hush_You!)*