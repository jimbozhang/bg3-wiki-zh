# Jara

| | |
| --- | --- |
| 配音 | - [Liane-Rose Bunce](https://www.imdb.com/name/nm3332140/) |
| 体型 | [中级](Medium.md "Medium") |
| 类型 | [类人生物](Humanoid.md "Humanoid") |
| 种族 | [人类](Human.md "Human") |
| 等级 | 10 |
| 生命值 | 43 |
| 护甲等级 | 11 |
| 先攻 | +1 |
| 移动速度 | 9 m (30 ft) |
| 经验值 | 250 |

## Tactician Difficulty Stats

### Level

10

### HP

45

### AC

11

### Initiative

+1

### Proficiency

+4

### Movement

9 m (30 ft)

### Weight

75 kg (150 lb)

### Darkvision

0 m (0 ft)

### XP reward

250

| Ability scores | STR | DEX | CON | INT | WIS | CHA |
| --- | --- | --- | --- | --- | --- | --- |
|  | 8 / (-1) | 13 / (+1) | 16 / (+3) | 20 / (+5) | 10 / (0) | 12 / (+1) |

## Involvement

Jara can first be seen in [Naoise Nallinto's](Naoise_Nallinto.md "Naoise Nallinto") room, in a makeout session with her. Upon being approached by the party, an extended cutscene begins, in which Jara recognizes the party and is commanded to transform into a Mind Flayer by the [Netherbrain](Netherbrain.md "Netherbrain"). This results in combat.

## Combat

Jara has both her regular abilities and mind flayer abilities:

### Attacks and abilities

[枯萎术](Blight.md "Blight") (+ )

Damage: 8~64

8d8⁠[黯蚀](Necrotic.md "Necrotic")

对目标造成 8d8⁠⁠[黯蚀](Necrotic.md "Necrotic") 伤害。植物会受到此法术的最大伤害，并且在对抗它的 [豁免检定](Saving_throw.md "Saving Throw") 上具有 [劣势](Disadvantage.md "Disadvantage")。

[体质](Constitution.md "Constitution") [豁免检定](Saving_throws.md "Saving throws")
Range: 9 m (30 ft)

[召唤元素生物：火元素](Conjure_Elemental_colon__Fire_Elemental.md "Conjure Elemental: Fire Elemental") (+ )
火元素可以使用 [闷燃之触](Smouldering_Touch.md "Smouldering Touch")、[爆裂余烬](Erupting_Cinder.md "Erupting Cinder")，并可以使你的敌人 [燃烧](Burning_(Condition).md "Burning (Condition)")。

Range: 18 m (60 ft)
Recharge: Per combat

[法术反制](Counterspell.md "Counterspell") (+ )

尝试阻止一个正在施放的法术。

如果该法术的等级高于你用于法术反制的法术位，你必须使用你的施法关键属性进行一次 [属性检定](Ability_Check.md "属性检定") _\[[查看注释](Counterspell.md#Notes "Counterspell")\]_ 来阻止它。检定的难度等于 10 加上你试图反制的法术的等级。

此法术可以在你处于 [沉默](Silenced_(Condition).md "Silenced (Condition)") 状态时施放。

[火焰箭](Fire_Bolt.md "Fire Bolt") ()

Damage: 1~10

1d10⁠[火焰](Fire.md "Fire")

投掷一团火焰。

Range: 18 m (60 ft)

[浮空术](Levitate_(Illithid_Power).md "Levitate (Illithid Power)") ()
轻松地飘浮到目的地。

Range: 18 m (60 ft)

[马友夫强酸箭](Melf's_Acid_Arrow.md "Melf's Acid Arrow") (+ )

Damage: 6~24

4d4⁠[强酸](Acid.md "Acid")

\+ 2d4⁠[强酸](Acid.md "Acid")

射出一支绿色的箭，爆裂成强酸喷雾。立即造成 ⁠[强酸](Acid.md "强酸") 伤害，并在目标下一回合结束时造成 ⁠[强酸](Acid.md "强酸") 伤害。

如果法术未命中，目标仍会受到一半的初始伤害，但在其下一回合结束时不会受到伤害。

Range: 18 m (60 ft)

[心灵震爆](Mind_Blast.md "Mind Blast") ()

Damage: 4~32 + modifiers

4d8 + [施法调整值](Spells.md#Spellcasting_ability "Spells")⁠[心灵](Psychic.md "Psychic")

喷出一股锥形的心灵能量波，并可能 [震慑](Stunned_(Condition).md "Stunned (Condition)") 范围内的目标。

[智力](Intelligence.md "Intelligence") [豁免检定](Saving_throws.md "Saving throws")
Recharge: [长休](Long_Rest.md "Long rest")

[识破隐形](See_Invisibility_(spell).md "See Invisibility (spell)") (+ )
能够看到 [隐形](Invisible_(Condition).md "Invisible (Condition)") 生物，并可能向他人揭示它们。

[敏捷](Dexterity.md "Dexterity") [豁免检定](Saving_throws.md "Saving throws")
Range: Self

[减速](Slow.md "Slow") (+ )
改变最多 6 个敌人周围的时间以使其减速。他们无法走远，无法做太多事情，并且更容易被击中。

[感知](Wisdom.md "Wisdom") [豁免检定](Saving_throws.md "Saving throws")
Range: 18 m (60 ft)

[触手鞭笞](Tentacle_Whip_(Mind_Flayer).md "Tentacle Whip (Mind Flayer)") ()

Damage: 6~24

2d10 + 4⁠[心灵](Psychic.md "Psychic")

用你的触手猛击，并可能震慑你的目标。

[智力](Intelligence.md "Intelligence") [豁免检定](Saving_throws.md "Saving throws")
Melee: 1.5 m (5 ft)

### Loot

- [红宝石戒指](Ruby_Ring.md "Ruby Ring")

## Notes

- 尽管是 [焰拳法师](Flaming_Fist_Wizard.md "Flaming Fist Wizard")，Jara 的肖像画显示她穿着焰拳 [皮甲](Leather_Armour.md "Leather Armour")，而不是他们所穿的 [简易长袍](Simple_Robe.md "Simple Robe")。同一件焰拳皮甲可以在房间的泉水附近整齐地叠放着。

## Bugs

- 尽管此角色可以被 [回生术](Revivify.md "Revivify") 复活，但复活后她仍然是灵吸怪（并且敌对）。

---
*Source: [Jara](https://bg3.wiki/wiki/Jara)*