# 有魅力的哨兵

## 有魅力的哨兵

- 模型
- 肖像画

## 属性

### 体型

中级

### 类型

[不死生物](Undead.md "不死生物")

### 种族

[暗夜法官](Dark_Justiciar.md "暗夜法官")

### 职业

[圣武士](Paladin.md "圣武士")

### 副职

[复仇之誓](Oath_of_Vengeance.md "复仇之誓")

- 平衡

- 探索者

- 硬核

## 平衡难度属性

### 等级

7

### 生命值

120

### 护甲等级

14

### 先攻

0

### 熟练项

+3

### 移动速度

9 米 (30 英尺)

### 重量

30 千克 (60 磅)

### 高级黑暗视觉

12 米 (40 英尺)

### 经验值奖励

60

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 14 / (+2) | 10 / (0) | 12 / (+1) | 10 / (0) | 10 / (0) | 20 / (+5) |

## 抗性 [(info)](Resistances.md "抗性")

- - ## 额外资源

- - - ## 状态

- \_(状态)[**守护灵光**](Aura_of_Protection_(Aura)_(Condition).md "守护灵光 (灵光) (状态)")

该生物及其 3 米 (10 英尺) 内的盟友获得该生物的 [魅力](Charisma.md "魅力") 调整值作为其 [豁免检定](Saving_throw.md "豁免检定") 的加值。

## 特性

- [**高级黑暗视觉**](Darkvision.md "高级黑暗视觉")

可以在黑暗中看到一定距离。

- [**影子召唤**](Living_Shadow.md "影子召唤")

当处于 [轻度遮蔽](Lightly_Obscured.md "轻度遮蔽") 或 [重度遮蔽](Heavily_Obscured.md "重度遮蔽") 时，该角色对所有伤害具有 [抗性](Damage_types.md "伤害类型")，除了 ⁠[力场](Force.md "力场")、⁠[心灵](Psychic.md "心灵") 或 ⁠[光耀](Radiant.md "光耀")。

- [**借机攻击**](Opportunity_Attack.md "借机攻击")

攻击离开触及范围的敌人。

- [**魅力典范**](Paragon_of_Charisma.md "魅力典范")

所有 [魅力](Charisma.md "魅力") [豁免检定](Saving_throw.md "豁免检定") 和法术 [攻击掷骰](Attack_rolls.md "攻击掷骰") 具有 [优势](Advantage.md "优势")。

- [**莎尔视界**](Shar's_Sight.md "莎尔视界")

暗夜法官不会被 [目盲](Blinded_(Condition).md "目盲 (状态)")。

- [**日照弱点**](Sunlight_Weakness.md "日照弱点")

在阳光下时，[攻击掷骰](Attack_rolls.md "攻击掷骰") 和 [豁免检定](Saving_throws.md "豁免检定") 具有 [劣势](Disadvantage.md "劣势")。

## 探索者难度属性

### 等级

7

### 生命值

84

### 护甲等级

14

### 先攻

0

### 熟练项

+5

### 移动速度

9 米 (30 英尺)

### 重量

30 千克 (60 磅)

### 高级黑暗视觉

12 米 (40 英尺)

### 经验值奖励

60

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | +10 | +5 | +6 | +5 | +5 | +10 |

## 抗性 [(info)](Resistances.md "抗性")

- - ## 额外资源

- - - ## 状态

- \_(状态)[**守护灵光**](Aura_of_Protection_(Aura)_(Condition).md "守护灵光 (灵光) (状态)")

该生物及其 3 米 (10 英尺) 内的盟友获得该生物的 [魅力](Charisma.md "魅力") 调整值作为其 [豁免检定](Saving_throw.md "豁免检定") 的加值。

## 特性

- [**高级黑暗视觉**](Darkvision.md "高级黑暗视觉")

可以在黑暗中看到一定距离。

- [**影子召唤**](Living_Shadow.md "影子召唤")

当处于 [轻度遮蔽](Lightly_Obscured.md "轻度遮蔽") 或 [重度遮蔽](Heavily_Obscured.md "重度遮蔽") 时，该角色对所有伤害具有 [抗性](Damage_types.md "伤害类型")，除了 ⁠[力场](Force.md "力场")、⁠[心灵](Psychic.md "心灵") 或 ⁠[光耀](Radiant.md "光耀")。

- [**借机攻击**](Opportunity_Attack.md "借机攻击")

攻击离开触及范围的敌人。

- [**魅力典范**](Paragon_of_Charisma.md "魅力典范")

所有 [魅力](Charisma.md "魅力") [豁免检定](Saving_throw.md "豁免检定") 和法术 [攻击掷骰](Attack_rolls.md "攻击掷骰") 具有 [优势](Advantage.md "优势")。

- [**莎尔视界**](Shar's_Sight.md "莎尔视界")

暗夜法官不会被 [目盲](Blinded_(Condition).md "目盲 (状态)")。

- [**日照弱点**](Sunlight_Weakness.md "日照弱点")

在阳光下时，[攻击掷骰](Attack_rolls.md "攻击掷骰") 和 [豁免检定](Saving_throws.md "豁免检定") 具有 [劣势](Disadvantage.md "劣势")。

## 硬核难度属性

### 等级

7

### 生命值

156

### 护甲等级

14

### 先攻

0

### 熟练项

+3

### 移动速度

9 米 (30 英尺)

### 重量

30 千克 (60 磅)

### 高级黑暗视觉

12 米 (40 英尺)

### 经验值奖励

60

| 属性值 | 运动 | 体操 | 察觉技能 |
| --- | --- | --- | --- |
|  | 2 | 0 | 0 |

## 抗性 [(info)](Resistances.md "抗性")

- - ## 额外资源

- - - ## 状态

- \_(状态)[**守护灵光**](Aura_of_Protection_(Aura)_(Condition).md "守护灵光 (灵光) (状态)")

该生物及其 3 米 (10 英尺) 内的盟友获得该生物的 [魅力](Charisma.md "魅力") 调整值作为其 [豁免检定](Saving_throw.md "豁免检定") 的加值。

## 特性

- [**高级黑暗视觉**](Darkvision.md "高级黑暗视觉")

可以在黑暗中看到一定距离。

- [**影子召唤**](Living_Shadow.md "影子召唤")

当处于 [轻度遮蔽](Lightly_Obscured.md "轻度遮蔽") 或 [重度遮蔽](Heavily_Obscured.md "重度遮蔽") 时，该角色对所有伤害具有 [抗性](Damage_types.md "伤害类型")，除了 ⁠[力场](Force.md "力场")、⁠[心灵](Psychic.md "心灵") 或 ⁠[光耀](Radiant.md "光耀")。

- [**借机攻击**](Opportunity_Attack.md "借机攻击")

攻击离开触及范围的敌人。

- [**魅力典范**](Paragon_of_Charisma.md "魅力典范")

所有 [魅力](Charisma.md "魅力") [豁免检定](Saving_throw.md "豁免检定") 和法术 [攻击掷骰](Attack_rolls.md "攻击掷骰") 具有 [优势](Advantage.md "优势")。

- [**莎尔视界**](Shar's_Sight.md "莎尔视界")

暗夜法官不会被 [目盲](Blinded_(Condition).md "目盲 (状态)")。

- [**日照弱点**](Sunlight_Weakness.md "日照弱点")

在阳光下时，[攻击掷骰](Attack_rolls.md "攻击掷骰") 和 [豁免检定](Saving_throws.md "豁免检定") 具有 [劣势](Disadvantage.md "劣势")。

- [**神佑**](Divine_Health.md "神佑")

免疫 [疾病](Disease.md "疾病") 效果。

- [**坚韧复仇**](Relentless_Avenger.md "坚韧复仇")

如果你用 [借机攻击](Opportunity_Attack.md "借机攻击") 命中敌人，你的 [移动速度](Movement_speed.md "移动速度") 在你的下一回合增加 4.5 米 / 15 英尺。

## 技术信息

###

UID

`S_TWN_CentralSquare_GuardianOfCharisma` ###

UUID

`86b97d31-f124-4579-b88e-ac34312d21cf` ###

属性

`TWN_DarkJusticiar_Charisma` **有魅力的哨兵** 是一种独特的 [不死生物](Undead.md "不死生物")，守卫着 [雷斯文小镇](Reithwin_Town.md "雷斯文小镇") 广场中 [凯瑟里克·索姆](Ketheric_Thorm.md "凯瑟里克·索姆") 雕像下方通往 [莎尔](Shar.md "莎尔") 的秘密神殿。它的能力类似于 [圣武士](Paladin.md "圣武士")，并且与其角色相符，它拥有极高的 [魅力](Charisma.md "魅力")。

## 参与

如果队伍成员未能通过 [感知](Wisdom.md "感知")、[智力](Intelligence.md "智力") 和 [魅力](Charisma.md "魅力") 的三项 [豁免检定](Saving_throw.md "豁免检定") 测试，损坏雕像，或拿走 [莎尔的仪式匕首](Ritual_Dagger_of_Shar.md "莎尔的仪式匕首")，它会召唤不死生物 [暗夜法官](Dark_Justiciar.md "暗夜法官")。它与 [博学的哨兵](Wise_Sentinel.md "博学的哨兵") 和 [聪明的哨兵](Intelligent_Sentinel.md "聪明的哨兵") 一起，考虑到其大量的 [生命值](Hit_Points.md "生命值") 和特殊能力，会带来一场艰难的战斗。

## 战斗

### 攻击与能力

[主手攻击](Main_Hand_Attack.md "主手攻击") () – [巨剑](Greatsword.md "巨剑")

伤害: 4~14

2d6 + 2⁠[挥砍](Slashing.md "挥砍")

用装备的武器进行近战攻击。

近战: 1.5 米 (5 英尺)

[至圣斩](Divine_Smite.md "至圣斩") (+ )

伤害: 3~24

正常武器伤害

\+ 2d8⁠[光耀](Radiant.md "光耀")

\+ 1d8⁠[光耀](Radiant.md "光耀")

你的武器造成额外 2d8⁠⁠[光耀](Radiant.md "光耀")[DRS](Damage_rider_as_source.md "伤害来源") 伤害，当攻击 [邪魔](Fiends.md "邪魔") 或 [不死生物](Undead.md "不死生物") 时，增加 1d8⁠⁠[光耀](Radiant.md "光耀") 伤害。

范围: 正常武器范围

[暗影步](Shadow_Step.md "暗影步") ()
从 [阴影](Lightly_Obscured.md "轻度遮蔽") 传送到阴影。之后，你的下一次近战 [攻击掷骰](Attack_roll.md "攻击掷骰") 具有 [优势](Advantage.md "优势")。

范围: 18 米 (60 英尺)

[灾祸术](Bane_(spell).md "灾祸术 (法术)") (+ )
最多 3 个生物获得 **-1d4** 的 [攻击掷骰](Attack_roll.md "攻击掷骰") 和 [豁免检定](Saving_throw.md "豁免检定") 惩罚。

[魅力](Charisma.md "魅力") [豁免检定](Saving_throws.md "豁免检定")
范围: 9 米 (30 英尺)

[猎人印记](Hunter's_Mark.md "猎人印记") (+ )

伤害: 1~6

1d6⁠[武器](Weapon.md "武器")

标记一个生物作为你的猎物，每当用武器攻击命中时，造成额外 1d6⁠⁠[武器](Weapon.md "武器") 伤害。

如果目标在法术结束前死亡，你可以使用 [重新施展猎人印记](Reapply_Hunter's_Mark.md "重新施展猎人印记") 来标记一个新的生物。

范围: 18 米 (60 英尺)

[检察官的威能](Inquisitor's_Might.md "检察官的威能") (+ )
你或盟友的武器攻击造成额外魅力调整值⁠⁠[光耀](Radiant.md "光耀") 伤害，并且可以 [眩晕](Dazed_(Condition).md "眩晕 (状态)") 敌人 1 回合。

范围: 9 米 (30 英尺)

---
*Source: [Charismatic Sentinel](https://bg3.wiki/wiki/Charismatic_Sentinel)*