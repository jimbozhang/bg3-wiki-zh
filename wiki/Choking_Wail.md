# 窒息哀号

**窒息哀号**是[悔恨面容](Visage_of_Regret.md "悔恨面容")使用的职业动作，用于对区域内的敌人造成伤害并使其麻痹。

## 描述

释放一声哀号，对附近的敌人造成伤害并使其[麻痹](Paralysed_(Condition).md "麻痹（状态）")。

## 属性

消耗
[动作](Actions.md#Resources "动作")
伤害：4~18

2d8 + 2⁠[心灵](Psychic.md "心灵")

详情
[体质](Constitution.md "体质") [豁免检定](Saving_throws.md "豁免检定")（[DC](DC.md "DC") 12）（豁免成功时：目标仍承受全额伤害，但不会被[麻痹](Paralysed_(Condition).md "麻痹（状态）")。）
范围：4米（13英尺）半径
充能：每回合

## 技术细节

UID

`Shout_Paralyze_TollCollector_Face`

法术标志

`[IsEnemySpell](https://bg3.wiki/w/index.php?title=IsEnemySpell_\(spell_flag\)&action=edit&redlink=1) "IsEnemySpell \(spell flag\) \(page does not exist\)")`, `[IsHarmful](IsHarmful_(spell_flag).md)`

## 状态：麻痹

**[麻痹](Paralysed_(Condition).md "麻痹（状态）")**

持续时间：2回合

[体质](Constitution.md "体质") [豁免检定](Saving_throws.md "豁免检定")（[DC](DC.md "DC") 12）

- 受影响实体无法移动或使用[动作](Actions.md#Resources "动作")、[附赠动作](Actions.md#Resources "动作")或[反应](Actions.md#Reactions "动作")。
- 如果攻击者在3米（10英尺）范围内，对实体的攻击总是[重击](Critical_Hit.md "重击")。
- 对实体的攻击掷骰具有[优势](Advantage.md "优势")。
- 受影响实体自动在所有[力量](Strength.md "力量")和[敏捷](Dexterity.md "敏捷")[豁免检定](Saving_throw.md "豁免检定")中失败。

## 如何习得

由生物使用：[悔恨面容](Visage_of_Regret.md "悔恨面容")

---
*Source: [Choking Wail](https://bg3.wiki/wiki/Choking_Wail)*