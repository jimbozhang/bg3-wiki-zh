# 莎尔之狼（维康妮亚）

## 莎尔之狼

## 属性

### 体型

[中级](Medium.md "中级")

### 类型

[野兽](Beast.md "野兽")

### 种族

[狼](Wolf.md "狼")

- 平衡
- 探索者

## 平衡难度属性

### 等级

11

### 生命值

91

### 护甲等级

13

### 先攻

+4

### 熟练项加值

+4

### 重量

60 kg (120 lb)

### 高级黑暗视觉

0 m (0 ft)

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 12 / (+1) | 18 / (+4) | 15 / (+2) | 3 / (-4) | 12 / (+1) | 6 / (-2) |

## 特性

- [**兽性狂怒**](Bestial_Fury.md "兽性狂怒")

为所有伙伴提供一次额外攻击。

- [**伙伴羁绊**](Companion's_Bond.md "伙伴羁绊")

获得等于其熟练项加值的护甲等级和伤害加值。

- [**训练有素**](Exceptional_Training.md "训练有素")

可以使用附赠动作进行[疾走](Dash_(bonus_action).md)、[撤离](Disengage_colon__Bonus_Action.md "撤离：附赠动作")和[协助](Help_(bonus_action).md)。

- [**额外攻击**](Extra_Attack.md "额外攻击")

在徒手或武器攻击后可以进行一次额外攻击。

- [**借机攻击**](Opportunity_Attack.md "借机攻击")

攻击离开触及范围的敌人。

- [**集群战术**](Pack_Tactics.md "集群战术")

3米（10英尺）范围内的盟友对与你相邻的敌人进行攻击掷骰时具有[优势](Advantage.md "优势")。

- [**猎物的气味**](Prey's_Scent.md "猎物的气味")

你对被游侠[猎人印记](Hunter's_Mark.md "猎人印记")的目标造成额外1d6点伤害。

## 探索者难度属性

### 等级

11

### 生命值

182

### 护甲等级

13

### 先攻

+4

### 熟练项加值

+6

### 重量

60 kg (120 lb)

### 高级黑暗视觉

0 m (0 ft)

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | +1 | +4 | +2 | -4 | +1 | -2 |

## 特性

- [**兽性狂怒**](Bestial_Fury.md "兽性狂怒")

为所有伙伴提供一次额外攻击。

- [**伙伴羁绊**](Companion's_Bond.md "伙伴羁绊")

获得等于其熟练项加值的护甲等级和伤害加值。

- [**训练有素**](Exceptional_Training.md "训练有素")

可以使用附赠动作进行[疾走](Dash_(bonus_action).md)、[撤离](Disengage_colon__Bonus_Action.md "撤离：附赠动作")和[协助](Help_(bonus_action).md)。

- [**额外攻击**](Extra_Attack.md "额外攻击")

在徒手或武器攻击后可以进行一次额外攻击。

- [**借机攻击**](Opportunity_Attack.md "借机攻击")

攻击离开触及范围的敌人。

- [**集群战术**](Pack_Tactics.md "集群战术")

3米（10英尺）范围内的盟友对与你相邻的敌人进行攻击掷骰时具有[优势](Advantage.md "优势")。

- [**猎物的气味**](Prey's_Scent.md "猎物的气味")

你对被游侠[猎人印记](Hunter's_Mark.md "猎人印记")的目标造成额外1d6点伤害。

## 技术

###

UID

`LOW_HouseOfGrief_Wolf_Viconia` ###

UUID

`4a9caf4d-ea3f-4d94-88c6-2bfc8c8ca923` ###

Stats

`Companion_Wolf_11` 本文讨论的是维康妮亚在使用“映射恐怖：狼群”时应用的狼形态。关于应用于莎尔学徒的较弱狼形态，请参见[莎尔之狼](Sharran_Wolf.md "莎尔之狼")。

**莎尔之狼**是[维康妮亚·迪佛](Viconia_DeVir.md "维康妮亚·迪佛")可能使用[映射恐怖：狼群](Mapped_Terror_colon__Wolves.md "映射恐怖：狼群")变形而成的[狼](Wolf.md "狼")形态。她还会将附近的[莎尔学徒](Sharran_Novice.md "莎尔学徒")变形为此形态的[较弱版本](Sharran_Wolf.md "莎尔之狼")。

## 战斗

此形态的属性值和被动特性与11级的[狼伙伴](Wolf_Companion.md "狼伙伴")相同，但其动作不同。这也意味着此形态在探索者难度下要强得多，因为它受益于通常为队伍狼伙伴提供的增益。

当狼的生命值耗尽时，维康妮亚会像正常的[荒野形态](Wild_Shape.md "荒野形态")一样变回类人形态。

### 攻击与能力

[啃咬](Bite_(Sharran_Wolf,_Viconia).md "啃咬（莎尔之狼，维康妮亚）") ()

伤害: 1~12

1d12⁠[心灵](Psychic.md "心灵")

啃咬目标。

范围: 0.75 m (3 ft)

[邪恶之嚎](Sinister_Howl.md "邪恶之嚎")
推动自己和族群突破极限，为族群的每个成员提供一次额外的[动作](Actions.md#Resources "动作")。

范围: 自身
充能: 每回合

---
*Source: [Sharran Wolf (Viconia)](https://bg3.wiki/wiki/Sharran_Wolf_(Viconia)*