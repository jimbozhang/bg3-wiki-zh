...
**动作**是[博德之门3](Baldur's_Gate_3.md "博德之门3")中[生物](Creatures.md "Creatures")在其回合中采取的行为。动作通常用于造成[伤害](Damage.md "Damage")或施加有害的[状态](Conditions.md "Conditions")，但也可以用于[治疗](Healing.md "Healing")或援助盟友。

一种特殊形式的动作——称为**反应**——可用于响应某些触发条件，包括在*其他*生物的回合中。

采取动作通常需要行动生物消耗三种[资源](Resources.md "Resources")之一：
...
情境动作
情境动作因其他能力、特性或状态而解锁。它会保留在角色的快捷栏旁，直到创建它的条件过期。一个这样的例子是[强制策动](Forced_Manoeuvre.md "Forced_Manoeuvre")，作为[策动攻击（近战）](Manoeuvring_Attack_(Melee).md "Manoeuvring Attack (Melee)")的结果。
武器动作
...
另一种资源——反应——用于在其他生物的回合中采取有条件的[反应](Reactions.md "Reactions")。

所有生物在每回合开始时拥有每种资源的1点充能，但可以从[各种来源](Sources_of_additional_actions.md "Sources_of additional actions")获得额外充能。

### 例外

额外动作

- 允许生物每回合采取*额外*动作的动作——例如[动作如潮](Action_Surge.md "Action Surge")——*不*消耗其提供的资源。

自由攻击

- 拥有[额外攻击](Extra_Attack.md "Extra Attack")特性的职业可以在其回合中采取一次攻击动作后，再进行一次自由攻击动作，而无需消耗动作资源。
- 11级[战士](Fighter.md "Fighter")拥有[精通额外攻击](Improved_Extra_Attack.md "Improved Extra Attack")特性，可以在其回合中采取一次攻击动作后，再进行两次自由攻击动作，而无需消耗动作资源。

## 反应

图1 - 激活的反应。每个图标上的对话气泡表示这些反应设置为“询问”。

反应是响应触发条件而采取的动作，即使触发于其他生物的回合中也可以采取。

某些反应要求行动生物消耗[反应](#reactions)资源，而其他反应则不需要。不需要消耗的通常是使用反应系统的职业特性，例如[至圣斩](Divine_Smite.md "Divine_Smite")、[鲁莽攻击](Reckless_Attack.md "Reckless_Attack")或[偷袭](Sneak_Attack.md "Sneak_Attack")。<sup>[\[1\]](#cite_note-1)</sup>

### 反应标签页

图2 - 反应标签页。每个反应都可以通过左侧的复选框启用或禁用。可以使用“询问”设置进一步配置，以便在每次触发反应时提示玩家确认。

反应通过*反应标签页*为每个角色管理，该标签页位于角色表的法术书页面。也可以通过按下快捷栏上的激活反应之一或按下快捷键（默认：L）立即访问该标签页。

反应可以设置为自动采取，或者可以设置为首先提示玩家。

如果采取反应会消耗任何资源，它们会列在反应名称下方。

### 限制

如果任何反应提示确认，则角色不能对同一事件应用多个反应；UI会阻止为每个角色选择多个反应。

事件触发的反应集在任何反应生效前确定。这意味着，如果攻击掷骰或豁免检定通常会忽略反应（例如，因为掷骰成功且反应是盟友的掷骰加值），即使掷骰结果被另一个反应改变，也会忽略该反应。
...
- 任何[隐形状态](SG_Invisible.md "SG Invisible")，这会阻止除[受诅咒幽鬼](Accursed_Spectre_(reaction).md "Accursed Spectre (reaction)")、[致盲伏击](Blinding_Ambush.md "Blinding Ambush")和[怪异魔法涌动](Weird_Magic_Surge.md "Weird Magic Surge")之外的所有反应
...
- 大多数通常被非伪装、非星形形态的变形状态（如其他荒野形态）禁用的反应，如果生物同时具有伪装或星形形态状态，则会被错误地启用。
...
1. [↑](#cite_ref-1) 许多在*Baldur's Gate 3*中作为反应实现的职业特性，在基于的D&D 5版中并非反应。这一改变可能是因为反应在游戏开发后期才添加。
