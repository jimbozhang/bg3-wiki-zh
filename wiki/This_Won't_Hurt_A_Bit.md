# 这不会疼的

**这不会疼的**是[马鲁斯·索姆](Malus_Thorm.md "马鲁斯·索姆")的其中一个职业动作，允许他向目标注射[手术针管](Syringe.md "手术针管")。他必须先让他的[安雅姐妹](Sister_Anya.md "安雅姐妹")将工具递给他。

## 描述

用手术针管刺击敌人并使其[中毒](Poisoned_(Condition).md "中毒（状态）")。

## 属性

消耗
[动作](Actions.md#Resources "动作")
伤害：8~64 + 调整值

8d8 + [敏捷调整值](Dexterity.md#Dexterity_modifier_chart "敏捷")⁠[穿刺](Piercing.md "穿刺")

详情
[敏捷](Dexterity.md "敏捷") [豁免检定](Saving_throws.md "豁免检定")（[DC](DC.md "DC") 18）（豁免成功时：目标仍承受一半伤害。）
范围：2.5 米（8 英尺）

## 技术细节

UID

`Target_SurgeryAttack_Syringe_Surgeon`

法术标志

`[IsEnemySpell](https://bg3.wiki/w/index.php?title=IsEnemySpell_\(spell_flag\)&action=edit&redlink=1) "IsEnemySpell \(spell flag\) \(page does not exist\)")`, `[IsHarmful](IsHarmful_(spell_flag).md)`, `[IsMelee](IsMelee_(spell_flag).md)`

## 状态：中毒

**[中毒](Poisoned_(Condition).md "中毒（状态）")**

持续时间：2 驱散

[敏捷](Dexterity.md "敏捷") [豁免检定](Saving_throws.md "豁免检定")（[DC](DC.md "DC") 18）

- 在[攻击掷骰](Attack_roll.md "攻击掷骰")和[检定](Checks.md "检定")上承受[劣势](Disadvantage.md "劣势")。

## 如何习得

由法术授予：

- [把我的手术针管递给我](Hand_Me_My_Syringe.md "把我的手术针管递给我")

被生物使用：[马鲁斯·索姆](Malus_Thorm.md "马鲁斯·索姆")

---
*Source: [This Won't Hurt A Bit](https://bg3.wiki/wiki/This_Won't_Hurt_A_Bit)*