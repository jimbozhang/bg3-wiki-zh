...
自正式发布以及Norbyte制作的[此](https://github.com/Norbyte/dos2de_collada_exporter)插件以来，直接从Blender导出为GR2时不再需要以下步骤。但是，如果您希望导出为DAE并遇到UV错误，以下方法将有效。

本教程来自[tyvianpear](https://www.nexusmods.com/baldursgate3/users/73681713)在DbtR Discord服务器中分享的帖子。

## 步骤

1. 打开两个Blender实例：2.79b和3.X（最新版）

1. 将**LaughingLeaders' Blender Helpers for 2.79b**安装到2.79b中

1. 首先在3.X中打开您的模型

1. 将整个模型及其所有网格复制并粘贴到2.79b中

1. 最小化（或关闭）3.X，直到最后才会再次用到它

1. 通过拖动顶部角落的条纹三角形之一，在**属性选项卡**旁边打开另一个窗口

1. 在新窗口中，点击**项目大纲**下的**属性**按钮，选择**UV/图像编辑器**

1. 点击您要首先检查的网格（为安全起见，请逐个检查每个网格）

1. 按**TAB**进入**编辑模式**

1. 点击垂直的**辅助选项卡**，确保**选择所有问题**框已勾选

1. 点击**检查UV三角形**

1. 在Blender的最顶部，您会看到一条通知，告知发现了多少个UV问题。它们还会以橙色在您的网格和右侧的图表中突出显示

1. 在模型的左侧，您会在**工具选项卡**中看到**移除重合顶点**

1. 点击它，您会在顶部看到一条消息，_移除了0个顶点_，因为合并距离太短，无法合并任何重叠的顶点

1. 尝试逐步更改合并距离，尽可能小。大约在0.0004左右，您会看到顶部显示_移除了X个顶点_

1. 密切关注您的网格，确保它不会过度变形

1. 每次顶点移除/合并后，选择并取消选择（在编辑模式下按CTRL-A）网格，并重复之前的**检查UV三角形**和**移除重合顶点**步骤

1. 并非所有UV错误都会显示在图表上，因此请密切关注顶部的通知，该通知会告知您有多少个UV错误

1. 当您合并了所有导致问题的顶点后，您会看到顶部显示_未发现UV问题_

1. 对模型中的每个网格完成此操作后，将整个内容复制并粘贴回3.X
