# 争端解决者

争端解决者是一把不常见的+1 [巨棒](Greatclubs.md "巨棒")，它赋予持有者[毒雾](Poison_Mist.md "毒雾")武器动作，该动作会造成额外的毒素伤害并制造一片毒云。

没有什么比挥舞巨棒更能快速解决分歧了。而这一把还散发着恶臭。

## 属性

伤害

1d8 + 1 (2~9) + [力量调整值](Damage_Roll.md#Modifiers "伤害掷骰") ⁠[钝击](Bludgeoning.md "钝击")

详情
[巨棒](Greatclubs.md "巨棒")
稀有度：不常见
惑控学派：**+ 1**
[双手](Two-Handed.md "双手")
[可蘸取](Dippable.md "可蘸取")
[可消耗](List_of_Items_consumable_by_Gale.md "盖尔可消耗物品列表")（由[盖尔](Gale.md "盖尔")消耗）
近战：1.5 米（5 英尺）
重量：4.5 千克（9 磅）
价格：65 金币
UID `MAG_Poison_Greatclub` UUID `fddc251b-22d8-4036-a9fb-85fba8635cf0` ### 武器动作

_如果你拥有[熟练项](Proficiency.md "熟练项")，装备在**主手**以获得：_

[震荡猛击](Concussive_Smash.md "震荡猛击") ()
用全力击中敌人以造成伤害，并可能使其[眩晕](Dazed_(Condition).md "眩晕（状态）")。（充能：[短休](Short_rest.md "短休")。）

[摔翻打击](Backbreaker.md "摔翻打击") ()
在攻击中施加额外力量，以可能使敌人[倒伏](Prone_(Condition).md "倒伏（状态）")。（充能：[短休](Short_rest.md "短休")。）

[韧性](Tenacity.md "韧性") ()
当你攻击未命中时，仍然造成等于你[力量](Strength.md "力量")[调整值](Ability_Scores.md#Ability_score_modifiers "属性值")的⁠[钝击](Bludgeoning.md "钝击")伤害（最低为1）。

### 特殊武器动作

此武器还赋予以下动作：

[毒雾](Poison_Mist.md "毒雾") ()
命中时，造成等于你[熟练项加值](Proficiency_Bonus.md "熟练项加值")的额外⁠[中毒](Poison.md "中毒")伤害，并使目标被[毒云](Poison_Cloud.md "毒云")包围，可能使其中的生物[中毒](Poisoned_(Condition).md "中毒（状态）")。（充能：[短休](Short_rest.md "短休")。）

## 获取地点

- [月出之塔](Moonrise_Towers.md "月出之塔") X: -196 Y: -195：由主楼层的[米格](Mig.md "米格")携带

## 备注

_关于毒雾：_

- 毒雾创造的[毒云](Poison_Cloud.md "毒云")内的目标，其豁免检定将对抗你的[法术豁免DC](Spell_save_DC.md "法术豁免DC")，而非你的[武器动作DC](Weapon_action_DC.md "武器动作DC")。

## 图库

- 游戏内外观

## 外部链接

- ⁠[争端解决者](https://forgottenrealms.fandom.com/wiki/Argument_Solver) 在 [被遗忘的国度 Wiki](https://forgottenrealms.fandom.com/wiki/Main_Page)

---
*Source: [Argument Solver](https://bg3.wiki/wiki/Argument_Solver)*