# 印记斩（远程）

**印记斩（远程）** 是一个[2级塑能学派法术](Spells.md "Spells")。此法术是[印记斩](Branding_Smite.md "印记斩")的变体。它允许施法者使他们的远程武器闪耀星界光耀，从而用光标记目标，阻止其变为[隐形](Invisible_(Condition).md "隐形（状态）")。

## 描述

当你攻击时，你的远程武器闪耀着星界光耀，并可能用光标记你的目标，阻止其变为[隐形](Invisible_(Condition).md "隐形（状态）")。

## 属性

消耗
[动作](Actions.md#Resources "动作")
命中后消耗
[附赠动作](Actions.md#Resources "动作") + [2级法术位](Spells.md#Spell_slots "法术")
伤害：2~12

普通武器伤害

\+ 2d6⁠[光耀](Radiant.md "光耀")

详情
远程武器 [攻击掷骰](Attack_roll.md "攻击掷骰")
射程：普通武器射程
[专注](Concentration.md "专注")

## 更高等级

每提升一级，额外造成1d6⁠⁠[光耀](Radiant.md "光耀")伤害。

## 技术细节

UID

`Projectile_Smite_Branding`

法术标志

`[HasHighGroundRangeExtension](HasHighGroundRangeExtension_(spell_flag).md)`, `[HasVerbalComponent](HasVerbalComponent_(spell_flag).md)`, `[IsConcentration](IsConcentration_(spell_flag).md)`, `[IsHarmful](IsHarmful_(spell_flag).md)`, `[IsSpell](IsSpell_(spell_flag).md)`, `[RangeIgnoreVerticalThreshold](https://bg3.wiki/w/index.php?title=RangeIgnoreVerticalThreshold_\(spell_flag\)&action=edit&redlink=1) "RangeIgnoreVerticalThreshold \(spell flag\) \(page does not exist\)")`

## 状态：印记斩

**[印记斩](Branding_Smite_(Condition).md "印记斩（状态）")**

持续时间：10 驱散

- 无法变为[隐形](Condition.md#Invisible "状态")。

## 如何习得

种族：

- 角色等级 5：[扎瑞尔的提夫林](Zariel_Tiefling.md "扎瑞尔的提夫林")

（_角色等级_ 是多职业角色所有职业等级的总和。）

此法术是以下法术的变体：
[印记斩](Branding_Smite.md "印记斩")

## 备注

- 有关附加功能信息，请参见[至圣斩](Smite.md "至圣斩")。
- 此法术与大多数其他至圣斩法术不同，它适用于远程武器。
- 扎瑞尔[提夫林](Tiefling.md "提夫林") 每[长休](Long_Rest.md "长休")只能施放一次该法术。

## 外部链接

- ⁠[Branding smite](https://forgottenrealms.fandom.com/wiki/Branding_smite) 在 [被遗忘的国度维基](https://forgottenrealms.fandom.com/wiki/Main_Page) 上

---
*Source: [Branding Smite (Ranged)](https://bg3.wiki/wiki/Branding_Smite_(Ranged)*