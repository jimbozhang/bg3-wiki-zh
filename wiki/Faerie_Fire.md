# 妖火

**妖火**是一个[1级塑能学派法术](Spells.md "法术")。它允许施法者召唤出耀眼的光芒。任何被半径范围内的光芒照射到的生物会暂时散发微光，并阻止[隐形](Invisible_(Condition).md "隐形（状态）")。

## 描述

光芒范围内的所有目标都会变得可见，并且对它们的[攻击掷骰](Attack_roll.md "攻击掷骰")具有[优势](Advantage.md "优势")。

## 属性

消耗
[动作](Actions.md#Resources "动作") + [1级法术位](Spells.md#Spell_slots "法术")
详情
[敏捷](Dexterity.md "敏捷") [豁免检定](Saving_throws.md "豁免检定")（[法术豁免DC](Dice_rolls.md#Spell_save_DC "掷骰")）
范围：18米（60英尺）
范围效果：6米（20英尺）半径
[专注](Concentration.md "专注")

## 更高法术位施放

以更高法术位施放此法术不会获得额外收益。

## 技术细节

UID

`Target_FaerieFire`

法术标志

`[HasVerbalComponent](HasVerbalComponent_(spell_flag).md)`, `[IsConcentration](IsConcentration_(spell_flag).md)`, `[IsHarmful](IsHarmful_(spell_flag).md)`, `[IsSpell](IsSpell_(spell_flag).md)`, `[RangeIgnoreVerticalThreshold](https://bg3.wiki/w/index.php?title=RangeIgnoreVerticalThreshold_\(spell_flag\)&action=edit&redlink=1) "RangeIgnoreVerticalThreshold \(spell flag\) \(page does not exist\)")`

## 状态：妖火

**[妖火](Faerie_Fire_(Condition).md "妖火（状态）")**

持续时间：10驱散

- 散发微光。受到的攻击具有[优势](Advantage.md "优势")。无法[隐形](Condition.md#Invisible "状态")。

## 如何习得

职业：

- 职业等级1：[吟游诗人](Bard.md "吟游诗人")、[德鲁伊](Druid.md "德鲁伊")、[光明领域](Light_Domain.md "光明领域")（领域法术）和[至高妖精](The_Archfey.md "至高妖精")
- 职业等级3：[蜂群守护者](Swarmkeeper.md "蜂群守护者")

种族：

- 角色等级3：[卓尔](Drow.md "卓尔")和[半卓尔](Drow_Half-Elf.md "半卓尔")

（*角色等级*是多职业角色所有职业等级的总和。）

由特性授予：

- [魔法学徒：吟游诗人](Magic_Initiate_colon__Bard.md "魔法学徒：吟游诗人")
- [魔法学徒：德鲁伊](Magic_Initiate_colon__Druid.md "魔法学徒：德鲁伊")

## 备注

- [卓尔](Drow.md "卓尔")和[半卓尔](Half-Elf.md#Drow_Half-Elves "半精灵")的种族版本法术只能每[长休](Long_Rest.md "长休")施放一次。

## 相关物品

- [地狱骑士团长弓](Hellrider_Longbow.md "地狱骑士团长弓") - 使用此武器造成伤害时，通过[敏捷](Dexterity.md "敏捷")[豁免检定](Saving_throw.md "豁免检定")（[DC](Dice_rolls.md#Save_DCs "掷骰")12）施加[妖火](Faerie_Fire_(Condition).md "妖火（状态）")。
- [艾克斯阎蒂](Xyanyde.md "艾克斯阎蒂") - 每[短休](Short_Rest.md "短休")一次，使用此武器未命中时施加[妖火](Faerie_Fire_(Condition).md "妖火（状态）")。
- [狩猎饰环](Circlet_of_Hunting.md "狩猎饰环") - 对被[妖火](Faerie_Fire_(Condition).md "妖火（状态）")标记的生物进行攻击掷骰时获得+1d4加值。

## 外部链接

- ⁠[妖火](https://forgottenrealms.fandom.com/wiki/Faerie_fire) 在 [被遗忘的国度Wiki](https://forgottenrealms.fandom.com/wiki/Main_Page)

---
*Source: [Faerie Fire](https://bg3.wiki/wiki/Faerie_Fire)*