...
- 伤害：
  - 对于具有[投掷](Thrown.md "投掷")属性的武器，伤害和伤害类型与武器的近战伤害相同。
...
- 投掷是一个明确定义的动作，在游戏代码中具有详尽的交互列表。投掷是其自己的动作类型，与武器攻击分开，即使与[投掷](Thrown.md "投掷")武器一起使用也是如此。[武器动作](Weapon_actions.md "武器动作")明确定义为与武器攻击配合使用，因此不能与投掷一起使用。另一个主要后果是投掷武器不会触发大多数“命中”被动，除非下面提到的那些。此外，投掷武器不会应用任何固有的额外伤害（例如，[闪亮的碎颅锤](Shining_Staver-of-Skulls.md "闪亮的碎颅锤")的1d4⁠⁠[光耀](Radiant.md "光耀")伤害）或来自[涂层](Coatings.md "涂层")或[蘸取](Dip.md "蘸取")的任何伤害。
  - 以下交互被明确编码为与投掷配合工作：
