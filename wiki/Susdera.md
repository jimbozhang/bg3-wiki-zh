# 萨斯德拉

## 萨斯德拉

灵魂侍女

- 游戏内
- 模型
- 肖像画

### 身份

女性

### 声音 / 演员

- [Lucy Paterson](https://www.imdb.com/name/nm13687455/)
- [Luisa Guerreiro](https://www.imdb.com/name/nm3708833/)

### 阵营

[米尔寇教团](Cult_of_Myrkul.md "Cult of Myrkul")

### 职业

米尔寇死灵法师

## 属性

### 体型

[中级](Medium.md "Medium")

### 类型

[类人生物](Humanoid.md "Humanoid")

### 种族

[人类](Human.md "Human")

### 原型

[灵魂侍女](Mistress_of_Souls.md "Mistress of Souls")

### 职业

[法师](Wizard.md "Wizard")

### 副职

[塑能学派](Evocation_School.md "Evocation School")

- 平衡

- 探索者

- 硬核

## 平衡难度属性

### 等级

8

### 生命值

63

### 护甲等级

15

### 先攻

+2

### 熟练项

+3

### 移动

9 米 (30 英尺)

### 重量

75 千克 (150 磅)

### 高级黑暗视觉

0 米 (0 英尺)

### 经验值奖励

140

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 10 / (0) | 14 / (+2) | 17 / (+3) | 19 / (+4) | 14 / (+2) | 13 / (+1) |

## 额外资源

- - - - ## 状态

- [**魔法护甲**](Mage_Armour_(Condition).md "Mage Armour (Condition)")

基础[护甲等级](Armour_Class.md "Armour Class")为 13

## 特性

- [额外攻击](Extra_Attack.md "额外攻击")

在徒手或武器攻击后可以进行一次额外攻击。

- [坟墓魔法](Grave_Magic.md "坟墓魔法")

当你用法术造成伤害时，额外造成 1d8⁠⁠[黯蚀](Necrotic.md "Necrotic") 伤害。

- [借机攻击](Opportunity_Attack.md "借机攻击")

攻击离开攻击范围的敌人。

- [法术塑形](Sculpt_Spells.md "法术塑形")

盟友自动通过使用者的法术豁免检定。

- [**韧性**](Tenacity_(passive_feature).md "Tenacity (passive feature)")

近战攻击未命中时，仍会造成等于 [力](STR.md "STR") 调整值或至少 1 点的[钝击](Bludgeoning.md "Bludgeoning") 伤害。

## 探索者难度属性

### 等级

8

### 生命值

45

### 护甲等级

15

### 先攻

+2

### 熟练项

+5

### 移动

9 米 (30 英尺)

### 重量

75 千克 (150 磅)

### 高级黑暗视觉

0 米 (0 英尺)

### 经验值奖励

140

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 0 | +2 | +3 | +7 | +5 | +1 |

## 额外资源

- - - - ## 状态

- [**魔法护甲**](Mage_Armour_(Condition).md "Mage Armour (Condition)")

基础[护甲等级](Armour_Class.md "Armour Class")为 13

## 特性

- [额外攻击](Extra_Attack.md "额外攻击")

在徒手或武器攻击后可以进行一次额外攻击。

- [坟墓魔法](Grave_Magic.md "坟墓魔法")

当你用法术造成伤害时，额外造成 1d8⁠⁠[黯蚀](Necrotic.md "Necrotic") 伤害。

- [借机攻击](Opportunity_Attack.md "借机攻击")

攻击离开攻击范围的敌人。

- [法术塑形](Sculpt_Spells.md "法术塑形")

盟友自动通过使用者的法术豁免检定。

- [**韧性**](Tenacity_(passive_feature).md "Tenacity (passive feature)")

近战攻击未命中时，仍会造成等于 [力](STR.md "STR") 调整值或至少 1 点的[钝击](Bludgeoning.md "Bludgeoning") 伤害。

## 硬核难度属性

### 等级

8

### 生命值

84

### 护甲等级

15

### 先攻

+2

### 熟练项

+3

### 移动

9 米 (30 英尺)

### 重量

75 千克 (150 磅)

### 高级黑暗视觉

0 米 (0 英尺)

### 经验值奖励

140

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 10 / (0) | 14 / (+2) | 17 / (+3) | 19 / (+4) | 14 / (+2) | 13 / (+1) |

## 额外资源

- - - - ## 状态

- [**魔法护甲**](Mage_Armour_(Condition).md "Mage Armour (Condition)")

基础[护甲等级](Armour_Class.md "Armour Class")为 13

## 特性

- [额外攻击](Extra_Attack.md "额外攻击")

在徒手或武器攻击后可以进行一次额外攻击。

- [坟墓魔法](Grave_Magic.md "坟墓魔法")

当你用法术造成伤害时，额外造成 1d8⁠⁠[黯蚀](Necrotic.md "Necrotic") 伤害。

- [借机攻击](Opportunity_Attack.md "借机攻击")

攻击离开攻击范围的敌人。

- [法术塑形](Sculpt_Spells.md "法术塑形")

盟友自动通过使用者的法术豁免检定。

- [**韧性**](Tenacity_(passive_feature).md "Tenacity (passive feature)")

近战攻击未命中时，仍会造成等于 [力](STR.md "STR") 调整值或至少 1 点的[钝击](Bludgeoning.md "Bludgeoning") 伤害。

## 技术

###

UID

`S_MOO_RoofCultist` ###

UUID

`ebfd7bc7-0fd5-4dee-8fd5-308ded4d0a65` ###

Stats

`Human_Cultist_Myrkul_MasterOfSouls` **萨斯德拉**是[米尔寇教团](Cult_of_Myrkul.md "Cult of Myrkul")的一名死灵法师<sup>[\[1\]](#cite_note-1)</sup>。她可以在[月出之塔](Moonrise_Towers.md "Moonrise Towers")的顶部找到，并在战斗中协助[凯瑟里克·索姆](Ketheric_Thorm.md "Ketheric Thorm")对抗队伍。

## 目录

- [1 参与](#参与)
- [2 战斗](#战斗)
  - [2.1 攻击与能力](#攻击与能力)
- [3 战利品](#战利品)
- [4 笔记](#笔记)

## 参与

如果在完成[寻找凯瑟里克·索姆的遗物](Find_Ketheric_Thorm's_Relic.md "Find Ketheric Thorm's Relic")（即释放或杀死[暗夜之歌](Aylin.md "Aylin")从而结束凯瑟里克的不朽）*之后*进入月出之塔的屋顶，会触发与他的过场动画，随后与他及其盟友战斗。萨斯德拉随后成为凯瑟里克对抗队伍的盟友之一。

如果在完成[寻找凯瑟里克·索姆的遗物](Find_Ketheric_Thorm's_Relic.md "Find Ketheric Thorm's Relic")*之前*进入月出之塔的屋顶，萨斯德拉会阻止并斥责他们回头。回答“我想去哪就去哪”会让她和所有八个[死灵侍僧](Necromite.md "Necromite")变得敌对。[凯瑟里克·索姆](Ketheric_Thorm.md "Ketheric Thorm")不会变得敌对，除非过于靠近他。然而，这次无法击败他，因为他仍然是不朽的。

与狂热者、专家和[绝对真神教团](Cult_of_the_Absolute.md "Cult of the Absolute")的其他成员不同，萨斯德拉和其他[灵魂侍女](Mistress_of_Souls.md "Mistress of Souls")似乎完全清楚绝对真神是什么，以及他们正在实施的宗教骗局。<sup>[\[2\]](#cite_note-2)</sup> 这种更深刻的理解（以及随之而来的蔑视）也被服务于索姆将军的其他高级米尔寇死灵法师所共享，[克雷萨·骨女](Kressa_Bonedaughter.md "Kressa Bonedaughter")哀叹她所面对的被感染者的软弱奴性，而[巴萨扎尔](Balthazar.md "Balthazar")则使用玩家角色被蠕虫感染的状态作为辱骂。

## 战斗

### 攻击与能力

[冻僵](Bone_Chill.md "Bone Chill") ()

伤害: 1~8

1d8⁠[黯蚀](Necrotic.md "Necrotic")

阻止目标在你的下一回合前进行治疗。目标无法协助[倒地](Downed_(Condition).md "Downed (Condition)")角色。不死生物目标在[攻击掷骰](Attack_roll.md "Attack Roll")上获得[劣势](Disadvantage.md "Disadvantage")。

射程: 18 米 (60 英尺)

[火球术](Fireball.md "火球术") (+ )

伤害: 8~48

8d6⁠[火焰](Fire.md "Fire")

从指尖射出明亮的火焰，在接触时爆炸，对附近的一切造成 8d6⁠⁠[火焰](Fire.md "Fire") 伤害。

[敏捷](Dexterity.md "敏捷") [豁免](Saving_throws.md "Saving throws")
射程: 18 米 (60 英尺)

[孵化死亡](Incubate_Death.md "Incubate Death") ()

伤害: 6~27

3d8 + 3⁠[黯蚀](Necrotic.md "Necrotic")

生成一个骸骨蒴苞。1 回合后，蒴苞爆炸，造成 3d8[黯蚀](Necrotic.md "Necrotic") 伤害并诞生一个死灵侍僧。

[敏捷](Dexterity.md "敏捷") [豁免](Saving_throws.md "Saving throws")
射程: 24 米 (80 英尺)

[主手攻击](Main_Hand_Attack.md "Main Hand Attack") () – [米尔寇的残忍链枷](Myrkulite_Scourge.md "Myrkulite Scourge")

伤害: 3~15

1d8 + 1⁠[钝击](Bludgeoning.md "Bludgeoning")

\+ 1d6⁠[黯蚀](Necrotic.md "Necrotic")

使用装备的武器进行近战攻击。

近战: 1.5 米 (5 英尺)

## 战利品

- [骸骨钥匙](Bone_Key.md "Bone Key")
- [米尔寇的残忍链枷](Myrkulite_Scourge.md "Myrkulite Scourge")

## 笔记

1. [↑](#cite_ref-1) 尽管在多处被称为死灵法师，但所有灵魂侍女都是塑能学派法师。
1. [↑](#cite_ref-2) 与[拉迪娅](Radija.md "Radija")交谈时，如果通过洞悉检定或尝试与她自己的寄生虫链接，会揭示她没有寄生虫，并且她（以及[克雷萨·骨女](Kressa_Bonedaughter.md "Kressa Bonedaughter"))视队伍成员为被蒙蔽的感染工具，充满蔑视。

---
*Source: [Susdera](https://bg3.wiki/wiki/Susdera)*