# 野心之神 (状态)

**野心之神**

- 凡人向新晋神祇盖尔·德卡里奥斯（The Once-Wizard, High Aspirant, The Lord of Zeal）低声祈求——但至今无人能获这位野心之神的接见。直到今夜。

## 属性

[更多属性](Status_properties.md "状态属性")：

- [ApplyToDead](Status_properties/ApplyToDead.md "状态属性/ApplyToDead")
- [DisableOverhead](Status_properties/DisableOverhead.md "状态属性/DisableOverhead")
- [DisableCombatlog](Status_properties/DisableCombatlog.md "状态属性/DisableCombatlog")
- [DisablePortraitIndicator](Status_properties/DisablePortraitIndicator.md "状态属性/DisablePortraitIndicator")
- [IgnoreResting](Status_properties/IgnoreResting.md "状态属性/IgnoreResting")

## 赋予能力

[神圣欢宴](Divine_Revelry.md "神圣欢宴") ()
从深水城之口召唤一桶影暗麦酒，并对所有附近生物施放[奥图迷舞](Otto's_Irresistible_Dance_(Condition).md "奥图迷舞 (状态)")。

[塑形翼猫](Formsculpt_colon__Tressym.md "塑形：翼猫") ()
将一个生物的形态重塑为盖尔心爱宠物的模样。

[毁灭真言](Power_Word_colon__Ruin.md "毁灭真言") ()
向盖尔编织的魔网低语灾难之词，召唤三道纯粹的毁灭光束，每道造成 10d6+40⁠⁠[力场](Force.md "力场") 伤害。

## 状态：可能性灵光 (光环)

**\_(状态)[可能性灵光](Aura_of_Possibility_(Aura)_(Condition).md "可能性灵光 (光环) (状态)")**

- 盖尔·德卡里奥斯（野心之神）附近的任何盟友，所有[检定](Ability_checks.md "检定")获得 +10 加值。

## 备注

若盖尔在终章成功成神，他将拥有此状态。

## 野心之神的来源

_维基数据库中未定义_

## 拥有野心之神的生物

_维基数据库中未定义_

---
*Source: [God of Ambition (Condition)](https://bg3.wiki/wiki/God_of_Ambition_(Condition)*