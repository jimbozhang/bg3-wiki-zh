# 守护灵光 (光环) (状态)

**守护灵光**

- 该生物及其3米（10英尺）范围内的盟友获得该生物的[魅力](Charisma.md "魅力")作为其[豁免检定](Saving_throw.md "豁免检定")的加值。

## 属性

[堆叠ID](Stack_ID.md "堆叠ID")：`AURA_OF_PROTECTION` [状态组](Status_groups.md "状态组")：[SG_RemoveOnRespec](SG_RemoveOnRespec.md "SG RemoveOnRespec")

[堆叠优先级](Stack_priority.md "堆叠优先级")：10

[更多属性](Status_properties.md "状态属性")：

- [忽略休息](Status_properties/IgnoreResting.md "状态属性/忽略休息")

## 状态：守护灵光

| 状态 | 效果 |
| --- | --- |
| [守护灵光](Aura_of_Protection_(Condition).md "守护灵光 (状态)") | 获得等于散发灵光生物的[魅力](Charisma.md "魅力")的[豁免检定](Saving_throw.md "豁免检定")加值。 |
| 守护灵光 | 该生物及其3米（10英尺）范围内的盟友获得该生物的[魅力](Charisma.md "魅力")作为其[豁免检定](Saving_throw.md "豁免检定")的加值。 |

⁠半径：3米（10英尺）

- 获得等于散发灵光生物的[魅力](Charisma.md "魅力")的[豁免检定](Saving_throw.md "豁免检定")加值。

## 守护灵光的来源

- [守护灵光](Aura_of_Protection.md "守护灵光")

## 拥有守护灵光的生物

- [安德斯](Anders.md "安德斯")
- [变节毁灭者](Apostate_Destroyer.md "变节毁灭者")
- [阿登](Arden.md "阿登")
- [本内尔](Bennel.md "本内尔")
- [布拉梅尔斯](Brammels.md "布拉梅尔斯")
- [布林](Brinn.md "布林")
- [玛克隆帕冠军](Champion_of_Mahkloompah.md "玛克隆帕冠军")
- [有魅力的哨兵](Charismatic_Sentinel.md "有魅力的哨兵")
- [清账屋执法者](Counting_House_Enforcer.md "清账屋执法者")
- [德维拉·方塔海德](Devella_Fountainhead.md "德维拉·方塔海德")
- [达格](Dugg.md "达格")
- [埃德维吉](Edvidge.md "埃德维吉")
- [法拉](Falla.md "法拉")
- [堕落斗士](Fallen_Champion.md "堕落斗士")
- [法里尔](Faril.md "法里尔")
- [福斯图姆](Forstum.md "福斯图姆")
- [地狱骑士圣武士](Hellrider_Paladin.md "地狱骑士圣武士")
- [汉古尔](Humgoole.md "汉古尔")
- [约加尔](Iogar.md "约加尔")
- [卡罗尔](Karol.md "卡罗尔")
- [凯瑟里克·索姆](Ketheric_Thorm.md "凯瑟里克·索姆")
- [克伦](Klen.md "克伦")
- [克里兹特](Krizt.md "克里兹特")
- [马利克（狂热信徒）](Malik_(zealot).md "马利克（狂热信徒）")
- [明萨拉](Minthara.md "明萨拉")
- [莫夫蒂](Mofty.md "莫夫蒂")
- [穆尼什](Munish.md "穆尼什")
- [纳迪](Nadi.md "纳迪")
- [尼克洛](Nickelow.md "尼克洛")
- [尼拉姆](Niram.md "尼拉姆")
- [弃誓骑士](Oathbreaker_Knight.md "弃誓骑士")
- [破誓恶棍](Oathless_Wretch.md "破誓恶棍")
- [雷南](Renan.md "雷南")
- [里尔维斯](Rylveth.md "里尔维斯")
- [索雷尼](Soreni.md "索雷尼")
- [塔斯利姆](Taslim.md "塔斯利姆")
- [维基](Vikki.md "维基")
- [至上真神狂热信徒](Zealot_of_the_Absolute.md "至上真神狂热信徒")
- [赛夫洛](Zevlor.md "赛夫洛")
- [齐拉](Zira.md "齐拉")

## 具有相同堆叠ID的状态

| 状态 | 效果 |
| --- | --- |
| [守护灵光](Aura_of_Protection_(Condition).md "守护灵光 (状态)") | 获得等于散发灵光生物的[魅力](Charisma.md "魅力")的[豁免检定](Saving_throw.md "豁免检定")加值。 |
| 守护灵光 | 该生物及其3米（10英尺）范围内的盟友获得该生物的[魅力](Charisma.md "魅力")作为其[豁免检定](Saving_throw.md "豁免检定")的加值。 |

\_(状态)守护灵光

- 该生物及其3米（10英尺）范围内的盟友获得该生物的[魅力](Charisma.md "魅力")作为其[豁免检定](Saving_throw.md "豁免检定")的加值。

## 简要预览

该生物及其3米（10英尺）范围内的盟友获得该生物的[魅力](Charisma.md "魅力")作为其[豁免检定](Saving_throw.md "豁免检定")的加值。

---
*Source: [Aura of Protection (Aura) (Condition)](https://bg3.wiki/wiki/Aura_of_Protection_(Aura)*