# 执法者短棒

本文介绍的内容**[无法通过正常游戏](Category_colon_Inaccessible.md "Category:Inaccessible")**获取，但仍可通过第三方工具访问。

**执法者短棒**是[短棒](Clubs.md "短棒")武器家族中一种不常见、轻度附魔（+1）的变体。它是一种[简易近战武器](Weapons.md "武器")，可单手挥舞。它是一种[轻型](Light_(weapon_property).md) 武器，任何人都无需特殊训练即可双持。

有时，恐惧的钝性应用（以及，不可否认，一些挥舞的肘击）是持续混乱与重建秩序之间的区别。

## 属性

伤害

1d4 + 1 (2~5) + [力量调整值](Damage_Roll.md#Modifiers "伤害掷骰") ⁠[钝击](Bludgeoning.md "钝击")

详情
[短棒](Clubs.md "短棒")
稀有度：不常见
附魔：**+1**
单手
[轻型](Light_(weapon_property).md "轻型（武器属性）")
近战：1.5 米（5 英尺）
重量：0.9 千克（1.8 磅）
价格：65 金币
UID `MAG_Enforcer_NonLethalFright_Club` UUID `d3cd1f93-64bb-4d57-bafc-19ee1cefc564` Stats `MAG_Enforcer_NonLethalFright_Club` ### 特殊

**持有此物品者获得：**

[手下留情](Non-Lethality.md "手下留情")
此武器仅造成非致命伤害。

### 武器动作

_如果你拥有[熟练项](Proficiency.md "熟练项")，装备在**主手**中可获得：_

[震荡猛击](Concussive_Smash.md "震荡猛击") ()
全力击中敌人以造成伤害，并可能使其[眩晕](Dazed_(Condition).md "眩晕（状态）")。（充能：[短休](Short_rest.md "短休")。）

[弱化打击](Weakening_Strike.md "弱化打击") ()
以非致命攻击瞄准敌人的手，可能施加[弱腕](Weak_Grip_(Condition).md "弱腕（状态）")。（充能：[短休](Short_rest.md "短休")。）

[摔翻打击](Backbreaker.md "摔翻打击") ()
在攻击中施加额外力量，可能使敌人[倒伏](Prone_(Condition).md "倒伏（状态）")。（充能：[短休](Short_rest.md "短休")。）

---
*Source: [Enforcer Club](https://bg3.wiki/wiki/Enforcer_Club)*