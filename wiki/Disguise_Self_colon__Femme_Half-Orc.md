# 易容术：女性半兽人

**易容术：女性半兽人** 是一个 [等级 1 幻术学派法术](Spells.md "Spells")。它是 [伪装术](Disguise_Self.md "Disguise Self") 的一个变体。此法术允许施法者暂时将自己的外观变为女性 [半兽人](Half-Orc.md "Half-Orc") 的形态。

## 描述

将自己伪装成具有女性体型的半兽人。

## 属性

消耗
[动作](Actions.md#Resources "动作") + [一级法术位](Spells.md#Spell_slots "Spells")
详情
范围：自身
[仪式](Spells.md#Ritual_spells "Spells")

## 更高环阶

以此法术的更高环阶施放不会获得额外收益。

## 技术细节

UID

`Shout_DisguiseSelf_HalfOrc_Female`

法术标志

`[HasSomaticComponent](HasSomaticComponent_(spell_flag).md)`, `[HasVerbalComponent](HasVerbalComponent_(spell_flag).md)`, `[IsLinkedSpellContainer](IsLinkedSpellContainer_(spell_flag).md)`, `[IsSpell](IsSpell_(spell_flag).md)`

## 状态：伪装术

**[伪装术](Disguise_Self_(Condition).md "伪装术 (状态)")**

持续时间：直至 [长休](Long_Rest.md "长休")

- 外观完全改变。

## 如何习得

此法术是以下法术的变体：
[伪装术](Disguise_Self.md "伪装术")

---
*Source: [Disguise Self: Femme Half-Orc](https://bg3.wiki/wiki/Disguise_Self:_Femme_Half-Orc)*