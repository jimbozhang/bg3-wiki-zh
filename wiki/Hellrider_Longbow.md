# 地狱骑士团长弓

地狱骑士团长弓是一件稀有度为“精良”的+1 [长弓](Longbows.md "长弓")，能为使用者提供高额的[先攻](Initiative.md "先攻")加成和[优势](Advantage.md "优势")进行[察觉技能](Perception.md "察觉技能")[属性检定](Ability_Check.md "属性检定")。在攻击命中时，它还可能对目标施加[妖火](Faerie_Fire_(Condition).md) 状态。

这把长弓由银剑斯坦尼斯拉夫的盾牌残骸锻造而成，于数年前在[城堡拉文洛夫](https://forgottenrealms.fandom.com/wiki/Castle_Ravenloft)的废墟附近被发现。

## 属性

伤害

1d8 + 1 (2~9) + [敏捷调整值](Damage_Roll.md#Modifiers "伤害掷骰") [穿刺](Piercing.md "穿刺")

详情
[长弓](Longbows.md "长弓")
稀有度：精良
附魔：**+ 1**
[双手](Two-Handed.md "双手")
[可蘸取](Dippable.md "可蘸取")
射程：18 米 (60 英尺)
重量：1.1 千克 (2.2 磅)
价格：770 gp / 1550 gpHHonour
UID `MAG_WYR_Hellrider_Longbow` UUID `75c90d55-c262-446d-937c-8adbef1e3e93` Stats `MAG_WYR_Hellrider_Longbow` ### 特殊效果

**持有此物品者获得：**

[加强感知](Heightened_Awareness.md "加强感知")
[先攻](Initiative.md "先攻")掷骰获得+3加成，并在进行[察觉技能](Perception.md "察觉技能")[属性检定](Ability_Check.md "属性检定")时具有[优势](Advantage.md "优势")。

[邪魔火焰](Fiendish_Fire.md "邪魔火焰")
每回合一次，被此武器击中的生物可能遭受[妖火](Faerie_Fire_(Condition).md "妖火 (状态)")效果。

### 武器动作

_若你拥有[熟练项](Proficiency.md "熟练项")，装备在**主手**可获得：_

[腿筋射击](Hamstring_Shot.md "腿筋射击")
射击敌人的大腿，可能使其[移动速度](Movement_speed.md "移动速度")降低50%。(充能：[短休](Short_rest.md "短休")。)

[备战（远程）](Brace_(Ranged).md "备战（远程）")
消耗6米（20英尺）的[移动](Movement_speed.md "移动速度")。在本回合剩余时间内，进行两次远程伤害掷骰并取较高值。(充能：[短休](Short_rest.md "短休")。)

## 获取地点

- [利文顿](Rivington.md "利文顿") X: 43 Y: -101：在[强制征用的谷仓](Requisitioned_Barn.md "强制征用的谷仓")附近向[弗格·德罗戈](Ferg_Drogher.md "弗格·德罗戈")购买

## 备注

- 若[影心](Shadowheart.md "影心")在队伍中，[弗格·德罗戈](Ferg_Drogher.md "弗格·德罗戈")不会出售任何物品（包括此物品），除非她在[第二幕](Act_Two.md "第二幕")中提前杀死了[暗夜之歌](Aylin.md "暗夜之歌")。

_关于邪魔火焰：_

- 妖火状态仅持续一回合，并在目标下一回合开始时移除，因此实际上仅在施加当回合有效。

## 错误

_关于邪魔火焰：_

- 虽然**邪魔火焰**豁免检定会在使用者造成的所有伤害（包括法术和其他武器）时触发，但效果实际上仅在伤害源自此特定武器时才会施加。如果目标对法术或其他近战攻击的豁免检定失败，则不会施加状态效果。

---
*Source: [Hellrider Longbow](https://bg3.wiki/wiki/Hellrider_Longbow)*