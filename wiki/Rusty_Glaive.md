# 锈长柄刀

**锈长柄刀**是[长柄刀](Glaives.md "长柄刀")武器家族的常见变体。它是一种需要双手持握的[近战武器](Weapons.md "近战武器")。它在近战中为使用者提供[额外范围](Extra_Reach.md "额外范围")。

因腐朽而损毁。

## 属性

伤害

1d10 (1~10) + [力量调整值](Damage_Roll.md#Modifiers "伤害掷骰") ⁠[挥砍](Slashing.md "挥砍")

详情
[长柄刀](Glaives.md "长柄刀")
稀有度：普通
惑控学派：无
[双手](Two-Handed.md "双手")
[额外范围](Extra_Reach.md "额外范围")
[可蘸取](Dippable.md "可蘸取")
范围：2.5 米 (8 英尺)
重量：2.7 千克 (5.4 磅)
价格：6 金币
UID `WPN_HUM_Glaive_A_0_Rusty_A` UUID `be861724-206e-483e-a42d-9dd131030ad` ### 武器动作

_如果你拥有[熟练项](Proficiency.md "熟练项")，装备在**主手**以获得：_

[突进攻击](Rush_Attack.md "突进攻击") ()
向前冲锋并攻击路径上的第一个敌人，可能将其推至[失衡](Off_Balance_(Condition).md "失衡（状态）")。（充能：[短休](Short_rest.md "短休")。）

[割裂](Lacerate.md "割裂") ()
劈砍目标的要害部位，使其[流血](Bleeding_(Condition).md "流血（状态）")。（充能：[短休](Short_rest.md "短休")。）

[备战（近战）](Brace_(Melee).md "备战（近战）") ()
消耗 6 米 (20 英尺) 的[移动](Movement_speed.md "移动速度")。在本回合剩余时间内，掷两次近战伤害并取最高值。（充能：[短休](Short_rest.md "短休")。）

## 获取地点

- 在[利文顿](Rivington.md "利文顿")大门附近 X: 10 Y: -131 处的一堆废弃武器和护甲上

\[[Category:挥砍伤害来源]

---
*Source: [Rusty Glaive](https://bg3.wiki/wiki/Rusty_Glaive)*