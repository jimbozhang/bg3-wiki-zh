# 风暴新星

另请参阅：[收集能量](Gather_Power.md "收集能量")

**风暴新星**是仅限[安苏](Ansur.md "安苏")使用的独特职业动作。在[收集能量](Gather_Power.md "收集能量")后，安苏将使用此动作释放毁灭性的闪电伤害，击中所有未找到掩护的生物。

## 描述

用闪电爆炸轰击整个竞技场，可能使你的敌人[感电](Shocked_(Condition).md "感电（状态）")。

## 属性

消耗
[动作](Actions.md#Resources "动作")
伤害：18~180

18d10⁠[闪电](Lightning.md "闪电")

详情
[敏捷](Dexterity.md "敏捷") [豁免检定](Saving_throws.md "豁免检定")（[法术豁免DC](Dice_rolls.md#Spell_save_DC "掷骰")）（豁免成功：目标仍承受一半伤害且不会被击退。）
范围：自身
区域效果：40米（133英尺）半径
击退：2米（7英尺）
充能：每回合

## 技术细节

UID

`Projectile_SuperNova_Dragon_Skeletal`

法术标志

`[CanAreaDamageEvade](CanAreaDamageEvade_(spell_flag).md)`, `[CannotTargetCharacter](https://bg3.wiki/w/index.php?title=CannotTargetCharacter_\(spell_flag\)&action=edit&redlink=1) "CannotTargetCharacter \(spell flag\) \(page does not exist\)")`, `[CannotTargetItems](https://bg3.wiki/w/index.php?title=CannotTargetItems_\(spell_flag\)&action=edit&redlink=1) "CannotTargetItems \(spell flag\) \(page does not exist\)")`, `[HasSomaticComponent](HasSomaticComponent_(spell_flag).md)`, `[IsHarmful](IsHarmful_(spell_flag).md)`, `[RangeIgnoreSourceBounds](https://bg3.wiki/w/index.php?title=RangeIgnoreSourceBounds_\(spell_flag\)&action=edit&redlink=1) "RangeIgnoreSourceBounds \(spell flag\) \(page does not exist\)")`, `[RangeIgnoreTargetBounds](https://bg3.wiki/w/index.php?title=RangeIgnoreTargetBounds_\(spell_flag\)&action=edit&redlink=1) "RangeIgnoreTargetBounds \(spell flag\) \(page does not exist\)")`, `[RangeIgnoreVerticalThreshold](https://bg3.wiki/w/index.php?title=RangeIgnoreVerticalThreshold_\(spell_flag\)&action=edit&redlink=1) "RangeIgnoreVerticalThreshold \(spell flag\) \(page does not exist\)")`

## 状态：感电

**[感电](Shocked_(Condition).md "感电（状态）")**

持续时间：2回合

[敏捷](Dexterity.md "敏捷") [豁免检定](Saving_throws.md "豁免检定")（[法术豁免DC](Dice_rolls.md#Spell_save_DC "掷骰")）

- 无法进行[反应](Reaction.md "反应")，且在使用[敏捷](Dexterity.md "敏捷")的[属性检定](Ability_Check.md "属性检定")和[豁免检定](Saving_throws.md "豁免检定")时具有[劣势](Disadvantage.md "劣势")。

## 状态：倒伏

**[倒伏](Prone_(Condition).md "倒伏（状态）")**

持续时间：2回合

[敏捷](Dexterity.md "敏捷") [豁免检定](Saving_throws.md "豁免检定")（[法术豁免DC](Dice_rolls.md#Spell_save_DC "掷骰")）

- 受影响的生物无法移动或进行[动作](Actions.md#Resources "动作")、[附赠动作](Actions.md#Resources "动作")或[反应](Actions.md#Reactions "动作")，且在[力量](Strength.md "力量")和[敏捷](Dexterity.md "敏捷")的[豁免检定](Saving_throw.md "豁免检定")时具有[劣势](Disadvantage.md "劣势")。
- 在距离生物3米（10英尺）内进行的攻击对倒伏生物具有[优势](Advantage.md "优势")。
- 倒伏生物必须花费其一半的[移动速度](Movement_speed.md "移动速度")才能站起。
  - 拥有[运动员](Athlete.md "运动员")专长的角色仅需花费1.5米（5英尺）的移动速度即可站起。

## 如何习得

由以下生物使用：[安苏](Ansur.md "安苏")

## 错误

- 风暴新星本应通过豁免检定掷骰来减半其伤害，但由于该法术通过脚本施放，此豁免检定掷骰不会发生，所有被风暴新星击中的角色都会受到全额伤害并被击退。

---
*Source: [Stormheart Nova](https://bg3.wiki/wiki/Stormheart_Nova)*