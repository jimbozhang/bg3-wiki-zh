...
**[克雷斯卡的强酸滴落](Adrip_with_Kereska's_Acid_(Condition).md "克雷斯卡的强酸滴落 (状态)")**
|
...
- 游戏中的工具提示根据效果的编码方式有些模糊，内容如下：_“当受威胁时，受影响实体的法术伤害有 +2 加值。”_ 然而，拥有奥术蓄能状态会对*任何* [受威胁](Threatened_(Condition).md "受威胁 (状态)") 的生物造成额外法术伤害；使用者不需要被威胁。
...
- 由于编码错误，此状态还授予吟游诗人激励资源和 d6 吟游诗人激励的反应。使用其中一个反应会消耗吟游诗人激励充能，但不会移除精通吟游诗人激励状态，使非吟游诗人角色在状态结束前拥有一个空的、不可补充的吟游诗人激励充能槽。
...
- 如果 [倒伏](Prone_(Condition).md "倒伏 (状态)") 的豁免检定触发任何需要确认的反应，则倒伏不会被应用，并且无论结果如何或是否选择了任何反应，豁免检定都不会记录在战斗日志中。如果豁免检定由多个残响来源引起，则可能会出现多个连续的确认提示，而不是一个，并且缺少命名相关实体的文本行。
...
- 描述错误地使用了与 [濡湿](Wet_(Condition).md "濡湿 (状态)") 相同的描述，尽管这两种状态具有不同的效果。
