# 操纵死尸：飞行食尸鬼

**操纵死尸：飞行食尸鬼** 是一个 [5级死灵学派法术](Spells.md "Spells")。此法术允许施法者操纵一具尸体来创建一个 [飞行食尸鬼](Flying_Ghoul.md "Flying_Ghoul")。需要一具尸体，并且持续到长休。

## 描述

创建一个擅长近战的 [飞行食尸鬼](Flying_Ghoul.md "Flying Ghoul")。

目标必须是中级或小型尸体。

## 属性

消耗
[动作](Actions.md#Resources "动作") + [5级法术位](Spells.md#Spell_slots "法术位")
详情
射程：3米（10英尺）
目标：中级或小型尸体

## 更高环阶施法

以更高环阶施法此法术不会获得额外收益。

## 技术细节

UID

`Target_AnimateDead_FlyingGhoul`

法术标志

`[HasSomaticComponent](HasSomaticComponent_(spell_flag).md)`, `[HasVerbalComponent](HasVerbalComponent_(spell_flag).md)`, `[IgnorePreviouslyPickedEntities](IgnorePreviouslyPickedEntities_(spell_flag).md)`, `[IsSpell](IsSpell_(spell_flag).md)`

## 生物：飞行食尸鬼

| [力量](Strength.md "力量") 16 (+3) | [敏捷](Dexterity.md "敏捷") 17 (+3) | [体质](Constitution.md "体质") 10 | [智力](Intelligence.md "智力") 11 | [感知](Wisdom.md "感知") 10 | [魅力](Charisma.md "魅力") 8 (-1) |
| --- | --- | --- | --- | --- | --- |
|  |  |  |  |  |  |
| 持续时间 | 直到长休 |  |  |  |  |
| 生命值 | 30 |  |  |  |  |
| 护甲等级 | 15 |  |  |  |  |
| 移动速度 | 12米（40英尺） |  |  |  |  |

[爪击](Claws_(Flying_Ghoul).md "爪击（飞行食尸鬼）") ()
受影响的生物可以用致命的爪子猛击，并可能使目标[麻痹](Paralysed_(Condition).md "麻痹（状态）")。

[吞噬](Devour_(Animate_Dead_Ghoul).md "吞噬（操纵死尸食尸鬼）") ()
啃咬一个被击倒、倒伏或沉睡的目标，并造成3d10 + 力量调整值[挥砍](Slashing.md "挥砍")伤害。治疗等同于该数值的生命值。

飞向目标位置

## 如何学习

此法术是以下法术的变体：
[操纵死尸](Animate_Dead.md "操纵死尸")

---
*Source: [Animate Dead: Flying Ghoul](https://bg3.wiki/wiki/Animate_Dead:_Flying_Ghoul)*