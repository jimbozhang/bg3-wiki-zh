...
不要与[缴械攻击](Disarming_Attack.md "缴械攻击")混淆

**缴械打击**是一种[武器动作](Weapon_action.md "武器动作")，可供[重甲的](Proficient.md "重甲的")使用三叉戟并将其握在主手的角色使用。
...
- 成功的[力量](Strength.md "力量") [豁免检定](Saving_throw.md "豁免检定")可防止武器掉落，但伤害仍会生效。
- 此动作本质上是[战斗大师](Battle_Master.md "战斗大师")战士可用的[缴械攻击（近战）](Disarming_Attack_(Melee).md "缴械攻击（近战）")动作的较弱变体。

## 错误

- 此动作的工具提示显示“未命中：不消耗卓越骰”。这是一个简单的显示错误；该动作不需要[卓越骰](Battle_Master.md#Level_3 "战斗大师")，命中成功时也不会消耗任何骰子。
