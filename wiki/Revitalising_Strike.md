# 再生打击

**再生打击** 是由 [跳跳](Hoppy.md "跳跳") 赋予的 [武器动作](Weapon_action.md "武器动作")。

## 描述

猛击敌人，使其受伤并治疗自身的伤势。

命中时，治疗 1d6⁠⁠[治疗](Healing.md "治疗")。你造成额外的 ⁠[黯蚀](Necrotic.md "黯蚀") 伤害，伤害值等于你的 [熟练项加值](Proficiency_Bonus.md "熟练项加值")。

## 属性

消耗
[动作](Actions.md#Resources "动作")
伤害：

普通武器伤害

\+ [熟练项加值](Proficiency_Bonus.md "熟练项加值")⁠[黯蚀](Necrotic.md "黯蚀")

治疗：1~6

1d6⁠[治疗](Healing.md "治疗") (对自身)

详情
近战武器 [攻击掷骰](Attack_roll.md "攻击掷骰")
射程：普通武器射程
充能：[短休](Short_rest.md "短休")
必须对武器 [熟练](Proficiency.md#Weapon_proficiency "熟练项")

## 技术细节

UID

`Target_MAG_WeaponAction_RevitalizingStrike`

法术标志

`[IsDefaultWeaponAction](https://bg3.wiki/w/index.php?title=IsDefaultWeaponAction_\(spell_flag\)&action=edit&redlink=1) "IsDefaultWeaponAction \(spell flag\) \(page does not exist\)")`, `[IsHarmful](IsHarmful_(spell_flag).md)`, `[IsMelee](IsMelee_(spell_flag).md)`

## 学习方式

由物品赋予：

- [跳跳](Hoppy.md "跳跳")

## 视觉效果

加载视频

本地文件

本地文件可能会收集个人数据。

继续 关闭

<https://bg3.wiki/wiki/File:Revitalising_Strike_Action.mp4>

---
*Source: [Revitalising Strike](https://bg3.wiki/wiki/Revitalising_Strike)*