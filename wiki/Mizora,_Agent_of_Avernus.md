# 米佐拉，阿弗纳斯代理人

**米佐拉，阿弗纳斯代理人** 是一个独特的类动作，仅在 [与主脑对峙](Confront_the_Elder_Brain.md "与主脑对峙") 任务期间可用。它允许队伍召唤 [米佐拉](Mizora.md "米佐拉") 在最终战斗中协助他们。

## 描述

召唤威尔的契约女主人，降下强大的地狱术法，并可能使你的敌人 [崩溃](Disintegrate.md "崩溃")。

仅在战斗中可用。
战斗结束后会解散，直到重新召唤。
如果在战斗中被杀死，则无法再次召唤。

## 属性

消耗
[动作](Actions.md#Resources "动作")
详情
范围：18 米（60 英尺）

## 技术细节

UID

`Target_END_AllyAbility_MizoraSummon`

法术标志

`[CannotTargetCharacter](https://bg3.wiki/w/index.php?title=CannotTargetCharacter_\(spell_flag\)&action=edit&redlink=1) "CannotTargetCharacter \(spell flag\) \(page does not exist\)")`, `[CannotTargetItems](https://bg3.wiki/w/index.php?title=CannotTargetItems_\(spell_flag\)&action=edit&redlink=1) "CannotTargetItems \(spell flag\) \(page does not exist\)")`, `[HasVerbalComponent](HasVerbalComponent_(spell_flag).md)`, `[IgnoreVisionBlock](IgnoreVisionBlock_(spell_flag).md)`, `[IsLinkedSpellContainer](IsLinkedSpellContainer_(spell_flag).md)`, `[Wildshape](https://bg3.wiki/w/index.php?title=Wildshape_\(spell_flag\)&action=edit&redlink=1) "Wildshape \(spell flag\) \(page does not exist\)")`

## 生物：米佐拉

| [力量](Strength.md "力量") 14 (+2) | [敏捷](Dexterity.md "敏捷") 15 (+2) | [体质](Constitution.md "体质") 16 (+3) | [智力](Intelligence.md "智力") 14 (+2) | [感知](Wisdom.md "感知") 12 (+1) | [魅力](Charisma.md "魅力") 20 (+5) |
| --- | --- | --- | --- | --- | --- |
|  |  |  |  |  |  |
| 持续时间 | 直至战斗结束 |  |  |  |  |
| 生命值 | 75 |  |  |  |  |
| 护甲等级 | 12 |  |  |  |  |

## 法术

- [摄心目光](Eyebite.md "摄心目光")
- [行动自如](Freedom_of_Movement.md "行动自如")
- [怪物定身术](Hold_Monster.md "怪物定身术")
- [崩溃](Disintegrate.md "崩溃")
- [加速术](Haste.md "加速术")
- [任意门](Dimension_Door.md "任意门")
- [疯狂](Crown_of_Madness.md "疯狂")
- [命令术](Command.md "命令术")
- [繁彩球](Chromatic_Orb.md "繁彩球")

## 超魔

米佐拉拥有 12 个 [术法点](Sorcery_Point.md "术法点")，可用于以下 [超魔](Metamagic.md "超魔") 选项：

- [超魔：远距法术](Metamagic_colon__Distant_Spell.md "超魔：远距法术")
- [超魔：延长法术](Metamagic_colon__Extended_Spell.md "超魔：延长法术")
- [超魔：成双法术](Metamagic_colon__Twinned_Spell.md "超魔：成双法术")
- [超魔：高强法术](Metamagic_colon__Heightened_Spell.md "超魔：高强法术")
- [超魔：瞬发法术](Metamagic_colon__Quickened_Spell.md "超魔：瞬发法术")

## 如何习得

此法术是以下法术的变体：
[召集盟友（类动作）](Gather_Your_Allies_(class_action).md "召集盟友（类动作）")

其他习得方式：

- 通过在 [召集盟友](Gather_Your_Allies.md "召集盟友") 期间获得 [米佐拉](Mizora.md "米佐拉") 作为盟友。

## 注释

- 当被召唤时，米佐拉会调侃道：_“我以为你永远不会问呢。”_

---
*Source: [Mizora, Agent of Avernus](https://bg3.wiki/wiki/Mizora,_Agent_of_Avernus)*