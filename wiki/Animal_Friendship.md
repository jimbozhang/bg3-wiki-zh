# 化兽为友

**化兽为友**是一个[法术](Spells.md "法术")。它魅惑一个[野兽](Beast.md "Beast")，使其相信施法者并无恶意。

## 描述

说服野兽不要攻击你。

该生物的[智力](Intelligence.md "Intelligence")必须为3或更低。
如果施法者或盟友伤害目标，则状态提前结束。
在更高难度模式下，法术结束时目标可能变得敌对。

## 属性

消耗
[动作](Actions.md#Resources "动作") + [1环法术位](Spells.md#Spell_slots "法术")
详情
[感知](Wisdom.md "Wisdom") [豁免检定](Saving_throws.md "Saving throws")（[法术豁免DC](Dice_rolls.md#Spell_save_DC "Dice rolls")）
射程：18米（60英尺）

## 升环施法

[升环施法](Upcasting.md "Upcasting")：每比1环高1环，影响一个额外目标。

## 技术细节

UID

`Target_AnimalFriendship`

法术标志

`[HasHighGroundRangeExtension](HasHighGroundRangeExtension_(spell_flag).md)`, `[HasSomaticComponent](HasSomaticComponent_(spell_flag).md)`, `[HasVerbalComponent](HasVerbalComponent_(spell_flag).md)`, `[IgnorePreviouslyPickedEntities](IgnorePreviouslyPickedEntities_(spell_flag).md)`, `[IsSpell](IsSpell_(spell_flag).md)`

## 状态：魅惑

**[魅惑](Charmed_(Condition).md "魅惑 (状态)")**

持续时间：10回合

- 无法攻击施法者。施法者在对话中进行[魅力](Charisma.md "Charisma")[属性检定](Ability_Check.md "属性检定")时具有[优势](Advantage.md "Advantage")。

## 学习方式

职业：

- 职业等级1：[吟游诗人](Bard.md "Bard")、[德鲁伊](Druid.md "Druid")和[自然领域](Nature_Domain.md "Nature Domain")（领域法术）
- 职业等级2：[游侠](Ranger.md "Ranger")

由特性提供：

- [魔法学徒：吟游诗人](Magic_Initiate_colon__Bard.md "魔法学徒：吟游诗人")
- [魔法学徒：德鲁伊](Magic_Initiate_colon__Druid.md "魔法学徒：德鲁伊")

由物品提供：

- [驯兽师铁链](Beastmaster's_Chain.md "驯兽师铁链")（充能：[长休](Long_Rest.md "长休")）

其他学习方式：

- [法师](Wizard.md "Wizard") _无法_ 将[化兽为友卷轴](Scroll_of_Animal_Friendship.md "化兽为友卷轴")[抄录](Transcribing_scrolls.md "抄录卷轴")到他们的法术书中。

## 外部链接

- ⁠[Animal friendship](https://forgottenrealms.fandom.com/wiki/Animal_friendship) 在 [被遗忘的国度维基](https://forgottenrealms.fandom.com/wiki/Main_Page) 上

---
*Source: [Animal Friendship](https://bg3.wiki/wiki/Animal_Friendship)*