# 闪电回流

**闪电回流**是[西索博得·索姆](Thisobald_Thorm.md "西索博得·索姆")在拥有[闪电回流](Electrified_Reflux_(Condition).md "闪电回流（状态）")状态时可用的动作。他会在区域内呕吐带电的液体，并变得略微醉醺醺。

## 描述

啜饮一口酿酒师的特制烈酒后，他会呕吐出带电的液体，能够[震慑](Stunned_(Condition).md "震慑（状态）")被击中的目标。

## 属性

消耗
[动作](Actions.md#Resources "动作")
伤害：6~48 + 调整值

6d8 + [力量调整值](Strength.md#Strength_modifier_chart "力量")⁠[闪电](Lightning.md "闪电")

详情
[体质](Constitution.md "体质") [豁免检定](Saving_throws.md "豁免检定")（[DC](DC.md "DC") 16）（豁免成功：目标仍承受一半伤害。）
范围：4米（13英尺）
创造区域：带电的水
充能：每回合

## 状态：震慑

**[震慑](Stunned_(Condition).md "震慑（状态）")**

持续时间：2回合

[体质](Constitution.md "体质") [豁免检定](Saving_throws.md "豁免检定")（[DC](DC.md "DC") 16）

- 受影响的生物无法移动或使用[动作](Actions.md#Resources "动作")、[附赠动作](Actions.md#Resources "动作")或[反应](Actions.md#Reactions "动作")。
- 受影响的生物自动在[力量](Strength.md "力量")和[敏捷](Dexterity.md "敏捷")[豁免检定](Saving_throw.md "豁免检定")中失败。
- 可通过[行动自如](Freedom_of_Movement.md "行动自如")、[高等复原术](Greater_Restoration.md "高等复原术")、[解缚打击](Unshackling_Strike.md "解缚打击")移除。
- 对受影响生物的攻击掷骰具有[优势](Advantage.md "优势")。

## 状态：微醺

**[微醺](Moderately_Inebriated_(Condition).md "微醺（状态）")**

- 每啜饮一口特制烈酒，西索博得会变得更加醉醺醺。
- 他在魅力和敏捷[属性检定](Ability_Check.md "属性检定")上具有[劣势](Disadvantage.md "劣势")，并开始产生小魔鬼的幻觉。

## 区域：带电的水

**[带电的水](Electrified_Water.md "带电的水")**

持续时间：2回合

范围：2米（7英尺）半径

每回合造成1d4⁠⁠[闪电](Lightning.md "闪电")伤害。

类型：[地表](Area.md#Surface "区域")

### 状态：电击

**[电击](Electrocuted_(Condition).md "电击（状态）")**

- 每回合开始时承受1d4⁠⁠[闪电](Lightning.md "闪电")伤害。

## 学习方式

由以下生物使用：[西索博得·索姆](Thisobald_Thorm.md "西索博得·索姆")

## 备注

- 使用此动作后，西索博得可使用一次[强酸幻觉](Acid_Hallucination.md "强酸幻觉")。

---
*Source: [Electrified Regurgitation](https://bg3.wiki/wiki/Electrified_Regurgitation)*