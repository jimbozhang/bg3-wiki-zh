...
**状态**是角色状态的变化，包括有益和有害的。它们可以由某些[法术](Spells.md "Spells")、职业特性、地表和其他环境危害等施加。许多状态是暂时的，有些也可以通过正确的法术或药水移除。
...
忽略
新状态不被应用，现有状态保持原样，保留其当前持续时间。通常用于状态的装备变体，例如，如果你拥有[树肤术](Barkskin_(Condition).md) 直到长休，并装备了[树肤护甲](Barkskin_Armour.md "树肤护甲")，来自护甲的状态不会替换你直到长休拥有的状态。
覆盖
...
状态的持续时间由施加该状态的来源决定。有些状态没有持续时间。这可能是基于其他条件激活的状态，例如被动特性或站在某物附近。状态[轻度遮蔽](Lightly_Obscured_(Condition).md "轻度遮蔽 (状态)")是一个没有持续时间的状态，相反，如果你在光线不足的地方[隐藏](Hiding_(Condition).md "隐藏 (状态)")，它会自动应用，当不再隐藏时自动移除。还有一些状态不会随时间减少持续时间。这可能是持续时间因其他原因而减少的状态，例如[奥术守御](Arcane_Ward_(Condition).md "奥术守御 (状态)")，当你被击中时会减少持续时间。状态在对话或过场动画中也不会减少持续时间。然而，这仅适用于参与对话/过场动画的角色，而不仅仅是旁听的角色。
...
开始回合
状态在生物回合开始时减少持续时间，这是默认行为。具有此节拍类型的状态将持续时间比其声明持续时间少一回合。例如，1回合持续时间的[冰冻](Frozen_(Condition).md "冰冻 (状态)")效果（来自[冻僵](Chilled_(Condition).md "冻僵 (状态)")）将在冰冻生物的回合开始时立即结束，而不会使其失效一回合。2回合的[冰冻](Frozen_(Condition).md "冰冻 (状态)")效果（来自[冷冻](Encrusted_with_Frost_(Condition).md "冷冻 (状态)")）由于相同原因实际上只持续1回合。
结束回合
状态在生物回合结束时减少持续时间。[剑刃防护](Blade_Ward_(Condition).md "剑刃防护 (状态)")就是一个例子。如果一个生物对自己施加一个持续1回合的此类状态，它将在结束回合后立即失去该状态，例如使用[动作如潮](Action_Surge.md "动作如潮")。
开始轮次
状态在_轮次_开始时减少持续时间。轮次开始发生在先攻顺序中的第一个生物开始其回合之前。
结束轮次
状态在_轮次_结束时减少持续时间。轮次结束发生在先攻顺序中的最后一个生物结束其回合之后。
...
| [无敌](Status_properties/IsInvulnerable.md "状态属性/无敌") | 应用此状态的生物无法受到伤害。 |
...
1. [↑](#cite_ref-representative_1-0) 许多状态类型有一个特定的状态作为最基本的形式。
