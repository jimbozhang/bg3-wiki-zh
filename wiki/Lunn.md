# 莱辛

## 竖琴手莱辛

- 模型
- 肖像画

### 声音 / 演员

- [Michael Gukas](https://www.imdb.com/name/nm11933813/)

### 阵营

[竖琴手同盟](Harpers.md "Harpers") [巴尔](Bhaal.md "巴尔")

## 属性

### 体型

[中级](Medium.md "Medium")

### 类型

[怪兽](Monstrosity.md "Monstrosity")

### 种族

[变形怪](Doppelganger.md "Doppelganger")

- 平衡

- 探索者

- 硬核

## 平衡难度属性

### 等级

8

### 生命值

52

### 护甲等级

14

### 先攻

+4

### 熟练项

+3

### 移动

9 米 (30 英尺)

### 重量

75 千克 (150 磅)

### 高级黑暗视觉

24 米 (80 英尺)

### 经验值奖励

200

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 11 / (0) | 18 / (+4) | 14 / (+2) | 11 / (0) | 12 / (+1) | 14 / (+2) |

## 特性

- [高级黑暗视觉](Darkvision.md "高级黑暗视觉")

能在黑暗中看到一定距离。

- [**多重攻击：形态伏击**](Multiattack_colon__Morphic_Ambush.md "多重攻击：形态伏击")

如果目标被[伏击](Ambushed_(Condition).md "伏击 (状态)")，则可进行[多重攻击](Multiattack_(Doppelganger).md "多重攻击 (变形怪)")。

- [借机攻击](Opportunity_Attack.md "借机攻击")

攻击离开攻击范围的敌人。

## 探索者难度属性

### 等级

8

### 生命值

36

### 护甲等级

14

### 先攻

+4

### 熟练项

+5

### 移动

9 米 (30 英尺)

### 重量

75 千克 (150 磅)

### 高级黑暗视觉

24 米 (80 英尺)

### 经验值奖励

200

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 0 | +7 | +2 | 0 | +1 | +2 |

## 特性

- [高级黑暗视觉](Darkvision.md "高级黑暗视觉")

能在黑暗中看到一定距离。

- [**多重攻击：形态伏击**](Multiattack_colon__Morphic_Ambush.md "多重攻击：形态伏击")

如果目标被[伏击](Ambushed_(Condition).md "伏击 (状态)")，则可进行[多重攻击](Multiattack_(Doppelganger).md "多重攻击 (变形怪)")。

- [借机攻击](Opportunity_Attack.md "借机攻击")

攻击离开攻击范围的敌人。

## 硬核难度属性

### 等级

8

### 生命值

67

### 护甲等级

14

### 先攻

+4

### 熟练项

+3

### 移动

9 米 (30 英尺)

### 重量

75 千克 (150 磅)

### 高级黑暗视觉

24 米 (80 英尺)

### 经验值奖励

200

| 属性值 | 运动 | 体操 | 察觉技能 |
| --- | --- | --- | --- |
|  | 0 | 1 | 1 |

## 特性

- [高级黑暗视觉](Darkvision.md "高级黑暗视觉")

能在黑暗中看到一定距离。

- [读心者](Mindreader.md "读心者")

远程攻击对该生物具有[劣势](Disadvantage.md "劣势")。

- [**多重攻击：形态伏击**](Multiattack_colon__Morphic_Ambush.md "多重攻击：形态伏击")

如果目标被[伏击](Ambushed_(Condition).md "伏击 (状态)")，则可进行[多重攻击](Multiattack_(Doppelganger).md "多重攻击 (变形怪)")。

- [借机攻击](Opportunity_Attack.md "借机攻击")

攻击离开攻击范围的敌人。

## 技术信息

###

UID

`S_WYR_Axe_DoppelgangerAssassin_002` ###

UUID

`ba5b21ad-bcc4-407c-85a3-f0448a7094b5` ###

属性

`Doppelganger` ## 变体 1

###

UID

`WYR_Axe_DoppelgangerShape_02` ###

UUID

`f768841e-ce20-422b-85b1-64a48dbd5046` ###

属性

`Elf_Caster` **莱辛**曾是[竖琴手同盟](Harpers.md "Harpers")的一名[木精灵](Wood_elf.md "木精灵")成员。她最初的任务是调查[巴尔](Bhaal.md "巴尔")在城中的活动，并寻找[明斯克](Minsc.md "明斯克")的下落，但后来被一只名为**伦恩**的[变形怪](Doppelganger.md "变形怪")杀害并冒充。

## 目录

- [1 历史](#历史)
  - [1.1 巴尔教团复苏](#巴尔教团复苏)
- [2 参与](#参与)
  - [2.1 巴尔教团伏击](#巴尔教团伏击)
- [3 战斗](#战斗)
  - [3.1 攻击与能力](#攻击与能力)
- [4 战利品](#战利品)
- [5 相关任务](#相关任务)
- [6 备注](#备注)
- [7 参考文献](#参考文献)

## 历史

### 巴尔教团复苏

在[博德之门](Baldur's_Gate.md "博德之门")出现[巴尔](Bhaal.md "巴尔")活动的传闻后，[贾希拉](Jaheira.md "贾希拉")命令她的竖琴手们调查巴尔教团的阴谋，尽可能多地收集信息，而她则率领另一支竖琴手部队进入[幽影诅咒之地](Shadow-Cursed_Lands.md "幽影诅咒之地")以击败[凯瑟里克·索姆](Ketheric_Thorm.md "凯瑟里克·索姆")。然而，当贾希拉命令竖琴手们集中精力寻找拉什玛尔人时，莱辛和她的竖琴手小组成员要么被杀，要么被俘，随后被带到了[奥林](Orin.md "奥林")处。

该竖琴手小组的唯一幸存者，一名名叫[杰拉尔杜斯](Geraldus.md "杰拉尔杜斯")的新兵，被用作诱饵，引诱贾希拉和队伍前来接触，企图伏击并灭口<sup>[\[1\]](#cite_note-1)</sup>。

## 参与

### 巴尔教团伏击

主条目：[高阶竖琴手](The_High_Harper.md "高阶竖琴手")

在[飞龙关](Wyrm's_Crossing.md "飞龙关")与贾希拉同行时，她指出了[丹瑟隆的飞斧](Danthelon's_Dancing_Axe.md "丹瑟隆的飞斧")，这是一家由[恩索尔·丹瑟隆](Entharl_Danthelon.md "恩索尔·丹瑟隆")经营的商店，也是竖琴手安全屋的掩护。在商店下方的一个密室中，被冒充的莱辛、[切尔文](Chelvin.md "切尔文")、[莱斯廷](Leistine.md "莱斯廷")和[索诺斯](Thonos.md "索诺斯")正等待伏击贾希拉和队伍。

当队伍和贾希拉抵达时，杰拉尔杜斯试图通过使用旧的竖琴手暗号来传达在场的竖琴手是[变形怪](Doppelganger.md "变形怪")。如果贾希拉在队伍中，她会理解这个暗号并向队伍提到被冒充的切尔文看起来很眼熟，让她想起了[马库斯](Marcus_Falgor.md "马库斯·法尔戈尔")。未能识别该信号会触发与队伍的战斗，队伍将处于[受惊](Surprised_(Condition).md "受惊 (状态)")状态，而识别暗号并采取行动则会使被冒充的竖琴手们措手不及。

## 战斗

### 攻击与能力

[形态伏击](Morphic_Ambush.md "形态伏击") ()
经历一次惊人的变形以惊吓你的敌人。成功[伏击](Ambushed_(Condition).md "伏击 (状态)")的实体更容易受到你的攻击。

范围：自身
冷却：每场战斗

[多重攻击](Multiattack_(Doppelganger).md "多重攻击 (变形怪)") ()

伤害：12~32

2d6 + 4⁠[钝击](Bludgeoning.md "钝击")

\+ 2d6 + 4⁠[钝击](Bludgeoning.md "钝击")

降临在[伏击](Ambushed_(Condition).md "伏击 (状态)")的生物身上，用你的变形怪利爪将其开膛破肚。

近战：1.5 米 (5 英尺)
冷却：每回合

[远程攻击](Ranged_Attack.md "远程攻击") () – [轻弩](Light_Crossbow.md "轻弩")

伤害：5~12

1d8 + 4⁠[穿刺](Piercing.md "穿刺")

使用装备的武器进行远程攻击。

范围：18 米 (60 英尺)

## 战利品

- [火焰箭](Arrow_of_Fire.md "火焰箭")

## 相关任务

- ⁠[高阶竖琴手](The_High_Harper.md "高阶竖琴手")

## 备注

- 被冒充的竖琴手都共享相同的[变形怪](Doppelganger.md "变形怪")属性块，但游戏文件中存在莱辛未被冒充版本的属性，但在游戏中完全无法访问。
  - 变形怪形态重 65 千克。
  - 莱辛的原始版本拥有 10 力、12 敏、10 体、18 智、12 感和 14 魅；尽管拥有“Elf_Caster”属性，但她却是一个远程职业。
- 莱辛是[竖琴手拉珊卓](Lassandra.md "拉珊卓")的精确物理复制品，拥有完全相同的服装、相同的饰环、相同的外貌——除了她是金发而非黑发。
  - 然而，尽管莱辛与身为[人类](Human.md "人类")的拉珊卓完全相同，但根据游戏文件，莱辛实际上是一名[木精灵](Wood_elf.md "木精灵")。

## 参考文献

1. [↑](#cite_ref-1) _“她说——它说——我会像往常一样向你报告。把你引到这里来，然后……我别无选择。”_ — 杰拉尔杜斯

---
*Source: [Leysin](https://bg3.wiki/wiki/Leysin)*