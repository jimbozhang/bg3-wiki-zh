# 获得魔宠：小魔鬼

**获得魔宠：小魔鬼** 是一个 [法术](Spells.md "法术")。此法术是 [获得魔宠](Find_Familiar.md "Find Familiar") 法术的一个变体，可召唤一只小型小魔鬼。

## 描述

召唤一只具有小魔鬼形态的魔宠，它能够飞行、进入[隐形](Invisibility_(Imp).md "Invisibility (Imp)")状态，并对敌人使用[钉刺](Sting_(Imp).md "Sting (Imp)")。

## 属性

消耗
[动作](Actions.md#Resources "Actions") + [1级法术位](Spells.md#Spell_slots "Spells")
详情
范围：18米（60英尺）
充能：[短休](Short_rest.md "Short rest")
[仪式](Spells.md#Ritual_spells "Spells")

## 高等级施法

以更高等级施放此法术不会获得额外收益。

## 生物：魔宠：小魔鬼

| [力量](Strength.md "Strength") 6 (-2) | [敏捷](Dexterity.md "Dexterity") 17 (+3) | [体质](Constitution.md "Constitution") 13 (+1) | [智力](Intelligence.md "Intelligence") 11 | [感知](Wisdom.md "Wisdom") 12 (+1) | [魅力](Charisma.md "Charisma") 14 (+2) |
| --- | --- | --- | --- | --- | --- |
|  |  |  |  |  |  |
| 生命值 | 10 |  |  |  |  |
| 护甲等级 | 13 |  |  |  |  |
| 体型 | 小型 |  |  |  |  |

[爪击](Claws_(Imp).md "Claws (Imp)") ()
用致命的爪子猛击。

飞至目标位置

[隐形](Invisibility_(Imp).md "Invisibility (Imp)") ()
进入隐形状态。

钉刺目标。

## 如何习得

职业：

- 职业等级 3：[邪术师](Warlock.md "Warlock")（通过 [链之魔契](Pact_of_the_Chain.md "Pact of the Chain")）

此法术是以下法术的变体：
[获得魔宠](Find_Familiar.md "Find Familiar")

---
*Source: [Find Familiar: Imp](https://bg3.wiki/wiki/Find_Familiar:_Imp)*