# 歌唱之剑

本文介绍的内容**[无法通过正常游戏](Category_colon_Inaccessible.md "Category:Inaccessible")**获取。但仍可通过第三方工具访问。

**歌唱之剑**是[长剑](Longswords.md "Longswords")武器家族中一种罕见的、轻度附魔（+1）的特殊变体。它是一种[武器](Weapons.md "武器")，可以单手持用，或双手持用以获得额外伤害。

这把剑的每一次碰撞与交击，都来自[极乐境](https://forgottenrealms.fandom.com/wiki/Elysium)或[九层地狱第一层](https://forgottenrealms.fandom.com/wiki/Avernus)（或两者皆是）最后一支凡人乐团演奏的黑暗旋律。

## 属性

单手伤害

1d8 + 1 (2~9) + [力量调整值](Damage_Roll.md#Modifiers "伤害掷骰") [挥砍](Slashing.md "Slashing")

双手伤害

1d10 + 1 (2~11) + [力量调整值](Damage_Roll.md#Modifiers "伤害掷骰") [挥砍](Slashing.md "Slashing")

详情
[长剑](Longswords.md "Longswords")
稀有度：罕见
附魔：**+ 1**
[两用](Versatile.md "Versatile")
[可蘸取](Dippable.md "Dippable")
近战：1.5 米（5 英尺）
重量：1.35 千克（2.7 磅）
价格：85 gp
UID `MAG_Harpers_SingingSword` UUID `5e7030e9-d59c-4edf-8278-8981a8e8baef` ### 特殊效果

**持有此物品者获得：**

- **[生离死别：歌唱](Phalar_Aluve_colon__Sing.md "生离死别：歌唱")**
- **[生离死别：尖啸](Phalar_Aluve_colon__Shriek.md "生离死别：尖啸")**

### 武器动作

_如果你拥有[熟练项](Proficiency.md "熟练项")，装备在**主手**可获得：_

[割裂](Lacerate.md "割裂") ()
劈砍目标的要害部位，使其[流血](Bleeding_(Condition).md "流血（状态）")。（充能：[短休](Short_rest.md "短休")。）

[突进攻击](Rush_Attack.md "突进攻击") ()
向前冲锋并攻击路径上的第一个敌人，可能将其推至[失衡](Off_Balance_(Condition).md "失衡（状态）")。（充能：[短休](Short_rest.md "短休")。）

[剑柄打击](Pommel_Strike.md "剑柄打击") ()
对敌人进行一次非致命攻击，可能使其[眩晕](Dazed_(Condition).md "眩晕（状态）")。（充能：[短休](Short_rest.md "短休")。）

## 备注

- 这把剑很可能是[生离死别](Phalar_Aluve.md "生离死别")的早期版本，有几点显著差异：
  - 该剑将歌唱与尖啸列为两个独立的类别动作，但装备时不会授予其中任何一个。
  - 不存在 +1 [表演](Performance.md "表演")加值。
  - 它不被视为灵巧武器。
  - 描述和 UID 暗示了该武器不同的背景故事/历史。

---
*Source: [Singing Sword](https://bg3.wiki/wiki/Singing_Sword)*