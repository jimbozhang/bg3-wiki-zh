# 杰里斯 (Jerris)

## 杰里斯 (Jerris)

至上真神学徒 (Novice of the Absolute)

- 游戏内 (In-game)
- 模型 (Model)
- 肖像画 (Portrait)

### 身份 (Identity)

女性 (Female)

### 声音 / 演员 (Voice / Actor)

- [Jodie Steele](https://www.imdb.com/name/nm7632738/)

### 阵营 (Faction)

[至上真神教团](Cult_of_the_Absolute.md "Cult of the Absolute")

## 属性 (Stats)

### 体型 (Size)

[中级](Medium.md "Medium")

### 类型 (Type)

[类人生物](Humanoid.md "Humanoid")

### 种族 (Race)

[矮人](Dwarf.md "Dwarf")

### 亚种 (Subrace)

[灰矮人](Duergar.md "Duergar")

### 职业 (Class)

[游侠](Ranger.md "Ranger")

- 平衡 (Balanced)

- 探索者 (Explorer)

- 硬核 (Tactician)

## 平衡难度属性 (Balanced Difficulty Stats)

### 等级 (Level)

5

### HP

44

### AC

11

### 先攻 (Initiative)

+2

### 熟练项 (Proficiency)

+3

### 移动速度 (Movement)

7.5 m (25 ft)

### 重量 (Weight)

75 kg (150 lb)

### 高级黑暗视觉 (Darkvision)

24 m (80 ft)

### XP奖励 (XP reward)

75

| 属性值 (Ability scores) | 力量 (STR) | 敏捷 (DEX) | 体质 (CON) | 智力 (INT) | 感知 (WIS) | 魅力 (CHA) |
| --- | --- | --- | --- | --- | --- | --- |
|  | 12 / (+1) | 15 / (+2) | 12 / (+1) | 10 / (0) | 12 / (+1) | 12 / (+1) |

## 抗性 (Resistances) [(info)](Resistances.md "Resistances")

- ## 额外资源 (Additional Resources)

- - ## 特性 (Features)

- [**灰矮人坚韧**](Duergar_Resilience.md "Duergar Resilience")

对[幻术学派](Illusion.md "Illusion")和被[魅惑](Charmed_(status_group).md)或[麻痹](Paralysed_(Condition).md)的[豁免检定](Saving_throws.md "Saving throws")具有[优势](Advantage.md "Advantage")。

- [**矮人活力**](Dwarven_Resilience.md "Dwarven Resilience")

对被[中毒](Poisoned_(Condition).md)的[豁免检定](Saving_throw.md "Saving throw")具有[优势](Advantage.md "Advantage")，并对[中毒](Poison.md "Poison")伤害具有[抗性](Resistance.md "Resistance")。

- [**额外攻击**](Extra_Attack.md "Extra Attack")

在徒手或武器攻击后可以进行一次额外攻击。

- [**借机攻击**](Opportunity_Attack.md "Opportunity Attack")

攻击离开攻击范围的敌人。

- [**日照敏感**](Sunlight_Sensitivity.md "Sunlight Sensitivity")

在阳光下时，攻击掷骰和[察觉技能](Perception.md "Perception")检定具有劣势。

- [**高级黑暗视觉**](Superior_Darkvision.md "Superior Darkvision")

可以在黑暗中看到24 m / 80 ft。

## 探索者难度属性 (Explorer Difficulty Stats)

### 等级 (Level)

5

### HP

30

### AC

11

### 先攻 (Initiative)

+2

### 熟练项 (Proficiency)

+5

### 移动速度 (Movement)

7.5 m (25 ft)

### 重量 (Weight)

75 kg (150 lb)

### 高级黑暗视觉 (Darkvision)

24 m (80 ft)

### XP奖励 (XP reward)

75

| 属性值 (Ability scores) | 力量 (STR) | 敏捷 (DEX) | 体质 (CON) | 智力 (INT) | 感知 (WIS) | 魅力 (CHA) |
| --- | --- | --- | --- | --- | --- | --- |
|  | +4 | +5 | +1 | 0 | +1 | +1 |

## 抗性 (Resistances) [(info)](Resistances.md "Resistances")

- ## 额外资源 (Additional Resources)

- - ## 特性 (Features)

- [**灰矮人坚韧**](Duergar_Resilience.md "Duergar Resilience")

对[幻术学派](Illusion.md "Illusion")和被[魅惑](Charmed_(status_group).md)或[麻痹](Paralysed_(Condition).md)的[豁免检定](Saving_throws.md "Saving throws")具有[优势](Advantage.md "Advantage")。

- [**矮人活力**](Dwarven_Resilience.md "Dwarven Resilience")

对被[中毒](Poisoned_(Condition).md)的[豁免检定](Saving_throw.md "Saving throw")具有[优势](Advantage.md "Advantage")，并对[中毒](Poison.md "Poison")伤害具有[抗性](Resistance.md "Resistance")。

- [**额外攻击**](Extra_Attack.md "Extra Attack")

在徒手或武器攻击后可以进行一次额外攻击。

- [**借机攻击**](Opportunity_Attack.md "Opportunity Attack")

攻击离开攻击范围的敌人。

- [**日照敏感**](Sunlight_Sensitivity.md "Sunlight Sensitivity")

在阳光下时，攻击掷骰和[察觉技能](Perception.md "Perception")检定具有劣势。

- [**高级黑暗视觉**](Superior_Darkvision.md "Superior Darkvision")

可以在黑暗中看到24 m / 80 ft。

## 硬核难度属性 (Tactician Difficulty Stats)

### 等级 (Level)

5

### HP

57

### AC

11

### 先攻 (Initiative)

+2

### 熟练项 (Proficiency)

+3

### 移动速度 (Movement)

7.5 m (25 ft)

### 重量 (Weight)

75 kg (150 lb)

### 高级黑暗视觉 (Darkvision)

24 m (80 ft)

### XP奖励 (XP reward)

75

| 属性值 (Ability scores) | 运动 (Athletics) | 体操 (Acrobatics) | 察觉技能 (Perception) |
| --- | --- | --- | --- |
|  | 1 | 2 | 1 |

## 抗性 (Resistances) [(info)](Resistances.md "Resistances")

- ## 额外资源 (Additional Resources)

- - ## 特性 (Features)

- [**灰矮人坚韧**](Duergar_Resilience.md "Duergar Resilience")

对[幻术学派](Illusion.md "Illusion")和被[魅惑](Charmed_(status_group).md)或[麻痹](Paralysed_(Condition).md)的[豁免检定](Saving_throws.md "Saving throws")具有[优势](Advantage.md "Advantage")。

- [**矮人活力**](Dwarven_Resilience.md "Dwarven Resilience")

对被[中毒](Poisoned_(Condition).md)的[豁免检定](Saving_throw.md "Saving throw")具有[优势](Advantage.md "Advantage")，并对[中毒](Poison.md "Poison")伤害具有[抗性](Resistance.md "Resistance")。

- [**额外攻击**](Extra_Attack.md "Extra Attack")

在徒手或武器攻击后可以进行一次额外攻击。

- [**借机攻击**](Opportunity_Attack.md "Opportunity Attack")

攻击离开攻击范围的敌人。

- [**日照敏感**](Sunlight_Sensitivity.md "Sunlight Sensitivity")

在阳光下时，攻击掷骰和[察觉技能](Perception.md "Perception")检定具有劣势。

- [**高级黑暗视觉**](Superior_Darkvision.md "Superior Darkvision")

可以在黑暗中看到24 m / 80 ft。

## 技术信息 (Technical)

###

UID

`S_UND_DuergarSquadDilligent` ###

UUID

`a7338653-4d88-440e-aee6-4d5b3e611a2d` ###

Stats

`UND_KC_Duergar_Ranger`

**杰里斯 (Jerris)** 是一名灰矮人游侠，是[海盗格雷蒙](Greymon.md "Greymon")在[幽暗地域](Underdark.md "Underdark")的船员之一，出现在[第一幕](Act_One.md "Act_One")。

> “
>
> 错一步，你就得下湖喂鱼。
>
> „

— 杰里斯 (Jerris)

## 参与 (Involvement)

### 第一幕 (Act One)

从[幽暗地域](Underdark.md "Underdark")海滩登上船只后，一艘由海盗格雷蒙 (Corsair Greymon) 指挥的船驶近了队伍的船。格雷蒙登船，玩家角色必须通过一次检定来说服他让队伍通过前往[复仇之炉](Grymforge.md "Grymforge")。如果检定失败或选择暴力，格雷蒙会发起攻击，杰里斯会加入战斗站在他那一边。

## 战斗 (Combat)

### 攻击与能力 (Attacks and abilities)

[备战（远程）](Brace_(Ranged).md "Brace (Ranged)") () – [重弩](Heavy_Crossbow.md "Heavy Crossbow")
消耗6 m (20 ft)的[移动速度](Movement_speed.md "Movement Speed")。在本回合剩余时间内，掷两次远程伤害并取最高值。

充能：[短休](Short_rest.md "Short rest")

[诱捕攻击（远程）](Ensnaring_Strike_(Ranged).md "Ensnaring Strike (Ranged)") (+ + ) – [重弩](Heavy_Crossbow.md "Heavy Crossbow")

伤害：4~18

1d10 + 2⁠[穿刺](Piercing.md "Piercing")

\+ 1d6⁠[穿刺](Piercing.md "Piercing")

你的攻击会召唤出荆棘藤蔓，可能\_(状态)[诱捕](Ensnared_(Ensnaring_Strike)_(Condition).md "Ensnared (Ensnaring Strike) (Condition)")你的目标。

被诱捕的生物无法移动，并在每回合开始时受到1d6⁠⁠[穿刺](Piercing.md "Piercing")伤害。盟友可以使用他们的[协助](Help.md "Help")动作尝试撕开藤蔓。

[力量](Strength.md "Strength") [豁免检定](Saving_throws.md "Saving throws")
射程：18 m (60 ft)

[主手攻击](Main_Hand_Attack.md "Main Hand Attack") () – [匕首](Dagger.md "Dagger")

伤害：3~6

1d4 + 2⁠[穿刺](Piercing.md "Piercing")

用装备的武器进行近战攻击。

近战：1.5 m (5 ft)

[穿刺射击](Piercing_Shot.md "Piercing Shot") () – [重弩](Heavy_Crossbow.md "Heavy Crossbow")

伤害：3~12

1d10 + 2⁠[穿刺](Piercing.md "Piercing")

造成常规伤害，并可能施加[开放伤口](Gaping_Wounds_(Condition).md "Gaping Wounds (Condition)")，导致攻击时造成额外伤害。

[不死生物](Undead.md "Undead")和[构装生物](Constructs.md "Constructs")无法获得开放伤口。

[体质](Constitution.md "Constitution") [豁免检定](Saving_throws.md "Saving throws")
射程：常规武器射程
充能：[短休](Short_rest.md "Short rest")

[穿刺打击](Piercing_Strike.md "Piercing Strike") () – [匕首](Dagger.md "Dagger")

伤害：3~6

1d4 + 2⁠[穿刺](Piercing.md "Piercing")

刺击敌人并可能施加[开放伤口](Gaping_Wounds_(Condition).md "Gaping Wounds (Condition)")。

[不死生物](Undead.md "Undead")和[构装生物](Constructs.md "Constructs")无法获得开放伤口。

[体质](Constitution.md "Constitution") [豁免检定](Saving_throws.md "Saving throws")
射程：常规武器射程
充能：[短休](Short_rest.md "Short rest")

[远程攻击](Ranged_Attack.md "Ranged Attack") () – [重弩](Heavy_Crossbow.md "Heavy Crossbow")

伤害：3~12

1d10 + 2⁠[穿刺](Piercing.md "Piercing")

用装备的武器进行远程攻击。

射程：18 m (60 ft)

### 盟友 (Allies)

- [炼金术士阿格拉姆](Argram.md "Argram")
- [波纳](Bonna.md "Bonna")
- [格雷蒙](Greymon.md "Greymon")
- [赫克](Hurk.md "Hurk")
- [心灵大师凯特尔](Kettel.md "Kettel")

### 战利品 (Loot)

- [匕首](Dagger.md "Dagger")
- [重弩](Heavy_Crossbow.md "Heavy Crossbow")
- [皮甲](Leather_Armour.md "Leather Armour")

---
*Source: [Jerris](https://bg3.wiki/wiki/Jerris)*