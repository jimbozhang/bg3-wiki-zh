...
另请参阅 [倒伏 (状态类型)](Prone_(Condition_Type).md)
...
- 如果生物在其回合 _开始_ 时变为倒伏状态，它会使用其一半的可用移动速度站起。如果生物在其回合 _期间_ 被击倒，其回合将自动结束。
  - 执行 [协助](Help.md "协助") 动作可以移除倒伏状态，即使该状态是在同一回合内较早获得的，这实际上可以“取消跳过”该角色的回合并允许其正常行动。专注状态仍会被打断。
- 击倒生物并将其移动速度降低至 50% 以下不会自动跳过其下一回合，因为生物使用其 _可用_ 移动速度的 50% 来站起。
- 某些生物对倒伏状态免疫，包括没有实体的生物（例如 [阴影](Shadow.md "阴影") 和 [缚灵](Wraith.md "缚灵")）。
- [塔莎狂笑术](Tasha's_Hideous_Laughter.md "塔莎狂笑术") 会使目标遭受 [狂笑](Hideous_Laughter_(Condition).md)，其效果与倒伏相同，但技术上不被视为倒伏。因此，该法术的效果无法通过 [协助](Help.md "协助") 移除。
- 变为倒伏状态会立即结束 [专注](Concentration.md "专注")。
...
两回合是倒伏效果的典型持续时间。由于倒伏的持续时间在受影响生物回合开始时减少，因此 2 回合的持续时间实际上相当于 1 轮。具体行为如下：

- 在受影响生物回合开始时，剩余持续时间减少至 1 回合。
- 如果受影响生物可以花费移动速度，它会这样做并站起，剩余的倒伏回合会被移除。
  - 如果无法花费移动速度，它将保持倒伏状态并跳过其回合。在这种情况下，在下一回合开始时，生物会花费其一半的移动速度（如果能够）并站起，倒伏状态被移除。
- 如果倒伏是由某些来源引起的，例如 [凶蛮跳跃](Brutal_Leap.md "凶蛮跳跃")、[原始践踏](Primal_Stampede.md "原始践踏")、[残响](Reverberation_(Condition).md "残响 (状态)") 或 [投掷](Throw.md "投掷")，则在状态结束前无法将持续时间 1 增加到 2，这通常意味着直到下一回合。对于其他一些来源，例如 [冰](Ice_(surface).md "冰 (地表)")，可以施加一个持续时间为 2 的单独倒伏状态。
...
以下倒伏来源仅施加 1 回合的倒伏状态。由于倒伏的持续时间在受影响生物回合开始时减少，因此该状态会结束 _即使_ 该生物因某种原因缺乏站起所需的移动速度。由于生物通常在回合开始时恢复移动速度，这仅在该生物还受到移动限制状态影响时才相关。
...
- [蜂群：强力击退](Legion_of_Bees_colon__Mighty_Knockback.md "蜂群：强力击退")
