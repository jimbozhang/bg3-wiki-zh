# 安布尔勋爵的保镖

## 安布尔勋爵的保镖

- 游戏内
- 肖像

### 身份

男性

### 配音 / 演员

- [Leemore Marrett Jr.](https://www.imdb.com/name/nm3760477/)

## 属性

### 体型

[中级](Medium.md "中级")

### 类型

[类人生物](Humanoid.md "类人生物")

### 种族

[半精灵](Half-Elf.md "半精灵")

### 亚种族

[半高精灵](High_Half-Elf.md "半高精灵")

### 职业

[武僧](Monk.md "武僧")

- 平衡

- 探索者

- 硬核

## 平衡难度属性

### 等级

10

### 生命值

56

### 护甲等级

16

### 先攻

+4

### 熟练项

+4

### 移动

15 米 (50 英尺)

### 重量

75 千克 (150 磅)

### 黑暗视觉

12 米 (40 英尺)

### 经验值奖励

250

| 属性值 | 力量 | 敏捷 | 体质 | 智力 | 感知 | 魅力 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 8 / (-1) | 19 / (+4) | 12 / (+1) | 10 / (0) | 14 / (+2) | 14 / (+2) |

## 额外资源

- 10

## 特性

- [**增强无甲移动**](Advanced_Unarmoured_Movement.md "增强无甲移动")

[劣势地形](Difficult_Terrain_(Condition).md)不会减缓你的移动速度，当你未穿戴[护甲](Armour.md "护甲")或未使用[盾牌](Shields.md "盾牌")时，你可以额外[跳跃](Jump.md "跳跃") 6 米 / 20 英尺。

- [**黑暗视觉**](Darkvision.md "黑暗视觉")

可以在一定距离内于黑暗中视物。

- [**拨挡飞弹**](Deflect_Misses_(passive_feature).md "拨挡飞弹")

使用你的反应动作，将远程武器攻击的伤害减少 1d10 + 你的敏捷[调整值](Modifier.md "调整值") + 你的武僧等级。

- [**反射闪避**](Evasion.md "反射闪避")

成功的敏捷豁免检定使伤害归零；失败的敏捷豁免检定使伤害减半。

- [**额外攻击**](Extra_Attack.md "额外攻击")

在进行一次徒手攻击或武器攻击后，可以额外进行一次攻击。

- [**妖精血统**](Fey_Ancestry.md "妖精血统")

在对抗[魅惑](Charmed_(Condition).md)状态的[豁免检定](Saving_throw.md "豁免检定")中具有优势；不会被魔法[沉睡](Sleeping_(Condition).md)。

- [**真气驻拳**](Ki-Empowered_Strikes.md "真气驻拳")

你的徒手攻击被视为魔法攻击，用于克服敌人对非魔法伤害的抗性和免疫。

- [**武艺：附赠徒手打击**](Martial_Arts_colon__Bonus_Unarmed_Strike.md "武艺：附赠徒手打击")

使用[武僧武器](Monk.md#Monk_weapons "武僧")进行攻击后，或在徒手状态下，你可以使用附赠动作进行一次额外的徒手攻击。

- [**武艺：灵巧打击**](Martial_Arts_colon__Deft_Strikes.md "武艺：灵巧打击")

使用武僧武器和徒手攻击造成 1d4[钝击](Bludgeoning.md "钝击")伤害，除非其正常伤害更高。

- [**武艺：灵巧攻击**](Martial_Arts_colon__Dextrous_Attacks.md "武艺：灵巧攻击")

使用[武僧武器](Monk.md#Monk_weapons "武僧")和徒手攻击时，如果你的敏捷高于力量，则使用敏捷而非力量进行攻击。

- [**借机攻击**](Opportunity_Attack.md "借机攻击")

攻击一个离开你攻击范围的敌人。

- [**百病不侵**](Purity_of_Body.md "百病不侵")

你对毒素伤害[免疫](Immune.md "免疫")，并且由于纯净的真气在经脉中流淌，你不会中毒或受到疾病影响。

- [**轻身坠**](Slow_Fall.md "轻身坠")

当你坠落时，你可以使用反应动作获得对坠落伤害的[抗性](Resistance.md "抗性")。

- [**心如止水**](Stillness_of_Mind_(passive_feature).md "心如止水")

如果被[魅惑](Charmed_(Condition).md)或[恐慌](Frightened_(Condition).md)，自动施放[心如止水](Stillness_of_Mind.md "心如止水")来移除这些状态。

- [**无甲防御**](Unarmoured_Defence_(Monk).md "无甲防御")

你的反应能力与任何[护甲](Armour.md "护甲")一样有效。当你未穿戴护甲时，将你的[感知调整值](Wisdom.md#Wisdom_Modifier_Chart "感知")加到你的[护甲等级](Armour_Class.md "护甲等级")上。

- [**无甲移动**](Unarmoured_Movement.md "无甲移动")

当你未穿戴护甲或未使用盾牌时，你的移动速度增加 3 米 / 10 英尺。

## 探索者难度属性

### 等级

10

### 生命值

39

### 护甲等级

16

### 先攻

+4

### 熟练项

+6

### 移动

15 米 (50 英尺)

### 重量

75 千克 (150 磅)

### 黑暗视觉

12 米 (40 英尺)

### 经验值奖励

250

| 属性值 | 力量 | 敏捷 | 体质 | 智力 | 感知 | 魅力 |
| --- | --- | --- | --- | --- | --- | --- |
|  | +3 | +8 | +1 | 0 | +2 | +2 |

## 额外资源

- 10

## 特性

- [**增强无甲移动**](Advanced_Unarmoured_Movement.md "增强无甲移动")

[劣势地形](Difficult_Terrain_(Condition).md)不会减缓你的移动速度，当你未穿戴[护甲](Armour.md "护甲")或未使用[盾牌](Shields.md "盾牌")时，你可以额外[跳跃](Jump.md "跳跃") 6 米 / 20 英尺。

- [**黑暗视觉**](Darkvision.md "黑暗视觉")

可以在一定距离内于黑暗中视物。

- [**拨挡飞弹**](Deflect_Missiles_(passive_feature).md "拨挡飞弹")

使用你的反应动作，将远程武器攻击的伤害减少 1d10 + 你的敏捷[调整值](Modifier.md "调整值") + 你的武僧等级。

- [**反射闪避**](Evasion.md "反射闪避")

成功的敏捷豁免检定使伤害归零；失败的敏捷豁免检定使伤害减半。

- [**额外攻击**](Extra_Attack.md "额外攻击")

在进行一次徒手攻击或武器攻击后，可以额外进行一次攻击。

- [**妖精血统**](Fey_Ancestry.md "妖精血统")

在对抗[魅惑](Charmed_(Condition).md)状态的[豁免检定](Saving_throw.md "豁免检定")中具有优势；不会被魔法[沉睡](Sleeping_(Condition).md)。

- [**真气驻拳**](Ki-Empowered_Strikes.md "真气驻拳")

你的徒手攻击被视为魔法攻击，用于克服敌人对非魔法伤害的抗性和免疫。

- [**武艺：附赠徒手打击**](Martial_Arts_colon__Bonus_Unarmed_Strike.md "武艺：附赠徒手打击")

使用[武僧武器](Monk.md#Monk_weapons "武僧")进行攻击后，或在徒手状态下，你可以使用附赠动作进行一次额外的徒手攻击。

- [**武艺：灵巧打击**](Martial_Arts_colon__Deft_Strikes.md "武艺：灵巧打击")

使用武僧武器和徒手攻击造成 1d4[钝击](Bludgeoning.md "钝击")伤害，除非其正常伤害更高。

- [**武艺：灵巧攻击**](Martial_Arts_colon__Dextrous_Attacks.md "武艺：灵巧攻击")

使用[武僧武器](Monk.md#Monk_weapons "武僧")和徒手攻击时，如果你的敏捷高于力量，则使用敏捷而非力量进行攻击。

- [**借机攻击**](Opportunity_Attack.md "借机攻击")

攻击一个离开你攻击范围的敌人。

- [**百病不侵**](Purity_of_Body.md "百病不侵")

你对毒素伤害[免疫](Immune.md "免疫")，并且由于纯净的真气在经脉中流淌，你不会中毒或受到疾病影响。

- [**轻身坠**](Slow_Fall.md "轻身坠")

当你坠落时，你可以使用反应动作获得对坠落伤害的[抗性](Resistance.md "抗性")。

- [**心如止水**](Stillness_of_Mind_(passive_feature).md "心如止水")

如果被[魅惑](Charmed_(Condition).md)或[恐慌](Frightened_(Condition).md)，自动施放[心如止水](Stillness_of_Mind.md "心如止水")来移除这些状态。

- [**无甲防御**](Unarmoured_Defence_(Monk).md "无甲防御")

你的反应能力与任何[护甲](Armour.md "护甲")一样有效。当你未穿戴护甲时，将你的[感知调整值](Wisdom.md#Wisdom_Modifier_Chart "感知")加到你的[护甲等级](Armour_Class.md "护甲等级")上。

- [**无甲移动**](Unarmoured_Movement.md "无甲移动")

当你未穿戴护甲或未使用盾牌时，你的移动速度增加 3 米 / 10 英尺。

## 硬核难度属性

### 等级

10

### 生命值

72

### 护甲等级

16

### 先攻

+4

### 熟练项

+4

### 移动

15 米 (50 英尺)

### 重量

75 千克 (150 磅)

### 黑暗视觉

12 米 (40 英尺)

### 经验值奖励

250

| 属性值 | 力量 | 敏捷 | 体质 | 智力 | 感知 | 魅力 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 8 / (-1) | 19 / (+4) | 12 / (+1) | 10 / (0) | 14 / (+2) | 14 / (+2) |

## 额外资源

- 10

## 特性

- [**增强无甲移动**](Advanced_Unarmoured_Movement.md "增强无甲移动")

[劣势地形](Difficult_Terrain_(Condition).md)不会减缓你的移动速度，当你未穿戴[护甲](Armour.md "护甲")或未使用[盾牌](Shields.md "盾牌")时，你可以额外[跳跃](Jump.md "跳跃") 6 米 / 20 英尺。

- [**黑暗视觉**](Darkvision.md "黑暗视觉")

可以在一定距离内于黑暗中视物。

- [**拨挡飞弹**](Deflect_Missiles_(passive_feature).md "拨挡飞弹")

使用你的反应动作，将远程武器攻击的伤害减少 1d10 + 你的敏捷[调整值](Modifier.md "调整值") + 你的武僧等级。

- [**反射闪避**](Evasion.md "反射闪避")

成功的敏捷豁免检定使伤害归零；失败的敏捷豁免检定使伤害减半。

- [**额外攻击**](Extra_Attack.md "额外攻击")

在进行一次徒手攻击或武器攻击后，可以额外进行一次攻击。

- [**妖精血统**](Fey_Ancestry.md "妖精血统")

在对抗[魅惑](Charmed_(Condition).md)状态的[豁免检定](Saving_throw.md "豁免检定")中具有优势；不会被魔法[沉睡](Sleeping_(Condition).md)。

- [**真气驻拳**](Ki-Empowered_Strikes.md "真气驻拳")

你的徒手攻击被视为魔法攻击，用于克服敌人对非魔法伤害的抗性和免疫。

- [**武艺：附赠徒手打击**](Martial_Arts_colon__Bonus_Unarmed_Strike.md "武艺：附赠徒手打击")

使用[武僧武器](Monk.md#Monk_weapons "武僧")进行攻击后，或在徒手状态下，你可以使用附赠动作进行一次额外的徒手攻击。

- [**武艺：灵巧打击**](Martial_Arts_colon__Deft_Strikes.md "武艺：灵巧打击")

使用武僧武器和徒手攻击造成 1d4[钝击](Bludgeoning.md "钝击")伤害，除非其正常伤害更高。

- [**武艺：灵巧攻击**](Martial_Arts_colon__Dextrous_Attacks.md "武艺：灵巧攻击")

使用[武僧武器](Monk.md#Monk_weapons "武僧")和徒手攻击时，如果你的敏捷高于力量，则使用敏捷而非力量进行攻击。

- [**借机攻击**](Opportunity_Attack.md "借机攻击")

攻击一个离开你攻击范围的敌人。

- [**百病不侵**](Purity_of_Body.md "百病不侵")

你对毒素伤害[免疫](Immune.md "免疫")，并且由于纯净的真气在经脉中流淌，你不会中毒或受到疾病影响。

- [**轻身坠**](Slow_Fall.md "轻身坠")

当你坠落时，你可以使用反应动作获得对坠落伤害的[抗性](Resistance.md "抗性")。

- [**心如止水**](Stillness_of_Mind_(passive_feature).md "心如止水")

如果被[魅惑](Charmed_(Condition).md)或[恐慌](Frightened_(Condition).md)，自动施放[心如止水](Stillness_of_Mind.md "心如止水")来移除这些状态。

- [**无甲防御**](Unarmoured_Defence_(Monk).md "无甲防御")

你的反应能力与任何[护甲](Armour.md "护甲")一样有效。当你未穿戴护甲时，将你的[感知调整值](Wisdom.md#Wisdom_Modifier_Chart "感知")加到你的[护甲等级](Armour_Class.md "护甲等级")上。

- [**无甲移动**](Unarmoured_Movement.md "无甲移动")

当你未穿戴护甲或未使用盾牌时，你的移动速度增加 3 米 / 10 英尺。

## 技术信息

###

UID

`S_WYR_WyrmRock_MessHallBodyguard_002` ###

UUID

`214fd102-81e0-4492-8bef-389c66dcd7dc` ###

Stats

`WYR_MessHallBodyguard_02` **安布尔勋爵的保镖**是一位[半精灵](Half-elven.md "半精灵")[武僧](Monk.md "武僧")，他陪同主人前往[第三幕](Act_Three.md "第三幕")的仪式，见证[恩维尔·戈塔什](Enver_Gortash.md "恩维尔·戈塔什")被任命为[博德之门大公爵](Archduke_of_Baldur's_Gate.md "博德之门大公爵")。

> “
>
> 地狱的圣所——仪式似乎已经持续了好几天了。
>
> „

— 安布尔勋爵的保镖

## 相关剧情

安布尔勋爵的保镖可以在[飞龙岩要塞](Wyrm's_Rock_Fortress.md "飞龙岩要塞")一楼的餐厅找到。他随主人[佩特里克·安布尔勋爵](Petric_Amber.md "佩特里克·安布尔勋爵")一同前来，在等待勋爵从[礼堂](Audience_Hall.md "礼堂")下来时，他与其他贵族随从闲聊。

如果首次与他交谈，他会与[碎盾勋爵的侍从](Lord_Shattershield's_Attendant.md "碎盾勋爵的侍从")和[杜林伯德夫人的侍从](Lady_Durinbold's_Attendant.md "杜林伯德夫人的侍从")讨论[钢铁卫士](Steel_Watcher.md "钢铁卫士")，三人会询问队伍对这些机器人的看法。他质疑另外两人对构装体实用性的肯定态度，解释了靠近它们时感到的不安。

如果之后再次与他交谈，他会抱怨仪式仍在进行，已经持续了好几天。

如果对抗戈塔什，安布尔勋爵的保镖会与其他所有受邀宾客及其随从一同被发现死在礼堂中。

## 战斗

### 攻击与能力

[疾风连击](Flurry_of_Blows.md "疾风连击") (+ )

伤害：

徒手⁠[钝击](Bludgeoning.md "钝击")

\+ 徒手⁠[钝击](Bludgeoning.md "钝击")

快速连续出拳两次。

近战：1.5 米 (5 英尺)

[徒手打击](Unarmed_Strike_(Monk).md "徒手打击 (武僧)")

伤害：5~8

1d4 + 4⁠[钝击](Bludgeoning.md "钝击")

以附赠动作快速攻击敌人。

近战：1.5 米 (5 英尺)

---
*Source: [Lord Amber's Bodyguard](https://bg3.wiki/wiki/Lord_Amber's_Bodyguard)*
