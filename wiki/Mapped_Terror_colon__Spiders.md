# 内心恐惧：蜘蛛

关于其他版本的内心恐惧，请参见 [内心恐惧（消歧义）](Mapped_Terror_(disambiguation)..md)

**内心恐惧：蜘蛛** 是一个 [法术](Spells.md "法术")。此法术通过将 [维康妮亚·迪佛](Viconia_DeVir.md "Viconia_DeVir") 及其两名盟友转变为凶猛的蜘蛛来恐吓目标。队伍成员无法学习此法术。

## 描述

通过将你的追随者转变为蜘蛛，利用目标内心深处的恐惧之一。

## 属性

消耗
[动作](Actions.md#Resources "动作") + [6级法术位](Spells.md#Spell_slots "法术位")
详情
[感知](Wisdom.md "Wisdom") [豁免检定](Saving_throws.md "Saving throws")
范围：18米（60英尺）
充能：每次战斗

## 更高施法等级

以更高等级施放此法术不会获得额外收益。

## 技术细节

UID

`Target_LOW_HouseOfGrief_ExploitFear_Spider_2`

法术标志

`[HasSomaticComponent](HasSomaticComponent_(spell_flag).md)`, `[HasVerbalComponent](HasVerbalComponent_(spell_flag).md)`, `[IsEnemySpell](https://bg3.wiki/w/index.php?title=IsEnemySpell_\(spell_flag\)&action=edit&redlink=1) "IsEnemySpell \(spell flag\) \(page does not exist\)")`, `[IsHarmful](IsHarmful_(spell_flag).md)`

## 状态：恐慌

**[恐慌](Frightened_(Condition).md "恐慌（状态）")**

持续时间：2回合

[感知](Wisdom.md "Wisdom") [豁免检定](Saving_throws.md "Saving throws") ([DC](DC.md "DC") 16)

- 受影响的生物无法移动。恐慌的生物在 [属性检定](Ability_Check.md "属性检定") 和 [攻击掷骰](Attack_roll.md "攻击掷骰") 上具有 [劣势](Disadvantage.md "劣势")。

## 状态：内心恐惧：蜘蛛

**[内心恐惧：蜘蛛](Heartform_Terror_colon__Spider_(Condition).md "内心恐惧：蜘蛛（状态）")**

[感知](Wisdom.md "Wisdom") [豁免检定](Saving_throws.md "Saving throws") ([DC](DC.md "DC") 16)

- 此生物已将其形态扭曲为你恐惧的一种。

### 生物：莎尔信徒蜘蛛

| [力量](Strength.md "Strength") 12 (+1) | [敏捷](Dexterity.md "Dexterity") 18 (+4) | [体质](Constitution.md "Constitution") 13 (+1) |
| --- | --- | --- |
|  |  |  |
| 生命值 | 24 |  |
| 护甲等级 | 15 |  |

[蛛形跳跃](Arachnid_Jump.md "蛛形跳跃") ()
向上、向下和横向跳跃。你的 [力量](Strength.md "Strength") 影响你跳跃的距离。（充能：每回合。）

啃咬一个目标。

[额外攻击](Extra_Attack.md "额外攻击")
在进行徒手或武器攻击后，你可以进行一次额外的免费攻击。

## 如何学习

由以下生物使用：[维康妮亚·迪佛](Viconia_DeVir.md "Viconia DeVir")

## 备注

- 由 [维康妮亚·迪佛](Viconia_DeVir.md "Viconia DeVir") 在 [内心映射](Mapping_of_the_Heart.md "内心映射") 期间对一名角色使用 [惩治心形](Castigate_Heartform.md "惩治心形") 后施放，该角色曾声称恐惧“蜘蛛”。
- [恐慌](Frightened_(Condition).md "恐慌（状态）") 状态适用于被惩治的目标，而变形状态适用于维康妮亚及其两名盟友。有趣的是，这种有益的变形仍会触发 [豁免检定](Saving_throw.md "豁免检定")。
- 蜘蛛变形的功能类似于 [荒野形态](Wild_Shape.md "荒野形态")，当蜘蛛的生命值耗尽后，角色会恢复其之前的形态。

---
*Source: [Mapped Terror: Spiders](https://bg3.wiki/wiki/Mapped_Terror:_Spiders)*