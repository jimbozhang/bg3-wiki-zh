...
采取动作通常要求行动生物消耗三种[资源](Resources.md "Resources")之一：
...
所有生物均可使用的动作。
职业动作
通过[特性](Features.md "Features")（例如从[职业](Classes.md "Classes")或[装备](Equipment.md "Equipment")获得）可用的动作。[NPC](Non-player_characters.md "Non-player_characters")采取的许多动作也被视为职业动作。
种族动作
通过种族特性可用的动作，且仅从[种族](Race.md "Race")获得。
情境动作
情境动作因其他能力、特性或状态而解锁。它会保留在角色快捷栏的侧面，直到创建它的条件过期。一个例子是[强制策动](Forced_Manoeuvre.md "Forced_Manoeuvre")，这是[策动攻击（近战）](Manoeuvring_Attack_(Melee).md "策动攻击（近战）")的结果。
武器动作
[武器动作](Weapon_actions.md "Weapon_actions")是通过装备[武器](Weapons.md "Weapons")可用的动作。
法术
[法术](Spells.md "Spells")由施法者或其他生物通过使用[卷轴](Scrolls.md "Scrolls")施放。
...
额外的资源——反应——用于在其他生物的回合采取有条件的[反应](Reactions.md "Reactions")。

所有生物在每回合开始时拥有每种资源的1点充能，但可以从[各种来源](Sources_of_additional_actions.md "Sources_of_additional_actions")获得额外充能。
...
- 拥有[额外攻击](Extra_Attack.md "Extra Attack")特性的职业可以在其回合采取攻击动作后，再采取一次自由攻击动作，而不消耗动作资源。
- 11级[战士](Fighter.md "Fighter")拥有[精通额外攻击](Improved_Extra_Attack.md "Improved Extra Attack")特性，可以在其回合采取攻击动作后，再采取两次自由攻击动作，而不消耗动作资源。

## 反应

图1 - 激活的反应。每个图标上的对话气泡表示这些反应设置为“询问”。

反应是响应触发条件而采取的动作，即使触发发生在其他生物的回合也可以采取。

有些反应要求行动生物消耗[反应](#reactions)资源，而其他反应则不需要。不需要消耗的通常是使用反应系统的职业特性，例如[至圣斩](Divine_Smite.md "Divine_Smite")、[鲁莽攻击](Reckless_Attack.md "Reckless_Attack")或[偷袭](Sneak_Attack.md "Sneak_Attack")。<sup>[\[1\]](#cite_note-1)</sup>

### 反应标签页

图2 - 反应标签页。每个反应都可以通过左侧的复选框启用或禁用。可以使用“询问”设置进一步配置，以便在每次触发反应时提示玩家确认。

每个角色的反应通过*反应标签页*管理，该标签页位于角色表的法术书页面。也可以通过按下快捷栏上的激活反应之一或按下快捷键（默认为 L）立即访问该标签页。

反应可以设置为自动采取，或者将游戏设置为首先提示玩家。

如果采取反应会消耗任何资源，它们会列在反应名称下方。

### 限制

如果任何反应要求确认，角色不能对同一事件应用多个反应；UI 会阻止为每个角色选择多个反应。

事件触发的反应集合在任何反应生效前就已确定。这意味着，如果攻击掷骰或豁免检定通常会忽略某个反应（例如，因为掷骰成功而反应是来自盟友的掷骰加值），即使掷骰结果被另一个反应改变，也会忽略该反应。

#### 反应忽略的事件

大多数反应只“看到”直接攻击或法术，因此通常只触发动作的即时效果。如果伤害和豁免检定源自以下来源，反应系统会忽略它们：

- 次要或延迟效果：来自地表、环境危害或持续状态的伤害（例如，内置于[区域](Areas.md "Areas")效果、[人类定身术](Hold_Person.md "Hold Person")等状态、反应或爆炸物品——箭矢和发射的烟花除外——中的豁免或伤害）
- 间接动作：由单独的反应（[炼狱叱喝](Hellish_Rebuke.md "Hellish_Rebuke")）或被动特性（[火焰护盾](Fire_Shield.md "Fire_Shield")或[荆棘丛生](Spike_Growth.md "Spike_Growth")）发起的动作
...
- 由以下装备授予的被动特性：
  ...
  - [玛科赫什基](Markoheshkir.md "Markoheshkir")（[酸性](Adrip_with_Kereska's_Acid_(Condition).md "克雷斯卡的酸液滴落（状态）")）
...
- 以下 NPC 被动特性：
  ...
  - [恐惧形态](Form_of_Dread.md "Form of Dread")([[3)](#cite_note-hiddenpassive-3))
...
- 任何[逃跑状态](Fleeing_(status_group).md) 如[恐惧](Fearful_(Condition).md "恐惧（状态）")
- 任何[失能状态](Incapacitated_(status_group).md) 如[人类定身术](Hold_Person_(Condition).md "人类定身术（状态）")
- 任何[隐形状态](SG_Invisible.md "SG Invisible")，这会阻止所有反应，除了[受诅咒的幽鬼](Accursed_Spectre_(reaction).md "受诅咒的幽鬼（反应）")、[致盲伏击](Blinding_Ambush.md "Blinding Ambush")和[怪异魔法涌动](Weird_Magic_Surge.md "Weird Magic Surge")
- 任何[变形状态](Polymorphed_(status_group).md) 除了[伪装](SG_Disguise.md "SG Disguise")和[星形形态](Starry_Form.md "Starry Form") _\[[参见：错误](#bugs)\]_
- 任何[束缚状态](Restrained_(status_group).md) 如[诱捕](Ensnared_(Condition).md "诱捕（状态）")
- 任何[震慑状态](Stunned_(status_group).md), 即[震慑](Stunned_(Condition).md "震慑（状态）")
- 任何[昏迷状态](Unconscious_(status_group).md) 如[倒伏](Prone_(Condition).md "倒伏（状态）")
...
- 对[惊惧斩](Staggering_Smite.md "Staggering Smite")、[雷鸣打击](Thunderous_Smite.md "Thunderous Smite")和[激愤斩](Wrathful_Smite.md "Wrathful Smite")以及[推翻大个](Topple_the_Big_Folk.md "Topple the Big Folk")的豁免检定的反应会被忽略。
...
1. [↑](#cite_ref-1) 《博德之门3》中许多作为反应实现的职业特性，在《博德之门3》所基于的 D&D 5e 中并非反应。这一改变可能是因为反应在游戏开发后期才添加。
...
4. [↑](#cite_ref-4) 源自 `Mods/Shared/Scripts/thoth/helpers/CommonConditions.khn` 中 `IsAbleToReact` 的定义、`EnableCondition` 字段数据（用于变形）以及 `InterruptWhileInvisible` 标志。反应可能同时检查变形和 `IsAbleToReact`，只检查其中一个，或都不检查。
