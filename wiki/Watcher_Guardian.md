# 守望者卫士

## 守望者卫士

- 半兽人模型
- 半兽人肖像画

### 声音 / 演员

- [James Alexander](https://www.imdb.com/name/nm9298458/)

### 阵营

[卫士](City_Watch.md "City Watch")

## 属性

### 体型

中型

### 类型

[类人生物](Humanoid.md "Humanoid")

### 种族

[半兽人](Half-Orc.md "Half-Orc") 或 [黄金矮人](Gold_Dwarf.md "Gold Dwarf") 或 [提夫林](Tiefling.md "Tiefling")

### 职业

[战士](Fighter.md "Fighter")

- 平衡

- 探索者

- 硬核

## 平衡难度属性

### 等级

10

### 生命值

84

### 护甲等级

18

### 先攻

+2

### 熟练项

+4

### 移动速度

9 米 (30 英尺)

### 重量

75 千克 (150 磅)

### 高级黑暗视觉

12 米 (40 英尺)

### 经验值奖励

250

| 属性值 | 力量 | 敏捷 | 体质 | 智力 | 感知 | 魅力 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 18 / (+4) | 14 / (+2) | 15 / (+2) | 11 / (0) | 10 / (0) | 12 / (+1) |

## 特性

- [**额外攻击**](Extra_Attack.md "Extra Attack")

在徒手或武器攻击后可以进行一次额外攻击。

- [**巨武器大师：全力一击**](Great_Weapon_Master_colon__All_In.md "Great Weapon Master: All In")

当你使用你[重甲的](Proficient.md "Proficient")的近战武器并双手持握进行攻击时，[攻击掷骰](Attack_roll.md "Attack Roll")会承受-5的减值，但其伤害会增加10点。

- [**巨武器大师：附赠攻击**](Great_Weapon_Master_colon__Bonus_Attack.md "Great Weapon Master: Bonus Attack")

当你用近战武器攻击造成[重击](Critical_Hit.md "Critical Hit")或击杀目标时，你可以在该回合使用[附赠动作](Actions.md#Resources "Actions")进行另一次近战武器攻击。

- [**巨武器战斗**](Great_Weapon_Fighting.md "Great Weapon Fighting")

当投掷[双手](Two-Handed.md "Two-Handed")近战伤害时，任何掷出1或2的骰子可以重掷一次。

- [**借机攻击**](Opportunity_Attack.md "Opportunity Attack")

攻击离开触及范围的敌人。

## 探索者难度属性

### 等级

10

### 生命值

58

### 护甲等级

18

### 先攻

+2

### 熟练项

+6

### 移动速度

9 米 (30 英尺)

### 重量

75 千克 (150 磅)

### 高级黑暗视觉

12 米 (40 英尺)

### 经验值奖励

250

| 属性值 | 力量 | 敏捷 | 体质 | 智力 | 感知 | 魅力 |
| --- | --- | --- | --- | --- | --- | --- |
|  | +8 | +2 | +7 | 0 | 0 | +1 |

## 特性

- [**额外攻击**](Extra_Attack.md "Extra Attack")

在徒手或武器攻击后可以进行一次额外攻击。

- [**巨武器大师：全力一击**](Great_Weapon_Master_colon__All_In.md "Great Weapon Master: All In")

当你使用你[重甲的](Proficient.md "Proficient")的近战武器并双手持握进行攻击时，[攻击掷骰](Attack_roll.md "Attack Roll")会承受-5的减值，但其伤害会增加10点。

- [**巨武器大师：附赠攻击**](Great_Weapon_Master_colon__Bonus_Attack.md "Great Weapon Master: Bonus Attack")

当你用近战武器攻击造成[重击](Critical_Hit.md "Critical Hit")或击杀目标时，你可以在该回合使用[附赠动作](Actions.md#Resources "Actions")进行另一次近战武器攻击。

- [**巨武器战斗**](Great_Weapon_Fighting.md "Great Weapon Fighting")

当投掷[双手](Two-Handed.md "Two-Handed")近战伤害时，任何掷出1或2的骰子可以重掷一次。

- [**借机攻击**](Opportunity_Attack.md "Opportunity Attack")

攻击离开触及范围的敌人。

## 硬核难度属性

### 等级

10

### 生命值

109

### 护甲等级

18

### 先攻

+2

### 熟练项

+4

### 移动速度

9 米 (30 英尺)

### 重量

75 千克 (150 磅)

### 高级黑暗视觉

12 米 (40 英尺)

### 经验值奖励

250

| 属性值 | 力量 | 敏捷 | 体质 | 智力 | 感知 | 魅力 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 18 / (+4) | 14 / (+2) | 15 / (+2) | 11 / (0) | 10 / (0) | 12 / (+1) |

## 特性

- [**额外攻击**](Extra_Attack.md "Extra Attack")

在徒手或武器攻击后可以进行一次额外攻击。

- [**巨武器大师：全力一击**](Great_Weapon_Master_colon__All_In.md "Great Weapon Master: All In")

当你使用你[重甲的](Proficient.md "Proficient")的近战武器并双手持握进行攻击时，[攻击掷骰](Attack_roll.md "Attack Roll")会承受-5的减值，但其伤害会增加10点。

- [**巨武器大师：附赠攻击**](Great_Weapon_Master_colon__Bonus_Attack.md "Great Weapon Master: Bonus Attack")

当你用近战武器攻击造成[重击](Critical_Hit.md "Critical Hit")或击杀目标时，你可以在该回合使用[附赠动作](Actions.md#Resources "Actions")进行另一次近战武器攻击。

- [**巨武器战斗**](Great_Weapon_Fighting.md "Great Weapon Fighting")

当投掷[双手](Two-Handed.md "Two-Handed")近战伤害时，任何掷出1或2的骰子可以重掷一次。

- [**借机攻击**](Opportunity_Attack.md "Opportunity Attack")

攻击离开触及范围的敌人。

## 技术信息

###

UID

`S_END_WatcherSummon001` ###

UUID

`1f0ff035-b76b-4a2c-a172-ca088089cbba` ###

属性

`END_CityWatchMelee_Dwarf` ## 变体 1

###

UID

`S_END_WatcherSummon002` ###

UUID

`011f6bf8-87ab-46b7-bc70-546571df75ec` ###

属性

`END_CityWatchMelee_HalfOrc` ## 变体 2

###

UID

`S_END_WatcherSummon003` ###

UUID

`751f3dd2-70fe-4de0-bff5-10e00d4f6ffc` ###

属性

`END_CityWatchMelee_Tiefling` **守望者卫士**是在[与主脑对峙](Confront_the_Elder_Brain.md "Confront the Elder Brain")期间可召唤的盟友，他们运用巨武器大师的技艺保护城市免受[绝对真理教派势力](Absolute.md "Absolute")的入侵。

> “
>
> 城市卫士将加入你！
>
> „

— 守望者卫士的战斗口号

## 目录

- [1 参与情况](#involvement)
  - [1.1 第三幕](#act-three)
- [2 战斗](#combat)
  - [2.1 攻击与能力](#attacks-and-abilities)
  - [2.2 战利品](#loot)
- [3 画廊](#gallery)
- [4 注释](#notes)
- [5 参考文献](#references)

## 参与情况

### 第三幕

在[至高大殿](High_Hall.md "High Hall")的初始战斗中，一支从[上城区下水道](Upper_City_Sewers.md "Upper City Sewers")潜入的大量[绝对真理教派信徒](Absolute.md "Absolute")摧毁并击溃了[城市卫士](City_Watch.md "City Watch")，迫使其残部逃离。面对惨重的伤亡，可用于夺回至高大殿的剩余城市卫士力量，是以[瓦莱里亚警官](Valeria.md "Valeria")的命令下的一小队守望者卫士，以及一群试图逃离至高大殿庭院的[惊慌失措的守望者](Watchman_Gyordi.md "Watchman Gyordi")。<sup>[\[1\]](#cite_note-1)</sup>

如果瓦莱里亚已被拯救并作为盟友参与最终战斗，可以使用[召集盟友](Gather_Your_Allies_(class_action).md "Gather Your Allies (class action)")的变体[卫士](The_Watch.md "The Watch")来召唤这些守望者卫士。召唤一名男性[半兽人](Half-Orc.md "Half-orc")、一名女性[提夫林](Tiefling.md "Tiefling")和一名女性[黄金矮人](Gold_Dwarf.md "Gold dwarf")在队伍身边战斗。

## 战斗

守望者卫士是强大的近战[战士](Fighter.md "Fighter")，凭借其高[护甲等级](Armour_Class.md "Armour Class")和生命值池能够承受大量伤害，同时凭借其[巨武器大师：全力一击](Great_Weapon_Master_colon__All_In.md "Great Weapon Master: All In")被动造成显著伤害。

### 攻击与能力

[劈砍](Cleave.md "Cleave") () – [巨剑](Greatsword.md "Greatsword")

伤害：

1/2 正常武器伤害

挥舞武器进行大范围弧线攻击，最多同时攻击3个敌人。他们各自承受你武器通常造成伤害的一半。

充能：[短休](Short_rest.md "Short rest")

[主手攻击](Main_Hand_Attack.md "Main Hand Attack") () – [巨剑](Greatsword.md "Greatsword")

伤害：6~16

2d6 + 4⁠[挥砍](Slashing.md "Slashing")

用装备的武器进行近战攻击。

近战：1.5 米 (5 英尺)

[剑柄打击](Pommel_Strike.md "Pommel Strike") () – [巨剑](Greatsword.md "Greatsword")

伤害：1~4

1d4⁠[钝击](Bludgeoning.md "Bludgeoning")

对敌人进行非致命攻击，并可能使其[眩晕](Dazed_(Condition).md "Dazed (Condition)")。

[体质](Constitution.md "Constitution") [豁免检定](Saving_throws.md "Saving throws")
近战：1.5 米 (5 英尺)
充能：[短休](Short_rest.md "Short rest")

[突进攻击](Rush_Attack.md "Rush Attack") () – [巨剑](Greatsword.md "Greatsword")

伤害：5~8

1d4 + 4⁠[穿刺](Piercing.md "Piercing")

向前冲锋并攻击路径上的第一个敌人，可能将其推至[失衡](Off_Balance_(Condition).md "Off Balance (Condition)")状态。

[力量](Strength.md "Strength") [豁免检定](Saving_throws.md "Saving throws")
范围：9 米 (30 英尺)
充能：[短休](Short_rest.md "Short rest")

### 战利品

- [巨剑](Greatsword.md "Greatsword")
- [头盔](Helmet.md "Helmet")
- [板甲](Plate_Armour.md "Plate Armour")

## 画廊

- 矮人模型

- 提夫林模型

- 矮人肖像画

- 提夫林肖像画

## 注释

- 该原型的某些成员可能因属于不同种族而具有略微不同的被动技能、移动速度、重量和体型。
- 这些可召唤的守望者卫士与从庭院逃离的有名守望者具有相同的属性，即[乔迪](Gyordi.md "Gyordi")、[埃伯](Ebber.md "Ebber")、[弗莱](Fry.md "Fry")和[古奇](Goodge.md "Goodge")。
  - 然而，这些有名守望者的护甲类似于染成蓝色、棕色和黑色的[胸甲 +1](Breastplate_+1.md "Breastplate +1")，而不是守望者卫士和阵亡守望者所穿的专为[卫士](City_Watch.md "City Watch")设计的护甲。
- [至高大殿](High_Hall.md "High Hall")的庭院中散布着其他几具守望者卫士的尸体，其中一些被染成不一致的蓝色和黄色，以及蓝色和红色的色调。

## 参考文献

1. [↑](#cite_ref-1) [刘易斯·莫雷尔](Lewis_Morell.md "Lewis Morell")的[死者交谈](Speak_with_Dead.md "Speak with Dead")对话。

---
*Source: [Watcher Guardian](https://bg3.wiki/wiki/Watcher_Guardian)*