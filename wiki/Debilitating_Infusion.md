# 衰弱灌注

本文介绍的内容**[无法](Category_colon_Inaccessible.md "Category:Inaccessible")通过正常游戏流程获取**。但仍可通过第三方工具访问。

**衰弱灌注**是一种[戏法](Spells.md "Spells")。

## 描述

当你对生物造成武器伤害时，该生物对你施放的下一个法术的[豁免检定](Saving_throw.md "Saving Throw")将承受1d4减值。

## 属性

消耗
[附赠动作](Actions.md#Resources "Actions")
详情
范围：自身
充能：[短休](Short_rest.md "Short rest")

## 技术细节

UID

`Shout_MAG_SpellBlade_Augmentation`

## 状态：法师灌注

**[法师灌注](Magi's_Infusion_(Condition).md "法师灌注 (状态)")**

持续时间：1 驱散

- 当你对生物造成伤害时，该生物对你下一个法术的[豁免检定](Saving_throw.md "Saving Throw")将承受1d4减值。

## 如何习得

_(信息尚未添加。)_

---
*Source: [Debilitating Infusion](https://bg3.wiki/wiki/Debilitating_Infusion)*