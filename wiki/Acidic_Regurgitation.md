# 强酸回流

**强酸回流**是[西索博得·索姆](Thisobald_Thorm.md "西索博得·索姆")在拥有[强酸反流](Acid_Reflux_(Condition).md "强酸反流（状态）")状态时可用的动作。他会在区域内呕吐酸液，并变得略微更加醉醺醺。

## 描述

啜饮一口酿酒师的特制烈酒会导致他呕吐酸液，覆盖生物并形成酸液水洼。

## 属性

消耗
[动作](Actions.md#Resources "动作")
伤害：6~48 + 调整值

6d8 + [力量调整值](Strength.md#Strength_modifier_chart "力量")⁠[强酸](Acid.md "强酸")

详情
[体质](Constitution.md "体质") [豁免检定](Saving_throws.md "豁免检定")（[DC](DC.md "DC") 16）（豁免成功：目标仍承受一半伤害。）
范围：4 米（13 英尺）
创建区域：强酸
充能：每回合

## 技术细节

UID

`Projectile_Belch_Acid_Brewer`

法术标志

`[IgnoreSilence](IgnoreSilence_(spell_flag).md)`, `[IsEnemySpell](https://bg3.wiki/w/index.php?title=IsEnemySpell_\(spell_flag\)&action=edit&redlink=1) "IsEnemySpell \(spell flag\) \(page does not exist\)")`, `[IsHarmful](IsHarmful_(spell_flag).md)`

## 状态：中度醉醺醺

**[中度醉醺醺](Moderately_Inebriated_(Condition).md "中度醉醺醺（状态）")**

- 每啜饮一口特制烈酒，西索博得都会更加醉醺醺。
- 他在魅力和敏捷[属性检定](Ability_Check.md "属性检定")上具有[劣势](Disadvantage.md "劣势")，并开始出现小魔鬼幻觉。

## 区域：强酸

**[强酸](Acid_(surface).md "强酸（地表）")**

持续时间：2 回合

范围：2 米（7 英尺）半径

使[护甲等级](Armour_Class.md "护甲等级")降低 2。

类型：[地表](Area.md#Surface "区域")

### 状态：强酸

**[强酸](Acid_(Condition).md "强酸（状态）")**

- 使[护甲等级](Armour_Class.md "护甲等级")降低 2。

## 如何习得

由生物使用：[西索博得·索姆](Thisobald_Thorm.md "西索博得·索姆")

## 备注

- 使用此动作后，西索博得可以使用一次[火焰幻觉](Fire_Hallucination.md "火焰幻觉")。

---
*Source: [Acidic Regurgitation](https://bg3.wiki/wiki/Acidic_Regurgitation)*