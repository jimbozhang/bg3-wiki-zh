# 次级咒法：造水术

此法术是[造水/枯水术](Create_or_Destroy_Water.md "造水/枯水术")的特殊职业版本，它还允许[咒法学派](Conjuration_School.md "咒法学派")法师每短休一次召唤一片水域。此动作等同于1环版本，但不消耗法术位且无法升环施法。

## 描述

召唤雨水。它会熄灭暴露的火焰并形成一片[水](Water_(surface).md "水（地表）")地表。

## 属性

消耗
[动作](Actions.md#Resources "动作")
详情
范围：9米（30英尺）
区域效果：4米（13英尺）半径
创造区域：水
充能：[短休](Short_rest.md "短休")

## 技术细节

UID

`Target_CreateWater_MinorConjuration`

法术标志

`[HasSomaticComponent](HasSomaticComponent_(spell_flag).md)`, `[HasVerbalComponent](HasVerbalComponent_(spell_flag).md)`, `[RangeIgnoreVerticalThreshold](https://bg3.wiki/w/index.php?title=RangeIgnoreVerticalThreshold_\(spell_flag\)&action=edit&redlink=1) "RangeIgnoreVerticalThreshold \(spell flag\) \(page does not exist\)")`

## 状态：濡湿

**[濡湿](Wet_(Condition).md "濡湿（状态）")**

持续时间：3回合

- [免疫](Immune.md "免疫")[燃烧](Burning_(Condition).md "燃烧（状态）")和[狂野魔法：燃烧](Wild_Magic_colon__Burning_(Condition).md "狂野魔法：燃烧（状态）")。
- [抗性](Resistant.md "抗性")[火焰](Fire.md "火焰")伤害。
- [易伤](Vulnerable.md "易伤")[闪电](Lightning.md "闪电")和[寒冷](Cold.md "寒冷")伤害。

## 区域：水

**[水](Water_(surface).md "水（地表）")**

区域效果：2米（7英尺）半径

熄灭无保护的火焰。可被电击或冻结。

类型：[地表](Area.md#Surface "区域")

## 学习方式

职业：

- 职业等级2：[咒法学派](Conjuration_School.md "咒法学派")

---
*Source: [Minor Conjuration: Create Water](https://bg3.wiki/wiki/Minor_Conjuration:_Create_Water)*