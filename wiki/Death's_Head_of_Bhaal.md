# 巴尔的死亡牧首

## 巴尔的死亡牧首

### 声音 / 演员

- [Madeleine Akua](https://www.imdb.com/name/nm12471663/)
- [Stefan Chanyaem](https://www.imdb.com/name/nm9175408/)

### 阵营

[巴尔教团](Bhaal.md "巴尔")

## 属性

### 体型

中型

### 类型

类人生物

### 种族

[人类](Human.md "人类")

- 平衡

- 硬核

## 平衡难度属性

### 等级

8

### 生命值

76

### 护甲等级

15

### 先攻

+5

### 熟练项

+3

### 移动速度

9 米 (30 英尺)

### 重量

75 千克 (150 磅)

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 20 / (+5) | 20 / (+5) | 20 / (+5) | 14 / (+2) | 13 / (+1) | 16 / (+3) |

## 状态

- \_(状态)[**谋杀灵光**](Aura_of_Murder_(NPC)_(Condition).md "谋杀灵光 (NPC) (状态)")

2 米 (7 英尺) 范围内的敌人对穿刺伤害[易伤](Vulnerable.md "易伤")。

- [**不可撼动**](Unstoppable_(Condition).md "不可撼动 (状态)")

将下一次受到的伤害降低至 1 点，并消耗一层不可撼动。无法被外力移动。

## 特性

- [**高级黑暗视觉**](Darkvision.md "高级黑暗视觉")

可以在黑暗中看到一定距离。

- [**额外攻击**](Extra_Attack.md "额外攻击")

在徒手或武器攻击后可以进行一次额外攻击。

- [**魔法抗性**](Magic_Resistance.md "魔法抗性")

对法术和其他魔法效果的[豁免检定](Saving_throws.md "豁免检定")具有[优势](Advantage.md "优势")。

- [**谋杀渴求**](Murder_Hungry.md "谋杀渴求")

每次杀死一个生物时，获得 1 层不可撼动。

- [**借机攻击**](Opportunity_Attack.md "借机攻击")

攻击离开触及范围的敌人。

- [**弱肉强食**](Prey_Upon_the_Weak.md "弱肉强食")

生命值等于或低于一半的目标受到 +1d4 穿刺伤害。

## 硬核难度属性

### 等级

8

### 生命值

98

### 护甲等级

15

### 先攻

+5

### 熟练项

+3

### 移动速度

9 米 (30 英尺)

### 重量

75 千克 (150 磅)

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | +5 | +5 | +5 | +2 | +1 | +3 |

## 状态

- \_(状态)[**谋杀灵光**](Aura_of_Murder_(NPC)_(Condition).md "谋杀灵光 (NPC) (状态)")

2 米 (7 英尺) 范围内的敌人对穿刺伤害[易伤](Vulnerable.md "易伤")。

- [**不可撼动**](Unstoppable_(Condition).md "不可撼动 (状态)")

将下一次受到的伤害降低至 1 点，并消耗一层不可撼动。无法被外力移动。

## 特性

- [**高级黑暗视觉**](Darkvision.md "高级黑暗视觉")

可以在黑暗中看到一定距离。

- [**额外攻击**](Extra_Attack.md "额外攻击")

在徒手或武器攻击后可以进行一次额外攻击。

- [**魔法抗性**](Magic_Resistance.md "魔法抗性")

对法术和其他魔法效果的[豁免检定](Saving_throws.md "豁免检定")具有[优势](Advantage.md "优势")。

- [**谋杀渴求**](Murder_Hungry.md "谋杀渴求")

每次杀死一个生物时，获得 1 层不可撼动。

- [**借机攻击**](Opportunity_Attack.md "借机攻击")

攻击离开触及范围的敌人。

- [**弱肉强食**](Prey_Upon_the_Weak.md "弱肉强食")

生命值等于或低于一半的目标受到 +1d4 穿刺伤害。

- [**哨兵：借机优势**](Sentinel_colon__Opportunity_Advantage.md "哨兵：借机优势")

你对[借机攻击](Opportunity_Attack.md "借机攻击")具有[优势](Advantage.md "优势")。

- [**哨兵：诱捕**](Sentinel_colon__Snare.md "哨兵：诱捕")

当你用[借机攻击](Opportunity_Attack.md "借机攻击")命中一个生物时，它在本回合剩余时间内无法移动。

- [**哨兵：复仇**](Sentinel_colon__Vengeance.md "哨兵：复仇")

当近战范围内的敌人攻击你的盟友时，你可以使用[反应](Reaction.md "反应")对该敌人进行一次武器攻击。目标盟友不能拥有哨兵专长。

## 技术

###

属性

`Human_Cultist_Bhaal_DeathsHead` ## 变体 1

###

属性

`Dwarf_Cultist_Bhaal_DeathsHead` ## 变体 2

###

属性

`Tiefling_Cultist_Bhaal_DeathsHead` **巴尔的死亡牧首**是一种 NPC 原型，他们是[巴尔教团](Bhaal.md "巴尔")的成员，遍布于[巴尔神殿](Bhaal_Temple.md "巴尔神殿")和[第三幕](Act_Three.md "第三幕")期间的其他区域。

## 战斗

### 攻击与能力

[华舞](Flourish.md "华舞") () – [凶残切割](Murderous_Cut.md "凶残切割")

伤害：1~4

1d4⁠[钝击](Bludgeoning.md "钝击")

佯攻以可能使对手[失衡](Off_Balance_(Condition).md "失衡 (状态)")。

[敏](Dexterity.md "敏捷") [豁免](Saving_throws.md "豁免检定")
范围：正常武器范围
充能：[短休](Short_rest.md "短休")

[割裂](Lacerate.md "割裂") () – [凶残切割](Murderous_Cut.md "凶残切割")

伤害：7~10

1d4 + 6⁠[穿刺](Piercing.md "穿刺")

斩击目标的要害部位使其[流血](Bleeding_(Condition).md "流血 (状态)")。

[不死生物](Undead.md "不死生物")和[构装生物](Constructs.md "构装生物")无法流血。

[体](Constitution.md "体质") [豁免](Saving_throws.md "豁免检定")
范围：正常武器范围
充能：[短休](Short_rest.md "短休")

[主手攻击](Main_Hand_Attack.md "主手攻击") () – [凶残切割](Murderous_Cut.md "凶残切割")

伤害：7~10

1d4 + 6⁠[穿刺](Piercing.md "穿刺")

用装备的武器进行近战攻击。

近战：1.5 米 (5 英尺)

[穿刺打击](Piercing_Strike.md "穿刺打击") () – [凶残切割](Murderous_Cut.md "凶残切割")

伤害：7~10

1d4 + 6⁠[穿刺](Piercing.md "穿刺")

刺击敌人并可能造成[开放伤口](Gaping_Wounds_(Condition).md "开放伤口 (状态)")。

[不死生物](Undead.md "不死生物")和[构装生物](Constructs.md "构装生物")无法获得开放伤口。

[体](Constitution.md "体质") [豁免](Saving_throws.md "豁免检定")
范围：正常武器范围
充能：[短休](Short_rest.md "短休")

[震慑凝视](Stunning_Gaze.md "震慑凝视") ()
目标为 18 米 (60 英尺) 范围内它能看到的一个生物。目标必须通过[感知](Wisdom.md "感知")[豁免检定](Saving_throw.md "豁免检定")，否则将被[震慑](Stunned_(Condition).md "震慑 (状态)")直到其下一回合结束。**硬核难度：** 此动作不需要[专注](Concentration.md "专注")。

[感](Wisdom.md "感知") [豁免](Saving_throws.md "豁免检定")
范围：18 米 (60 英尺)
充能：每回合

### 显著战利品

- [皮甲](Leather_Armour.md "皮甲")
- [凶残切割](Murderous_Cut.md "凶残切割")

## 已知成员

### 繁花山脊公园

- [希莱恩](Cirian.md "希莱恩")
- [巴尔的死亡牧首法斯博得](Spaulder.md "法斯博得")

### 清账屋

- [巴尔的死亡牧首费奥纳](Feona.md "费奥纳")
- [巴尔的死亡牧首马'利切](M'alice.md "马'利切")

### 下城区下水道

- [巴尔的死亡牧首布莱斯](Blythe.md "布莱斯")
- [巴尔的死亡牧首诺什莫兹](Nov_Shmoz.md "诺什莫兹")

### 巴尔神殿

- [巴尔的死亡牧首德利西亚](Derysia.md "德利西亚")
- [巴尔的死亡牧首埃尼](Eni.md "埃尼")
- [巴尔的死亡牧首法斯蒂安](Fustian.md "法斯蒂安")
- [巴尔的死亡牧首莱瑞亚](Glyria.md "莱瑞亚")

## 图库

- 渲染图：Wayne Chu

- 渲染图：Wayne Chu

## 注释

- 由于属于不同种族，此原型的部分成员可能具有略微不同的被动特性、移动速度、重量和体型。

---
*Source: [Death's Head of Bhaal](https://bg3.wiki/wiki/Death's_Head_of_Bhaal)*