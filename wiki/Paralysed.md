# 麻痹 (状态)

**麻痹**

- 受影响实体无法移动或进行[动作](Actions.md#Resources "动作")、[附赠动作](Actions.md#Resources "动作")或[反应](Actions.md#Reactions "动作")。
- 如果攻击者在3米（10英尺）范围内，对实体的攻击总是[重击](Critical_Hit.md "重击")。
- 对实体的攻击掷骰具有[优势](Advantage.md "优势")。
- 受影响实体自动失败所有[力量](Strength.md "力量")和[敏捷](Dexterity.md "敏捷")[豁免检定](Saving_throw.md "豁免检定")。

[失能](Incapacitated_(status_group).md)

## 属性

[状态组](Status_groups.md "状态组")：[失能 (状态组)](SG_Incapacitated.md "失能 (状态组)")、[SG 状态](SG_Condition.md "SG 状态")、[SG 麻痹](SG_Paralyzed.md "SG 麻痹")

[更多属性](Status_properties.md "状态属性")：

- [InitiateCombat](Status_properties/InitiateCombat.md "状态属性/InitiateCombat")
- [OverheadOnTurn](Status_properties/OverheadOnTurn.md "状态属性/OverheadOnTurn")

## 麻痹的来源

- [腐蚀攻击](Caustic_Strikes.md "腐蚀攻击")
- [窒息哀号](Choking_Wail.md "窒息哀号")
- [爪击（操纵死尸体食尸鬼）](Claws_(Animate_Dead_Ghoul).md "爪击（操纵死尸体食尸鬼）")
- [爪击（死亡之舞）](Claws_(Danse_Macabre).md "爪击（死亡之舞）")
- [爪击（堕落古尔猎人）](Claws_(Fallen_Gur_Hunter).md "爪击（堕落古尔猎人）")
- [爪击（飞行食尸鬼）](Claws_(Flying_Ghoul).md "爪击（飞行食尸鬼）")
- [爪击（妖鬼）](Claws_(Ghast).md "爪击（妖鬼）")
- [爪击（食尸鬼）](Claws_(Ghoul).md "爪击（食尸鬼）")
- [爪击（凌弱怪）](Claws_(Meenlock).md "爪击（凌弱怪）")
- [残暴之触](Ghoulish_Touch.md "残暴之触")
- [多重攻击（弗林德）](Multiattack_(Flind).md "多重攻击（弗林德）")
- [麻痹重击](Paralysing_Critical.md "麻痹重击")
- [麻痹射线](Paralysing_Ray.md "麻痹射线")
- [麻痹射线](Paralyzing_Ray.md "麻痹射线")
- [麻痹链枷](Flail_of_Paralysis.md "麻痹链枷")

## 拥有麻痹状态的生物

- [潘蒂娜](Pandirna.md "潘蒂娜")

## 简要预览

受麻痹影响的生物无法移动或进行动作。所有对受影响生物的攻击都具有优势，近战攻击总是重击。

---
*Source: [Paralysed (Condition)](https://bg3.wiki/wiki/Paralysed_(Condition)*