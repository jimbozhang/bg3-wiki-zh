# 布拉梅尔斯

## 现金守卫打手 布拉梅尔斯

- 模型
- 肖像画

### 身份

男性

### 声音 / 演员

- [安德鲁·温科特](https://www.imdb.com/name/nm1218328/)

### 阵营

[清账屋](The_Counting_House.md "清账屋")

## 属性

### 体型

[中级](Medium.md "中级")

### 类型

[类人生物](Humanoid.md "类人生物")

### 种族

[人类](Human.md "人类")

### 职业

[圣武士](Paladin.md "圣武士")

- 平衡

- 探索者

- 硬核

## 平衡难度属性

### 等级

9

### 生命值

85

### 护甲等级

19

### 先攻

+1

### 熟练项

+4

### 移动

9 米 (30 英尺)

### 重量

75 千克 (150 磅)

### 高级黑暗视觉

0 米 (0 英尺)

### 经验值奖励

250

| 属性值 | 力量 | 敏捷 | 体质 | 智力 | 感知 | 魅力 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 18 / (+4) | 12 / (+1) | 14 / (+2) | 10 / (0) | 12 / (+1) | 15 / (+2) |

## 额外资源

- - 4

- - - ## 状态

- \_(状态)[**守护灵光**](Aura_of_Protection_(Aura)_(Condition).md "守护灵光 (灵光) (状态)")

该生物及其 3 米 (10 英尺) 范围内的盟友获得该生物的 [魅力](Charisma.md "魅力") 作为其 [豁免检定](Saving_throw.md "豁免检定") 的加值。

## 特性

- [冲锋](Charger.md "冲锋")

让你的速度强化你的猛攻。攻击时，你可以 [推击](Charger_colon__Shove.md "冲锋：推击") 或造成额外的 [武器伤害](Charger_colon__Weapon_Attack.md "冲锋：武器攻击")。

- [神佑](Divine_Health.md "神佑")

免疫 [疾病](Disease.md "疾病") 效果。

- [额外攻击](Extra_Attack.md "额外攻击")

在徒手或武器攻击后可以进行一次额外攻击。

- [借机攻击](Opportunity_Attack.md "借机攻击")

攻击离开攻击范围的敌人。

- [**韧性**](Tenacity_(passive_feature).md "韧性 (被动特性)")

近战攻击未命中时，仍会造成等于 [力](STR.md "力") 调整值或最低 1 点的 [钝击](Bludgeoning.md "钝击") 伤害。

## 探索者难度属性

### 等级

9

### 生命值

59

### 护甲等级

19

### 先攻

+1

### 熟练项

+6

### 移动

9 米 (30 英尺)

### 重量

75 千克 (150 磅)

### 高级黑暗视觉

0 米 (0 英尺)

### 经验值奖励

250

| 属性值 | 力量 | 敏捷 | 体质 | 智力 | 感知 | 魅力 |
| --- | --- | --- | --- | --- | --- | --- |
|  | +6 | +3 | +5 | +2 | +7 | +8 |

## 额外资源

- - 4

- - - ## 状态

- \_(状态)[**守护灵光**](Aura_of_Protection_(Aura)_(Condition).md "守护灵光 (灵光) (状态)")

该生物及其 3 米 (10 英尺) 范围内的盟友获得该生物的 [魅力](Charisma.md "魅力") 作为其 [豁免检定](Saving_throw.md "豁免检定") 的加值。

## 特性

- [冲锋](Charger.md "冲锋")

让你的速度强化你的猛攻。攻击时，你可以 [推击](Charger_colon__Shove.md "冲锋：推击") 或造成额外的 [武器伤害](Charger_colon__Weapon_Attack.md "冲锋：武器攻击")。

- [神佑](Divine_Health.md "神佑")

免疫 [疾病](Disease.md "疾病") 效果。

- [额外攻击](Extra_Attack.md "额外攻击")

在徒手或武器攻击后可以进行一次额外攻击。

- [借机攻击](Opportunity_Attack.md "借机攻击")

攻击离开攻击范围的敌人。

- [**韧性**](Tenacity_(passive_feature).md "韧性 (被动特性)")

近战攻击未命中时，仍会造成等于 [力](STR.md "力") 调整值或最低 1 点的 [钝击](Bludgeoning.md "钝击") 伤害。

## 硬核难度属性

### 等级

9

### 生命值

110

### 护甲等级

19

### 先攻

+1

### 熟练项

+4

### 移动

9 米 (30 英尺)

### 重量

75 千克 (150 磅)

### 高级黑暗视觉

0 米 (0 英尺)

### 经验值奖励

250

| 属性值 | 运动 | 体操 | 察觉技能 |
| --- | --- | --- | --- |
|  | 4 | 1 | 1 |

## 额外资源

- - 4

- - - ## 状态

- \_(状态)[**守护灵光**](Aura_of_Protection_(Aura)_(Condition).md "守护灵光 (灵光) (状态)")

该生物及其 3 米 (10 英尺) 范围内的盟友获得该生物的 [魅力](Charisma.md "魅力") 作为其 [豁免检定](Saving_throw.md "豁免检定") 的加值。

## 特性

- [冲锋](Charger.md "冲锋")

让你的速度强化你的猛攻。攻击时，你可以 [推击](Charger_colon__Shove.md "冲锋：推击") 或造成额外的 [武器伤害](Charger_colon__Weapon_Attack.md "冲锋：武器攻击")。

- [神佑](Divine_Health.md "神佑")

免疫 [疾病](Disease.md "疾病") 效果。

- [额外攻击](Extra_Attack.md "额外攻击")

在徒手或武器攻击后可以进行一次额外攻击。

- [借机攻击](Opportunity_Attack.md "借机攻击")

攻击离开攻击范围的敌人。

- [**韧性**](Tenacity_(passive_feature).md "韧性 (被动特性)")

近战攻击未命中时，仍会造成等于 [力](STR.md "力") 调整值或最低 1 点的 [钝击](Bludgeoning.md "钝击") 伤害。

## 技术

###

UID

`S_LOW_CountingHouse_Guard_06` ###

UUID

`ae3168fe-dd18-421b-aa51-4d86d1245871` ###

属性

`LOW_CountingHouse_Melee_Human` **现金守卫打手 布拉梅尔斯** ，是位于 [博德之门](Baldur's_Gate.md "博德之门") [下城区](Lower_City.md "下城区") [清账屋](The_Counting_House.md "清账屋") 的现金守卫中的一名人类成员。

## 目录

- [1 参与](#参与)
- [2 战斗](#战斗)
  - [2.1 攻击与能力](#攻击与能力)
  - [2.2 值得一提的战利品](#值得一提的战利品)
- [3 闲聊](#闲聊)
- [4 外部链接](#外部链接)

## 参与

布拉梅尔斯可以在外巡逻的码头找到。

## 战斗

### 攻击与能力

[摔翻打击](Backbreaker.md "摔翻打击") () – [巨锤](Maul.md "巨锤")

伤害: 5~8

1d4 + 4⁠[钝击](Bludgeoning.md "钝击")

在你的打击中施加额外的力量，可能将你的敌人击至 [倒伏](Prone_(Condition).md "倒伏 (状态)")。

[力量](Strength.md "力量") [豁免检定](Saving_throws.md "豁免检定")
范围: 正常武器范围
充能: [短休](Short_rest.md "短休")

[致盲斩](Blinding_Smite.md "致盲斩") (+ + )

伤害: 3~24

正常武器伤害

\+ 3d8⁠[光耀](Radiant.md "光耀")

唤起一道天堂之光，可能 \_(状态)[目盲](Blinded_(Blinding_Smite)_(Condition).md "目盲 (致盲斩) (状态)") 你的目标

[体质](Constitution.md "体质") [豁免检定](Saving_throws.md "豁免检定")
范围: 正常武器范围

[震荡猛击](Concussive_Smash.md "震荡猛击") () – [巨锤](Maul.md "巨锤")

伤害: 6~16

2d6 + 4⁠[钝击](Bludgeoning.md "钝击")

全力击中敌人以造成伤害，并可能使其 [眩晕](Dazed_(Condition).md "眩晕 (状态)")。

[体质](Constitution.md "体质") [豁免检定](Saving_throws.md "豁免检定")
范围: 正常武器范围
充能: [短休](Short_rest.md "短休")

[至圣斩](Divine_Smite.md "至圣斩") (+ )

伤害: 3~24

正常武器伤害

\+ 2d8⁠[光耀](Radiant.md "光耀")

\+ 1d8⁠[光耀](Radiant.md "光耀")

你的武器造成额外的 2d8⁠⁠[光耀](Radiant.md "光耀")[伤害来源](Damage_rider_as_source.md "伤害来源") 伤害，当攻击 [邪魔](Fiends.md "邪魔") 或 [不死生物](Undead.md "不死生物") 时，增加 1d8⁠⁠[光耀](Radiant.md "光耀") 伤害。

范围: 正常武器范围

[圣疗：高等治疗](Lay_on_Hands_colon__Greater_Healing.md "圣疗：高等治疗") (+ 2 )

治疗:

4 × (圣武士等级)⁠[治疗](Healing.md "治疗")

用神圣力量灌注你的双手以治疗盟友。

近战: 1.5 米 (5 英尺)

[主手攻击](Main_Hand_Attack.md "主手攻击") () – [巨锤](Maul.md "巨锤")

伤害: 6~16

2d6 + 4⁠[钝击](Bludgeoning.md "钝击")

用你的装备武器进行近战攻击。

近战: 1.5 米 (5 英尺)

[移动射击](Mobile_Shot.md "移动射击") () – [手弩](Hand_Crossbow.md "手弩")

伤害: 2~7

1d6 + 1⁠[穿刺](Piercing.md "穿刺")

在本回合使用 [疾走](Dash.md "疾走") 或 [撤离](Disengage.md "撤离") 后，你可以以附赠动作进行一次远程攻击。

范围: 15 米 (50 英尺)
充能: [短休](Short_rest.md "短休")

[穿刺射击](Piercing_Shot.md "穿刺射击") () – [手弩](Hand_Crossbow.md "手弩")

伤害: 2~7

1d6 + 1⁠[穿刺](Piercing.md "穿刺")

造成常规伤害，并可能施加 [开放伤口](Gaping_Wounds_(Condition).md "开放伤口 (状态)")，导致攻击时造成额外伤害。

[不死生物](Undead.md "不死生物") 和 [构装生物](Constructs.md "构装生物") 无法获得开放伤口。

[体质](Constitution.md "体质") [豁免检定](Saving_throws.md "豁免检定")
范围: 正常武器范围
充能: [短休](Short_rest.md "短休")

[远程攻击](Ranged_Attack.md "远程攻击") () – [手弩](Hand_Crossbow.md "手弩")

伤害: 2~7

1d6 + 1⁠[穿刺](Piercing.md "穿刺")

用你的装备武器进行远程攻击。

范围: 18 米 (60 英尺)

[韧性](Tenacity.md "韧性") () – [巨锤](Maul.md "巨锤")

伤害: 4

4⁠[钝击](Bludgeoning.md "钝击")

当你未命中攻击时，仍会造成等于你的 [力量](Strength.md "力量") [调整值](Ability_Scores.md#Ability_score_modifiers "属性值") 的 [钝击](Bludgeoning.md "钝击") 伤害（最低为 1）。

### 值得一提的战利品

- [港口塔楼钥匙](Docktower_Key.md "港口塔楼钥匙")
- [手弩](Hand_Crossbow.md "手弩")
- [头盔](Helmet.md "头盔")
- [巨锤](Maul.md "巨锤")
- [板甲](Plate_Armour.md "板甲")

## 闲聊

- 现金守卫打手 布拉梅尔斯
  安静无事。本该如此。
- 现金守卫打手 布拉梅尔斯
  *打哈欠。*
- 现金守卫打手 布拉梅尔斯
  好一阵子没见到格利特比尔德了。没抱怨。

## 外部链接

- ⁠[布拉梅尔斯](https://forgottenrealms.fandom.com/wiki/Brammels) 在 [被遗忘的国度维基](https://forgottenrealms.fandom.com/wiki/Main_Page)

---
*Source: [Brammels](https://bg3.wiki/wiki/Brammels)*