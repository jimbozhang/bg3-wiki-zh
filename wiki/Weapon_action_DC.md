...
许多调整值可能被加到掷骰中。调整值可以是加到结果上的奖励，也可以是从结果中减去的惩罚。一个掷骰可能有来自多个来源的奖励和惩罚；在这种情况下，它们被加在一起并表示为一个调整值。例如，一个带有+5奖励和-2惩罚的20面骰掷骰表示为1d20+3。
...
每当生物尝试一个可能失败的动作时，它会掷一个20面骰对抗一个目标数字，以确定尝试是否成功或失败，然后添加任何适用的调整值。如果结果等于或超过目标数字，则尝试成功。如果结果低于目标数字（或对于某些掷骰，如果未调整的骰子掷出1），则尝试失败。
...
**豁免检定**
陷阱、法术、状态和其他伤害来源可能允许生物有机会避免或减少其效果，这被称为豁免检定或豁免。要尝试豁免，生物会掷一个20面骰对抗目标[豁免DC](#save-dcs)。与豁免相关的属性由伤害来源的类型决定。对于必须避免的物理威胁，进行敏捷豁免检定。对于中毒效果，进行体质豁免检定。对于魔法或非物质效果，豁免检定可能是智力、感知或魅力，但最常见的是感知。
**属性检定**
...
生物从掷骰的多个优势或劣势来源中不会获得额外的好处或惩罚，并且仍然只掷两次。此外，如果生物在一次掷骰中同时具有优势和劣势，这些条件会相互抵消，只掷一个20面骰，即使它们有多个来源。
...
- [知识领域](Knowledge_Domain.md "Knowledge Domain")[牧师](Cleric.md "Cleric")在1级时从以下技能中选择两个获得专精：[奥秘](Arcana.md "Arcana")、[历史](History.md "History")、[自然](Nature.md "Nature")或[宗教](Religion.md "Religion")
...
熟练项加值不会添加到伤害掷骰中，除非所使用的攻击另有说明，例如[浸影打击](Shadowsoaked_Blow.md "Shadowsoaked_Blow")，或由特性启用，例如[咒剑士诅咒](Hexblade's_Curse.md "Hexblade's Curse")。
...
当我们将此表达式应用于20面骰时，结果是具有优势相当于平均奖励+3.325。
...
具有优势或劣势对增加或减少大成功和失败的机会有显著影响。例如，当掷骰具有优势时，获得大失败的唯一方法是同时掷出_两个_1。此结果的概率为$\frac{1}{20} \cdot \frac{1}{20} = \frac{1}{400}$（或0.25%）。相反，掷出大成功要容易得多——在400种可能的掷骰结果中，有39种会导致20（第一个骰子掷出20，第二个骰子掷出1、2、3、...20，加上第二个骰子掷出20，第一个骰子掷出1、2、3、...20，减去一个，以便两个20的结果不会被重复计算）。此结果的概率为$39/400$（或9.75%）。对于掷骰具有劣势的情况则相反：大成功的概率为0.25%，大失败的概率为9.75%。

实际上，掷骰具有优势意味着大失败的可能性_降低20倍_，而大成功的可能性几乎增加_一倍_，而对于劣势则相反。
...
1. ↑ [2.0](#cite_ref-lucky_half_2-0) [2.1](#cite_ref-lucky_half_2-1) [2.2](#cite_ref-lucky_half_2-2) [半身人幸运](Halfling_Luck.md "Halfling Luck")允许几乎所有自然1掷骰的第一次实例被重掷。虽然可能再次掷出1，但这很罕见，因此允许此种族特性抵消大多数大失败。
