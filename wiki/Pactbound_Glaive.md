# 魔契长柄刀

由[刃之魔契](Pact_of_the_Blade.md "刃之魔契")召唤的武器。它使用持有者的[魅力调整值](Abilities.md#Ability_score_modifiers "属性")，其伤害为魔法伤害。

由邪术师的契约主授予的可召唤长柄刀。其表面闪烁着跨维度的能量。

## 属性

伤害

1d10 (1~10) + [力量调整值](Damage_Roll.md#Modifiers "伤害掷骰") ⁠[挥砍](Slashing.md "挥砍")

详情
[长柄刀](Glaives.md "长柄刀")
稀有度：普通
惑控学派：无
[双手](Two-Handed.md "双手")
[额外范围](Extra_Reach.md "额外范围")
[可蘸取](Dippable.md "可蘸取")
范围：2.5 米 (8 英尺)
重量：2.7 千克 (5.4 磅)
价格：0 金币
UID `WPN_HUM_Glaive_Pact` UUID `e7dab8bd-8037-4d8e-9c4e-ebec705912aa` ### 武器动作

_如果你拥有[熟练项](Proficiency.md "熟练项")，装备在**主手**以获得：_

[突进攻击](Rush_Attack.md "突进攻击") ()
向前冲锋并攻击路径上的第一个敌人，可能将其推至[失衡](Off_Balance_(Condition).md "失衡（状态）")状态。（充能：[短休](Short_rest.md "短休")。）

[割裂](Lacerate.md "割裂") ()
劈砍目标的要害部位，使其[流血](Bleeding_(Condition).md "流血（状态）")。（充能：[短休](Short_rest.md "短休")。）

[备战（近战）](Brace_(Melee).md "备战（近战）") ()
消耗 6 米 (20 英尺) 的[移动](Movement_speed.md "移动速度")。在本回合剩余时间内，掷两次近战伤害并取最高值。（充能：[短休](Short_rest.md "短休")。）

## 状态：魔契武器

**[魔契武器](Pact_Weapon_(Condition).md "魔契武器（状态）")**

- 与此武器绑定的邪术师始终对其[熟练项](Proficiency.md "熟练项")。武器的伤害为魔法伤害。
- 武器使用施法者的[魅力调整值](Abilities.md#Ability_score_modifiers "属性")进行[攻击掷骰](Attack_roll.md "攻击掷骰")和伤害掷骰。
- 防止武器掉落或以其他方式被缴械。

## 获取地点

- 通过邪术师的[刃之魔契：长柄刀](Pact_of_the_Blade_colon__Glaive.md "刃之魔契：长柄刀")召唤

---
*Source: [Pactbound Glaive](https://bg3.wiki/wiki/Pactbound_Glaive)*