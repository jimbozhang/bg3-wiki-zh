# 灵巧动作：撤离

**灵巧动作：撤离**是[游荡者](Rogue.md "游荡者")的附赠动作。此能力允许游荡者使用其附赠动作从敌人处[撤离](Disengage.md "撤离")并避免借机攻击。通常这需要一个完整动作才能完成。

## 描述

安全撤退。移动不会引发[借机攻击](Attack_of_Opportunity.md "借机攻击")。

## 属性

消耗
[附赠动作](Actions.md#Resources "动作")
详情
范围：自身

## 技术细节

UID

`Shout_Disengage_CunningAction`

`Shout_LOW_Dolor_Disengage`

法术标志

`[NoCameraMove](https://bg3.wiki/w/index.php?title=NoCameraMove_\(spell_flag\)&action=edit&redlink=1) "NoCameraMove \(spell flag\) \(page does not exist\)")`

## 状态：撤离

**[撤离](Disengage_(Condition).md "撤离 (状态)")**

持续时间：1 驱散

- 可以移动而不引发[借机攻击](Attack_of_Opportunity.md "借机攻击")。

## 如何习得

职业：

- 职业等级 2：[游荡者](Rogue.md "游荡者")

## 备注

- 此动作也与[诡诈之雾披风](Cloak_of_Cunning_Brume.md "诡诈之雾披风")配合使用，使其成为触发该物品的一种非常划算的方式。

---
*Source: [Cunning Action: Disengage](https://bg3.wiki/wiki/Cunning_Action:_Disengage)*