# 鹰之光彩

**鹰之光彩**是一个[等级 2 变化学派法术](Spells.md "法术")。此法术允许施法者赋予生物鹰的优雅。受影响的生物在[魅力](Charisma.md "魅力")[检定](Ability_Check.md "属性检定")上获得[优势](Advantage.md "优势")。

## 描述

生物在[魅力](Charisma.md "魅力")[检定](Ability_Check.md "属性检定")上获得[优势](Advantage.md "优势")。

## 属性

消耗
[动作](Actions.md#Resources "动作") + [等级 2 法术位](Spells.md#Spell_slots "法术")
详情
近战：1.5 米（5 英尺）
[专注](Concentration.md "专注")

## 高等级施法

当此法术以 3 级或更高法术位施法时，你可以为每个高于 2 级的法术位等级额外指定一个目标。

## 技术细节

UID

`Target_EnhanceAbility_EaglesSplendor`

法术标志

`[HasSomaticComponent](HasSomaticComponent_(spell_flag).md)`, `[HasVerbalComponent](HasVerbalComponent_(spell_flag).md)`, `[IgnorePreviouslyPickedEntities](IgnorePreviouslyPickedEntities_(spell_flag).md)`, `[IsConcentration](IsConcentration_(spell_flag).md)`, `[IsLinkedSpellContainer](IsLinkedSpellContainer_(spell_flag).md)`, `[IsMelee](IsMelee_(spell_flag).md)`, `[IsSpell](IsSpell_(spell_flag).md)`

## 状态：鹰之光彩

**[鹰之光彩](Eagle's_Splendour_(Condition).md "鹰之光彩 (状态)")**

持续时间：直至[长休](Long_Rest.md "长休")

- 在[魅力](Charisma.md "魅力")[检定](Ability_Check.md "属性检定")上拥有[优势](Advantage.md "优势")。

## 如何习得

此法术是以下法术的变体：
[强化属性](Enhance_Ability.md "强化属性")

## 外部链接

- ⁠[Eagle's splendor](https://forgottenrealms.fandom.com/wiki/Eagle%27s_splendor) on the [Forgotten Realms Wiki](https://forgottenrealms.fandom.com/wiki/Main_Page)

---
*Source: [Eagle's Splendour](https://bg3.wiki/wiki/Eagle's_Splendour)*