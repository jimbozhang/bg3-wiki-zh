# 红帽子（生物）

## 红帽子

- 模型
- 抢先体验

### 声音 / 演员

- [Robert G. Slade](https://www.imdb.com/name/nm0805195/)

## 属性

### 体型

[小型](Small.md "Small")

### 类型

[妖精](Fey.md "Fey")

### 种族

[红帽子](Redcap.md "Redcap")

- 平衡

- 探索者

- 硬核

## 平衡难度属性

### 等级

4

### 生命值

45

### 护甲等级

14

### 先攻

+1

### 熟练项

+2

### 移动

9 米 (30 英尺)

### 重量

35 千克 (70 磅)

### 经验值奖励

40

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 18 / (+4) | 13 / (+1) | 18 / (+4) | 10 / (0) | 12 / (+1) | 9 / (-1) |

## 特性

- [**高级黑暗视觉**](Darkvision.md "Darkvision")

能在黑暗中看到一定距离。

- [**多重攻击：嗜血**](Multiattack_colon__Bloodlust.md "多重攻击：嗜血")

红帽子在[嗜血](Bloodlust_(Condition).md "嗜血（状态）")效果下可以进行[多重攻击](Multiattack_(Redcap).md "多重攻击（红帽子）")。

- [**借机攻击**](Opportunity_Attack.md "借机攻击")

攻击离开攻击范围的敌人。

## 探索者难度属性

### 等级

4

### 生命值

31

### 护甲等级

14

### 先攻

+1

### 熟练项

+4

### 移动

9 米 (30 英尺)

### 重量

35 千克 (70 磅)

### 经验值奖励

40

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | +4 | +1 | +4 | 0 | +1 | -1 |

## 特性

- [**高级黑暗视觉**](Darkvision.md "Darkvision")

能在黑暗中看到一定距离。

- [**多重攻击：嗜血**](Multiattack_colon__Bloodlust.md "多重攻击：嗜血")

红帽子在[嗜血](Bloodlust_(Condition).md "嗜血（状态）")效果下可以进行[多重攻击](Multiattack_(Redcap).md "多重攻击（红帽子）")。

- [**借机攻击**](Opportunity_Attack.md "借机攻击")

攻击离开攻击范围的敌人。

## 硬核难度属性

### 等级

4

### 生命值

58

### 护甲等级

14

### 先攻

+1

### 熟练项

+2

### 移动

9 米 (30 英尺)

### 重量

35 千克 (70 磅)

### 经验值奖励

40

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 18 / (+4) | 13 / (+1) | 18 / (+4) | 10 / (0) | 12 / (+1) | 9 / (-1) |

## 特性

- [**高级黑暗视觉**](Darkvision.md "Darkvision")

能在黑暗中看到一定距离。

- [**多重攻击：嗜血**](Multiattack_colon__Bloodlust.md "多重攻击：嗜血")

红帽子在[嗜血](Bloodlust_(Condition).md "嗜血（状态）")效果下可以进行[多重攻击](Multiattack_(Redcap).md "多重攻击（红帽子）")。

- [**借机攻击**](Opportunity_Attack.md "借机攻击")

攻击离开攻击范围的敌人。

- [**沉浸杀戮**](Steeped_in_Slaughter.md "沉浸杀戮")

对[流血](Bleeding_(Condition).md "流血（状态）")的敌人进行[攻击掷骰](Attack_rolls.md "攻击掷骰")时具有[优势](Advantage.md "优势")。

此页面适用于生物。关于红帽子种族，请参见[红帽子](Redcap.md "红帽子")。

**红帽子**是为[埃赛尔婶婶](Auntie_Ethel.md "埃赛尔婶婶")服务的[妖精](List_of_creature_types.md#Fey "生物类型列表")生物。它们可以在[第一幕](Act_One.md "第一幕")期间在[日照湿地](Sunlit_Wetlands.md "日照湿地")找到。这些小型、嗜血的生物诞生于嗜血欲望，必须将帽子浸泡在血液中才能生存。

如果队伍尚未克服湿地的幻象，它们最初不会敌对，因为它们会伪装成绵羊。然而，如果埃赛尔对队伍敌对，它们也会变得敌对。

## 目录

- [1 战斗](#combat)
  - [1.1 攻击与能力](#attacks-and-abilities)
  - [1.2 战术](#tactics)
  - [1.3 重要战利品](#notable-loot)
- [2 伙伴好感度](#companion-approval)
- [3 画廊](#gallery)

## 战斗

### 攻击与能力

[铁意追击](Ironbound_Pursuit.md "铁意追击") ()

伤害：7~28

3d8 + 4⁠[钝击](Bludgeoning.md "钝击")

发动飞踢，将目标击退 2 米 / 7 英尺并使其[倒伏](Prone_(Condition).md "倒伏（状态）")。

[敏](Dexterity.md "敏捷") [豁免检定](Saving_throws.md "豁免检定")
近战：1.5 米 (5 英尺)

[主手攻击](Main_Hand_Attack.md "主手攻击") () – [镰刀](Sickle.md "镰刀")

伤害：5~8

1d4 + 4⁠[挥砍](Slashing.md "挥砍")

用装备的武器进行近战攻击。

近战：1.5 米 (5 英尺)

仅在[嗜血](Bloodlust_(Condition).md "嗜血（状态）")效果下可用：

[多重攻击](Multiattack_(Redcap).md "多重攻击（红帽子）") ()

伤害：

正常武器伤害

\+ 正常武器伤害

\+ 正常武器伤害

对目标进行多次攻击

近战：1.5 米 (5 英尺)
充能：每回合

### 战术

**红帽子**在[第一幕](Act_One.md "第一幕")早期是非常危险的敌人，因为它们拥有高[力量](Strength.md "力量")和[生命值](Hit_Points.md "生命值")。它们很可能会从[鲜血贤者](Redcap_Blood_Sage.md "红帽子鲜血贤者")那里获得嗜血，从而能够进行[多重攻击](Multiattack.md "多重攻击")。它们的飞踢会击倒敌人，并且它们对流血的敌人获得[优势](Advantage.md "优势")，这可能是由鲜血贤者造成的。

虽然战斗力强大，但它们对大多数针对[敏捷](Dexterity.md "敏捷")、[智力](Intelligence.md "智力")和[感知](Wisdom.md "感知")的法术和能力缺乏抗性。远程攻击对它们很有效，因为它们需要穿越[劣势地形](Difficult_Terrain.md "劣势地形")才能接近队伍。

### 重要战利品

- [骨帽菇](Bonecap.md "骨帽菇")
- [湿地发蓬](Wispweed.md "湿地发蓬")

## 伙伴好感度

- 对红帽子咩咩叫 +1 +1 +1
  - 第二次对红帽子咩咩叫 +1 +1 +1

## 画廊

- 由 Nikolai Chugunov 渲染

- 由 Nikolai Chugunov 渲染

- 由 Jane Katsubo 绘制的概念艺术

---
*Source: [Redcap (creature)](https://bg3.wiki/wiki/Redcap_(creature)*