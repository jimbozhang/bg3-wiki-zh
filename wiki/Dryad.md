# 树精

## 树精

- 模型
- 肖像画

## 属性

### 体型

中级

### 类型

妖精

### 种族

[妖精](Fey.md "妖精")

- 平衡

- 探索者

- 硬核

## 平衡难度属性

### 等级

6

### 生命值

62

### 护甲等级

13

### 先攻

+1

### 熟练项

+3

### 施法DC

13（感知）

### 移动

9米（30英尺）

### 重量

55千克（110磅）

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 10 / (0) | 12 / (+1) | 11 / (0) | 14 / (+2) | 15 / (+2) | 18 / (+4) |

## 特性

- [**借机攻击**](Opportunity_Attack.md "借机攻击")

攻击离开触及范围的敌人。

- [**魔法抗性**](Magic_Resistance.md "魔法抗性")

对法术和其他魔法效果的[豁免检定](Saving_throws.md "豁免检定")具有[优势](Advantage.md "优势")。

- [**森林之握**](Grasp_of_the_Forest.md "森林之握")

武器攻击可以使目标[缠绕](Entangled_(Condition).md "缠绕（状态）")。

## 探索者难度属性

### 等级

6

### 生命值

43

### 护甲等级

13

### 先攻

+1

### 熟练项

+5

### 施法DC

15（感知）

### 移动

9米（30英尺）

### 重量

55千克（110磅）

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 0 | +1 | 0 | +2 | +2 | +4 |

## 特性

- [**借机攻击**](Opportunity_Attack.md "借机攻击")

攻击离开触及范围的敌人。

- [**魔法抗性**](Magic_Resistance.md "魔法抗性")

对法术和其他魔法效果的[豁免检定](Saving_throws.md "豁免检定")具有[优势](Advantage.md "优势")。

- [**森林之握**](Grasp_of_the_Forest.md "森林之握")

武器攻击可以使目标[缠绕](Entangled_(Condition).md "缠绕（状态）")。

## 硬核难度属性

### 等级

6

### 生命值

80

### 护甲等级

13

### 先攻

+1

### 熟练项

+3

### 施法DC

15（感知）

### 移动

9米（30英尺）

### 重量

55千克（110磅）

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 10 / (0) | 12 / (+1) | 11 / (0) | 14 / (+2) | 15 / (+2) | 18 / (+4) |

## 特性

- [**借机攻击**](Opportunity_Attack.md "借机攻击")

攻击离开触及范围的敌人。

- [**魔法抗性**](Magic_Resistance.md "魔法抗性")

对法术和其他魔法效果的[豁免检定](Saving_throws.md "豁免检定")具有[优势](Advantage.md "优势")。

- [**森林之握**](Grasp_of_the_Forest.md "森林之握")

武器攻击可以使目标[缠绕](Entangled_(Condition).md "缠绕（状态）")。

**树精**是一种在[博德之门3](Baldur's_Gate_3.md "博德之门3")中出现的不可游玩的[妖精](List_of_creature_types.md#Fey "生物类型列表")种族。她们引导强大的[德鲁伊力量](Druid.md "德鲁伊")，使她们能够缠绕敌人并召唤[菘蓝树人](Wood_Woad.md "菘蓝树人")寻求帮助。

## 关于树精

树精可以通过法术[召唤林地妖精](Conjure_Woodland_Being.md "召唤林地妖精")召唤，这是7级[德鲁伊](Druid.md "德鲁伊")可用的四级法术。

## 战斗

### 攻击与能力

[纠缠](Entangle_(Dryad).md "纠缠（树精）") ()
创造一片藤蔓地表，减慢生物速度，并可能使其[缠绕](Entangled_(Condition).md "缠绕（状态）")。

[力量](Strength.md "力量") [豁免检定](Saving_throws.md "豁免检定")
范围：18米（60英尺）

[坠落连理](Fallen_Lover.md "坠落连理") ()
召唤一个与你的树誓缚的[菘蓝树人](Wood_Woad.md "菘蓝树人")与你并肩作战。

范围：9米（30英尺）
充能：[短休](Short_rest.md "短休")

[荆棘丛生](Spike_Growth.md "荆棘丛生") (+ )

伤害：2~8

2d4⁠[穿刺](Piercing.md "穿刺")

将一块地面塑造成坚硬的尖刺。在尖刺上行走的生物每移动1.5米（5英尺）会受到2d4⁠⁠[穿刺](Piercing.md "穿刺")伤害。

尖刺是[劣势地形](Difficult_Terrain.md "劣势地形")，使生物的[移动速度](Movement_speed.md "移动速度")减半。

范围：18米（60英尺）

[强化橡棍术](Strengthened_Shillelagh.md "强化橡棍术") ()
你的[长棍](Quarterstaff.md "长棍")或[短棒](Club.md "短棒")变为魔法武器：造成4d8[钝击](Bludgeoning.md "钝击")伤害，并使用你的[施法关键属性](Ability_score_modifier.md "属性值调整值")进行[攻击掷骰](Attack_roll.md "攻击掷骰")。

范围：自身

## 树精角色

- [泽希诺](Zethino.md "泽希诺")

## 树精召唤物

- [树精（召唤林地妖精）](Dryad_(Conjure_Woodland_Being).md)

## 注释

- 树精的默认攻击造成3d6+1⁠⁠[钝击](Bludgeoning.md "钝击")伤害。
- 除了[森林之握](Grasp_of_the_Forest.md "森林之握")外，默认攻击和[强化橡棍术](Strengthened_Shillelagh.md "强化橡棍术")还有另一个被动效果，可以使目标[缠绕](Entangled_(Condition).md "缠绕（状态）")。它没有特定名称，战斗日志中的豁免显示为_对抗主手攻击_。它使用[DC](Dice_rolls.md#Save_DCs "掷骰") 13 [敏捷](Dexterity.md "敏捷") [豁免检定](Saving_throw.md "豁免检定")。
- 如果溅出，树精的血液会在地面上变成[扭曲的藤蔓](Twisting_Vines.md "扭曲的藤蔓")。

## 画廊

- 渲染：Ellie Porfyridou

- 渲染：Ellie Porfyridou

## 外部链接

- ⁠[树精](https://forgottenrealms.fandom.com/wiki/Dryad) 在 [被遗忘的国度维基](https://forgottenrealms.fandom.com/wiki/Main_Page)

---
*Source: [Dryad](https://bg3.wiki/wiki/Dryad)*