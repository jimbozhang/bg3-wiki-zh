...
**高等隐形术** 是一个 [4级幻术学派法术](Spells.md "法术")。它允许施法者为盟友提供更持久的[隐形](Invisibility_(spell).md "隐形")形态。

## 描述

使一个生物隐形。攻击它的攻击掷骰具有[劣势](Disadvantage.md "劣势")。它进行的攻击掷骰具有[优势](Advantage.md "优势")。

当在攻击、施法或与物品互动时，进行越来越难的[隐匿](Stealth.md "隐匿")[检定](Ability_Check.md "属性检定")失败时，隐形状态会解除。
...
- 当与物品互动、施法或攻击时，实体需要成功通过一次[隐匿](Stealth.md "隐匿")[检定](Ability_Check.md "属性检定")以维持隐形。每次成功尝试后，检定难度会增加。
