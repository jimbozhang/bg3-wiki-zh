# 树精 (召唤林地妖精)

## 树精

- 模型
- 肖像画

## 属性

### 体型

中级

### 类型

妖精

### 种族

[妖精](Fey.md "Fey")

- 平衡
- 探索者

## 平衡难度属性

### 等级

6

### 生命值

22

### 护甲等级

13

### 先攻

+2

### 熟练项

+3

### 移动

9 米 (30 英尺)

### 重量

50 千克 (100 磅)

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 10 / (0) | 14 / (+2) | 11 / (0) | 14 / (+2) | 15 / (+2) | 18 / (+4) |

## 抗性 [(info)](Resistances.md "Resistances")

- - ## 特性

- [**借机攻击**](Opportunity_Attack.md "Opportunity Attack")

攻击离开触及范围的敌人。

- [**永久武装**](Permanently_Armed.md "Permanently Armed")

无法被缴械。

- [**魔法抗性**](Magic_Resistance.md "Magic Resistance")

对法术和其他魔法效果的[豁免检定](Saving_throws.md "Saving_throws")具有[优势](Advantage.md "Advantage")。

- [**自然步伐**](Nature's_Step.md "Nature's Step")

改变你周围的地形，使该区域附近的盟友能够轻松移动。

你和范围内 4 米 / 13 英尺内的盟友不受[劣势地形](Difficult_Terrain_(Condition).md "劣势地形 (状态)")影响，不会被[束缚](Restrained_(Condition).md "束缚 (状态)")或[麻痹](Paralysed_(Condition).md "麻痹 (状态)")，并获得对[中毒](Poison.md "中毒")伤害的[抗性](Resistance.md "Resistance")。

- [**森林之握**](Grasp_of_the_Forest.md "Grasp of the Forest")

武器攻击可以使目标[缠绕](Entangled_(Condition).md "缠绕 (状态)")。

## 探索者难度属性

### 等级

6

### 生命值

44

### 护甲等级

13

### 先攻

+2

### 熟练项

+5

### 移动

9 米 (30 英尺)

### 重量

50 千克 (100 磅)

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 0 | +2 | 0 | +2 | +2 | +4 |

## 抗性 [(info)](Resistances.md "Resistances")

- - ## 特性

- [**借机攻击**](Opportunity_Attack.md "Opportunity Attack")

攻击离开触及范围的敌人。

- [**永久武装**](Permanently_Armed.md "Permanently Armed")

无法被缴械。

- [**魔法抗性**](Magic_Resistance.md "Magic Resistance")

对法术和其他魔法效果的[豁免检定](Saving_throws.md "Saving_throws")具有[优势](Advantage.md "Advantage")。

- [**自然步伐**](Nature's_Step.md "Nature's Step")

改变你周围的地形，使该区域附近的盟友能够轻松移动。

你和范围内 4 米 / 13 英尺内的盟友不受[劣势地形](Difficult_Terrain_(Condition).md "劣势地形 (状态)")影响，不会被[束缚](Restrained_(Condition).md "束缚 (状态)")或[麻痹](Paralysed_(Condition).md "麻痹 (状态)")，并获得对[中毒](Poison.md "中毒")伤害的[抗性](Resistance.md "Resistance")。

- [**森林之握**](Grasp_of_the_Forest.md "Grasp of the Forest")

武器攻击可以使目标[缠绕](Entangled_(Condition).md "缠绕 (状态)")。

**树精**是一种[妖精](List_of_creature_types.md#Fey "生物类型列表")生物，可通过法术[召唤林地妖精](Conjure_Woodland_Being.md "召唤林地妖精")召唤，这是 7 级[德鲁伊](Druid.md "德鲁伊")可用的四级法术。它们引导强大的德鲁伊力量，使它们能够缠绕敌人并召唤[菘蓝树人](Wood_Woad.md "菘蓝树人")协助战斗。

## 目录

- [1 战斗](#combat)
  - [1.1 攻击与能力](#attacks-and-abilities)
  - [1.2 物品](#items)
- [2 外部链接](#external-links)

## 战斗[&action=edit&section=1> "编辑章节源代码：战斗") | &veaction=edit&section=1> "编辑章节：战斗")]

### 攻击与能力[&action=edit&section=2> "编辑章节源代码：攻击与能力") | &veaction=edit&section=2> "编辑章节：攻击与能力")]

[强化橡棍术](Strengthened_Shillelagh.md "强化橡棍术") ()
你的[长棍](Quarterstaff.md "长棍")或[短棒](Club.md "短棒")变为魔法武器：造成 4d8[钝击](Bludgeoning.md "钝击")伤害，并使用你的[施法关键属性](Ability_score_modifier.md "属性值调整值")进行[攻击掷骰](Attack_roll.md "攻击掷骰")。

范围：自身

[纠缠](Entangle_(Dryad).md "纠缠 (树精)") ()
创造一片藤蔓地表，减慢生物速度，并可能使其[缠绕](Entangled_(Condition).md "缠绕 (状态)")。

[力量](Strength.md "力量") [豁免检定](Saving_throws.md "Saving_throws")
范围：18 米 (60 英尺)

[坠落连理](Fallen_Lover.md "坠落连理") ()
召唤一个与你的树誓缚的[菘蓝树人](Wood_Woad.md "菘蓝树人")与你并肩作战。

范围：9 米 (30 英尺)
充能：[短休](Short_rest.md "短休")

[荆棘丛生](Spike_Growth.md "荆棘丛生") ()

伤害：2~8

2d4⁠[穿刺](Piercing.md "穿刺")

将一片地面塑造成坚硬的尖刺。在尖刺上行走的生物每移动 1.5 米 (5 英尺) 会受到 2d4⁠⁠[穿刺](Piercing.md "穿刺")伤害。

尖刺是[劣势地形](Difficult_Terrain.md "劣势地形")，使生物的[移动速度](Movement_speed.md "移动速度")减半。

范围：18 米 (60 英尺)

### 物品[&action=edit&section=3> "编辑章节源代码：物品") | &veaction=edit&section=3> "编辑章节：物品")]

- [扭曲的橡木](Twisted_Oak_Crook.md "扭曲的橡木")
- 2x [神莓](Goodberry_(Item).md "神莓 (物品)")

## 外部链接[&action=edit&section=4> "编辑章节源代码：外部链接") | &veaction=edit&section=4> "编辑章节：外部链接")]

- ⁠[树精](https://forgottenrealms.fandom.com/wiki/Dryad) 在 [被遗忘的国度维基](https://forgottenrealms.fandom.com/wiki/Main_Page) 上

---
*Source: [Dryad (Conjure Woodland Being)](https://bg3.wiki/wiki/Dryad_(Conjure_Woodland_Being)*