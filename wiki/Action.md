...
**动作** 是 _博德之门3_ 中 [生物](Creatures.md "生物") 在其回合内采取的行为。动作通常用于造成 [伤害](Damage.md "伤害") 或施加有害 [状态](Conditions.md "状态")，但也可用于 [治疗](Healing.md "治疗") 或援助盟友。
...
情境动作
情境动作因其他能力、特性或状态而解锁。它会保留在角色的快捷栏旁，直到创建它的条件过期。一个例子是 [强制策动](Forced_Manoeuvre.md "强制策动")，作为 [策动攻击（近战）](Manoeuvring_Attack_(Melee).md "策动攻击（近战）") 的结果。
武器动作
...
- 允许生物每回合采取 _额外_ 动作的动作——例如 [动作如潮](Action_Surge.md "动作如潮")——_不会_ 消耗其提供的资源。
...
某些反应需要行动生物消耗 [反应](#reactions) 资源，而其他反应则不需要。不需要消耗的通常是使用反应系统的职业特性，例如 [至圣斩](Divine_Smite.md "至圣斩")、[鲁莽攻击](Reckless_Attack.md "鲁莽攻击") 或 [偷袭](Sneak_Attack.md "偷袭")。<sup>[\[1\]](#cite_note-1)</sup>

### 反应标签页

图 2 - 反应标签页。每个反应都可以通过左侧的复选框启用或禁用。可以通过“询问”设置进一步配置，以便在每次触发反应时提示玩家确认。

反应通过 _反应标签页_ 为每个角色管理，该标签页位于角色的法术书页面。也可以通过按下快捷栏上的激活反应之一或按下快捷键（默认：L）立即访问该标签页。
...
如果任何反应要求确认，角色无法对同一事件应用多个反应；UI 会阻止为每个角色选择多个反应。

在任何反应生效之前，事件触发的反应集合就已确定。这意味着，如果攻击掷骰或豁免检定通常会忽略某个反应（例如，因为掷骰成功且反应是来自盟友的掷骰加值），那么即使掷骰结果被另一个反应改变，也会忽略该反应。

#### 反应忽略的事件

大多数反应只“看到”直接攻击或法术，因此通常只触发于动作的即时效果。如果伤害和豁免检定源自以下来源，反应系统会忽略它们：

- 次要或延迟效果：来自地表、环境危害或持续状态的伤害（例如，内置在 [区域](Areas.md "区域") 效果、[人类定身术](Hold_Person.md "人类定身术") 等状态、反应或爆炸物品（箭和发射的烟花除外）中的豁免检定或伤害）
- 间接动作：由单独的反应（[炼狱叱喝](Hellish_Rebuke.md "炼狱叱喝")）或被动特性（[火焰护盾](Fire_Shield.md "火焰护盾") 或 [荆棘丛生](Spike_Growth.md "荆棘丛生")）发起的动作

**例外情况**：有关特定触发条件的完整列表——例如 [专注](Concentration.md "专注") 豁免或 [死亡豁免检定](Death_Saving_Throw.md "死亡豁免检定")——请参见下面列出的例外情况：<sup>[\[2\]](#cite_note-2)</sup>
...
- 由以下装备授予的被动特性：
  ...
  - [玛科赫什基](Markoheshkir.md "玛科赫什基") ([酸性](Adrip_with_Kereska's_Acid_(Condition).md "克雷斯卡酸液滴落（状态）"))
...
- 任何 [隐形状态](SG_Invisible.md "隐形状态组")，这会阻止除 [受诅咒幽鬼](Accursed_Spectre_(reaction).md "受诅咒幽鬼（反应）")、[致盲伏击](Blinding_Ambush.md "致盲伏击") 和 [怪异魔法涌动](Weird_Magic_Surge.md "怪异魔法涌动") 之外的所有反应
...
后一组状态不会阻止受影响的生物采取不消耗 [反应](#reactions) 资源的反应。此类反应尤其包括 [传奇动作](Legendary_actions.md "传奇动作")，由 [荣誉模式](Honour_Mode.md "荣誉模式") 中的首领执行。这些动作像反应一样触发，但使用不同的资源，因此不受这些状态的影响。
...
- 对豁免检定的反应忽略了 [惊惧斩](Staggering_Smite.md "惊惧斩")、[雷鸣打击](Thunderous_Smite.md "雷鸣打击") 和 [激愤斩](Wrathful_Smite.md "激愤斩") 以及 [推翻大个](Topple_the_Big_Folk.md "推翻大个") 的豁免检定。
...
1. [↑](#cite_ref-1) 许多在 _博德之门3_ 中作为反应实现的职业特性，在基于 D&D 第五版的 _博德之门3_ 中并非反应。这一改变可能是因为反应在游戏开发后期才添加。
...
4. [↑](#cite_ref-4) 源自 `Mods/Shared/Scripts/thoth/helpers/CommonConditions.khn` 中 `IsAbleToReact` 的定义、`EnableCondition` 字段数据（用于变形）以及 `InterruptWhileInvisible` 标志。反应可能同时检查变形和 `IsAbleToReact`，只检查其中一个，或两者都不检查。
