# 悲伤

悲伤是一把稀有的+1 [长柄刀](Glaives.md "长柄刀")，它让持有者每回合一次以[附赠动作](Actions.md#Resources "动作")获得[悲伤鞭笞](Sorrowful_Lash.md "悲伤鞭笞")戏法。

每一滴洒落的鲜血，都意味着灵魂的一部分消亡——直至只剩悔恨。

## 属性

伤害

1d10 + 1 (2~11) + [力量调整值](Damage_Roll.md#Modifiers "伤害掷骰") ⁠[挥砍](Slashing.md "挥砍")

详情
[长柄刀](Glaives.md "长柄刀")
稀有度：稀有
惑控学派：**+ 1**
[双手](Two-Handed.md "双手")
[额外范围](Extra_Reach.md "额外范围")
[可蘸取](Dippable.md "可蘸取")
范围：2.5 米 (8 英尺)
重量：2.7 千克 (5.4 磅)
价格：380 金币
UID `UNI_DEN_HalsinBlade` UUID `0de9e072-7048-4d07-8bbe-79e0591674c2` Stats `DEN_HalsinBlade` ### 特殊

**持有此物品获得：**

[悲伤鞭笞](Sorrowful_Lash.md "悲伤鞭笞") ()
以戏法施放（充能：每回合。）

### 武器动作

_如果你有[熟练项](Proficiency.md "熟练项")，装备在**主手**以获得：_

[备战（近战）](Brace_(Melee).md "备战（近战）") ()
消耗 6 米 (20 英尺) 的[移动](Movement_speed.md "移动速度")。在你的回合剩余时间内，掷两次近战伤害并取最高值。（充能：[短休](Short_rest.md "短休")。）

[割裂](Lacerate.md "割裂") ()
劈砍目标的要害部位，使其[流血](Bleeding_(Condition).md "流血（状态）")。（充能：[短休](Short_rest.md "短休")。）

[突进攻击](Rush_Attack.md "突进攻击") ()
向前冲锋并攻击路径上的第一个敌人，可能使其[失衡](Off_Balance_(Condition).md "失衡（状态）")。（充能：[短休](Short_rest.md "短休")。）

## 获取地点

- [隐藏金库](Hidden_Vault.md "隐藏金库") X: 183 Y: -82：在隐藏金库的狼祭坛的石板上

## 注释

- 通过解决[德鲁伊林地](Druid_Grove.md "德鲁伊林地")中的[闲庭图书馆](Enclave_Library.md "闲庭图书馆")来解锁祭坛。
- [哈尔辛](Halsin.md "哈尔辛")床边的[德鲁伊手册](Druid_Notebook.md "德鲁伊手册")暗示，悲伤最初属于[凯瑟里克·索姆](Ketheric_Thorm.md "凯瑟里克·索姆")。
- 除了[邪念](Dark_Urge.md "邪念")以外的所有角色在第一次拾取这把长柄刀时都会发表评论。

## 图库

- 游戏中的外观

---
*Source: [Sorrow](https://bg3.wiki/wiki/Sorrow)*