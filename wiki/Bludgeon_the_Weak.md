# 弱点钝击

**弱点钝击**是一个[法术](Spells.md "法术")。该法术是[标点弱点](Brand_the_Weak.md "Brand the Weak")的变体，使敌人对[钝击](Bludgeoning.md "Bludgeoning")伤害处于[易伤](Vulnerable.md "Vulnerable")状态。

## 描述

使一个敌人对[钝击](Bludgeoning.md "Bludgeoning")伤害处于[易伤](Vulnerable.md "Vulnerable")状态。

此效果持续 3 驱散，或直到目标受到伤害。

## 属性

消耗
[附赠动作](Actions.md#Resources "Actions")
详情
范围：18 米（60 英尺）
充能：[长休](Long_Rest.md "Long rest")

## 更高等级

以更高等级施放此法术不会获得额外收益。

## 技术细节

UID

`Target_MAG_Gish_WeaknessBranding_Bludgeoning`

法术标志

`[IsHarmful](IsHarmful_(spell_flag).md)`, `[IsLinkedSpellContainer](IsLinkedSpellContainer_(spell_flag).md)`, `[IsSpell](IsSpell_(spell_flag).md)`

## 状态：弱点钝击

**[弱点钝击](Bludgeon_the_Weak_(Condition).md "弱点钝击 (状态)")**

持续时间：3 驱散

- 受影响实体在 3 驱散内对[钝击](Bludgeoning.md "Bludgeoning")伤害处于[易伤](Vulnerable.md "Vulnerable")状态，或直到其受到伤害。

## 如何习得

此法术是以下法术的变体：
[标点弱点](Brand_the_Weak.md "Brand the Weak")

---
*Source: [Bludgeon the Weak](https://bg3.wiki/wiki/Bludgeon_the_Weak)*