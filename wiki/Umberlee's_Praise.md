# 安博里的赞美

本文介绍的内容**[无法通过正常游戏获取](Category_colon_Inaccessible.md "Category:Inaccessible")**。但仍可通过第三方工具访问。

**安博里的赞美**是一个[法术](Spells.md "法术")。此法术会魅惑大范围区域内的所有人。

## 描述

施法者周围的所有人都会被[魅惑](Charmed.md "Charmed")。

每当你成功躲避一次攻击，其中一个镜像就会消失。

## 属性

消耗
[动作](Actions.md#Resources "动作") + [2级法术位](Spells.md#Spell_slots "法术位")
详情
[感知](Wisdom.md "Wisdom") [豁免检定](Saving_throws.md "Saving throws")
射程：18米（60英尺）

## 更高环阶

以此法术的更高环阶施放不会获得额外收益。

## 技术细节

UID

`Shout_CHA_ShellKid_Stone_Spell`

## 状态：魅惑

**[魅惑](Charmed_(Condition).md "魅惑 (状态)")**

持续时间：10驱散

- 无法攻击施法者。施法者在对话中进行[魅力](Charisma.md "Charisma")[属性检定](Ability_Check.md "属性检定")时具有[优势](Advantage.md "Advantage")。

## 如何习得

由物品提供：

- [泼妇女王的祝福](The_Bitch_Queen's_Blessing.md "泼妇女王的祝福")

## 备注

- 描述具有误导性，此法术完全不提供镜像术。
- 宝石和安博里的赞美无法通过正常游戏获取，需借助第三方工具。

---
*Source: [Umberlee's Praise](https://bg3.wiki/wiki/Umberlee's_Praise)*