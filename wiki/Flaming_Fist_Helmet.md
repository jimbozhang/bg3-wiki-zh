A **焰拳头盔**（Flaming Fist Helmet）是一种常见的[头盔](Headwear.md "Headwear")。

这顶头盔虽无装饰，却因此更显威严，它诉说着焰拳所激发的静谧高贵与同等静谧的恐惧。

## 属性

- [头盔](Helmets.md "Helmets")
- 所需熟练项：[中甲](Medium_Armour.md "Medium_Armour")
- 稀有度：普通
- 重量：1 kg (2 lb)
- 价格：40 gp
- UID `ARM_Helmet_Metal_FlamingFist_A` UUID `83219bb0-800e-46fe-9dd8-24d98ae63f3c` ### 特殊效果

佩戴此物品者获得：

- [体质](Constitution.md "Constitution") [豁免检定](Saving_throw.md "Saving Throw") +1

## 获取地点

- 在[瑰晨修道院小径](Rosymorn_Monastery_Trail.md "Rosymorn_Monastery_Trail")的北向入口处
- 由[焰拳](Flaming_Fist.md "Flaming Fist")成员携带
- [阴影之拥修道院](Cloister_of_Sombre_Embrace.md "Cloister_of_Sombre_Embrace") X: -332 Y: -1606：渗透训练室的架子上

# 变体

**焰拳头盔**是一种常见的[头盔](Headwear.md "Headwear")。

虽然朴实无华，但正因为如此而显得令人生畏，这款头盔体现了焰拳所激发的无声威严与同样无声的恐惧。

## Properties

- [Helmets](Helmets.md "Helmets")
- Required Proficiency: [Medium Armour](Medium_Armour.md "Medium Armour")
- Rarity: Common
- 重量: 1 kg (2 lb)
- Price: 40 gp
- UID `ARM_Helmet_Metal_FlamingFist_B` UUID `5c98c7c8-97f3-4675-bf22-920168450949` ### Special

The wearer of this item gains:

- [Constitution](Constitution.md "Constitution") [Saving Throws](Saving_throw.md "Saving Throw") +1

## Where to find

- Carried by members of the [Flaming Fist](Flaming_Fist.md "Flaming Fist")

# 变体

A **Flaming Fist Helmet** is a common [Helmet](Headwear.md "Headwear").

The brutish thuggery of the Enforcer's position is summarised in this helmet's design.

## Properties

- [Helmets](Helmets.md "Helmets")
- Required Proficiency: [Medium Armour](Medium_Armour.md "Medium Armour")
- Rarity: Common
- 重量: 1 kg (2 lb)
- Price: 40 gp
- UID `ARM_Helmet_Metal_FlamingFist_C` UUID `0466c509-9947-441a-ad51-85ad4f407f38` ### Special

The wearer of this item gains:

- [Constitution](Constitution.md "Constitution") [Saving Throws](Saving_throw.md "Saving Throw") +1

## Where to find

- Carried by members of the [Flaming Fist](Flaming_Fist.md "Flaming Fist")

---
*Source: [Flaming Fist Helmet](https://bg3.wiki/wiki/Flaming_Fist_Helmet)*