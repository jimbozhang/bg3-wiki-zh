...
一种作为平坦层存在于地板上的区域。地表的效果可以通过使用[飞行术](Fly_(class_action).md "飞行术（职业动作）")或[魔法飓风：飞行](Tempestuous_Magic_colon__Flight.md "魔法飓风：飞行")等飞越它们来避免。地表包括[强酸](Acid_(surface).md "强酸（地表）")地表或[冰](Ice_(surface).md "冰（地表）")地表，也可以包括[蛛网术](Web.md "蛛网术")等法术。这些区域可以在游戏中被检视，并显示为地表。
...
一种作为云存在于地板上方的区域。它们可以被[造风术](Gust_of_Wind.md "造风术")吹散。这些区域可以在游戏中被检视，并显示为云。
...
在战斗日志中显示为被召唤的区域。召唤区域通常通过向进入其中的生物施加状态来运作。召唤区域可以彼此叠加放置，但不能与相同的区域叠加，因为一个生物一次只能拥有一个施加的状态实例。这些区域无法在游戏中被检视，但可能在其源法术上找到一个代表它们的状态，该状态与进入时施加的状态不同。
...
游戏中存在多种可以改变区域的交互类型。这些交互在 `SurfaceChange()` 函数中找到。并非所有交互都适用于所有地表，请参阅区域页面上的“交互”以查看其与什么交互以及如何交互。
...
| Ignite | 爆炸区域或点燃它 | 可以引爆烟粉并将多个地表变成火焰 | [火球术术](Fireball.md "火球术术"), [火焰箭](Fire_Bolt.md "火焰箭") |
