# 卓尔毒药云

**卓尔毒药云**是一种[云](Cloud.md "Cloud")，会对穿过它的单位施加减益效果。在云内时，角色每回合必须进行一次体质[豁免检定](Saving_throw.md "Saving Throw")，否则将[中毒](Poisoned_(Condition).md "Poisoned (Condition)")并[沉睡](Sleeping_(Condition).md "Sleeping (Condition)")。云本身不会造成任何伤害。

## 描述

有几率使目标[中毒](Poisoned_(Condition).md "Poisoned (Condition)")。

## 属性

类型：[云](Area.md#Cloud "区域")

## 状态：卓尔毒药侵袭

**[卓尔毒药侵袭](Stricken_with_Drow_Poison_(Condition).md "卓尔毒药侵袭 (状态)")**

[体质](Constitution.md "Constitution") [豁免检定](Saving_throws.md "Saving throws")（[骰子掷骰](DC.md "骰子掷骰") 13）

- 受影响实体[中毒](Poisoned_(Condition).md "Poisoned (Condition)")并已[沉睡](Sleeping_(Condition).md "Sleeping (Condition)")。
- 持续至受影响实体成功通过一次[体质](Constitution.md "Constitution")[豁免检定](Saving_throw.md "Saving Throw")或受到伤害为止。

## 创建方式

此区域可由以下方式创建：

物品：

- [卓尔毒药](Drow_Poison.md "卓尔毒药")

---
*Source: [Drow Poison Cloud](https://bg3.wiki/wiki/Drow_Poison_Cloud)*