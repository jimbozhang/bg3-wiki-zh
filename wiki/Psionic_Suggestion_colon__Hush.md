# 心灵低语：静默

**心灵低语：静默** 是一个 [1级惑控学派法术](Spells.md "Spells")。此法术允许施法者暂时沉默其目标，阻止他们施放带有言语成分的法术。无法被队友学习。

## 描述

用一道心灵命令[沉默](Silenced_(Condition).md "Silenced (Condition)")一个生物。

## 属性

消耗
[动作](Actions.md#Resources "Actions")
详情
[感知](Wisdom.md "Wisdom") [豁免检定](Saving_throws.md "Saving throws")
射程：18米（60英尺）

## 更高法术位

以更高法术位施放此法术不会获得额外收益。

## 技术细节

UID

`Target_END_Daisy_Command_Hush`

法术标志

`[HasSomaticComponent](HasSomaticComponent_(spell_flag).md)`, `[HasVerbalComponent](HasVerbalComponent_(spell_flag).md)`, `[IgnorePreviouslyPickedEntities](IgnorePreviouslyPickedEntities_(spell_flag).md)`, `[IsHarmful](IsHarmful_(spell_flag).md)`, `[IsSpell](IsSpell_(spell_flag).md)`

## 状态：沉默

**[沉默](Silenced_(Condition).md "Silenced (Condition)")**

持续时间：2 驱散

[感知](Wisdom.md "Wisdom") [豁免检定](Saving_throws.md "Saving throws") ([法术豁免DC](Dice_rolls.md#Spell_save_DC "Dice rolls"))

- 生物无法说话或施放带有言语成分的法术，并且免疫[雷鸣](Thunder.md "Thunder")伤害。

## 如何学习

由以下生物使用：[梦境守护者](Dream_Guardian.md "Dream Guardian")

---
*Source: [Psionic Suggestion: Hush](https://bg3.wiki/wiki/Psionic_Suggestion:_Hush)*