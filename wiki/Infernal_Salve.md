# 地狱抚慰

**地狱抚慰**是一个[6级塑能学派法术](Spells.md "Spells")。此法术允许拉斐尔使用灵魂治疗他的邪魔同伴。队伍成员无法学习此法术。

## 描述

消耗1个灵魂来治疗一个邪魔。

对不死生物和构装生物无效。

## 属性

消耗
[附赠动作](Actions.md#Resources "Actions")
治疗：3~18

3d6⁠[治疗](Healing.md "Healing")

详情
范围：18米（60英尺）

## 升环施法

[升环施法](Upcasting.md "Upcasting")：每比1环高1环，额外治疗1d4⁠⁠[生命值](Healing.md "Healing")。

## 技术细节

UID

`Target_LOW_Raphael_Heal`

法术标志

`[HasVerbalComponent](HasVerbalComponent_(spell_flag).md)`, `[IsSpell](IsSpell_(spell_flag).md)`, `[RangeIgnoreVerticalThreshold](https://bg3.wiki/w/index.php?title=RangeIgnoreVerticalThreshold_\(spell_flag\)&action=edit&redlink=1) "RangeIgnoreVerticalThreshold \(spell flag\) \(page does not exist\)")`

## 如何学习

被以下生物使用：[拉斐尔](Raphael.md "Raphael")

---
*Source: [Infernal Salve](https://bg3.wiki/wiki/Infernal_Salve)*