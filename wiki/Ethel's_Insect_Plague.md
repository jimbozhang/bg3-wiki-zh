# 埃赛尔的虫群瘟疫

本文介绍的是该法术的一个独特NPC变体。关于普通法术，请参见[虫群瘟疫](Insect_Plague.md "虫群瘟疫")。关于其他用途，请参见[虫群瘟疫（消歧义）](Insect_Plague_(disambiguation)..md)

**埃赛尔的虫群瘟疫**是一个[5级咒法学派法术](Spells.md "Spells")。该法术允许[埃赛尔婶婶](Auntie_Ethel.md "埃赛尔婶婶")在战场的一部分区域上创造一个造成伤害、带来削弱效果的昆虫群。队伍成员无法学习此法术。

## 描述

蝗虫攻击范围内的所有人，使该区域成为[劣势地形](Difficult_Terrain.md "劣势地形")，并对[察觉技能](Perception.md "Perception")的[检定](Ability_Check.md "属性检定")施加[劣势](Disadvantage.md "劣势")。

埃赛尔及其盟友免疫该效果。

## 属性

消耗
[动作](Actions.md#Resources "动作")
伤害：4~40

4d10⁠[穿刺](Piercing.md "穿刺")

详情
[体质](Constitution.md "Constitution") [豁免检定](Saving_throws.md "Saving throws")（豁免成功时：目标仍承受一半伤害。）
范围：18米（60英尺）
范围效果：6米（20英尺）半径
创造区域：虫群瘟疫
[专注](Concentration.md "专注")

## 更高等级

以更高等级施放此法术不会获得额外收益。

## 技术细节

UID

`Target_LOW_Hag_InsectPlague`

法术标志

`[CannotTargetItems](https://bg3.wiki/w/index.php?title=CannotTargetItems_\(spell_flag\)&action=edit&redlink=1) "CannotTargetItems \(spell_flag\) \(page does not exist\)")`, `[HasSomaticComponent](HasSomaticComponent_(spell_flag).md)`, `[HasVerbalComponent](HasVerbalComponent_(spell_flag).md)`, `[IsConcentration](IsConcentration_(spell_flag).md)`, `[IsHarmful](IsHarmful_(spell_flag).md)`, `[IsSpell](IsSpell_(spell_flag).md)`

## 区域：虫群瘟疫

**[虫群瘟疫](Insect_Plague_(area).md "虫群瘟疫 (区域)")**

持续时间：10驱散

范围效果：6米（20英尺）半径

成群的叮咬蝗虫充斥空气。它们每驱散造成4d10⁠⁠[穿刺](Piercing.md "穿刺")伤害，使该区域成为[劣势地形](Difficult_Terrain_(Condition).md "劣势地形 (状态)")，并对[察觉技能](Perception.md "Perception")的[检定](Checks.md "检定")施加[劣势](Disadvantage.md "劣势")。

类型：[召唤](Area.md#Summoned "区域")

### 状态：虫群瘟疫（光环）

**\_(状态)[虫群瘟疫](Insect_Plague_(Aura)_(Condition).md "虫群瘟疫 (光环) (状态)")**

- 成群的叮咬蝗虫充斥空气。它们每驱散造成4d10⁠⁠[穿刺](Piercing.md "穿刺")伤害，使该区域成为[劣势地形](Difficult_Terrain_(Condition).md "劣势地形 (状态)")，并对[察觉技能](Perception.md "Perception")的[检定](Ability_Check.md "属性检定")施加[劣势](Disadvantage.md "劣势")。

## 如何学习

由生物使用：[埃赛尔婶婶](Auntie_Ethel.md "埃赛尔婶婶")

## 外部链接

- ⁠[虫群瘟疫](https://forgottenrealms.fandom.com/wiki/Insect_plague) 在 [被遗忘的国度维基](https://forgottenrealms.fandom.com/wiki/Main_Page) 上

---
*Source: [Ethel's Insect Plague](https://bg3.wiki/wiki/Ethel's_Insect_Plague)*