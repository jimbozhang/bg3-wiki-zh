# 支配野兽

**支配野兽**是一个[法术](Spells.md "法术")。它允许施法者暂时魅惑一个野兽，使其与施法者并肩作战，而非对抗施法者。

## 描述

使一个[野兽](Beast.md "Beast")与你并肩作战。每当该野兽受到伤害时，它必须进行一次[感知](Wisdom.md "Wisdom") [豁免检定](Saving_throw.md "Saving Throw")来对抗你的支配。

## 属性

消耗
[动作](Actions.md#Resources "动作") + [4级法术位](Spells.md#Spell_slots "法术位")
详情
[WIS](Wisdom.md "感知") [豁免检定](Saving_throws.md "豁免检定")
射程：18米（60英尺）
[专注](Concentration.md "专注")

## 升环施法效应

以此法术升环施法不会获得额外收益。

## 技术细节

UID

`Target_DominateBeast`

法术标志

`[HasHighGroundRangeExtension](HasHighGroundRangeExtension_(spell_flag).md)`, `[HasSomaticComponent](HasSomaticComponent_(spell_flag).md)`, `[HasVerbalComponent](HasVerbalComponent_(spell_flag).md)`, `[IsConcentration](IsConcentration_(spell_flag).md)`, `[IsHarmful](IsHarmful_(spell_flag).md)`, `[IsSpell](IsSpell_(spell_flag).md)`

## 状态：受控（支配野兽）

**\_(状态)[受控](Dominated_(Dominate_Beast)_(Condition).md "受控（支配野兽）（状态）")**

持续时间：10驱散

[WIS](Wisdom.md "感知") [豁免检定](Saving_throws.md "豁免检定") ([法术豁免DC](Dice_rolls.md#Spell_save_DC "掷骰"))

- 受影响实体将跟随并跟随此法术的施法者并肩作战。
- 每当它受到伤害时，它可以通过一次成功的感知[豁免检定](Saving_throw.md "豁免检定")来打破施法者对它的控制。

## 如何习得

职业：

- 职业等级7：[德鲁伊](Druid.md "德鲁伊")、[术士](Sorcerer.md "术士")、[自然领域](Nature_Domain.md "自然领域")（领域法术）、[至高妖精](The_Archfey.md "至高妖精")、[旧日支配者](The_Great_Old_One.md "旧日支配者")、[魔契领主](The_Hexblade.md "魔契领主")，以及[大地结社](Circle_of_the_Land.md "大地结社")（山岳或幽暗地域）
- 职业等级10：[吟游诗人](Bard.md "吟游诗人")（通过[魔法秘密](Magical_Secrets.md "魔法秘密")）

由物品提供：

- [纪念戒指](Keepsake_Ring.md "纪念戒指")（充能：[长休](Long_Rest.md "长休")）

## 外部链接

- ⁠[支配野兽](https://forgottenrealms.fandom.com/wiki/Dominate_beast) 在 [被遗忘的国度Wiki](https://forgottenrealms.fandom.com/wiki/Main_Page) 上

---
*Source: [Dominate Beast](https://bg3.wiki/wiki/Dominate_Beast)*