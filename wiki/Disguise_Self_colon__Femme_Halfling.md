# 伪装术：女性半身人

**伪装术：女性半身人** 是一个 [等级 1 幻术学派法术](Spells.md "Spells")。它是 [伪装术](Disguise_Self.md "Disguise Self") 的一个变体。此法术允许施法者暂时将自己的外观改变为女性 [轻足半身人](Halfling.md#Lightfoot_Halflings "Halfling") 的形态。

## 描述

将自己伪装成具有女性体型的半身人。

## 属性

消耗
[动作](Actions.md#Resources "动作") + [一级法术位](Spells.md#Spell_slots "法术位")
详情
范围：自身
[仪式](Spells.md#Ritual_spells "法术")

## 更高环阶施法

以更高环阶施放此法术不会获得额外收益。

## 技术细节

UID

`Shout_DisguiseSelf_Halfling_Female`

法术标志

`[HasSomaticComponent](HasSomaticComponent_(spell_flag).md)`, `[HasVerbalComponent](HasVerbalComponent_(spell_flag).md)`, `[IsLinkedSpellContainer](IsLinkedSpellContainer_(spell_flag).md)`, `[IsSpell](IsSpell_(spell_flag).md)`

## 状态：伪装术

**[伪装术](Disguise_Self_(Condition).md "伪装术 (状态)")**

持续时间：直至 [长休](Long_Rest.md "长休")

- 外观完全改变。

## 如何习得

此法术是以下法术的变体：
[伪装术](Disguise_Self.md "伪装术")

---
*Source: [Disguise Self: Femme Halfling](https://bg3.wiki/wiki/Disguise_Self:_Femme_Halfling)*