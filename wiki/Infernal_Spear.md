# 地狱短矛

地狱短矛是一把稀有的+1 [短矛](Spears.md "Spears")，造成额外的3点[中毒](Poison.md "Poison")伤害，并可能在命中时使目标[中毒](Poisoned_(Condition).md "中毒（状态）")。

这把武器的纹饰中编织着精确的地狱符文，暗示其创作者精通地狱奥秘。

## 属性

单手伤害

1d6 + 1 (2~7) + [力量调整值](Damage_Roll.md#Modifiers "伤害掷骰") ⁠[穿刺](Piercing.md "Piercing")

双手伤害

1d8 + 1 (2~9) + [力量调整值](Damage_Roll.md#Modifiers "伤害掷骰") ⁠[穿刺](Piercing.md "Piercing")

额外伤害

3 ⁠[中毒](Poison.md "Poison")

详情
[短矛](Spears.md "Spears")
稀有度：稀有
惑控学派：**+ 1**
[两用](Versatile.md "Versatile")
[投掷攻击](Thrown.md "Thrown")
近战：1.5 米 (5 英尺)
重量：1.35 千克 (2.7 磅)
价格：190 金币
UID `MAG_Infernal_Spear` UUID `e8f6c0aa-de5c-45a5-97fd-599addc036f1` ### 特殊

**持有此物品获得：**

[地狱武器](Infernal_Weapon.md "Infernal_Weapon")
命中时，此武器造成额外3点[中毒](Poison.md "Poison")伤害，并可能使目标[中毒](Poisoned_(Condition).md "中毒（状态）")。

### 武器动作

_如果你拥有[熟练项](Proficiency.md "Proficiency")，装备在**主手**可获得：_

[突进攻击](Rush_Attack.md "Rush Attack")
向前冲锋并攻击路径上的第一个敌人，可能将其推至[失衡](Off_Balance_(Condition).md "失衡（状态）")状态。（充能：[短休](Short_rest.md "Short rest")。）

## 状态：中毒

**[中毒](Poisoned_(Condition).md "中毒（状态）")**

持续时间：2 回合

[体质](Constitution.md "Constitution") [豁免检定](Saving_throws.md "Saving throws")（[DC](DC.md "DC") 12）

- 在[攻击掷骰](Attack_roll.md "Attack Roll")和[检定](Checks.md "Checks")上承受[劣势](Disadvantage.md "Disadvantage")。

## 获取地点

- [飞龙岩要塞](Wyrm's_Rock_Fortress.md "飞龙岩要塞") X: -13 Y: 202：由[铁执政官米安](Mian.md "Mian")携带
- [飞龙岩要塞](Wyrm's_Rock_Fortress.md "飞龙岩要塞") X: -10 Y: 212：由[铁执政官纳夫](Nuff.md "Nuff")携带

## 错误

- 游戏中未明确说明，但此短矛在命中时若目标未通过[DC](Dice_rolls.md#Save_DCs "掷骰") 12的[体质](Constitution.md "Constitution") [豁免检定](Saving_throw.md "Saving Throw")，则会[中毒](Poisoned_(Condition).md "中毒（状态）")。此效果的豁免检定未在战斗日志中显示。

_关于地狱武器：_

- [中毒](Poisoned_(Condition).md "中毒（状态）")豁免检定的掷骰结果无论成功或失败均未在战斗日志中显示。由于其[DC](Dice_rolls.md#Save_DCs "掷骰") 12的[体质](Constitution.md "Constitution") [豁免检定](Saving_throw.md "Saving throw")较低，可能让人误以为中毒效果完全无效。

---
*Source: [Infernal Spear](https://bg3.wiki/wiki/Infernal_Spear)*