# 奉献之盾

奉献之盾是一件非常稀有的[盾牌](Shields.md "Shields")，它提供一个1级[法术位](Spell.md "Spell")、[盾牌猛击](Shield_Bash.md "Shield Bash")，并允许持有者每[长休](Long_Rest.md "Long Rest")一次施放3级[奉献之盾：协助](Shield_of_Devotion_colon__Aid.md "Shield_of_Devotion: Aid")。

关于[维伦的圣武士](https://forgottenrealms.fandom.com/wiki/Paladins_of_Velen)的任何传说都古老到几乎肯定是道听途说。然而，无可辩驳的是他们在这面古老盾牌上留下的强大祝福，至今仍能激发令人心跳加速的勇气。

## 属性

+2

[护甲等级](Armour_Class.md "Armour Class")

- [盾牌](Shields.md "Shields")
- 所需熟练项：[盾牌](Shields.md "Shields")
- 稀有度：非常稀有
- 重量：2.7 千克 (5.4 磅)
- 价格：570 金币 / 740 金币（荣誉模式）
- UID `MAG_BG_OfDevotion_Shield` UUID `a0bc3295-c01d-405e-8396-e0fa7e1e5340` Stats `MAG_BG_OfDevotion_Shield` ### 特殊效果

此物品的穿戴者获得：

[盾牌猛击](Shield_Blow.md "Shield_Blow") ()
当敌人用近战攻击命中你时，你可以使用你的[反应](reaction.md "reaction")将其击至[倒伏](Prone_(Condition).md "Prone (Condition)")，除非它们成功通过[力量](Strength.md "Strength")[豁免检定](Saving_throw.md "Saving throw")。

[奉献之盾：协助](Shield_of_Devotion_colon__Aid.md "Shield_of_Devotion: Aid") ()
治疗自身并使你的生命值上限提高10点[生命值](Hit_Points.md "Hit points")。（充能：[长休](Long_Rest.md "Long rest")。）

[额外法术位](Additional_Spell_Slot.md "Additional Spell Slot")
你获得1个[1级法术位](Spells.md#Spell_slots "Spells")。

## 获取地点

- [终焉光芒旅店](Last_Light_Inn.md "Last Light Inn") X: -31 Y: 130：由[终焉光芒旅店](Last_Light_Inn.md "Last Light Inn")[传送点](Waypoint.md "Waypoint")附近的[军需官塔利](Talli.md "Talli")出售

## 备注

- 此物品提供的[支援术](Aid.md "Aid")版本仅影响持有者，并且始终以3级施放。

## 错误

_关于额外法术位：_

- 尽管物品上有法术位耗尽状态，额外法术位仍可每[长休](Long_Rest.md "Long Rest")一次为每个角色提供一个额外的法术位。
- 如果该法术位用于创建[术法点](Sorcery_Point.md "Sorcery Point")，额外法术位可多次为同一角色提供额外的法术位。

## 外部链接

- ⁠[奉献之盾](https://forgottenrealms.fandom.com/wiki/Shield_of_devotion) 在 [被遗忘的国度维基](https://forgottenrealms.fandom.com/wiki/Main_Page) 上

---
*Source: [Shield of Devotion](https://bg3.wiki/wiki/Shield_of_Devotion)*