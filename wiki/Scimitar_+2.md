# 弯刀 +2

**弯刀 +2** 是一种稀有、中等附魔（+2）的 [弯刀](Scimitars.md "Scimitars") 武器变体。它是一种 [军用近战武器](Weapons.md "Weapons")，可单手挥舞。作为 [灵巧](Finesse.md "Finesse") 武器，它能受益于使用者的敏捷，而不仅仅是力量。它是一种 [轻型](Light_(weapon_property).md) 武器，任何人都无需特殊训练即可双持。

这把刀刃的每一次挥动都伴随着一道拱形的电蓝色残影。

## 属性

伤害

1d6 + 2 (3~8) + [力量或敏捷调整值](Damage_Roll.md#Modifiers "伤害掷骰") ⁠[挥砍](Slashing.md "Slashing")

详情
[弯刀](Scimitars.md "Scimitars")
稀有度：稀有
附魔：**+ 2**
单手
[灵巧](Finesse.md "Finesse")
[轻型](Light_(weapon_property).md "轻型（武器属性）")
[可蘸取](Dippable.md "可蘸取")
近战：1.5 米（5 英尺）
重量：1.35 千克（2.7 磅）
价格：95 金币
UID `WPN_Scimitar_2` UUID `5193af64-48c1-406f-90bf-87f7f01b4684` ### 武器动作

_如果你拥有[熟练项](Proficiency.md "熟练项")，装备在**主手**可获得：_

[割裂](Lacerate.md "割裂") ()
劈砍目标的要害点，使其[流血](Bleeding_(Condition).md "流血（状态）")。（充能：[短休](Short_rest.md "短休")。）

[华舞](Flourish.md "华舞") ()
佯攻，可能使对手[失衡](Off_Balance_(Condition).md "失衡（状态）")。（充能：[短休](Short_rest.md "短休")。）

## 获取地点

- 根据主角等级，由不同商人出售

---
*Source: [Scimitar +2](https://bg3.wiki/wiki/Scimitar_+2)*