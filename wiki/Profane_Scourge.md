# 亵渎灾祸

**亵渎灾祸** 是由 [不死灾祸](The_Undead_Bane.md "不死灾祸") 赋予的 [武器动作](Weapon_action.md "武器动作")。

## 描述

以征服之怒进行打击，将你的 [熟练项加值](Proficiency_Bonus.md "熟练项加值") 加入伤害掷骰。如果你击中 [邪魔](Fiend.md "邪魔") 或 [不死生物](Undead.md "不死生物")，则额外造成 2d6⁠⁠[挥砍](Slashing.md "挥砍")[DRS](Damage_rider_as_source.md "伤害来源附加") 伤害，并可能对其施加 [灾祸术](Bane_(Condition).md "灾祸术（状态）")。

## 属性

消耗
[动作](Actions.md#Resources "动作")
伤害：2~12 + 调整值

普通武器伤害

\+ [熟练项加值](Proficiency_Bonus.md "熟练项加值")⁠[挥砍](Slashing.md "挥砍")

\+ 2d6⁠[挥砍](Slashing.md "挥砍")（条件性）

详情
近战武器 [攻击掷骰](Attack_roll.md "攻击掷骰")
[力量](Strength.md "力量") [豁免检定](Saving_throws.md "豁免检定")（[武器动作DC](Dice_rolls.md#Weapon_action_DC "掷骰") + 2）
射程：普通武器射程
充能：[短休](Short_rest.md "短休")
必须对武器 [熟练](Proficiency.md#Weapon_proficiency "熟练项")

## 状态：灾祸术

**[灾祸术](Bane_(Condition).md "灾祸术（状态）")**

持续时间：3 回合

[力量](Strength.md "力量") [豁免检定](Saving_throws.md "豁免检定")（[武器动作DC](Dice_rolls.md#Weapon_action_DC "掷骰") + 2）

- 对 [攻击掷骰](Attack_roll.md "攻击掷骰") 和 [豁免检定](Saving_throw.md "豁免检定") 有 **-1d4** 惩罚。

## 学习方式

由物品赋予：

- [不死灾祸](The_Undead_Bane.md "不死灾祸")

## 备注

- 对于非邪魔或不死生物的目标，你仍然获得 [熟练项加值](Proficiency_Bonus.md "熟练项加值")⁠⁠[挥砍](Slashing.md "挥砍") 伤害，但不会获得 2d6⁠⁠[挥砍](Slashing.md "挥砍") 伤害，也不会获得 [灾祸术](Bane_(Condition).md "灾祸术（状态）") 效果。

## 视觉效果

<https://bg3.wiki/wiki/File:Profane_Scourge_Action.mp4>

---
*Source: [Profane Scourge](https://bg3.wiki/wiki/Profane_Scourge)*