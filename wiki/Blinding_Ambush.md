...
**伏击目盲**是 [尤格](Yurgir.md "尤格") 使用的一种 [传奇动作](Legendary_action.md "传奇动作")，当被 [警戒追猎](Watchful_Hunt.md "警戒追猎") 标记的目标攻击或施放法术时触发。这是一种近战攻击，会对被标记的目标造成光耀伤害并可能使其目盲。尤格还拥有此攻击的 [远程变体](Blinding_Ambush_(ranged).md)。

## 描述

尝试在 [被追猎](Hunted_(Condition).md "被追猎（状态）") 的生物攻击或对你施放法术前使其 [目盲](Blinded_(Condition).md "目盲（状态）")。
...
由以下生物使用：[尤格](Yurgir.md "尤格")

## 备注

- 与远程版本不同，近战版本的 [目盲](Blinded_(Condition).md "目盲（状态）") 效果有豁免检定。
