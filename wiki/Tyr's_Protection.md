# 提尔的守护

另见：[虔诚护盾](Shield_of_Faith.md "虔诚护盾")

**提尔的守护**是一个[1级防护学派法术](Spells.md "法术"). 该法术允许施法者用神圣的保护环绕自己或一名盟友，并获得[护甲等级](Armour_Class.md "护甲等级")加值。

## 描述

保护一个生物免受攻击：使其[护甲等级](Armour_Class.md "护甲等级")提高2点。

当正义神剑被卸下时，法术结束。

## 属性

消耗
[附赠动作](Actions.md#Resources "动作")
详情
范围：18米（60英尺）
充能：[短休](Short_rest.md "短休")
[专注](Concentration.md "专注")

## 更高等级

以更高等级施放此法术不会获得额外收益。

## 技术细节

UID

`Target_PLA_ShieldOfFaith_SwordOfJustice`

法术标志

`[HasSomaticComponent](HasSomaticComponent_(spell_flag).md)`, `[HasVerbalComponent](HasVerbalComponent_(spell_flag).md)`, `[IsConcentration](IsConcentration_(spell_flag).md)`, `[IsSpell](IsSpell_(spell_flag).md)`

## 状态：虔诚护盾

**[虔诚护盾](Shield_of_Faith_(Condition).md "虔诚护盾 (状态)")**

持续时间：直至[长休](Long_Rest.md "长休")

- 获得+2的[护甲等级](Armour_Class.md "护甲等级")加值。

## 如何习得

由物品提供：

- [正义神剑](Sword_of_Justice.md "正义神剑")（充能：[短休](Short_rest.md "短休")）

---
*Source: [Tyr's Protection](https://bg3.wiki/wiki/Tyr's_Protection)*