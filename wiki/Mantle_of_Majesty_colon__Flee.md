# 威严斗篷：逃离

**威严斗篷：逃离**是[迷惑学院](College_of_Glamour.md "迷惑学院")吟游诗人的职业动作，使用附赠动作命令敌人逃离。它对被[魅惑](Charmed_(Condition).md "魅惑（状态）")的敌人保证生效。

## 描述

命令一个生物在其回合逃离你，并且不做其他任何事情。

被[魅惑](Charmed_(Condition).md "魅惑（状态）")的目标自动豁免检定失败。

## 属性

消耗
[附赠动作](Actions.md#Resources "动作")
详情
[感知](Wisdom.md "感知") [豁免检定](Saving_throws.md "豁免检定")（[法术豁免DC](Dice_rolls.md#Spell_save_DC "骰子掷骰")）
范围：18米（60英尺）
充能：[长休](Long_Rest.md "长休")
[专注](Concentration.md "专注")

## 状态：威严斗篷

**[威严斗篷](Mantle_of_Majesty_(Condition).md "威严斗篷（状态）")**

持续时间：10 驱散

- 专注于威严斗篷。仍可在每回合对任何目标施放该法术的任何变体。

## 状态：命令：逃离

**[命令：逃离](Command_colon__Flee_(Condition).md "命令：逃离（状态）")**

持续时间：1 驱散

[感知](Wisdom.md "感知") [豁免检定](Saving_throws.md "豁免检定")（[法术豁免DC](Dice_rolls.md#Spell_save_DC "骰子掷骰")）

- 必须以最快的方式尽可能远离施法者移动。
- 不能采取[动作](Action.md "动作")、[附赠动作](Bonus_action.md "附赠动作")或[反应](Reaction.md "反应")。

## 如何学习

职业：

- 职业等级 6：[迷惑学院](College_of_Glamour.md "迷惑学院")

此法术是以下法术的变体：
[威严斗篷：命令](Mantle_of_Majesty_colon__Command.md "威严斗篷：命令")

---
*Source: [Mantle of Majesty: Flee](https://bg3.wiki/wiki/Mantle_of_Majesty:_Flee)*