# 费金

## 费金

- 游戏内
- 肖像画

### 身份

女性

### 家乡

[博德之门](Baldur's_Gate.md "博德之门")

## 属性

### 体型

[中级](Medium.md "中级")

### 类型

[类人生物](Humanoid.md "类人生物")

### 种族

[卓尔](Drow.md "卓尔")

### 亚种

[罗丝誓约卓尔](Lolth-Sworn_Drow.md "罗丝誓约卓尔")

- 平衡

- 探索者

- 硬核

## 平衡难度属性

### 等级

1

### 生命值

8

### 护甲等级

10

### 先攻

0

### 熟练项

+2

### 移动

9 米 (30 英尺)

### 重量

75 千克 (150 磅)

### 高级黑暗视觉

16 米 (53 英尺)

### 经验值奖励

1

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 10 / (0) | 10 / (0) | 10 / (0) | 10 / (0) | 10 / (0) | 10 / (0) |

## 特性

- [高级黑暗视觉](Darkvision.md "高级黑暗视觉")

能在黑暗中看到一定距离。

- [妖精血统](Fey_Ancestry.md "妖精血统")

对[魅惑](Charmed_(Condition).md)的[豁免检定](Saving_throw.md "豁免检定")具有[优势](Advantage.md "优势")；魔法无法使其[沉睡](Sleeping_(Condition).md)。

- [借机攻击](Opportunity_Attack.md "借机攻击")

攻击离开攻击范围的敌人。

- [日照敏感](Sunlight_Sensitivity.md "日照敏感")

在阳光下时，攻击掷骰和[察觉技能](Perception.md "察觉技能")检定具有[劣势](Disadvantage.md "劣势")。

## 探索者难度属性

### 等级

1

### 生命值

4

### 护甲等级

10

### 先攻

0

### 熟练项

+4

### 移动

9 米 (30 英尺)

### 重量

75 千克 (150 磅)

### 高级黑暗视觉

16 米 (53 英尺)

### 经验值奖励

1

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 0 | 0 | 0 | 0 | 0 | 0 |

## 特性

- [高级黑暗视觉](Darkvision.md "高级黑暗视觉")

能在黑暗中看到一定距离。

- [妖精血统](Fey_Ancestry.md "妖精血统")

对[魅惑](Charmed_(Condition).md)的[豁免检定](Saving_throw.md "豁免检定")具有[优势](Advantage.md "优势")；魔法无法使其[沉睡](Sleeping_(Condition).md)。

- [借机攻击](Opportunity_Attack.md "借机攻击")

攻击离开攻击范围的敌人。

- [日照敏感](Sunlight_Sensitivity.md "日照敏感")

在阳光下时，攻击掷骰和[察觉技能](Perception.md "察觉技能")检定具有[劣势](Disadvantage.md "劣势")。

## 硬核难度属性

### 等级

1

### 生命值

7

### 护甲等级

10

### 先攻

0

### 熟练项

+2

### 移动

9 米 (30 英尺)

### 重量

75 千克 (150 磅)

### 高级黑暗视觉

16 米 (53 英尺)

### 经验值奖励

1

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 10 / (0) | 10 / (0) | 10 / (0) | 10 / (0) | 10 / (0) | 10 / (0) |

## 特性

- [高级黑暗视觉](Darkvision.md "高级黑暗视觉")

能在黑暗中看到一定距离。

- [妖精血统](Fey_Ancestry.md "妖精血统")

对[魅惑](Charmed_(Condition).md)的[豁免检定](Saving_throw.md "豁免检定")具有[优势](Advantage.md "优势")；魔法无法使其[沉睡](Sleeping_(Condition).md)。

- [借机攻击](Opportunity_Attack.md "借机攻击")

攻击离开攻击范围的敌人。

- [日照敏感](Sunlight_Sensitivity.md "日照敏感")

在阳光下时，攻击掷骰和[察觉技能](Perception.md "察觉技能")检定具有[劣势](Disadvantage.md "劣势")。

## 技术细节

###

UID

`S_LOW_FakeSendai_Corpse_001` ###

UUID

`86ccda32-2196-40fb-92ea-ed534c8021ae` **费金**是一名死去的[卓尔](Drow.md "卓尔")，其尸体可在第三章的[下城区](Lower_City.md "下城区")找到。

> “
>
> 这本该是森代，一名计划向其兄弟发动战争的巴尔之子。但我们先找到了她。
>
> „

— [贾希拉](Jaheira.md "贾希拉")，在探索现场时

## 目录

- [1 参与情况](#参与情况)
- [2 相关任务](#相关任务)
- [3 战利品](#战利品)
- [4 备注](#备注)
- [5 外部链接](#外部链接)

## 参与情况

费金的尸体可在[拉韦尔尼卡之家](Lavernica's_Home.md "拉韦尔尼卡之家")地下室后方的一个洞穴中找到。

费金被[奥林](Orin.md "奥林")杀害。她的尸体被用作奥林“艺术”的一部分，作为[森代](List_of_references_to_previous_Baldur's_Gate_games.md#Characters_from_Baldur's_Gate_1_and_2_appearing_as_NPCs "博德之门前作参考列表")的肖像——森代是一名一个多世纪前被击败的巴尔之子，同时还有许多其他女性卓尔的尸体（全部与她本人外观相同），以及数名灰矮人、土元素，甚至一名夺心魔。

费金的尸体可被搜刮，获得[寻找小丑德里波斯](Find_Dribbles_the_Clown.md "寻找小丑德里波斯")的身体部位之一，以及[奥林的仪式笔记](Orin's_Ritual_Note.md "奥林的仪式笔记")的第三份。

## 相关任务

- [寻找小丑德里波斯](Find_Dribbles_the_Clown.md "寻找小丑德里波斯")
- [调查谋杀案](Investigate_the_Murders.md "调查谋杀案")

## 战利品

- [小丑的断腿](Clown's_Severed_Leg.md "小丑的断腿")
- [森代，第三人](Sendai,_the_Third.md "森代，第三人")

## 备注

- 费金的游戏内模型，以及[森代的回声](Echo_of_Sendai.md "森代的回声")（[谋杀裁判所](Murder_Tribunal.md "谋杀裁判所")的森代幽魂）和伊桑·蒂博（[底城废墟](Undercity_Ruins.md "底城废墟")中另一处尸体场景的死去卓尔）的模型，均设计为复制博德之门前作中森代的形象。
- 该角色的名字可能参考了蒙提·派森的《布莱恩的一生》中的一个场景，其中提到了一位名叫 Incontinentia Buttocks 的罗马贵族女性及其丈夫 Biggus Dickus。

## 外部链接

- [森代](https://forgottenrealms.fandom.com/wiki/Sendai) 在 [被遗忘的国度 Wiki](https://forgottenrealms.fandom.com/wiki/Main_Page) 上

---
*Source: [Incontinentia Figgin](https://bg3.wiki/wiki/Incontinentia_Figgin)*