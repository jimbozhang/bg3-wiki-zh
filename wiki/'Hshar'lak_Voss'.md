# 'Hshar'lak Voss'

'**Hshar'lak Voss'** questions whether [Voss](Voss.md "Voss") is a traitor to ⁠[Vlaakith](Vlaakith.md "Vlaakith").

一张朴素、无装饰的便条。

## 属性

- [石板](Slates.md "Slates")

- 作者：未知

- 稀有度：普通

- 重量：.05 千克 (0.1 磅)

- 价格：14 金币

- UID `S_CRE_MIOffice_VossDocument` UUID `85158042-95b2-47e6-8dbb-28e804614e49` ## 位置

- ⁠[审判官的房间](Crèche_Y'llek.md#Inquisitor's_Chamber "Crèche Y'llek")，位于 X: 1345 Y: -669

## 文本

\[石板上刻满了密集的文字。几个词反复出现：'Voss'、'[hshar'lak](Githyanki_vocabulary.md#hshar'lak "吉斯洋基语词汇")'、'Vlaakith'、'忠诚'。文字上方用通用语写了一条注释。\]

Voss - 叛徒？

## 备注

- 内容暗示这原本应是一块 [吉斯洋基石板](Githyanki_Slate.md "吉斯洋基石板")

---
*Source: ['Hshar'lak Voss'](https://bg3.wiki/wiki/'Hshar'lak_Voss')*