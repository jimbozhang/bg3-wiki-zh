# 灾祸短剑

灾祸短剑是一把稀有的+1[短剑](Shortswords.md "短剑")，命中时会施加[灾祸术](Bane_(Condition).md "灾祸术（状态）")状态。这把剑的魔法力量只有在[誓缚](Bound_Weapon_(Condition).md "誓缚武器（状态）")于奥法骑士或是邪术师的[魔契武器](Pact_Weapon_(Condition).md "魔契武器（状态）")时才会生效。

法师[奥提卢克](https://forgottenrealms.fandom.com/wiki/Otiluke)曾写过关于誓缚武器及其在持有者与武器合二为一时凝聚的倾向。他也写过它们的危险——危险重重，以及它们的好处——让危险变得值得。

## 属性

伤害

1d6 + 1 (2~7) + [力量或敏捷调整值](Damage_Roll.md#Modifiers "伤害掷骰") [穿刺](Piercing.md "穿刺")

详情
[短剑](Shortswords.md "短剑")
稀有度：稀有
附魔：**+ 1**
单手
[灵巧](Finesse.md "灵巧")
[轻型](Light_(weapon_property).md "轻型（武器属性）")
[可蘸取](Dippable.md "可蘸取")
近战：1.5米（5英尺）
重量：0.9千克（1.8磅）
价格：190 gp / 250 gpHHonour
UID `MAG_Bonded_Baneful_Shortsword` UUID `3b578a43-222e-480b-8a22-2c424471099f` Stats `MAG_Bonded_Baneful_Shortsword` ### 特殊

**持有此物品者获得：**

[心仪武器](Favoured_Weapon.md "心仪武器")
此武器的伤害和[攻击掷骰](Attack_roll.md "攻击掷骰")具有+1加值。

[灾祸连接](Baneful_Bond.md "灾祸连接")
命中时，可能对目标施加[灾祸术](Bane_(Condition).md "灾祸术（状态）")。

### 武器动作

_如果你拥有[熟练项](Proficiency.md "熟练项")，装备在**主手**以获得：_

[华舞](Flourish.md "华舞")
佯攻以可能使对手[失衡](Off_Balance_(Condition).md "失衡（状态）")。（充能：[短休](Short_rest.md "短休")。）

[穿刺打击](Piercing_Strike.md "穿刺打击")
刺击敌人并可能施加[开放伤口](Gaping_Wounds_(Condition).md "开放伤口（状态）")。（充能：[短休](Short_rest.md "短休")。）

## 获取地点

- [黑檀湖洞穴](Ebonlake_Grotto.md "黑檀湖洞穴") X: 111 Y: -89：由[布尔格](Blurg.md "布尔格")出售

## 注释

- 此剑的魔法力量只有在[武器绑定](Weapon_Bond.md "武器绑定")于[奥法骑士](Eldritch_Knight.md "奥法骑士")或是[邪术师](Warlock.md "邪术师")的[魔契武器誓缚](Bind_Pact_Weapon.md "魔契武器誓缚")或[脆弱诅咒](Bind_Hexed_Weapon.md "脆弱诅咒武器誓缚")武器时才会生效。这些效果是武器+1附魔的额外加成。由于附魔和心仪武器加成叠加，此武器在誓缚时相当于一把+2武器。

_关于灾祸连接：_

- 灾祸连接不会在战斗日志中显示豁免检定掷骰。

## 错误

- 尽管描述只提到[誓缚武器](Bound_Weapon_(Condition).md)和[魔契武器誓缚](Bind_Pact_Weapon.md "魔契武器誓缚")能启用其**心仪武器**被动，但[魔刃邪术师](Hexblade.md "魔刃邪术师")的[脆弱诅咒武器誓缚](Bind_Hexed_Weapon.md "脆弱诅咒武器誓缚")同样有效。

---
*Source: [The Baneful](https://bg3.wiki/wiki/The_Baneful)*