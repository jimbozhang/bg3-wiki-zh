# 原始雷暴

**原始雷暴**是[风元素](Air_Elemental.md "风元素")独有的动作。它可以使附近所有生物[感电](Shocked_(Condition).md "感电（状态）")，使其易受该元素生物的[多重攻击](Multiattack_(Air_Elemental).md "多重攻击（风元素）")影响。

## 描述

呼啸的风暴狂风在你周围盘旋，猛烈冲击附近的生物并使其[感电](Shocked_(Condition).md "感电（状态）")。

## 属性

消耗
[动作](Actions.md#Resources "动作")
伤害：2~12

2d6⁠[闪电](Lightning.md "闪电")

详情
[力量](Strength.md "力量") [豁免检定](Saving_throws.md "豁免检定")（[法术豁免DC](Dice_rolls.md#Spell_save_DC "掷骰")）（豁免成功时：目标仍承受一半伤害）
范围：自身
范围效果：4米（13英尺）半径
目标：半径内所有其他生物

## 技术细节

UID

`Shout_Whirlwind_Elemental_Air`

法术标志

`[IsAttack](IsAttack_(spell_flag).md)`

## 状态：感电

**[感电](Shocked_(Condition).md "感电（状态）")**

持续时间：2驱散

[力量](Strength.md "力量") [豁免检定](Saving_throws.md "豁免检定")（[法术豁免DC](Dice_rolls.md#Spell_save_DC "掷骰")）

- 无法进行[反应](Reaction.md "反应")，且在使用[敏捷](Dexterity.md "敏捷")进行的[属性检定](Ability_Check.md "属性检定")和[豁免检定](Saving_throws.md "豁免检定")上具有[劣势](Disadvantage.md "劣势")。

## 学习方式

由以下生物使用：[风元素](Air_Elemental.md "风元素")

## 备注

- 风元素的[法术豁免DC](Spell_save_DC.md "法术豁免DC")基于智力，其调整值为-2。默认情况下，DC为10。
- 此攻击也会击中范围内的任何队伍成员或盟友。

---
*Source: [Primordial Gales](https://bg3.wiki/wiki/Primordial_Gales)*