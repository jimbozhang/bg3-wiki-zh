# 临时弓

**临时弓**是[短弓](Shortbows.md "短弓")武器家族的一种常见变体。它是一种需要双手持握的[军用远程武器](Weapons.md "武器")。

尽管只是一根柔韧的树枝和一根弦，但这把临时弓在紧要关头也能派上用场。

## 属性

伤害

1d6 (1~6) + [敏捷调整值](Damage_Roll.md#Modifiers "伤害掷骰") ⁠[穿刺](Piercing.md "穿刺")

额外伤害

1d10 (1~10) ⁠[黯蚀](Necrotic.md "黯蚀")

详情
[短弓](Shortbows.md "短弓")
稀有度：普通
惑控学派：无
[双手](Two-Handed.md "双手")
[可蘸取](Dippable.md "可蘸取")
射程：18米（60英尺）
重量：0.9千克（1.8磅）
价格：16金币
UID `WPN_HUM_Shortbow_A_Makeshift` UUID `82992370-cb61-4381-b390-891a3a9bd075` Stats `WPN_Shortbow_Makeshift` ### 武器动作

_如果你拥有[熟练项](Proficiency.md "熟练项")，装备在**主手**可获得：_

[腿筋射击](Hamstring_Shot.md "腿筋射击") ()
射击敌人的大腿，可能使其[移动速度](Movement_speed.md "移动速度")降低50%。（充能：[短休](Short_rest.md "短休")。）

## 获取地点

- 由[骷髅（操纵死尸）](Skeleton_(Animate_Dead).md "骷髅（操纵死尸）")携带

## 备注

- 不使用漏洞无法获得此物品。[[_verify_](bg3wiki_colon_Verification.md "bg3wiki:Verification")]
- 当玩家角色在[幽暗地域](Underdark.md "幽暗地域")的[观察者眼魔](Spectator.md "观察者眼魔")战斗中通过特定方式召唤骷髅时，可以从该骷髅弓箭手身上扒窃到此弓。[[_verify_](bg3wiki_colon_Verification.md "bg3wiki:Verification")]

---
*Source: [Makeshift Bow](https://bg3.wiki/wiki/Makeshift_Bow)*