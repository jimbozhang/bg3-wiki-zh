# 猎人匕首

猎人匕首是一把不常见的+1 [匕首](Daggers.md "Daggers")，在近战命中时会对目标施加[裂地](Ruptured_(Condition).md) to the target on a melee hit._\[[见：错误](#bugs)\]_

猎物常以为自己已逃脱捕食者的嗜血与狂怒——但总有更残酷的折磨等待着它。

## 属性

伤害

1d4 + 1 (2~5) + [力量或敏捷调整值](Damage_Roll.md#Modifiers "伤害掷骰") ⁠[穿刺](Piercing.md "穿刺")

详情
[Daggers](Daggers.md "Daggers")
稀有度：不常见
附魔：**+ 1**
单手
[灵巧](Finesse.md "Finesse")
[轻型](Light_(weapon_property).md "轻型（武器属性）")
[投掷攻击](Thrown.md "投掷攻击")
[可蘸取](Dippable.md "可蘸取")
近战：1.5 米（5 英尺）
重量：0.45 千克（0.9 磅）
价格：65 金币
UID `MAG_TheHunters_Dagger` UUID `e101101e-e4ba-48b6-9f82-267ca1cd84dc` Stats `MAG_TheHunters_Dagger` ### 特殊效果

**持有此物品者获得：**

[地动山摇](Rupturing.md "地动山摇")
命中时，目标陷入[裂地](Ruptured_(Condition).md "裂地（状态）")状态，持续3回合。

### 武器动作

_如果你拥有[熟练项](Proficiency.md "熟练项")，装备在**主手**可获得：_

[华舞](Flourish.md "华舞")
佯攻以可能使对手[失衡](Off_Balance_(Condition).md "失衡（状态）")。（充能：[短休](Short_rest.md "短休")。）

[穿刺打击](Piercing_Strike.md "穿刺打击")
刺击敌人并可能施加[开放伤口](Gaping_Wounds_(Condition).md "开放伤口（状态）")。（充能：[短休](Short_rest.md "短休")。）

[割裂](Lacerate.md "割裂")
劈砍目标要害使其[流血](Bleeding_(Condition).md "流血（状态）")。（充能：[短休](Short_rest.md "短休")。）

## 状态：裂地

**[裂地](Ruptured_(Condition).md "裂地（状态）")**

持续时间：3回合

- 每移动1米（3英尺）受到1d4⁠⁠[穿刺](Piercing.md "穿刺")伤害。每次触发伤害时，状态持续时间减少1。

## 获取地点

- [破碎圣所](Shattered_Sanctum.md "破碎圣所") X: 272 Y: -31：由[罗阿·月光](Roah_Moonglow.md "罗阿·月光")出售，位于圣所入口左侧深处

## 备注

_关于地动山摇：_

- 裂地状态仅对拥有此被动特性的武器和武器动作生效。
- 武器进行[投掷攻击](Thrown.md "投掷攻击")时不会施加裂地状态。
- 裂地状态的持续时间不会因多次命中叠加。每次后续命中都会将持续时间重置为3回合。

## 错误

- 猎人匕首在罗阿的商人库存刷新时，会像非独特装备一样重新出现，并可多次获取。

_关于地动山摇：_

- 游戏内描述称目标需通过[DC](Dice_rolls.md#Save_DCs "掷骰") 13 [体质](Constitution.md "体质") [豁免检定](Saving_throw.md "豁免检定")以避免裂地效果。然而，这并不正确，该状态总是会被施加。

---
*Source: [Hunter's Dagger](https://bg3.wiki/wiki/Hunter's_Dagger)*