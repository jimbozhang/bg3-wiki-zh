# 爆裂共鸣

**爆裂共鸣** 是一项被动特性，当你进行远程法术攻击时，会对目标施加残响。

## 描述

当穿戴者用远程[法术攻击](Spell_Attack.md "法术攻击")造成伤害时，对目标施加2回合的[残响](Reverberation_(Condition).md "残响 (状态)")。

## 状态：残响

**[残响](Reverberation_(Condition).md "残响 (状态)")**

持续时间：2回合

- 受影响实体每剩余回合在[力量](Strength.md "力量")、[敏捷](Dexterity.md "敏捷")和[体质](Constitution.md "体质")[豁免检定](Saving_throw.md "豁免检定")上承受-1减值。

- 当实体拥有5回合或以上的残响时，它会受到1d4⁠⁠[雷鸣](Thunder.md "雷鸣")[伤害机制](Damage_rider_as_source.md "伤害机制")伤害，并且必须成功通过[掷骰](Dice_rolls.md#Save_DCs "骰子投掷") 10的[体质](Constitution.md "体质")[豁免检定](Saving_throw.md "豁免检定")，否则会[倒伏](Prone_(Condition).md "倒伏 (状态)")。之后状态会被移除。

- 对⁠[雷鸣](Thunder.md "雷鸣")伤害[免疫](Damage_types.md#Immunity "伤害类型")的生物无法获得残响。

## 如何习得

由装备提供：

- [颤脊护符](Spineshudder_Amulet.md "颤脊护符")

## 备注

- 有非显而易见的方式施放法术攻击。至少来自[真菌迷药](Fungal_Bamboozler.md "真菌迷药")、[军团魔药水](Merregon_Potion.md "军团魔药水")和[毒素孢子弹](Noxious_Spore_Grenade.md "毒素孢子弹")的爆炸算作法术攻击。

---
*Source: [Crackling Resonance](https://bg3.wiki/wiki/Crackling_Resonance)*