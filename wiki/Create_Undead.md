# 唤起死灵

**唤起死灵**是一个[6级死灵学派法术](Spells.md "Spells")。它允许施法者活化一具尸体，创造一个不死仆从。

## 描述

将一具尸体变成一具邪恶的木乃伊，为你而战。

目标必须是一具中型或小型尸体。

## 属性

消耗
[动作](Actions.md#Resources "动作") + [6级法术位](Spells.md#Spell_slots "法术位")
详情
范围：3米（10英尺）
目标：小型或中型尸体

## 更高环阶施法

以更高环阶施法此法术不会获得额外收益。

## 技术细节

UID

`Target_CreateUndead`

法术标志

`[HasSomaticComponent](HasSomaticComponent_(spell_flag).md)`, `[HasVerbalComponent](HasVerbalComponent_(spell_flag).md)`, `[IsSpell](IsSpell_(spell_flag).md)`

## 生物：木乃伊（唤起死灵）

| [力量](Strength.md "力量") 16 (+3) | [敏捷](Dexterity.md "敏捷") 14 (+2) | [体质](Constitution.md "体质") 15 (+2) | [智力](Intelligence.md "智力") 6 (-2) | [感知](Wisdom.md "感知") 10 | [魅力](Charisma.md "魅力") 12 (+1) |
| --- | --- | --- | --- | --- | --- |
|  |  |  |  |  |  |
| 持续时间 | 直至长休 |  |  |  |  |
| 生命值 | 93 |  |  |  |  |
| 护甲等级 | 18 |  |  |  |  |

[恐惧凝视](Dreadful_Glare.md "恐惧凝视") ()
用你那不祥的凝视锁定目标，使其[恐慌](Frightened_(Condition).md "恐慌（状态）")。（充能：每回合。）

[多重攻击](Multiattack_(Mummy).md "多重攻击（木乃伊）") ()
对一个处于[恐慌](Frightened_(Condition).md "恐慌（状态）")状态的目标发动多次攻击。每次攻击造成2d6[钝击](Bludgeoning.md "钝击")和3d6[黯蚀](Necrotic.md "黯蚀")伤害。（充能：每回合。）

[腐烂之拳](Rotting_Fist.md "腐烂之拳") ()
将你的拳头砸向敌人，并可能使其感染[木乃伊腐毒](Mummy_Rot_(Condition).md "木乃伊腐毒（状态）")。

## 如何习得

职业：

- 职业等级11：[牧师](Cleric.md "牧师")、[邪术师](Warlock.md "邪术师")和[法师](Wizard.md "法师")

由物品提供：

- [地穴领主戒指](Crypt_Lord_Ring.md "地穴领主戒指")（充能：[长休](Long_Rest.md "长休")）

其他习得方式：

- 没有可供[法师](Wizard.md "法师")[抄录](Transcribing_scrolls.md "抄录卷轴")到法术书中的卷轴。

## 外部链接

- ⁠[唤起死灵](https://forgottenrealms.fandom.com/wiki/Create_undead) 在 [被遗忘的国度维基](https://forgottenrealms.fandom.com/wiki/Main_Page)

---
*Source: [Create Undead](https://bg3.wiki/wiki/Create_Undead)*