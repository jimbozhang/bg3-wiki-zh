# 啄咬眼珠 (渡鸦魔宠)

本文介绍渡鸦魔宠使用的攻击。其他版本请参见 [啄咬眼珠 (消歧义)](Rend_Vision_(disambiguation)..md)

**啄咬眼珠**是[获得魔宠：渡鸦](Find_Familiar_colon__Raven.md "获得魔宠：渡鸦")动作。这是一种近战攻击，造成微量穿刺伤害并使目标目盲。

## 描述

攻击生物的眼睛并使其[目盲](Blinded_(Condition).md "目盲 (状态)")。

## 属性

消耗
[动作](Actions.md#Resources "动作")
伤害: 1

1⁠[穿刺](Piercing.md "穿刺")

详情
近战 徒手 [攻击掷骰](Attack_roll.md "攻击掷骰")
近战: 1.5 米 (5 英尺)

## 状态: 目盲

**[目盲](Blinded_(Condition).md "目盲 (状态)")**

持续时间: 1 回合

- 受影响的生物在[攻击掷骰](Attack_roll.md "攻击掷骰")上具有[劣势](Disadvantage.md "劣势")。
- 受影响的生物的攻击和法术范围减少至 3 米 (10 英尺)。
- 对受影响生物的攻击掷骰具有[优势](Advantage.md "优势")。

## 如何习得

由生物使用: [魔宠渡鸦](Quothe.md "魔宠渡鸦")

## 备注

- 没有[豁免检定](Saving_throw.md "豁免检定")来避免[目盲](Blinded_(Condition).md "目盲 (状态)")效果。攻击只需命中即可应用效果。
- 与[荒野形态版本](Rend_Vision_(Dire_Raven)不同.md)，[目盲](Blinded_(Condition).md "目盲 (状态)")的持续时间少于一个完整的战斗回合；它在（目盲的）敌人的[回合](Turn-based_mode.md "回合制模式")开始时结束。

---
*Source: [Rend Vision (Raven Familiar)](https://bg3.wiki/wiki/Rend_Vision_(Raven_Familiar)*