# 跛足打击

**跛足打击**是角色在主手装备[战斧](Battleaxes.md "战斧")或[战镐](War_Picks.md "战镐")并[重甲的](Proficient.md "重甲的")使用时可用的[武器动作](Weapon_action.md "武器动作")。

## 描述

可能使目标[跛足](Maimed_(Condition).md "跛足（状态）")。目标无法移动。

飞行生物无法被跛足。

## 属性

消耗
[动作](Actions.md#Resources "动作")
伤害：1~4 + 调整值

1d4 + [力量或敏捷调整值](Finesse.md "灵巧")⁠[武器](Weapon.md "武器")

详情
近战武器[攻击掷骰](Attack_roll.md "攻击掷骰")
[体质](Constitution.md "体质")[豁免检定](Saving_throws.md "豁免检定")（[武器动作DC](Dice_rolls.md#Weapon_action_DC "掷骰") + 2）
近战：1.5 米（5 英尺）
充能：[短休](Short_rest.md "短休")
必须[熟练](Proficiency.md#Weapon_proficiency "熟练项")使用该武器

## 技术细节

UID

`Target_CripplingStrike`

法术标志

`[IsDefaultWeaponAction](https://bg3.wiki/w/index.php?title=IsDefaultWeaponAction_\(spell_flag\)&action=edit&redlink=1) "IsDefaultWeaponAction \(spell flag\) \(page does not exist\)")`, `[IsHarmful](IsHarmful_(spell_flag).md)`, `[IsMelee](IsMelee_(spell_flag).md)`

## 状态：跛足

**[跛足](Maimed_(Condition).md "跛足（状态）")**

持续时间：2 回合

[体质](Constitution.md "体质")[豁免检定](Saving_throws.md "豁免检定")（[武器动作DC](Dice_rolls.md#Weapon_action_DC "掷骰") + 2）

- 受影响的生物[移动速度](Movement_speed.md "移动速度")降至 0。
- 受影响的生物在敏捷[豁免检定](Saving_throw.md "豁免检定")上具有[劣势](Disadvantage.md "劣势")。
- 该状态可通过治疗移除。

## 学习方式

由物品提供：

- [莎尔的黄昏短矛](Shar's_Spear_of_Evening.md "莎尔的黄昏短矛")
- [塞伦涅的暗夜短矛](Sel%C3%BBne's_Spear_of_Night.md "塞伦涅的暗夜短矛")
- [尼鲁纳](Nyrulna.md "尼鲁纳")

通过[熟练](Proficiency.md "熟练")使用以下武器类型提供：

- [战斧](Battleaxes.md "战斧")
- [战镐](War_Picks.md "战镐")

## 注释

- 虽然此武器动作通常仅在战斧和战镐上可用，但其他类型的几种独特武器也提供此动作。

---
*Source: [Maiming Strike](https://bg3.wiki/wiki/Maiming_Strike)*