# 邪恶符号

本文介绍的内容**[无法通过正常游戏方式](Category_colon_Inaccessible.md "Category:Inaccessible")**获取，但仍可通过第三方工具访问。

**邪恶符号**是[刺剑](Rapiers.md "Rapiers")武器家族中一种罕见的、轻度附魔（+1）变体。它是一种单手挥舞的[武器](Weapons.md "武器")。作为[灵巧](Finesse.md "Finesse")武器，它能受益于使用者的敏捷，而不仅仅是力量。

在大多数人眼中，普通老鼠只是一堆有趣的疾病，但在散塔林会成员眼中，它们却是潜在的同事。

## 属性

伤害

1d8 + 1 (2~9) + [力量或敏捷调整值](Damage_Roll.md#Modifiers "伤害掷骰") ⁠[穿刺](Piercing.md "穿刺")

详情
[刺剑](Rapiers.md "Rapiers")
稀有度：罕见
附魔：**+ 1**
单手
[灵巧](Finesse.md "Finesse")
[可蘸取](Dippable.md "Dippable")
近战：1.5 米（5 英尺）
重量：0.9 千克（1.8 磅）
价格：85 金币
UID `MAG_Zhentarim_Ratcaller_Rapier` UUID `31a0245d-48fe-462e-b4bc-4bfe906e49fd` ### 武器动作

_如果你拥有[熟练项](Proficiency.md "熟练项")，装备在**主手**可获得：_

[穿刺打击](Piercing_Strike.md "穿刺打击")
刺击敌人并可能施加[开放伤口](Gaping_Wounds_(Condition).md "开放伤口（状态）")。（充能：[短休](Short_rest.md "短休")。）

[弱化打击](Weakening_Strike.md "弱化打击")
以非致命攻击瞄准敌人的手部，可能施加[弱腕](Weak_Grip_(Condition).md "弱腕（状态）")。（充能：[短休](Short_rest.md "短休")。）

[华舞](Flourish.md "华舞")
佯攻以可能使对手[失衡](Off_Balance_(Condition).md "失衡（状态）")。（充能：[短休](Short_rest.md "短休")。）

---
*Source: [Verminsign](https://bg3.wiki/wiki/Verminsign)*