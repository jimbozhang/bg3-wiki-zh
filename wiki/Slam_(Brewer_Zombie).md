# 猛击 (酿酒僵尸)

本文介绍的是 [残月酒厂](The_Waning_Moon.md "残月酒厂") 中僵尸使用的攻击版本。关于 [硬核](Tactician.md "硬核") 难度下使用的更强版本，请参见 [猛击 (酿酒僵尸, 硬核)](Slam_(Brewer_Zombie,_tactician).md)。其他用法，请参见 [猛击 (消歧义)](Slam_(disambiguation).md)。

**猛击** 是 [残月酒厂](The_Waning_Moon.md "残月酒厂") 中 [僵尸](Zombie.md "僵尸") 使用的基础攻击。这是一种徒手近战攻击，会造成额外的 [黯蚀](Necrotic.md "黯蚀") 伤害。在 [硬核](Tactician.md "硬核") 难度下，此能力会被一个[更强版本](Slam_(Brewer_Zombie,_tactician)取代.md)，该版本还能使目标[倒伏](Prone.md "Prone")。

## 描述

用你的赤手空拳击打目标。

## 属性

消耗
[动作](Actions.md#Resources "动作")
伤害：3~24 + 调整值

1d8 + [力量调整值](Strength.md#Strength_modifier_chart "力量")⁠[钝击](Bludgeoning.md "钝击")

\+ 2d8⁠[黯蚀](Necrotic.md "黯蚀")

详情
近战 徒手 [攻击掷骰](Attack_roll.md "攻击掷骰")
近战：1.5 米 (5 英尺)

## 学习方式

由以下生物使用：[瘟疫演员](Blighted_Actor.md "瘟疫演员")、[瘟疫酒保](Blighted_Barman.md "瘟疫酒保")、[瘟疫顾客](Blighted_Patron.md "瘟疫顾客") 和 [瘟疫吟游诗人](Blighted_Troubador.md "瘟疫吟游诗人")

---
*Source: [Slam (Brewer Zombie)](https://bg3.wiki/wiki/Slam_(Brewer_Zombie)*