# 贪婪藤蔓

关于该生物，请参见 [贪婪藤蔓（生物）](Grasping_Vine_(Creature)..md)

**贪婪藤蔓** 是一个 [法术](Spells.md "法术")。它允许施法者召唤一个 [贪婪藤蔓](Grasping_Vine_(Creature).md)，可以将敌人拉向自身。

## 描述

召唤一根巨大的藤蔓，能够将生物拖向自身。

## 属性

消耗
[附赠动作](Actions.md#Resources "Actions") + [4级法术位](Spells.md#Spell_slots "Spells")
详情
射程：9米（30英尺）
创造区域：扭曲的藤蔓

## 升环施法

以更高环阶施放此法术不会获得额外收益。

## 技术细节

UID

`Target_GraspingVine`

法术标志

`[CannotTargetCharacter](https://bg3.wiki/w/index.php?title=CannotTargetCharacter_\(spell_flag\)&action=edit&redlink=1) "CannotTargetCharacter \(spell_flag\) \(page does not exist\)")`, `[CannotTargetItems](https://bg3.wiki/w/index.php?title=CannotTargetItems_\(spell_flag\)&action=edit&redlink=1) "CannotTargetItems \(spell_flag\) \(page does not exist\)")`, `[HasSomaticComponent](HasSomaticComponent_(spell_flag).md)`, `[HasVerbalComponent](HasVerbalComponent_(spell_flag).md)`, `[IsSpell](IsSpell_(spell_flag).md)`

## 生物：贪婪藤蔓（生物）

| [力量](Strength.md "Strength") 16 (+3) | [敏捷](Dexterity.md "Dexterity") 16 (+3) | [体质](Constitution.md "Constitution") 10 | [智力](Intelligence.md "Intelligence") 2 (-4) | [感知](Wisdom.md "Wisdom") 2 (-4) | [魅力](Charisma.md "Charisma") 2 (-4) |
| --- | --- | --- | --- | --- | --- |
|  |  |  |  |  |  |
| 持续时间 | 10 驱散 |  |  |  |  |
| 生命值 | 20 |  |  |  |  |
| 护甲等级 | 13 |  |  |  |  |
| 移动速度 | 0 米 (0 英尺) |  |  |  |  |

[抓钩拉扯](Grasping_Pull.md "Grasping Pull") ()
抓住一个目标并将其拉向你 9 米 / 30 英尺。（充能：每驱散。）

## 区域：扭曲的藤蔓

**[扭曲的藤蔓](Twisting_Vines.md "Twisting Vines")**

持续时间：10 驱散

范围效果：3 米（10 英尺）半径

劣势地形 - 移动速度减半，生物可能被缠绕。被缠绕的生物无法移动。

类型：[地表](Area.md#Surface "Area")

### 状态：缠绕

**[缠绕](Entangled_(Condition).md "Entangled (Condition)")**

[力量](Strength.md "Strength") [豁免检定](Saving_throws.md "Saving throws") ([法术豁免DC](Dice_rolls.md#Spell_save_DC "Dice rolls"))

- 无法移动。
- 对该生物的[攻击掷骰](Attack_roll.md "Attack Roll")具有[优势](Advantage.md "Advantage")，而该生物的攻击掷骰和[敏捷](Dexterity.md "Dexterity") [豁免检定](Saving_throw.md "Saving Throw")具有[劣势](Disadvantage.md "Disadvantage")。

### 状态：劣势地形：藤蔓

**[劣势地形：藤蔓](Difficult_Terrain_colon__Vines_(Condition).md "Difficult Terrain: Vines (Condition)")**

- [移动速度](Movement_speed.md "Movement Speed")减半。

## 如何习得

职业：

- 职业等级 7：[德鲁伊](Druid.md "Druid")、[自然领域](Nature_Domain.md "Nature Domain")（领域法术）和[大地结社](Circle_of_the_Land.md "Circle of the Land")（森林或沼泽）

## 注释

- 在补丁 8 之前，此法术在升环施法时需要消耗[动作](Actions.md#Resources "Actions")而非[附赠动作](Actions.md#Resources "Actions")。

## 外部链接

- ⁠[贪婪藤蔓](https://forgottenrealms.fandom.com/wiki/Grasping_vine) 在 [被遗忘的国度维基](https://forgottenrealms.fandom.com/wiki/Main_Page)

---
*Source: [Grasping Vine](https://bg3.wiki/wiki/Grasping_Vine)*