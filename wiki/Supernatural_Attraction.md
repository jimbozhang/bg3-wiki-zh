# 超自然引力

本文介绍的是游戏性元素——例如角色、任务或机制——这些内容已从游戏中[移除](Category_colon_Legacy_content.md "Category:Legacy content")，且**不再可用**。

**超自然引力**是[邪术师](Warlock.md "Warlock")的[附赠动作](Actions.md#Resources "Actions")和[灵吸怪力量](Illithid_power.md "Illithid Power")，在[抢先体验](Early_Access.md "Early Access")中可用。此能力允许邪术师传送至其他寄生有蝌蚪的生物。

## 描述

你是更宏大整体的一部分。集中精神于你的连接，以传送至另一被感染的生物。

## 属性

消耗
[附赠动作](Actions.md#Resources "Actions")
详情
范围：18 米（60 英尺）
充能：[短休](Short_rest.md "Short rest")

## 技术细节

UID

`Target_SupernaturalAttraction`

法术标志

`[HasHighGroundRangeExtension](HasHighGroundRangeExtension_(spell_flag).md)`, `[RangeIgnoreVerticalThreshold](https://bg3.wiki/w/index.php?title=RangeIgnoreVerticalThreshold_\(spell_flag\)&action=edit&redlink=1) "RangeIgnoreVerticalThreshold \(spell flag\) \(page does not exist\)")`

---
*Source: [Supernatural Attraction](https://bg3.wiki/wiki/Supernatural_Attraction)*