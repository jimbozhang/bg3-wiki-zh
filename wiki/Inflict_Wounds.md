# 致伤术

**致伤术** 是一个 [1级死灵学派法术](Spells.md "Spells")。它允许施法者进行一次近战攻击，造成黯蚀伤害。

## 描述

用充满你双手的 [黯蚀](Necrotic.md "黯蚀") 能量腐化一个生物。

## 属性

消耗
[动作](Actions.md#Resources "动作") + [1级法术位](Spells.md#Spell_slots "法术位")
伤害: 3~30

3d10 [黯蚀](Necrotic.md "黯蚀")

详情
近战法术 [攻击掷骰](Attack_roll.md "攻击掷骰")
近战: 1.5 米 (5 英尺)

## 升环施法

[升环施法](Upcasting.md "升环施法"): 每升一环，额外造成 1d10 [黯蚀](Necrotic.md "黯蚀") 伤害。

## 技术细节

UID

`Target_InflictWounds`

法术标志

`[HasSomaticComponent](HasSomaticComponent_(spell_flag).md)`, `[HasVerbalComponent](HasVerbalComponent_(spell_flag).md)`, `[IsHarmful](IsHarmful_(spell_flag).md)`, `[IsMelee](IsMelee_(spell_flag).md)`, `[IsSpell](IsSpell_(spell_flag).md)`

## 学习方式

职业:

- 职业等级 1: [牧师](Cleric.md "牧师")
- 职业等级 3: [弃誓者](Oathbreaker.md "弃誓者") (誓言法术)

由特性提供:

- [魔法学徒：牧师](Magic_Initiate_colon__Cleric.md "魔法学徒：牧师")

由物品提供:

- [黑暗卫士的铁手套](Blackguard's_Gauntlets.md "黑暗卫士的铁手套") (充能: [长休](Long_Rest.md "长休"))

## 注释

致伤术的咒语是 **Morē**，拉丁语命令 _"die!"_（死吧！）

## 外部链接

- ⁠[致伤术](https://forgottenrealms.fandom.com/wiki/Inflict_wounds) 在 [被遗忘的国度维基](https://forgottenrealms.fandom.com/wiki/Main_Page) 上

---
*Source: [Inflict Wounds](https://bg3.wiki/wiki/Inflict_Wounds)*