# 月之尘埃

**月之尘埃**是一个[法术](Spells.md "法术")。它由[塞伦涅的暗夜短矛](Sel%C3%BBne's_Spear_of_Night.md "塞伦涅的暗夜短矛")授予。此能力会降低敌人在你周围的移动速度，并为你和你的盟友提供额外的[光耀](Radiant.md "光耀")伤害。

## 描述

用月光般的微光点亮你周围的区域，使敌人的移动变得困难，并增强盟友的伤害。

在回合开始时，光线内的每个敌对生物必须通过一次感知[豁免检定](Saving_throw.md "豁免检定")，否则必须将照亮区域视为[劣势地形](Difficult_Terrain.md "劣势地形")。光线内的每个盟友在近战攻击命中时，造成额外的1d4点[光耀](Radiant.md "光耀")伤害。

## 属性

消耗
[动作](Actions.md#Resources "动作")
详情
范围：18米（60英尺）
范围效果：9米（30英尺）半径
创建区域：舞光术
充能：[长休](Long_Rest.md "长休")
[专注](Concentration.md "专注")

## 技术细节

UID

`Target_SHA_SeluneBlessing_Moonfire`

法术标志

`[CannotTargetItems](https://bg3.wiki/w/index.php?title=CannotTargetItems_\(spell_flag\)&action=edit&redlink=1) "CannotTargetItems \(spell_flag\) \(page does not exist\)")`, `[HasHighGroundRangeExtension](HasHighGroundRangeExtension_(spell_flag).md)`, `[HasSomaticComponent](HasSomaticComponent_(spell_flag).md)`, `[HasVerbalComponent](HasVerbalComponent_(spell_flag).md)`, `[IsConcentration](IsConcentration_(spell_flag).md)`

## 状态：塞伦涅的祝福：月之烙印

**[塞伦涅的祝福：月之烙印](Selune's_Blessing_colon__Lunar_Brand_(Condition).md "塞伦涅的祝福：月之烙印（状态）")**

- 你的攻击造成额外的1d4点[光耀](Radiant.md "光耀")伤害。

## 状态：塞伦涅的祝福：月光陷阱

**[塞伦涅的祝福：月光陷阱](Selune's_Blessing_colon__Moonsnare_(Condition).md "塞伦涅的祝福：月光陷阱（状态）")**

[感知](Wisdom.md "感知") [豁免](Saving_throws.md "豁免检定")（[DC](DC.md "DC") 14）

- [移动速度](Movement_speed.md "移动速度")减半。

## 区域：舞光术

**[舞光术](Dancing_Lights_(area).md "舞光术（区域）")**

持续时间：10驱散
范围效果：9米（30英尺）半径
照亮9米（30英尺）半径。
类型：[召唤](Area.md#Summoned "区域")

## 如何习得

由物品授予：

- [塞伦涅的暗夜短矛](Sel%C3%BBne's_Spear_of_Night.md "塞伦涅的暗夜短矛")

## 备注

- 创建一个名为“塞伦涅的祝福：月之尘埃光环”的区域，该区域对范围内的生物施加“塞伦涅的祝福：月之烙印”和“塞伦涅的祝福：月光陷阱”，并持续10驱散。

## 错误

- 法术提示说明此光环仅适用于近战攻击，而状态提示说明此光环适用于“攻击”。实际上，它适用于武器和徒手攻击，但不适用于任何也是攻击的法术。

---
*Source: [Moonmote](https://bg3.wiki/wiki/Moonmote)*