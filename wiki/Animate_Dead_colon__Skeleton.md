# 操纵死尸：骷髅

**操纵死尸：骷髅**是一个[3级死灵学派法术](Spells.md "法术")。此法术允许施法者操纵一具尸体，创造一个专注于远程武器的[骷髅](Skeleton_(Animate_Dead).md "骷髅（操纵死尸）")。他们必须以一具尸体为目标，该骷髅会持续存在直到被杀死或直到下一次长休。

## 描述

创造一个专注于远程战斗的骷髅。

目标必须是一具中级或小型尸体。

## 属性

消耗
[动作](Actions.md#Resources "动作") + [3级法术位](Spells.md#Spell_slots "法术")
详情
射程：3米（10英尺）
目标：中级或小型尸体

## 更高法术位

以更高法术位施放此法术不会获得额外收益。

## 技术细节

UID

`Target_AnimateDead_Skeleton`

法术标志

`[HasSomaticComponent](HasSomaticComponent_(spell_flag).md)`, `[HasVerbalComponent](HasVerbalComponent_(spell_flag).md)`, `[IgnorePreviouslyPickedEntities](IgnorePreviouslyPickedEntities_(spell_flag).md)`, `[IsSpell](IsSpell_(spell_flag).md)`

## 生物：骷髅（操纵死尸）

| [力量](Strength.md "力量") 10 | [敏捷](Dexterity.md "敏捷") 16 (+3) | [体质](Constitution.md "体质") 8 (-1) | [智力](Intelligence.md "智力") 6 (-2) | [感知](Wisdom.md "感知") 8 (-1) | [魅力](Charisma.md "魅力") 5 (-3) |
| --- | --- | --- | --- | --- | --- |
|  |  |  |  |  |  |
| 持续时间 | 直到长休 |  |  |  |  |
| 生命值 | 19 |  |  |  |  |
| 护甲等级 | 11 |  |  |  |  |

[远程攻击](Ranged_Attack.md "远程攻击") ()
使用装备的武器进行一次远程攻击。

伤害：5~19

1d6 + 3⁠[穿刺](Piercing.md "穿刺")

\+ 1d10⁠[黯蚀](Necrotic.md "黯蚀")

## 如何习得

此法术是以下法术的变体：
[操纵死尸](Animate_Dead.md "操纵死尸")

---
*Source: [Animate Dead: Skeleton](https://bg3.wiki/wiki/Animate_Dead:_Skeleton)*