# 召唤初级元素生物：泥魔蝠

**召唤初级元素生物：泥魔蝠** 是一个 [法术](Spells.md "法术")。它是 [召唤初级元素生物](Conjure_Minor_Elemental.md "Conjure_Minor_Elemental") 的一个变体，可以召唤两个 [泥魔蝠](Mud_Mephit.md "Mud_Mephit")。

## 描述

从元素混沌中召唤两只泥魔蝠。它们可以施放 [泥水呼吸](Mud_Breath.md "Mud_Breath")，并在死亡时 [爆炸](Death_Burst_(Mud).md "Death Burst (Mud)")。

## 属性

消耗
[动作](Actions.md#Resources "Actions") + [4级法术位](Spells.md#Spell_slots "Spells")
详情
范围：18米（60英尺）

## 提升施法等级

以更高法术位施放此法术不会获得额外收益。

## 技术细节

UID

`Target_ConjureElementals_Minor_MudMephit`

法术标志

`[CannotTargetCharacter](https://bg3.wiki/w/index.php?title=CannotTargetCharacter_\(spell_flag\)&action=edit&redlink=1) "CannotTargetCharacter \(spell flag\) \(page does not exist\)")`, `[CannotTargetItems](https://bg3.wiki/w/index.php?title=CannotTargetItems_\(spell_flag\)&action=edit&redlink=1) "CannotTargetItems \(spell flag\) \(page does not exist\)")`, `[HasSomaticComponent](HasSomaticComponent_(spell_flag).md)`, `[HasVerbalComponent](HasVerbalComponent_(spell_flag).md)`, `[IsSpell](IsSpell_(spell_flag).md)`

## 生物：泥魔蝠

| [力量](Strength.md "Strength") 8 (-1) | [敏捷](Dexterity.md "Dexterity") 12 (+1) | [体质](Constitution.md "Constitution") 12 (+1) | [智力](Intelligence.md "Intelligence") 9 (-1) | [感知](Wisdom.md "Wisdom") 11 | [魅力](Charisma.md "Charisma") 7 (-2) |
| --- | --- | --- | --- | --- | --- |
|  |  |  |  |  |  |
| 持续时间 | 直至长休 |  |  |  |  |
| 生命值 | 18 |  |  |  |  |
| 护甲等级 | 11 |  |  |  |  |
| 体型 | 微型 |  |  |  |  |

[泥泞利爪](Muddy_Claws.md "Muddy_Claws") ()
用致命的拳头猛击。

[扔泥巴](Mud_Fling.md "Mud_Fling")()
扔出一团泥巴。

向敌人喷吐粘稠的泥水以 [束缚](Muddy_(Condition).md "Muddy (Condition)") 它。（充能：每回合。）

在粘稠泥巴的爆裂中爆炸。它会瞬间干燥在任何附近的非泥巴生物上，[束缚](Muddy_(Condition).md "Muddy (Condition)") 并伤害它们。

飞向目标位置

## 如何学习

此法术是以下法术的变体：
[召唤初级元素生物](Conjure_Minor_Elemental.md "Conjure_Minor_Elemental")

---
*Source: [Conjure Minor Elemental: Mud Mephits](https://bg3.wiki/wiki/Conjure_Minor_Elemental:_Mud_Mephits)*