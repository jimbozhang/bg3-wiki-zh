# 刃之魔契誓缚

注意不要与[脆弱诅咒武器誓缚](Bind_Hexed_Weapon.md "脆弱诅咒武器誓缚")混淆

**刃之魔契誓缚**是[邪术师](Warlock.md "邪术师")在3级选择[刃之魔契](Pact_of_the_Blade_(passive_feature)时可用的职业动作.md)。此动作用于绑定现有武器——若要召唤新武器，请使用[刃之魔契](Pact_of_the_Blade.md "刃之魔契")。

## 描述

绑定至主手武器。其伤害变为魔法伤害，你无法丢弃或投掷它，并且如果你尚未熟练，则会变得[重甲的](Proficient.md "重甲的")。

你同时只能拥有1把魔契武器，它使用你的[魅力](Charisma.md "魅力")调整值。

如果你与武器的距离超过1.5米（5英尺）超过10轮，武器将恢复其先前状态。

## 属性

消耗
[动作](Actions.md#Resources "动作")
详情
范围：自身

## 状态：魔契武器

**[魔契武器](Pact_Weapon_(Condition).md "魔契武器 (状态)")**

- 与该武器绑定的邪术师始终对其[熟练项](Proficiency.md "熟练项")。武器的伤害为魔法伤害。
- 武器使用施法者的[魅力调整值](Abilities.md#Ability_score_modifiers "属性")进行[攻击掷骰](Attack_roll.md "攻击掷骰")和伤害掷骰。
- 防止武器被丢弃或以其他方式缴械。

## 如何习得

职业：

- 职业等级3：[邪术师](Warlock.md "邪术师")

---
*Source: [Bind Pact Weapon](https://bg3.wiki/wiki/Bind_Pact_Weapon)*