# 消化液 (状态)

**消化液**

- 当被腐蚀性消化液浸湿时，该生物每[驱散](Turned_(Condition).md "驱散")受到 1d6⁠⁠[强酸](Acid.md "强酸")伤害，对[穿刺](Piercing.md "穿刺")和[钝击](Bludgeoning.md "钝击")伤害[易伤](Vulnerable.md "易伤")，并且如果同时处于[束缚](Restrained_(Condition).md "束缚 (状态)")状态，则可被[吞噬](Devour_(Shambling_Mound).md "吞噬 (蔓生怪)")。

- 水可以洗掉这些消化液。

## 属性

[堆叠 ID](Stack_ID.md "堆叠 ID")：`DIGESTIVE_SAP` [持续时间结束](Conditions.md#Duration "状态")：驱散开始时

[若已应用](Conditions.md#Stack_type "状态")：不应用

## 备注

- 在补丁 8 之前，此状态错误地未应用对钝击伤害的易伤。

## 消化液来源

- [消化液](Digestive_Sap.md "消化液")

## 拥有消化液的生物

_维基数据库中未定义_

## 具有相同堆叠 ID 的状态

- 当被腐蚀性消化液浸湿时，该生物每[驱散](Turned_(Condition).md "驱散")受到 1d6⁠⁠[强酸](Acid.md "强酸")伤害，对[穿刺](Piercing.md "穿刺")和[钝击](Bludgeoning.md "钝击")伤害[易伤](Vulnerable.md "易伤")，并且如果同时处于[束缚](Restrained_(Condition).md "束缚 (状态)")状态，则可被[吞噬](Devour_(Shambling_Mound).md "吞噬 (蔓生怪)")。

- 水可以洗掉这些消化液。

---
*Source: [Digestive Juices (Condition)](https://bg3.wiki/wiki/Digestive_Juices_(Condition)*