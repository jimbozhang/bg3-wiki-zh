# 惊心打击

**惊心打击**是角色[重甲的](Proficient.md "重甲的")使用[钉头锤](Morningstars.md "钉头锤")并将其握在主手时可用的[武器动作](Weapon_action.md "武器动作")。

## 描述

粉碎敌人的胸膛，并可能施加[胸外伤](Chest_Trauma_(Condition).md "胸外伤（状态）")。

[不死生物](Undead.md "不死生物")和[构装生物](Constructs.md "构装生物")无法获得胸外伤。

## 属性

消耗
[动作](Actions.md#Resources "动作")
伤害：1~4 + 调整值

1d4 + [力量或敏捷调整值](Finesse.md "灵巧")⁠[穿刺](Piercing.md "穿刺")

详情
近战武器[攻击掷骰](Attack_roll.md "攻击掷骰")
[体](Constitution.md "体质")[豁免检定](Saving_throws.md "豁免检定")（[武器动作DC](Dice_rolls.md#Weapon_action_DC "掷骰") + 2）
射程：正常武器射程
充能：[短休](Short_rest.md "短休")
必须[熟练](Proficiency.md#Weapon_proficiency "熟练项")使用该武器

## 技术细节

UID

`Target_HeartStopper`

法术标志

`[IsDefaultWeaponAction](https://bg3.wiki/w/index.php?title=IsDefaultWeaponAction_\(spell_flag\)&action=edit&redlink=1) "IsDefaultWeaponAction \(spell flag\) \(page does not exist\)")`, `[IsHarmful](IsHarmful_(spell_flag).md)`, `[IsMelee](IsMelee_(spell_flag).md)`

## 状态：胸外伤

**[胸外伤](Chest_Trauma_(Condition).md "胸外伤（状态）")**

持续时间：2 驱散

[体](Constitution.md "体质")[豁免检定](Saving_throws.md "豁免检定")（[武器动作DC](Dice_rolls.md#Weapon_action_DC "掷骰") + 2）

- 在体质[豁免检定](Saving_throws.md "豁免检定")上具有[劣势](Disadvantage.md "劣势")，且动作减少一个。
- 通过治疗移除。

## 如何习得

通过[熟练](Proficiency.md "熟练")使用以下武器类型获得：

- [钉头锤](Morningstars.md "钉头锤")

---
*Source: [Heartstopper](https://bg3.wiki/wiki/Heartstopper)*