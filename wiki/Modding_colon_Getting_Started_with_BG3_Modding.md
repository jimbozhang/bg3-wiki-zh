# Modding:开始博德之门3模组制作

- [模组资源](Modding_colon_Modding_resources.md "Modding:模组资源")

模组资源 - 网格、模型、工具等
[护甲/服装纹理贴图](Modding_colon_Armor/Clothing_Texture_Maps.md "Modding:护甲/服装纹理贴图") [身体模型](Modding_colon_Body_Models.md "Modding:身体模型") [自定义变形术](Modding_colon_Custom_polymorph.md "Modding:自定义变形术") [依赖项](Modding_colon_Dependencies.md "Modding:依赖项") [面部和身体纹理](Modding_colon_Face_and_body_textures.md "Modding:面部和身体纹理") [修复骨骼错误](Modding_colon_Fixing_bone_errors.md "Modding:修复骨骼错误") [修复颈部接缝](Modding_colon_Fixing_neck_seams.md "Modding:修复颈部接缝") [修复UV错误](Modding_colon_Fixing_UV_errors.md "Modding:修复UV错误") [开始博德之门3模组制作](Modding_colon_Getting_Started_with_BG3_Modding.md "Modding:开始博德之门3模组制作") [头发网格](Modding_colon_Hair_Meshes.md "Modding:头发网格") [头部模型](Modding_colon_Head_Models.md "Modding:头部模型") [模组故障排除](Modding_colon_Mod_troubleshooting.md "Modding:模组故障排除") [模组职业图标](Modding_colon_Modding_class_icons.md "Modding:模组职业图标") [NPC头部模型](Modding_colon_NPC_Head_Models.md "Modding:NPC头部模型") [提夫林号角模型](Tiefling_Horn_Models.md "提夫林号角模型") [非VT着色器](Modding_colon_Non-VT_Shaders.md "Modding:非VT着色器") [种族UUID](Modding_colon_Race_UUID.md "Modding:种族UUID") [示例模板](Modding_colon_Sample_Templates.md "Modding:示例模板") [法术标志](Spell_flags.md "法术标志") [纹理格式](Modding_colon_Texture_formatting.md "Modding:纹理格式") [工具](Modding_colon_Tools.md "Modding:工具") [宝藏表参考](Modding_colon_TreasureTables_References.md "Modding:宝藏表参考") [教程](Modding_colon_Tutorials.md "Modding:教程") [顶点颜色遮罩槽](Modding_colon_VertexColorMaskSlots.md "Modding:顶点颜色遮罩槽") [处理LSX文件](Modding_colon_Working_with_LSX_files.md "Modding:处理LSX文件")
模组指南
[BG3迷你工具](Modding_colon_BG3_Mini_Tool.md "Modding:BG3迷你工具") [Blender导出设置](Modding_colon_Blender_Export_Settings.md "Modding:Blender导出设置") [代码片段](Modding_colon_Code_Snippets.md "Modding:代码片段") [编写物品代码](Modding_colon_Coding_An_Item.md "Modding:编写物品代码) [兼容性框架](Modding_colon_Compatibility_Framework.md "Modding:兼容性框架") [创建自定义布料碰撞体](Modding_colon_Creating_a_custom_cloth_collider.md "Modding:创建自定义布料碰撞体") [在Blender中创建和导出网格](Modding_colon_Creating_and_Exporting_Meshes_in_Blender.md "Modding:在Blender中创建和导出网格") [创建物品图标](Modding_colon_Creating_Item_Icons.md "Modding:创建物品图标") [创建meta.lsx](Modding_colon_Creating_meta.lsx.md "Modding:创建meta.lsx") [创建模组](Modding_colon_Creating_mods.md "Modding:创建模组") [编辑角色创建预设](Modding_colon_Editing_a_Character_Creation_Preset.md "Modding:编辑角色创建预设") [编辑Equipment.txt](Modding_colon_Editing_Equipment.txt.md "Modding:编辑Equipment.txt") [如何查找虚拟纹理](Modding_colon_How_To_Find_A_Virtual_Texture.md "Modding:如何查找虚拟纹理") [安装模组](Modding_colon_Installing_mods.md "Modding:安装模组") [本地化](Modding_colon_Localization.md "Modding:本地化") [模组资源](Modding_colon_Modding_resources.md "Modding:模组资源") [打包模组](Modding_colon_Packaging_mods.md "Modding:打包模组") [纹理格式](Modding_colon_Texture_formatting.md "Modding:纹理格式") [宝藏表](Modding_colon_Treasure_Tables.md "Modding:宝藏表") [理解模组文件夹结构](Understanding_Mod_Folder_Structure.md "理解模组文件夹结构") [解包和转换文件](Modding_colon_Unpacking_and_converting_files.md "Modding:解包和转换文件") [处理LSX文件](Modding_colon_Working_with_LSX_files.md "Modding:处理LSX文件")
头发模组
[使用头发工具创建自定义头发](Modding_colon_Creating_Custom_Hair_with_Hair_Tool.md "Modding:使用头发工具创建自定义头发") [使用自定义纹理创建头发合并.lsf](Modding_colon_Creating_hair_merged.lsf_with_custom_texture.md "Modding:使用自定义纹理创建头发合并.lsf") [创建头发模组](Modding_colon_Creating_Hair_Mods.md "Modding:创建头发模组") [创建meta.lsx](Modding_colon_Creating_meta.lsx.md "Modding:创建meta.lsx") [自定义头发高光](Modding_colon_Custom_Hair_Highlights.md "Modding:自定义头发高光") [头发网格](Modding_colon_Hair_Meshes.md "Modding:头发网格") [头发模组文件设置](Guide_colon_Hair_Mod_File_Setup.md "Guide:头发模组文件设置") [头发模组网格设置](Modding_colon_Hair_Mod_Mesh_Setup.md "Modding:头发模组网格设置")
头部模组
[创建meta.lsx](Modding_colon_Creating_meta.lsx.md "Modding:创建meta.lsx") [面部和身体纹理](Modding_colon_Face_and_body_textures.md "Modding:面部和身体纹理") [修复颈部接缝](Modding_colon_Fixing_neck_seams.md "Modding:修复颈部接缝") [头部模组](Modding_colon_Head_Modding.md "Modding:头部模组") [头部模型](Modding_colon_Head_Models.md "Modding:头部模型") [头部故障排除](Modding_colon_Head_Troubleshooting.md "Modding:头部故障排除") [NPC头部模型](Modding_colon_NPC_Head_Models.md "Modding:NPC头部模型")
物品模组 - 护甲、服装、武器等
[护甲/服装纹理贴图](Modding_colon_Armor/Clothing_Texture_Maps.md "Modding:护甲/服装纹理贴图") [编写物品代码](Modding_colon_Coding_An_Item.md "Modding:编写物品代码") [在Blender中创建和导出网格](Modding_colon_Creating_and_Exporting_Meshes_in_Blender.md "Modding:在Blender中创建和导出网格") [创建护甲模组](Modding_colon_Creating_Armor_Mods.md "Modding:创建护甲模组") [创建物品图标](Modding_colon_Creating_Item_Icons.md "Modding:创建物品图标") [创建meta.lsx](Modding_colon_Creating_meta.lsx.md "Modding:创建meta.lsx") [创建服装纹理](Modding_colon_Creating_Outfit_Textures.md "Modding:创建服装纹理") [修复骨骼错误](Modding_colon_Fixing_bone_errors.md "Modding:修复骨骼错误") [如何查找虚拟纹理](Modding_colon_How_To_Find_A_Virtual_Texture.md "Modding:如何查找虚拟纹理") [纹理格式](Modding_colon_Texture_formatting.md "Modding:纹理格式") [宝藏表参考](Modding_colon_TreasureTables_References.md "Modding:宝藏表参考") [使用服装构建器重新调整服装](Modding_colon_Use_Outfit_Builder_To_Refit_Outfits.md "Modding:使用服装构建器重新调整服装") [顶点颜色遮罩槽](Modding_colon_VertexColorMaskSlots.md "Modding:顶点颜色遮罩槽")
种族/职业模组
[创建种族模组](Modding_colon_Creating_Race_Mods.md "Modding:创建种族模组") [编辑角色创建预设](Modding_colon_Editing_a_Character_Creation_Preset.md "Modding:编辑角色创建预设")

您是否对成为博德之门3模组制作者感到兴奋，但完全不知道从哪里开始？希望这篇文章能为您指明正确的方向。

## 目录

- [1 我需要什么才能开始？](#我需要什么才能开始？)
- [2 从哪些模组开始最好？](#从哪些模组开始最好？)
- [3 模组网站](#模组网站)
- [4 权限](#权限)
  - [4.1 权限示例](#权限示例)
  - [4.2 Nexusmods](#Nexusmods)
  - [4.3 其他模组制作者的权限](#其他模组制作者的权限)
  - [4.4 您的权限](#您的权限)

## 我需要什么才能开始？

您需要一些工具才能开始模组制作。请确保下载以下内容，因为它们基本上是模组制作的必备工具：

- [BG3 Multitool](https://github.com/ShinyHobo/BG3-Modders-Multitool) by ShinyHobo
  - 多功能工具是一个很棒的工具，提供了几个重要功能，其中最主要的是能够搜索游戏文件，以及将您的模组打包成可导入BG3MM的格式。
- [Lslib](https://github.com/Norbyte/lslib) by Norbyte
  - Lslib允许您在可编辑格式和游戏就绪格式之间转换文件。有关更多详细信息，请参见[模组:解包和转换文件](Modding_colon_Unpacking_and_converting_files.md "Modding:解包和转换文件")。
- [ScriptExtender](https://github.com/Norbyte/bg3se) by Norbyte
  - Script Extender为游戏添加了Lua/Osiris脚本支持。
- [Blender](https://www.blender.org/download/lts/3-6/) (推荐版本3.6)
  - 仅当您计划进行任何类型的网格编辑（头发、头部、衣服、武器等）时才需要此工具。
  - [Collada/GR2导入/导出Blender插件](https://github.com/Norbyte/dos2de_collada_exporter) by Norbyte
    - 用于导入/导出BG3使用的.GR2文件。
- [BG3 ModManager (BG3MM)](https://github.com/LaughingLeader/BG3ModManager) by LaughingLeader
  - BG3的便携式模组管理器。

## 从哪些模组开始最好？

您已经找到了想要使用的工具、教程、示例/模板。但您想知道是否有更简单的模组项目更适合开始，以便熟悉工具和文件格式。

如果您想创建角色创建（CC）模组，创建您自己的[化妆/纹身/高光/灰白颜色](https://www.nexusmods.com/baldursgate3/mods/132?tab=files)是一个很好的起点。它可以帮助您熟悉文件的某些格式，而不会不知所措。如果您对装备更感兴趣，也许可以尝试制作一个[简单的魔法戒指模组](https://www.nexusmods.com/baldursgate3/mods/278)。

但请记住：别人从某个地方开始并不意味着您也必须如此，而且尽量不要将自己与他人在掌握速度上进行比较。我们都在以自己的节奏学习和工作。以您感到舒适的速度进行，并且不要忘记享受乐趣！

## 模组网站

如果您制作模组是为了与他人分享，请选择您想要分享的网站。

- [Nexus Mods](https://www.nexusmods.com/baldursgate3) (最受欢迎)
- [Thunderstore](https://baldurs-gate-3.thunderstore.io)
- [Curse Forge](https://www.curseforge.com/baldurs-gate-3)

## 权限

在制作模组时，您可以为您的模组分配权限。这也意味着，如果您想在自己的模组中使用任何其他模组制作者的_编辑/自定义_资产，您也必须阅读他们的权限。这甚至包括该资产的唯一标识符（UUID），因为它是_旨在与现有用户提交的内容交互的_。

那么什么是权限？简而言之：它们是您和其他模组制作者围绕您模组中的自定义资产制定的规则。例如，经过大量编辑或从头开始制作的纹理、头部、服装、视觉效果等。例如，您是否希望有人能够获取您的模组，对其进行一些编辑，然后将其作为他们自己的模组上传。

没有人拥有原始资产，任何人都不能声称拥有它们。因此，多个使用相同资产的模组可以存在，例如多个添加相同NPC头发的模组可以存在。如果资产被编辑过，那么通常就会涉及权限。

在使用来自另一个模组的资产或为模组制作补丁之前，请征求许可。询问是否可以为他人的模组制作补丁，或者询问是否可以将一个模组的资产添加到另一个模组中，这是常见的礼节。

- 这使模组制作者意识到，任何需要的故障排除都可以更容易地进行。
- 大多数模组制作者会说可以，如果他们自己没有在制作补丁的话。

模组制作者也有权在被问及是否可以使用他们的资产时说不。尊重他们的意愿。

因此，始终征求许可，如果模组制作者说不，这并不意味着您可以继续使用他们的资产。这意味着停止您正在做的事情，并寻找其他可以使用的资产。

- 大多数模组制作者允许个人使用编辑，但这意味着不能将他们的资产上传到其他网站/ Discord服务器等。如有疑问，请询问！

### 权限示例

假设您找到了一个您喜欢的头部模组，并且您想将其用作创建自己头部的基础：

- 由于是编辑过的网格，您必须获得模组作者的许可才能将该网格用作您自己编辑的基础。是的，即使他们编辑的原始资产来自游戏。
  - 如果面部本身没有编辑，但耳朵被大量编辑，那么在这种情况下，您需要获得使用耳朵的许可。

### Nexusmods

如果您想在nexusmods上发布，您需要了解那里的权限如何运作。

[Nexusmods版权与您](https://forums.nexusmods.com/topic/771842-copyright-and-you/)

[Nexusmods提交指南](https://help.nexusmods.com/article/28-file-submission-guidelines)

### 其他模组制作者的权限

如果想搜索其他模组制作者的权限，可以在他们模组的主页上的“权限和鸣谢”下找到。您会发现：

- 其他用户的资产 -
  - 模组/文件中的资产是否属于作者，或来自免费使用的模组制作者资源
- 上传权限 -
  - 您是否被允许将模组/文件上传到其他网站
- 修改权限 -
  - 您是否必须在修改文件之前获得模组制作者的许可
- 转换权限 -
  - 您是否被允许将文件转换为在其他游戏中工作
- 资产使用权限 -
  - 您是否必须在使用模组中的任何资产之前获得模组制作者的许可

### 您的权限

如果创建模组，“权限”选项卡是第四个选项卡。您会发现以下问题：

- 是否允许用户在其他网站上上传我的文件？
  - 您是否允许他人将您的文件/模组上传到其他网站
- 是否允许用户将我的模组转换为在其他游戏中工作？
  - 您是否允许他人将您的资产转换到其他游戏
- 是否允许用户修改我的文件；包括发布错误修复或改进我的文件添加到游戏的功能，并将其作为单独的文件上传？
  - 您是否允许他人修改您的文件，包括错误修复或改进。
- 是否允许用户在我的文件中使用资产？
  - 您是否允许他人在自己的模组中使用您的资产。

如果您未经许可使用来自其他模组制作者的编辑资产，您可能会面临您的模组或至少所使用的资产被要求撤回的风险，然后模组才能重新上线。

---
*Source: [Modding:Getting Started with BG3 Modding](https://bg3.wiki/wiki/Modding:Getting_Started_with_BG3_Modding)*