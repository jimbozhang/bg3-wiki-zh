# 赫克

## 赫克

至上真神学徒

- 游戏内
- 模型
- 肖像画

### 身份

男性

### 声音 / 演员

- [Peter Hannah](https://www.imdb.com/name/nm0360307/)

### 阵营

[至上真神教团](Cult_of_the_Absolute.md "Cult of the Absolute")

## 属性

### 体型

[中级](Medium.md "Medium")

### 类型

[类人生物](Humanoid.md "Humanoid")

### 种族

[矮人](Dwarf.md "Dwarf")

### 亚种

[灰矮人](Duergar.md "Duergar")

### 职业

[游侠](Ranger.md "Ranger")

- 平衡

- 探索者

- 硬核

## 平衡难度属性

### 等级

5

### 生命值

44

### 护甲等级

11

### 先攻

+2

### 熟练项

+3

### 移动速度

7.5 米 (25 英尺)

### 重量

75 千克 (150 磅)

### 高级黑暗视觉

24 米 (80 英尺)

### 经验值奖励

75

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | 12 / (+1) | 15 / (+2) | 12 / (+1) | 10 / (0) | 12 / (+1) | 12 / (+1) |

## 抗性 [(info)](Resistances.md "Resistances")

- ## 额外资源

- - ## 特性

- [**灰矮人坚韧**](Duergar_Resilience.md "Duergar Resilience")

对[幻术学派](Illusion.md "Illusion")和被[魅惑](Charmed_(status_group).md)或[麻痹](Paralysed_(Condition).md)的[豁免检定](Saving_throws.md "Saving throws")具有[优势](Advantage.md "Advantage")。

- [**矮人活力**](Dwarven_Resilience.md "Dwarven Resilience")

对被[中毒](Poisoned_(Condition).md)的[豁免检定](Saving_throw.md "Saving throw")具有[优势](Advantage.md "Advantage")，并对[中毒](Poison.md "Poison")伤害具有[抗性](Resistance.md "Resistance")。

- [**额外攻击**](Extra_Attack.md "Extra Attack")

可以在徒手或武器攻击后进行一次额外攻击。

- [**借机攻击**](Opportunity_Attack.md "Opportunity Attack")

攻击离开攻击范围的敌人。

- [**日照敏感**](Sunlight_Sensitivity.md "Sunlight Sensitivity")

在阳光下时，[攻击掷骰](Attack_Roll.md "Attack Roll")和[察觉技能](Perception.md "Perception")检定具有[劣势](Disadvantage.md "Disadvantage")。

- [**高级黑暗视觉**](Superior_Darkvision.md "Superior Darkvision")

可以在黑暗中看到最多 24 米 / 80 英尺。

## 探索者难度属性

### 等级

5

### 生命值

30

### 护甲等级

11

### 先攻

+2

### 熟练项

+5

### 移动速度

7.5 米 (25 英尺)

### 重量

75 千克 (150 磅)

### 高级黑暗视觉

24 米 (80 英尺)

### 经验值奖励

75

| 属性值 | 力 | 敏 | 体 | 智 | 感 | 魅 |
| --- | --- | --- | --- | --- | --- | --- |
|  | +4 | +5 | +1 | 0 | +1 | +1 |

## 抗性 [(info)](Resistances.md "Resistances")

- ## 额外资源

- - ## 特性

- [**灰矮人坚韧**](Duergar_Resilience.md "Duergar Resilience")

对[幻术学派](Illusion.md "Illusion")和被[魅惑](Charmed_(status_group).md)或[麻痹](Paralysed_(Condition).md)的[豁免检定](Saving_throws.md "Saving throws")具有[优势](Advantage.md "Advantage")。

- [**矮人活力**](Dwarven_Resilience.md "Dwarven Resilience")

对被[中毒](Poisoned_(Condition).md)的[豁免检定](Saving_throw.md "Saving throw")具有[优势](Advantage.md "Advantage")，并对[中毒](Poison.md "Poison")伤害具有[抗性](Resistance.md "Resistance")。

- [**额外攻击**](Extra_Attack.md "Extra Attack")

可以在徒手或武器攻击后进行一次额外攻击。

- [**借机攻击**](Opportunity_Attack.md "Opportunity Attack")

攻击离开攻击范围的敌人。

- [**日照敏感**](Sunlight_Sensitivity.md "Sunlight Sensitivity")

在阳光下时，[攻击掷骰](Attack_Roll.md "Attack Roll")和[察觉技能](Perception.md "Perception")检定具有[劣势](Disadvantage.md "Disadvantage")。

- [**高级黑暗视觉**](Superior_Darkvision.md "Superior Darkvision")

可以在黑暗中看到最多 24 米 / 80 英尺。

## 硬核难度属性

### 等级

5

### 生命值

57

### 护甲等级

11

### 先攻

+2

### 熟练项

+3

### 移动速度

7.5 米 (25 英尺)

### 重量

75 千克 (150 磅)

### 高级黑暗视觉

24 米 (80 英尺)

### 经验值奖励

75

| 属性值 | 运动 | 体操 | 察觉技能 |
| --- | --- | --- | --- |
|  | 1 | 2 | 1 |

## 抗性 [(info)](Resistances.md "Resistances")

- ## 额外资源

- - ## 特性

- [**灰矮人坚韧**](Duergar_Resilience.md "Duergar Resilience")

对[幻术学派](Illusion.md "Illusion")和被[魅惑](Charmed_(status_group).md)或[麻痹](Paralysed_(Condition).md)的[豁免检定](Saving_throws.md "Saving throws")具有[优势](Advantage.md "Advantage")。

- [**矮人活力**](Dwarven_Resilience.md "Dwarven Resilience")

对被[中毒](Poisoned_(Condition).md)的[豁免检定](Saving_throw.md "Saving throw")具有[优势](Advantage.md "Advantage")，并对[中毒](Poison.md "Poison")伤害具有[抗性](Resistance.md "Resistance")。

- [**额外攻击**](Extra_Attack.md "Extra Attack")

可以在徒手或武器攻击后进行一次额外攻击。

- [**借机攻击**](Opportunity_Attack.md "Opportunity Attack")

攻击离开攻击范围的敌人。

- [**日照敏感**](Sunlight_Sensitivity.md "Sunlight Sensitivity")

在阳光下时，[攻击掷骰](Attack_Roll.md "Attack Roll")和[察觉技能](Perception.md "Perception")检定具有[劣势](Disadvantage.md "Disadvantage")。

- [**高级黑暗视觉**](Superior_Darkvision.md "Superior Darkvision")

可以在黑暗中看到最多 24 米 / 80 英尺。

## 技术信息

###

UID

`S_UND_DuergarSquadRatty` ###

UUID

`64e38ef0-9779-499c-9f0a-ea013ee81442` ###

属性

`UND_KC_Duergar_Ranger` **赫克**是一名灰矮人游侠，是[海盗格雷蒙](Greymon.md "Greymon")在[幽暗地域](Underdark.md "Underdark")的船员之一，出现在[第一幕](Act_One.md "Act_One")。

> “
>
> 对我们所有人来说都是漫长的一天，所以别耍花样。
>
> „

— 赫克

## 参与剧情

### 第一幕

在从[幽暗地域](Underdark.md "Underdark")海滩登上船只后，一艘由海盗格雷蒙驾驶的船驶近了队伍的船。格雷蒙登船，玩家角色必须通过一次检定来说服他让队伍通过前往[复仇之炉](Grymforge.md "Grymforge")。如果检定失败或选择暴力，格雷蒙会发起攻击，赫克会加入他的阵营战斗。

## 战斗

### 攻击与能力

[备战（远程）](Brace_(Ranged).md "Brace (Ranged)") () – [重弩](Heavy_Crossbow.md "Heavy Crossbow")
消耗 6 米 (20 英尺) 的[移动速度](Movement_speed.md "Movement Speed")。在你的回合剩余时间内，掷两次远程伤害并取最高值。

充能：[短休](Short_rest.md "Short rest")

[穿刺射击](Piercing_Shot.md "Piercing Shot") () – [重弩](Heavy_Crossbow.md "Heavy Crossbow")

伤害：3~12

1d10 + 2⁠[穿刺](Piercing.md "Piercing")

造成常规伤害，并可能施加[开放伤口](Gaping_Wounds_(Condition).md "Gaping Wounds (Condition)")，使攻击造成额外伤害。

[不死生物](Undead.md "Undead")和[构装生物](Constructs.md "Constructs")无法获得开放伤口。

[体质](Constitution.md "Constitution") [豁免检定](Saving_throws.md "Saving throws")
范围：正常武器范围
充能：[短休](Short_rest.md "Short rest")

[远程攻击](Ranged_Attack.md "Ranged Attack") () – [重弩](Heavy_Crossbow.md "Heavy Crossbow")

伤害：3~12

1d10 + 2⁠[穿刺](Piercing.md "Piercing")

使用装备的武器进行一次远程攻击。

范围：18 米 (60 英尺)

### 盟友

- [炼金术士阿格拉姆](Argram.md "Argram")
- [波纳](Bonna.md "Bonna")
- [格雷蒙](Greymon.md "Greymon")
- [杰里斯](Jerris.md "Jerris")
- [心灵大师凯特尔](Kettel.md "Kettel")

### 战利品

- [火焰箭](Arrow_of_Fire.md "Arrow_of_Fire") (如果未使用)
- [重弩](Heavy_Crossbow.md "Heavy Crossbow")
- [皮甲](Leather_Armour.md "Leather Armour")
- [皮革头盔](Leather_Helmet.md "Leather Helmet")

---
*Source: [Hurk](https://bg3.wiki/wiki/Hurk)*